...
\begin{proof}
    Let \(n \in \mathbf{N}\), let \(I_n = \{i \in \mathbf{N} : 1 \leq i \leq n\}\) and let \(X_1, \dots, X_n\) be bounded subsets of \(\mathbf{R}\).
    ...
    So suppose that \(n \neq 0\).
    Since \(X_1, \dots, X_n\) are bounded, \(\exists\ M_1, \dots, M_n \in \mathbf{R}^+\) such that \(\forall i \in I_n\), \(X_i \subseteq [-M_i, M_i]\).
    Let \(S = \{M_i : i \in I_n\}\).
    Clearly \(S\) is finite.
...
\begin{proof}
    Let \(n \in \mathbf{N}\) and let \(x_i \in \mathbf{R}\) for every \(i \in \mathbf{N} \land i \leq n\).
    Given any sequence \((a_m)_{m = 0}^\infty\) of real numbers which taks values in the singleton set \(\{x_i\}\), we know that \(\forall m \in \mathbf{N}\), \(a_m = x_i\).
...
\begin{proof}
    ...
    If not, then \(\exists\ \varepsilon \in \mathbf{R}^+\) such that \(S - \varepsilon < S\) and
    \[
