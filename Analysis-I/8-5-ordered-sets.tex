...
\begin{defn}[Partially ordered sets]\label{8.5.1}
...
\begin{defn}[Totally ordered set]\label{8.5.3}
...
\begin{defn}[Maximal and minimal elements]\label{8.5.5}
...
\begin{defn}[Well-ordered sets]\label{8.5.8}
...
\begin{prop}[Principle of strong induction]\label{8.5.10}
...
\begin{defn}[Upper bounds and strict upper bounds]\label{8.5.12}
...
\begin{proof}
  ...
  But then \(Y_{\infty} \cup \{s(Y_{\infty})\}\) is well-ordered (by \cref{ex:8.5.11}) and has \(x_0\) as its minimal element.
...
\begin{lem}[Zorn's lemma]\label{8.5.15}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies & y = \min\big((Y, \leq_X)\big) = \max\big((Y, \leq_X)\big) & \text{(by \cref{8.5.5})}
  \end{align*}
  ...
  \begin{itemize}
    \item If \(y <_X y_{\min}\), then \(y = \min\big((Y, \leq_X)\big)\) since
          \begin{align*}
            ...
            \implies & (y' = y) \lor (y_{\min} <_X y')          & \text{(by \cref{8.5.5})}               \\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies & Z \text{ is well-ordered}.                  & \text{(by \cref{8.5.8})}
  \end{align*}
  ...
  \begin{align*}
    ...
    \implies & (z_Y \leq_X z) \lor (z_{Y'} \leq_X z)            & \text{(by \cref{8.5.5})} \\
    \implies & (z_Y \leq_X z) \lor (z_Y \leq_X z_{Y'} \leq_X z) & \text{(by \cref{8.5.1})} \\
    \implies & z_Y \leq_X z                                                                \\
    \implies & z_Y = \min\big((Z, \leq_X)\big).                 & \text{(by \cref{8.5.5})}
  \end{align*}
  This means for any \(Z \subseteq Y \cup Y'\), \(\min\big((Z, \leq_X)\big)\) must exists.
  Thus by \cref{8.5.8} \((Y \cup Y', \leq_X)\) is well-ordered.
\end{proof}

\begin{ex}\label{ex:8.5.12}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies & (x = x') \land (y = y')                                                                       & \text{(by \cref{8.5.1})} \\
  ...
  \begin{align*}
    ...
    \implies & (x_1 <_X x_2 <_X x_3)                                                                                     & \text{(by \cref{8.5.1})} \\
             & \lor \big((x_1 = x_2 <_X x_3) \land (y_1 \leq_Y y_2)\big)                                                                            \\
             & \lor \big((x_1 <_X x_2 = x_3) \land (y_2 \leq_Y y_3)\big)                                                                            \\
             & \lor \big((x_1 = x_2 = x_3) \land (y_1 \leq_Y y_2 \leq_Y y_3)\big)                                        & \text{(by \cref{8.5.1})} \\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies & X = Y,                                              & \text{(by \cref{3.1.18})}
  \end{align*}
...
\begin{ex}\label{ex:8.5.16}
  Let \(X\) be a set, and let \(P\) be the set of all partial orderings of \(X\).
  (For instance, if \(X \coloneqq \N \setminus \{0\}\), then both the usual partial ordering \(\leq\), and the partial ordering in \cref{ex:8.5.3}, are elements of \(P\).)
...
\begin{proof}
  ...
  \begin{align*}
             & \forall (X, \leq) \in P                                                    \\
    \implies & (\forall x, y \in X, x =_X y \implies x \leq y) & \text{(by \cref{8.5.1})} \\
...
\begin{proof}
  ...
  Since \((\{y\}, =)\) is reflexive, anti-symmetric and transitive, by \cref{8.5.1} we know that \((\{y\}, =)\) is partially ordered.
  Since
  \[
    \forall y_1, y_2 \in \{y\}, y_1 = y_2 = y,
  \]
  by \cref{8.5.3} we know that \((\{y\}, =)\) is totally ordered.
  Since every non-emptyset subset of \(\{y\} = \{y\}\), by \cref{8.5.5} \(\min\big((\{y\}, =)\big) = y\) and by \cref{8.5.8} \((\{y\}, =)\) is well-ordered.
