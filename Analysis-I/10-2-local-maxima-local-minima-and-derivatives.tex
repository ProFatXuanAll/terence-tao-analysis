...
\begin{proof}
    ...
    \]
    By Definition \ref{9.3.6} we know that \(\forall \varepsilon \in \R^+\), \(\exists\ \delta' \in \R^+\) such that
    \[
...
\begin{proof}
    Since \(f'\) is bounded, by Definition \ref{9.6.1} we know that \(\exists\ M \in \R^+\) such that \(\abs{f'(x)} \leq M\) for every \(x \in \R\).
    By Exercise \ref{ex 10.2.6} we know that \(\abs{f(x) - f(y)} \leq M \abs{x - y}\) for every \(x, y \in \R\).
    Then we have
    \begin{align*}
