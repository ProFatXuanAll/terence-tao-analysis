...
\begin{proof}
    ...
    By Lemma \ref{5.1.15} we know that \(\big(\sum_{n = 0}^m f(n)\big)_{m = 0}^\infty\) is bounded by some \(M \in \R\).
...
\begin{proof}
    ...
    \[
        ...
        \end{cases}
    \]
    By Proposition \ref{11.6.1} \(f\) is Riemann integrable on \([1, N]\) for every \(N \in \R\) and \(N \geq 1\).
    If \(p \neq 1\), then we have
    \begin{align*}
    ...
    Since \(\int_{[1, N]} f = \frac{1}{1 - p} (N^{1 - p} - 1)\) and \(\{N^{1 - p} : N \in \R^+\}\) is unbounded, we know that \(\sup_{N > 1} \int_{[1, N]} f = +\infty\) and by Proposition \ref{11.6.4} \(\sum_{n = 1}^\infty \frac{1}{n^p}\) is divergent.
