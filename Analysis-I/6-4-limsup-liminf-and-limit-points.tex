...
\begin{proof}{(a)}
    ...
    Since \(c\) is a limit point of \((a_n)_{n = m}^\infty\), \(\forall\ \varepsilon \in \mathbf{R}^+\), we have \(\forall\ N \in \mathbf{N}\) and \(N \geq m\), \(\exists\ n \geq N\) such that \(\abs*{a_n - c} \leq \varepsilon\).
