...
\begin{defn}[Extended real number system]\label{6.2.1}
...
\begin{defn}[Negation of extended reals]\label{6.2.2}
...
\begin{defn}[Ordering of extended reals]\label{6.2.3}
...
\begin{defn}[Supremum of sets of extended reals]\label{6.2.6}
...
\begin{proof}{(a)}
  ...
  \begin{itemize}
    \item If \(+\infty \not\in E\), then we can further split into two cases:
          \begin{itemize}
            \item If \(-\infty \in E\), then by \cref{6.2.6} we know that \(\sup(E) = \sup(E \setminus \set{-\infty})\).
...
\begin{proof}{(c)}
  ...
  \begin{align*}
    ...
    \implies & -x \leq -M                                    \\
    \implies & \sup(-E) \leq -M          &  & \by{6.2.11}[b] \\
    \implies & -\sup(-E) \geq M                              \\
