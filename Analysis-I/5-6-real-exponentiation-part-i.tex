...
We define \(x^{1 / n}\), also known as the \emph{\(n^{\text{th}}\) roots of \(x\)}, by the formula
\[
...
Let \(k_1, k_2 \in \{k \in \mathds{N} : k > 0\}\) and without loss of generality assume that \(k_1 < k_2\).
Now we want to show that \(x^{1 / k_1} > x^{1 / k_2}\).
Suppose for sake of contadiction that \(x^{1 / k_1} \not> x^{1 / k_2}\).
Then by Proposition \ref{5.4.7}, exactly one of the following two statements is true:
\begin{enumerate}[label=(\Roman*)]
    \item \(x^{1 / k_1} = x^{1 / k_2}\).
    By Lemma \ref{5.6.6}(a), \(x = (x^{1 / k_1})^{k_1} = (x^{1 / k_2})^{k_1} = (x^{1 / k_2})^{k_2}\).
    Since \(x\) is positive, by Lemma \ref{5.6.6}(c) \(x^{1 / k_1}\) is also positive.
    So we have
    \begin{align*}
        & (x^{1 / k_2})^{k_2} / (x^{1 / k_2})^{k_1} = 1 \\
        \implies& (x^{1 / k_2})^{k_2 - k_1} = 1 & \text{(by Proposition \ref{5.6.3})}
    \end{align*}
    \item \(x^{1 / k_1} < x^{1 / k_2}\).
\end{enumerate}
\end{proof}
