...
\begin{proof}{(a)}
    ...
    By Definition \ref{8.2.1} we know that there exists a bijection \(p : \N \to X\) such that \(\sum_{n = 0}^\infty f\big(p(n)\big)\) converges.
...
\begin{proof}{(c)}
    ...
    \begin{itemize}
        \item One of \(X_1, X_2\) is finite and one is countable.
              ...
              Since \(X_2\) is countable, by Definition \ref{8.1.1} we know that \(\exists\ q_2 : \N \to X_2\) such that \(q_2\) is bijective.
        ...
        \item Both \(X_1, X_2\) are countable.
              Since \(X_1\) is countable, by Definition \ref{8.1.1} we know that \(\exists\ q_1 : \N \to X_1\) such that \(q_1\) is bijective.
...
\begin{proof}{(d)}
    ...
    So suppose that \(X\) is countable.
    By Definition \ref{8.2.1} we know that there exists a bijection \(p : \N \to X\) such that \(\sum_{n = 0}^\infty f\big(p(n)\big)\) converges.
    Since \(\phi\) is bijective, we know that \(Y\) is also countable and by Definition \ref{8.1.1} \(\exists\ q : \N \to Y\) such that \(q\) is bijective.
...
\begin{proof}
    ...
    In particular, for every \(n \in \N\) and \(M \in \N\) we have \(\sum_{m = 0}^M f(n, m) \leq L\), which implies by Proposition \ref{6.3.8} that \(\sum_{m = 0}^\infty f(n, m)\) is convergent for each \(n\).
    Similarly, for any \(N \in \N\) and \(M \in \N\) we have (by Corollary \ref{7.1.14})
    \[
        \sum_{n = 0}^N \sum_{m = 0}^M f(n, m) = \sum_{(n, m) \in X} f(n, m) \leq L
    \]
    where \(X\) is the set \(\{(n,m) \in \N \times \N : n \leq N, m \leq M\}\) which is finite by Proposition \ref{3.6.14}.
    ...
    (Since \(\N \times \N\) is countable by Corollary \ref{8.1.13}, we can find a bijection \(g : \N \to \N \times \N\) such that \(\sum_{i = 0}^\infty f(g(i)) = L\), which means \(\forall \varepsilon > 0\), \(\exists\ H \in \N\) such that \(\abs{\sum_{i = 0}^h f(g(i)) - L} \leq \varepsilon\) for all \(h \geq H\).
    ...
    (We can construct a bijection \(g : \N \to \N \times \N\) and then since \(\forall n \in \N\) we have \(f_+(g(n)) \leq \abs{f(g(n))}\) and \(\abs{f_-(g(n))} \leq \abs{f(g(n))}\), we know that \((f_+(g(n)))_{n = 0}^\infty\) and \((f_-(g(n)))_{n = 0}^\infty\) are absolutely convergent by comparison test (Proposition \ref{7.3.2}.))
...
\begin{proof}
    ...
    Since \(\sum_{x \in X} f(x)\) is absolutely convergent, by Definition \ref{8.2.1} \(\exists\ g : \N \to X\) where \(g\) is a bijection such that
    \[
    ...
    \]
    Since \(A\) is finite, by Exercise \ref{ex 3.6.3} \(\exists\ M \in \N\) such that \(g^{-1}(A)\) is bounded by \(M\).
    So we have
    \[
    ...
    \begin{align*}
        ...
        \implies & \forall n \in \N : \sum_{i = 0}^n \abs{f(g(i))} \leq L                              & \text{(by Definition \ref{7.1.6})}  \\
...
\begin{proof}
    ...
    Since \(\abs{f(x)} \in \R^+\), by Proposition \ref{5.4.12} we have
    \begin{align*}
                 & \exists\ N \in \Z^+ : \frac{1}{\abs{f(x)}} < N                                                        \\
        \implies & \abs{f(x)} > \frac{1}{N}                                                                              \\
        \implies & x \in S_N                                                     & \text{(by the definition of \(S_N\))} \\
        \implies & x \in \bigcup_{n \in \Z^+} S_n                                & \text{(by Axiom \ref{3.11})}          \\
        \implies & \{x \in X : f(x) \neq 0\} \subseteq \bigcup_{n \in \Z^+} S_n. & \text{(by Definition \ref{3.1.15})}
    \end{align*}
    Since \(\forall n \in \Z^+\), \(S_n\) is finite, by Exercise \ref{8.1.9} we know that \(\bigcup_{n \in \Z^+} S_n\) is at most countable.
    Since \(\{x \in X : f(x) \neq 0\} \subseteq \bigcup_{n \in \Z^+} S_n\), by Corollary \ref{8.1.7} we know that \(\{x \in X : f(x) \neq 0\}\) is at most countable.
\end{proof}
...
\begin{proof}{(a)}
    ...
    Since \(\sum_{x \in X} f(x)\) and \(\sum_{x \in X} g(x)\) are both absolutely convergent, by Definition \ref{8.2.4} \(\exists\ N, M \in \R\) such that
    \[
...
\begin{proof}{(b)}
    ...
    Since \(\sum_{x \in X} f(x)\) is absolutely convergent, by Definition \ref{8.2.4} \(\exists\ N \in \R\) such that
    \[
        N = \sup\Bigg\{\sum_{x \in A} \abs{f(x)} : A \subseteq X, A \text{ finite}\Bigg\} < \infty.
    \]
    Let \(A \subseteq X\) be a finite set.
...
\begin{proof}{(c)}
    ...
    Since \(\sum_{x \in X_1} h(x)\) and \(\sum_{x \in X_2} h(x)\) are absolutely convergent, by Definition \ref{8.2.4} \(\exists\ N, M \in \R\) such that
    \[
...
\begin{proof}
    ...
    By Proposition \ref{8.1.5} we can then find increasing bijections \(f_+ : \N \to A_+\) and \(f_- : \N \to A_-\).
...
\begin{proof}
    ...
    In Theorem \ref{8.2.8} we know that both \(A_+\) and \(A_-\) are countable, and there exist two increasing bijections \(f_+ : \N \to A_+\) and \(f_- : \N \to A_-\).
    ...
    \begin{itemize}
        ...
        \item We have \(\limsup_{j \to \infty} \sum_{i = 0}^j a_{n_j}\) diverges to \(\infty\).
              ...
              \]
              Since this is true for every \(M \in \R^+\), by Exercise \ref{ex 6.4.8} we thus have
              \[
