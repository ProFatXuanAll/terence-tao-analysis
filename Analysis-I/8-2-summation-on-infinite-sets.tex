...
\begin{proof}{(a)}
    ...
    Since \(\sum_{x \in X} f(x)\) and \(\sum_{x \in X} g(x)\) are both absolutely convergent, by Lemma \ref{8.2.3}, \(\exists\ N, M \in \mathbf{R}\) such that
    \[
    ...
    \]
    Let \(A \subsetneq X\) be a finite set.
    Then we have
    \begin{align*}
    ...
    Since
    \[
        \bigg\{x \in X : \big(f(x) = 0 \land g(x) \neq 0\big) \lor \big(f(x) \neq 0 \land g(x) = 0\big)\bigg\} \subseteq X_h,
    \]
\end{proof}
