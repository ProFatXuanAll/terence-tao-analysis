...
Let \(I\) be a bounded interval, let \(\alpha : X \to \mathbf{R}\) be a monotone function defined on some interval \(X\) which contains \(I\), and let \(f : I \to \mathbf{R}\) and \(g : I \to \mathbf{R}\) be piecewise constant functions on \(I\) such that both \(p.c. \int_I f \; d \alpha\) and \(p.c. \int_I g \; d \alpha\) are well-defined.
\begin{enumerate}
    ...
    \item We have \(p.c. \int_I (f - g) \; d \alpha = p.c. \int_I f \; d \alpha - p.c. \int_I g \; d \alpha\).
    \item If \(f(x) \geq 0\) for all \(x \in I\), then \(p.c. \int_I f \; d \alpha \geq 0\) if \(\alpha\) is monotone increasing and \(p.c. \int_I f \; d \alpha \leq 0\) if \(\alpha\) is monotone decreasing.
    \item If \(f(x) \geq g(x)\) for all \(x \in I\), then \(p.c. \int_I f \; d \alpha \geq p.c. \int_I g \; d \alpha\) if \(\alpha\) is monotone increasing and \(p.c. \int_I f \; d \alpha \leq p.c. \int_I g \; d \alpha\) if \(\alpha\) is monotone decreasing.
...
\begin{proof}{(d)}
    ...
    \begin{itemize}
        \item \(\alpha\) is monotone increasing.
              ...
              \begin{align*}
                  p.c. \int_I f \; d \alpha & = p.c. \int_{[\mathbf{P}]} f \; d \alpha & \text{(by Additional Corollary \ref{ac 11.8.5})} \\
                                            ...
                                            & = 0.
              \end{align*}
        \item \(\alpha\) is monotone decreasing.
              By Additional Corollary \ref{ac 11.8.6} we know that \(\alpha[J] \leq 0\) for all \(J \in \mathbf{P}\).
              Since \(f(x) \geq 0\) for all \(x \in I\), we have \(c_J \geq 0\) and \(c_J \alpha[J] \leq 0\) for all \(J \in \mathbf{P}\).
              Thus
              \begin{align*}
                  p.c. \int_I f \; d \alpha & = p.c. \int_{[\mathbf{P}]} f \; d \alpha & \text{(by Additional Corollary \ref{ac 11.8.5})} \\
                                            & = \sum_{J \in \mathbf{P}} c_J \alpha[J]  & \text{(by Definition \ref{11.8.5})}              \\
                                            & \leq \sum_{J \in \mathbf{P}} 0           & \text{(by Proposition \ref{7.1.11}(h))}          \\
                                            & = 0.
              \end{align*}
    \end{itemize}
\end{proof}
...
\begin{proof}{(e)}
    ...
    \begin{itemize}
        ...
        \item \(\alpha\) is monotone decreasing.
              Then by Additional Corollary \ref{ac 11.8.7}(d) we have
              \[
                  p.c. \int_I (f - g) \; d \alpha \leq 0 \implies p.c. \int_I f \; d \alpha \leq p.c. \int_I g \; d \alpha.
              \]
    \end{itemize}
\end{proof}
...
\begin{additional corollary}[Laws of Riemann-Stieltjes integration]\label{ac 11.8.11}
Let \(I\) be a bounded interval, let \(\alpha : X \to \mathbf{R}\) be a monotone increasing function defined on some interval \(X\) which contains \(I\).
...
\begin{additional corollary}[Laws of Riemann-Stieltjes integration]\label{ac 11.8.12}
Let \(I\) be a bounded interval, let \(\alpha : X \to \mathbf{R}\) be a monotone increasing function defined on some interval \(X\) which contains \(I\).
