...
\begin{proof}
    ...
    The function \(\tilde{h}\) is also a bijection from \(\{i \in \N : 1 \leq i \leq n\} \to X - \{x\}\) (cf. Lemma \ref{3.6.9}).
...
\begin{proof}
    ...
    i.e., we let \(P(n)\) be the assertion that Lemma \ref{7.1.13} is true for any set \(X\) with \(n\) elements, and any finite set \(Y\) and any function \(f : X \times Y \to \R\).
...
\begin{proof}
    ...
    i.e., we let \(P(n)\) be the assertion that Additional Corollary \ref{ac 7.1.6} is true for any set \(X\) with \(n\) elements, and any finite set \(Y\) and any function \(f : X \times Y \to \R\).
    We wish to prove \(P(n)\) for all natural numbers \(n\).
