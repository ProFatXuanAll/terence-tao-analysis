...
\begin{note}
  In light of \labelcrefs{ii:6.2.1}, we see that the derivative \(f'(x_0)\) can be interpreted as the number \(L\) for which \(\abs{f(x) - \big(f(x_0) + L(x - x_0)\big)}\) is small, in the sense that it tends to zero as \(x\) tends to \(x_0\), even if we divide out by the very small number \(\abs{x - x_0}\).
...
\begin{proof}
  ...
  \]
  Since \(x_0\) is an interior point, by \labelcrefs{ii:1.2.5} we know that
  \[
  ...
  \[
    ...
    \end{dcases}
  \]
  Since \(f\) is differentiable at \(x_0\) with derivative \(L_1\), by \labelcrefs{ii:6.2.2} we know that
  \begin{align*}
  ...
  \begin{align*}
     ...
     & = 0.
  \end{align*}
  By \labelcrefs{ii:1.1.6} and \labelcrefs{ii:1.1.2} we know that
  \begin{align*}
     & \norm*{f(x_0 + tv) - \big(f(x_0) + L_1(tv)\big)} + \norm*{f(x_0 + tv) - \big(f(x_0) + L_2(tv)\big)}   \\
  ...
  \begin{align*}
     ...
     & = 0.
  \end{align*}
  But \(v \neq 0\) implies \(\norm*{v} \neq 0\) (by \labelcrefs{ii:1.1.2.b}), so we must have \(L_2(v) = L_1(v)\), a contradiction.
  Thus, \(L_1 = L_2\).
\end{proof}

\begin{note}
  Because of \labelcrefs{ii:6.2.4}, we can now talk about \emph{the} derivative of \(f\) at interior points \(x_0\), and we will denote this derivative by \(f'(x_0)\).
...
\begin{note}
  Another consequence of \labelcrefs{ii:6.2.4} is that if you know that \(f(x) = g(x)\) for all \(x \in E\), and \(f, g\) are differentiable at \(x_0\), then you also know that \(f'(x_0) = g'(x_0)\) at every \emph{interior} point of \(E\).
...
\begin{ex}\label{ii:ex:6.2.1}
  Prove \labelcrefs{ii:6.2.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.2.1}.
\end{proof}

\begin{ex}\label{ii:ex:6.2.2}
  Prove \labelcrefs{ii:6.2.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.2.4}.
\end{proof}
