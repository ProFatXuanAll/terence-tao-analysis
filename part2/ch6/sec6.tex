...
\begin{proof}
  ...
  \]
  by \labelcrefs{ii:6.6.1} we know that \(x \mapsto x + 1\) is a contraction.
  ...
  \]
  by \labelcrefs{ii:6.6.1} we know that \(x \mapsto \dfrac{x}{2}\) is a strict contraction.
  ...
  \begin{align*}
    \forall x, y \in [0, 1], \abs{(x - x^2) - (y - y^2)} & = \abs{x - y - (x^2 - y^2)}    \\
                                                         ...
                                                         & \leq \abs{x - y}.
  \end{align*}
  Thus, by \labelcrefs{ii:6.6.1} we know that \(x \mapsto x - x^2\) is a contraction on \([0, 1]\).
...
\begin{proof}
  ...
  Since \(f\) is a strict contraction, by \labelcrefs{ii:6.6.1} we know that there exists a \(c \in (0, 1)\) such that
  \[
    d\big(f(x_1), f(x_2)\big) \leq c d(x_1, x_2).
  \]
  Since \(x_1, x_2\) are fixed points, by \labelcrefs{ii:6.6.3} we know that
  \[
  ...
  \]
  By \labelcrefs{ii:1.4.6} this means \((x_n)_{n = 1}^\infty\) is a Cauchy sequence in \((X, d)\).
  By hypothesis we know that \((X, d)\) is complete, thus by \labelcrefs{ii:1.4.10} there exists a \(y_0 \in X\) such that \(\lim_{n \to \infty} d(y_0, x_n) = 0\).

  Now we claim that \(y_0\) is the fixed point of \(f\).
  Suppose for the sake of contradiction that \(y_0\) is not the fixed point of \(f\).
  By \labelcrefs{ii:6.6.3} this means \(f(y_0) \neq y_0\), in other words, \(d\big(f(y_0), y_0\big) > 0\).
...
\begin{note}
  Basically, \labelcrefs{ii:6.6.6} says that any map \(f\) on a ball which is a ``small'' perturbation (since \(g\) is a strict contraction) of the identity map (since \(f(x) = x + g(x)\)), remains one-to-one and cannot create any internal holes in the ball
...
\begin{proof}
  ...
  \]
  so \(F\) is a strict contraction on \(B(0, r)\), and hence on the complete space \(\overline{B(0, r - \varepsilon)}\) (see \labelcrefs{ii:1.5.7} and \labelcrefs{ii:1.5.5}).
...
\begin{proof}
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:6.6.1} \(f\) is a contraction.
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:6.6.1} \(f\) is a strict contraction.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  by \labelcrefs{ii:6.6.1} we know that \(f\) is a strict contraction.
\end{proof}

\begin{ex}\label{ii:ex:6.6.5}
  Verify the claims in \labelcrefs{ii:6.6.2}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.6.2}.
\end{proof}
...
\begin{proof}
  ...
  \]
  Since \(x_0\) was arbitrary, by \labelcrefs{ii:2.1.1} this means \(f\) is continuous on \(X\) from \((X, d)\) to \((X, d)\).
\end{proof}

\begin{ex}\label{ii:ex:6.6.7}
  Prove \labelcrefs{ii:6.6.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.6.4}.
\end{proof}
...
\begin{ex}\label{ii:ex:6.6.8}
  ...
  From \labelcrefs{ii:6.6.4} we know that \(f\) has some fixed point \(x_0\), and \(g\) has some fixed point \(y_0\).
