\section{The implicit function theorem}\label{ii:sec:6.8}
...
\begin{thm}[Implicit function theorem]\label{ii:6.8.1}
...
\begin{proof}
  ...
  Thus, the inverse function theorem (\labelcrefs{ii:6.7.2}) applies, and we can find an open set \(V\) in \(E\) containing \(y\), and an open set \(W\) in \(\R^n\) containing \(F(y) = (y_1, \dots, y_{n - 1}, 0)\), such that \(F\) is a bijection from \(V\) to \(W\), and that \(F^{-1}\) is differentiable at \((y_1, \dots, y_{n - 1}, 0)\).
...
\begin{rmk}\label{ii:6.8.2}
  \labelcrefs{ii:eq 6.1} is sometimes derived using \emph{implicit differentiation}.
  ...
  \]
  which is \labelcrefs{ii:eq 6.1} in disguise
...
\begin{ex}\label{ii:ex:6.8.1}
  Let the notation and hypotheses be as in \labelcrefs{ii:6.8.1}.
  Show that, after shrinking the open sets \(U, V\) if necessary, that the function \(g\) becomes continuously differentiable on all of \(U\), and \labelcrefs{ii:eq 6.1} holds at all points of \(U\).
\end{ex}
