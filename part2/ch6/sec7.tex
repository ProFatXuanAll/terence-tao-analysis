...
\begin{proof}
  ...
  \begin{align*}
    T^{-1}(cx) & = T^{-1}\Big(c T\big(T^{-1}(x)\big)\Big)                    \\
               ...
               & = c T^{-1}(x).
  \end{align*}
  Thus, by \labelcrefs{ii:6.1.6} \(T^{-1}\) is a linear transformation.
\end{proof}
...
\begin{proof}
  ...
  Note from \labelcrefs{ii:6.7.1} that \(\big(f'(0)\big)^{-1}\) is a linear transformation.
  ...
  Since \(f'(0)\) and its inverse are both continuous, \(f'(0)(V')\) is open (see \labelcrefs{ii:2.1.5.a,ii:2.1.5.c}), and it certainly contains \(0\).
  ...
  By \labelcrefs{ii:6.6.6}, the map \(f = g + I\) is thus one-to-one on \(B(0, r)\), and the image \(f\big(B(0, r)\big)\) contains \(B(0, \dfrac{r}{2})\).
  ...
  By \labelcrefs{ii:3.1.5.b}, it suffices to show that
  \[
...
\begin{note}
  ...
  Recall that we have \(f'(x_0) = L_{D f(x_0)}\), so by \labelcrefs{ii:6.1.13,ii:6.1.16} we see that the linear transformation \(f'(x_0)\) is invertible iff the matrix \(D f(x_0)\) is.
...
\begin{note}
  ...
  this was remarked in the proof of \labelcrefs{ii:6.7.2}.
...
\begin{proof}
  ...
  By \labelcrefs{ii:1.2.15.a} we know that
  \[
...
\begin{ex}\label{ii:ex:6.7.2}
  Prove \labelcrefs{ii:6.7.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.7.1}.
\end{proof}
...
\begin{proof}
  ...
  Since \(f\) is continuously differentiable on \(\R^n\) and \(f'(x)\) is invertible, by inverse function theorem (\labelcrefs{ii:6.7.2}) we know that
  \[
  ...
  Fix such \(r\).
  By \labelcrefs{ii:1.2.15.f} we know that \(U \cap B_{(\R^n, d)}(x, r)\) is open in \((\R^n, d)\).
  ...
  By hypotheses we know that \(f^{-1}\) is differentiable on \(f\big(U \cap B_{(\R^n, d)}(x, r)\big)\), by \labelcrefs{ii:ex:6.4.2} we know that \(f^{-1}\) is continuous \(f\big(U \cap B_{(\R^n, d)}(x, r)\big)\).
  Thus, by \labelcrefs{ii:2.1.5.a,ii:2.1.5.c} we know that \(f\big(U \cap B_{(\R^n, d)}(x, r)\big)\) is open in \((\R, d)\).
  Now we have
  \begin{align*}
...
\begin{ex}\label{ii:ex:6.7.4}
  Let the notation and hypotheses be as in \labelcrefs{ii:6.7.2}.
