\section{Partial and directional derivatives}\label{ii:sec:6.3}

\begin{defn}[Directional derivative]\label{ii:6.3.1}
...
\begin{rmk}\label{ii:6.3.2}
  One should compare \labelcrefs{ii:6.3.1} with \labelcrefs{ii:6.2.2}.
...
\begin{proof}
  Since \(f\) is differentiable at \(x_0\), by \labelcrefs{ii:6.2.2} we know that \(f'(x_0) : \R^n \to \R^m\) exists and \(f'(x_0)\) is a linear transformation.
  If \(v = 0_{\R^n}\), then we have
  \begin{align*}
     & f'(x_0)(0_{\R^n})                                                                                                                             \\
     & = 0_{\R^m}                                                                               &  & \text{(cf. the proof of \labelcrefs{ii:6.2.4})} \\
...
\begin{rmk}\label{ii:6.3.6}
  One consequence of \labelcrefs{ii:6.3.5} is that total differentiability implies directional differentiability.
  However, the converse is not true;
  see \labelcrefs{ii:ex:6.3.3}.
\end{rmk}

\begin{defn}[Partial derivative]\label{ii:6.3.7}
...
\begin{ac}\label{ii:ac:6.3.1}
  ...
  Indeed, if we write \(f\) in components as \(f = (f_1, \dots, f_m)\), it is easy to see (by \labelcrefs{ii:1.1.18}) that
  \[
...
\begin{ac}\label{ii:ac:6.3.2}
  From \labelcrefs{ii:6.3.5} (and Proposition 9.5.3 from Analysis I), we know that if a function is differentiable at a point \(x_0\), then all the partial derivatives \(\dfrac{\partial f}{\partial x_j}\) exists at \(x_0\), and that
  \[
...
\begin{note}
  Just because the partial derivatives exist at a point \(x_0\), we cannot conclude that the function is differentiable there (\labelcrefs{ii:ex:6.3.3}).
  However, if we know that the partial derivatives not only exist, but are continuous, then we can in fact conclude differentiability, thanks to the \labelcrefs{ii:6.3.8}
\end{note}
...
\begin{ac}\label{ii:ac:6.3.3}
  From \labelcrefs{ii:6.3.8} and \labelcrefs{ii:6.3.5} we see that if the partial derivatives of a function \(f : E \to \R^m\) exist and are continuous on some set \(F\), then all the directional derivatives also exist at every interior point \(x_0\) of \(F\), and we have the formula
  \[
...
\begin{ac}\label{ii:ac:6.3.4}
  More generally, if \(f : E \to \R^m\) is a function taking values in \(\R^m\), with \(f = (f_1, \dots, f_m)\), and \(x_0\) is in the interior of the region where the partial derivatives of \(f\) exist and are continuous, then we have from \labelcrefs{ii:6.3.8} that
  \[
...
\begin{ex}\label{ii:ex:6.3.1}
  Prove \labelcrefs{ii:6.3.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.3.5}.
\end{proof}
...
\begin{ex}\label{ii:ex:6.3.3}
  ...
  Explain why this does not contradict \labelcrefs{ii:6.3.8}.
\end{ex}
...
\begin{proof}
  ...
  \begin{align*}
     ...
     & = 0,
  \end{align*}
  by \labelcrefs{ii:6.3.1} we know that \(f\) is differentiable in every direction \(v \in \R^2\) at \((0, 0)\).

  Next we show that \(f\) is not differentiable at \((0, 0)\).
  Suppose for the sake of contradiction that \(f\) is differentiable at \((0, 0)\).
  Then by \labelcrefs{ii:ac:6.3.2} we know that
  \[
  ...
  Now we show that this does not contradict \labelcrefs{ii:6.3.8}.
  ...
  \]
  by \labelcrefs{ii:ex:6.3.2} we know that \(\dfrac{\partial f}{\partial x}(0, 0) = 1\).
...
\begin{proof}
  ...
  By \labelcrefs{ii:ac:6.3.2} we know that
  \begin{align*}
  ...
  \]
  by mean value theorem (cf. the proof of \labelcrefs{ii:6.3.8}) we know that
  \begin{align*}
  ...
  Since \((\Omega, d)\) is open, by \labelcrefs{ii:1.2.15.a} we know that
  \[
  ...
  But by \labelcrefs{ii:2.4.1} we know that \((\Omega, d)\) is connected implies
  \begin{align*}
