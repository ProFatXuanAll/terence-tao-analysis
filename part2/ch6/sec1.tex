...
\begin{proof}
  ...
  \begin{align*}
    L_A(cx) & = L_A\big(c(x_j)_{1 \leq j \leq n}\big)                       &  & \by{ii:6.1.1} \\
            ...
            & = c L_A(x).                                                   &  & \by{ii:6.1.1}
  \end{align*}
  Thus, by \labelcrefs{ii:6.1.6} \(L_A\) is a linear transformation from \(\R^n\) to \(\R^m\).
\end{proof}
...
\begin{proof}
  ...
  Suppose for the sake of contradiction that we could find such a matrix \(B\) which was different from \(A\).
...
\begin{rmk}\label{ii:6.1.14}
  \labelcrefs{ii:6.1.13} establishes a one-to-one correspondence between linear transformations and matrices, and is one of the fundamental reasons why matrices are so important in linear algebra.
...
\begin{note}
  The composition \(T \circ S\) of two linear transformations \(T, S\) is again a linear transformation (\labelcrefs{ii:ex:6.1.2}).
...
\begin{proof}
  ...
  By \labelcrefs{ii:6.1.11} \(AB\) is well-defined.
  Let \(C = AB = (c_{ik})_{1 \leq i \leq m ; 1 \leq k \leq p}\).
  Then by \labelcrefs{ii:6.1.11} we have
  \[
...
\begin{ex}\label{ii:ex:6.1.1}
  Prove \labelcrefs{ii:6.1.2}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.1.2}.
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    (T \circ S)(cx) & = T\big(S(cx)\big)                     \\
                    ...
                    & = c (T \circ S)(x).
  \end{align*}
  Thus, by \labelcrefs{ii:6.1.6} we know that \(T \circ S\) is a linear transformation from \(\R^p\) to \(\R^m\).
\end{proof}

\begin{ex}\label{ii:ex:6.1.3}
  Prove \labelcrefs{ii:6.1.16}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.1.16}.
\end{proof}
...
\begin{proof}
  Since \(T\) is a linear transformations from \(\R^n\) to \(\R^m\), by \labelcrefs{ii:6.1.13} we know that there exists an \(m \times n\) matrix such that
  \[
  ...
  \]
  Since \(x_0\) was arbitrary, by \labelcrefs{ii:2.1.1} this means \(T\) is continuous on \(\R^n\) from \((\R^n, d_{l^1}|_{\R^n \times \R^n})\) to \((\R^m, d_{l^1}|_{\R^m \times \R^m})\).
\end{proof}
