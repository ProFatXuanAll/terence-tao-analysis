...
\begin{proof}
  By \labelcrefs{ii:6.2.2} we want to show that
  \[
  ...
  \end{align*}
  Since \(g'\big(f(x_0)\big)\) is a linear transformation, by \labelcrefs{ii:6.1.6} we know that
  \begin{align*}
     & \norm*{g'\big(f(x_0)\big) \big(f(x) - f(x_0)\big) - \Big(g'\big(f(x_0)\big) \circ f'(x_0)\Big) (x - x_0)} \\
     & = \norm*{g'\big(f(x_0)\big) \big(f(x) - f(x_0) - f'(x_0) (x - x_0)\big)}.
  \end{align*}
  By \labelcrefs{ii:ex:6.1.4} we know that
  \begin{align*}
    \exists M \in \R^+ : & \norm*{g'\big(f(x_0)\big) \big(f(x) - f(x_0) - f'(x_0) (x - x_0)\big)} \\
                         & \leq M \norm*{f(x) - f(x_0) - f'(x_0) (x - x_0)}                       \\
                         & \leq M \varepsilon \norm*{x - x_0}.
...
\begin{note}
  As a corollary of the chain rule and \labelcrefs{ii:6.1.16} (and \labelcrefs{ii:6.1.13}), we see that
  \[
...
\begin{eg}\label{ii:6.4.2}
  ...
  \begin{align*}
     ...
     & \leq \norm*{f(x) - f(x_0) - (x - x_0) \nabla f(x_0)^\top} + \norm*{g(x) - g(x_0) - (x - x_0) \nabla g(x_0)^\top}
  \end{align*}
  (note that the last line follow by \labelcrefs{ii:ex:1.1.8}),
  ...
  by \labelcrefs{ii:6.2.2} we know that the identity is true.
  ...
  \[
    \nabla (fg) = g \nabla f + f \nabla g.
  \]
  A similar argument gives the sum rule \(\nabla (f + g) = \nabla f + \nabla g\), or the difference rule \(\nabla (f - g) = \nabla f - \nabla g\), as well as the quotient rule (\labelcrefs{ii:ex:6.4.4}).
\end{eg}

\begin{note}
  We do record one further useful application of the chain rule.
  Let \(T : \R^n \to \R^m\) be a linear transformation.
  From \labelcrefs{ii:ex:6.4.1} we observe that \(T\) is continuously differentiable at every point, and in fact \(T'(x) = T\) for every \(x\).
...
\begin{proof}
  ...
  \end{align*}
  Since \(t_0\) was arbitrary, by \labelcrefs{ii:6.2.2} we know that
  \[
    \forall t \in \R, D h(t) = \big(x_1'(t), \dots, x_n'(t)\big)^\top.
  \]
...
\begin{proof}
  ...
  \begin{align*}
     ...
     & = 0
  \end{align*}
  and \(x_0\) was arbitrary, we conclude by \labelcrefs{ii:6.2.2} that
  \[
    \forall x \in \R^n, T'(x) = T.
  \]
  By \labelcrefs{ii:6.1.13} we know that
  \[
...
\begin{proof}
  By \labelcrefs{ii:6.2.2} we have
  \begin{align*}
    ...
    \implies & \forall \varepsilon \in \R^+, \exists \delta_1 \in \R^+ : \forall x \in E,                                                                 \\
             & \bigg(\norm*{x - x_0} < \delta_1 \implies \norm*{f(x) - f(x_0)} < \dfrac{\varepsilon}{2} \norm*{x - x_0} + \norm*{f'(x_0)(x - x_0)}\bigg).
  \end{align*}
  By \labelcrefs{ii:ex:6.1.4} we know that every linear transformation is continuous.
  Thus, we know that
  \begin{align*}
...
\begin{ex}\label{ii:ex:6.4.3}
  Prove \labelcrefs{ii:6.4.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:6.4.1}.
\end{proof}
...
\begin{proof}
  ...
  Since \((a, b)\) was arbitrary, by \labelcrefs{ii:6.2.2} we conclude that
  \[
    \forall (a, b) \in \R \times (\R \setminus \set{0}), D k(a, b) = \bigg(\dfrac{1}{b}, \dfrac{-a}{b^2}\bigg).
  \]

  Let \(h\) be the function in \labelcrefs{ii:6.4.2}.
  Using chain rule we have
  \begin{align*}
...
\begin{proof}
  ...
  \]
  and \(\dfrac{\partial f}{\partial x_i}\) is continuous on \(\R^3 \setminus \set{0_{\R^3}}\), by \labelcrefs{ii:6.3.8} we know that
  \begin{align*}
    \forall y \in \R^3 \setminus \set{0_{\R^3}}, D f(y) & = \nabla f(y)                                                                                                               \\
                                                        ...
                                                        & = \dfrac{y}{\norm*{y}}.
  \end{align*}
  By chain rule (\labelcrefs{ii:6.4.1}) we know that
  \begin{align*}
