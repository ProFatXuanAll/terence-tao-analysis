...
\begin{note}
  As we have just seen, continuous functions can be very badly behaved, for instance they can be nowhere differentiable (\labelcrefs{ii:3.7.4}).
...
\begin{proof}
  ...
  \]
  The function \(g\) is continuous on \([0, 1]\) since \(y \mapsto \dfrac{y - a}{b - a}\) is bijective on \([a, b]\), and so by \labelcrefs{ii:3.8.19} we may find a polynomial \(Q : [0, 1] \to \R\) such that \(\abs{Q(x) - g(x)} \leq \varepsilon\) for all \(x \in [0, 1]\).
...
\begin{note}
  Another way of stating \labelcrefs{ii:3.8.3} is as follows.
  ...
  or in other words, that the closure of the space of polynomials is the space of continuous functions (see \labelcrefs{ii:3.3.2}):
  \[
    \overline{P([a, b] \to \R)}_{\big(C([a, b] \to \R), d_\infty\big)} = C([a, b] \to \R).
  \]
  In particular, every continuous function on \([a, b]\) is the uniform limit of polynomials (see \labelcrefs{ii:3.4.4}).
...
\begin{proof}
  ...
  By \labelcrefs{ii:3.8.4} we know that \(g\) is supported on \([-1, 1]\).
  ...
  By \labelcrefs{ii:ex:3.8.2.b} we know that
  \[
  ...
  By \labelcrefs{ii:3.8.4} we have
  \[
  ...
  \end{align*}
  combine all the proofs above we conclude by \labelcrefs{ii:3.8.6} that \(f\) is an \((\varepsilon, \delta)\)-approximation to the identity.
\end{proof}
...
\begin{note}
  If \(f\) and \(g\) are continuous and compactly supported, then for each \(x\) the function \(f(y) g(x - y)\) (thought of as a function of \(y\)) is also continuous and compactly supported, so \labelcrefs{ii:3.8.9} makes sense.
\end{note}
...
\begin{prop}[Basic properties of convolution]\label{ii:3.8.11}
  ...
  \begin{enumerate}
    \item\label{ii:3.8.11.a} The convolution \(f * g\) is also a continuous, compactly supported function.
    \item\label{ii:3.8.11.b} (Convolution is commutative)
    We have \(f * g = g * f\);
    in other words
    \begin{align*}
      f * g(x) & = \int_{-\infty}^\infty f(y) g(x - y) \; dy \\
               ...
               & = g * f(x).
    \end{align*}
    \item\label{ii:3.8.11.c} (Convolution is linear)
    We have \(f * (g + h) = f * g + f * h\).
    Also, for any real number \(c\), we have \(f * (cg) = (cf) * g = c(f * g)\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{ii:3.8.11.a}]
  Since \(f, g\) are compactly supported, by \labelcrefs{ii:3.8.4} we know that
  \[
  ...
  \]
  Thus, by \labelcrefs{ii:3.8.4} \(f, g\) are supported on \([-M, M]\) and \([-2M, 2M]\).
  Observe that
  \begin{align*}
  ...
  Since \(z_x\) is continuous on \(\R\), by \labelcrefs{ii:3.8.4} and \labelcrefs{ii:3.8.9} we have
  \begin{align*}
  ...
  \end{align*}
  Since \(f, g\) are compactly supported and continuous on \(\R\), by \labelcrefs{ii:ex:3.8.3} we know that
  \[
...
\begin{proof}[\pf{ii:3.8.11.c}]
  Let \(x_0 \in \R\).
  Since \(g, h\) are compactly supported, by \labelcrefs{ii:3.8.4} we know that
  \[
  ...
  By \labelcrefs{ii:3.8.11.b} we thus have
  \[
...
\begin{rmk}\label{ii:3.8.12}
  ...
  \(f * \delta = \delta * f = f\).
  These results are slightly harder to prove than the ones in \labelcrefs{ii:3.8.11}, however, and we will not need them in this text.
\end{rmk}
...
\begin{proof}
  Since \(g\) is an \((\varepsilon, \delta)\)-approximation to the identity, by \labelcrefs{ii:3.8.6} we have
  \begin{itemize}
  ...
  \end{itemize}
  Since \(f\) is continuous on \(\R\), by \labelcrefs{ii:3.8.9} we have
  \begin{align*}
  ...
  \end{align*}
  By \labelcrefs{ii:ex:3.8.6} we have
  \[
...
\begin{proof}
  Let \(\varepsilon \in \R^+\).
  Since \(f\) is continuous on \(\R\) and supported on \([0, 1]\), by \labelcrefs{ii:ex:3.8.3} we know that \(f\) is bounded by some \(M \in \R^+\) and \(f\) is uniformly continuous on \(\R\).
  This means
  \[
  ...
  By \labelcrefs{ii:3.8.8} we know that there exists a polynomial \(P\) on \([-1, 1]\) such that \(P\) is an \((\dfrac{\varepsilon}{1 + 4M}, \delta)\)-approximation to the identity.
  By \labelcrefs{ii:3.8.6} we know that \(P\) is continuous on \(\R\) and supported on \([-1, 1]\).
  Since \(f\) is continuous on \(\R\) and supported on \([0, 1]\), by \labelcrefs{ii:3.8.13} we know that \(f * P\) is a polynomial on \([0, 1]\).
  Then by \labelcrefs{ii:3.8.14} we have
  \[
...
\begin{rmk}\label{ii:3.8.17}
  The function \(F\) obtained in \labelcrefs{ii:3.8.16} is sometimes known as the \emph{extension of \(f\) by zero}.
\end{rmk}
...
\begin{proof}
  Using \labelcrefs{ii:3.8.6} we can define an \(F : \R \to \R\) such that
  \[
  ...
  Then by \labelcrefs{ii:3.8.15} we have
  \[
    \forall \varepsilon \in \R^+, \exists P \in \R^{\R} : \begin{dcases}
...
\begin{proof}
  ...
  By \labelcrefs{ii:3.8.18}, we can thus find a polynomial \(Q : [0, 1] \to \R\) such that \(\abs{Q(x) - F(x)} \leq \varepsilon\) for all \(x \in [0, 1]\).
  But
  \[
...
\begin{ex}\label{ii:ex:3.8.1}
  Prove \labelcrefs{ii:3.8.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.8.5}.
\end{proof}

\begin{ex}\label{ii:ex:3.8.2}
  \quad
  \begin{enumerate}
    \item\label{ii:ex:3.8.2.a} Prove that for any real number \(0 \leq y \leq 1\) and any natural number \(n \geq 0\), that \((1 - y)^n \geq 1 - ny\).
    \item\label{ii:ex:3.8.2.b} Show that \(\int_{-1}^1 (1 - x^2)^n \; dx \geq \dfrac{1}{\sqrt{n}}\).
    \item\label{ii:ex:3.8.2.c} Prove \labelcrefs{ii:3.8.8}.
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:3.8.2.a}]
...
\begin{proof}[\pf{ii:ex:3.8.2.b}]
  ...
  \]
  By \labelcrefs{ii:ex:3.8.2.a} we have
  \[
...
\begin{proof}[\pf{ii:ex:3.8.2.c}]
  See \labelcrefs{ii:3.8.8}.
\end{proof}
...
\begin{proof}
  Since \(f\) is compactly supported, by \labelcrefs{ii:3.8.4} we know that there exists some \(a, b \in \R\) such that
  \[
    \forall x \notin [a, b], f(x) = 0.
  \]
  Since \([a, b]\) is closed and bounded in \((\R, d_{l^1}|_{\R \times \R})\), by \labelcrefs{ii:1.5.7} we know that \(\big([a, b], d_{l^1}|_{\R \times \R}\big)\) is compact.
  Since \(\big([a, b], d_{l^1}|_{\R \times \R}\big)\) is compact and \(f\) is continuous on \([a, b]\), by \labelcrefs{ii:2.3.2} we know that \(f\) is bounded.
  Since \(f\) is bounded and continuous on \([a, b]\), by \labelcrefs{ii:2.3.5} \(f\) is uniformly continuous on \([a, b]\).
  ...
  Suppose that \(a \neq b\).
  Since \(f\) in uniformly continuous on \([a, b]\), by \labelcrefs{ii:2.3.4} we have
  \[
...
\begin{ex}\label{ii:ex:3.8.4}
  Prove \labelcrefs{ii:3.8.11}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.8.11}.
\end{proof}
...
\begin{ex}\label{ii:ex:3.8.6}
  \quad
  \begin{enumerate}
    \item\label{ii:ex:3.8.6.a} Let \(g\) be an \((\varepsilon, \delta)\) approximation to the identity.
    Show that \(1 - 2 \varepsilon \leq \int_{[-\delta, \delta]} g \leq 1\).
    \item\label{ii:ex:3.8.6.b} Prove \labelcrefs{ii:3.8.14}.
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:3.8.6.a}]
  By \labelcrefs{ii:3.8.6} we know that
  \begin{itemize}
...
\begin{proof}[\pf{ii:ex:3.8.6.b}]
  See \labelcrefs{ii:3.8.14}.
\end{proof}

\begin{ex}\label{ii:ex:3.8.7}
  Prove \labelcrefs{ii:3.8.15}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.8.15}.
\end{proof}
...
\begin{proof}
  ...
  Then by \labelcrefs{ii:3.8.1} we have
  \[
  ...
  \begin{align*}
    \int_{[0, 1]} f(x) P(x) \; dx & = \int_{[0, 1]} f(x) \sum_{j = 0}^n c_j x^j \; dx \\
                                  ...
                                  & = 0.
  \end{align*}
  Since \([0, 1]\) is closed and bounded in \((\R, d_{l^1}|_{\R \times \R})\), by \labelcrefs{ii:1.5.7} we know that \(\big([0, 1], d_{l^1}|_{\R \times \R}\big)\) is compact.
  By \labelcrefs{ii:2.3.2} we know that \(f\) is bounded, i.e., there exists a \(M \in \R^+\) such that \(\abs{f(x)} \leq M\) for all \(x \in [0, 1]\).

  Since \(f\) is continuous on \([0, 1]\), by \labelcrefs{ii:3.8.3} we know that
  \[
    \forall \varepsilon \in \R^+, \exists P \in \R^{\R} : \begin{dcases}
      P \text{ is a polynomial on } [0, 1] \\
...
\begin{ex}\label{ii:ex:3.8.9}
  Prove \labelcrefs{ii:3.8.16}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.8.16}.
\end{proof}
