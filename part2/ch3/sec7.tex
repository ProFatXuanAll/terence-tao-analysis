...
\begin{proof}
  ...
  Now, since each \(f_n'\) is continuous on \([a, b]\), and \(f_n'\) converges uniformly to \(g\), we see by \labelcrefs{ii:3.3.2} that \(g\) is also continuous.
  By \labelcrefs{ii:3.6.1} we have
  \[
  ...
  Since \((f_n')_{n = 1}^\infty\) converges uniformly to \(g\) on \([a, b]\) with respect to \(d_{l^1}|_{\R \times \R}\), by \labelcrefs{ii:3.2.7} we have
  \begin{align*}
  ...
  \]
  and by \labelcrefs{ii:3.2.7} \((f_n)_{n = 1}^\infty\) converges uniformly to \(f\) with respect to \(d_{l^1}|_{\R \times \R}\).

  Since \(f_n\) is continuous on \([a, b]\) for each \(n \in \Z^+\), by \labelcrefs{ii:3.3.2} we know that \(f\) is also continuous on \([a, b]\).
...
\begin{note}
  Informally, \labelcrefs{ii:3.7.1} says that if \(f_n'\) converges uniformly, and \(f_n(x_0)\) converges for some \(x_0\), then \(f_n\) also converges uniformly, and
  \[
...
\begin{rmk}\label{ii:3.7.2}
  It turns out that \labelcrefs{ii:3.7.1} is still true when the functions \(f_n'\) are not assumed to be continuous, but the proof is more difficult;
  see \labelcrefs{ii:ex:3.7.2}.
\end{rmk}
...
\begin{cor}\label{ii:3.7.3}
  ...
  \]
  is the sup norm of \(f_n'\), as defined in \labelcrefs{ii:3.5.5}.
...
\begin{proof}
  ...
  By \labelcrefs{ii:ex:3.5.1} we know that
  \[
    \forall N \in \Z^+, F_N' = \sum_{n = 1}^N f_n' \in C([a, b] \to \R).
  \]
  Since \(\sum_{n = 1}^\infty \norm*{f_n'}_\infty\) converges and \(f_n' \in C\big([a, b] \to \R\big)\) for each \(n \in \Z^+\), by \labelcrefs{ii:3.5.7} we know that there exists some \(G : [a, b] \to \R\) such that \(\big(\sum_{n = 1}^N f_n'\big)_{N = 1}^\infty\) converges uniformly to \(G\) on \([a, b]\) with respect to \(d_{l^1}|_{\R \times \R}\).
  ...
  \]
  by \labelcrefs{ii:3.7.1} we know that there exists some \(F : [a, b] \to \R\) such that \((F_N)_{N = 1}^\infty\) converges uniformly to \(F\) on \([a, b]\) with respect to \(d_{l^1}|_{\R \times \R}\) and \(F' = G\).
  Then we have
  \begin{align*}
...
\begin{note}
  \labelcrefs{ii:3.7.4} was discovered by Weierstrass.
\end{note}
...
\begin{ex}\label{ii:ex:3.7.1}
  Complete the proof of \labelcrefs{ii:3.7.1}.
  Compare this theorem with Example 1.2.10 in Analysis I, and explain why this example does not contradict the theorem.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.7.1}.
  Since \(\lim_{n \to \infty} \dfrac{x^3}{\dfrac{1}{n} + x^2}\) is not continuous at \(x = 0\), is does not contradict to \labelcrefs{ii:3.7.1}.
\end{proof}

\begin{ex}\label{ii:ex:3.7.2}
  Prove \labelcrefs{ii:3.7.1} without assuming that \(f_n'\) is continuous.
  ...
  Use this to show that if \(d_\infty(f_n', f_m') \leq \varepsilon\), then \(\abs{\big(f_n(x) - f_m(x)\big) - \big(f_n(x_0) - f_m(x_0)\big)} \leq \varepsilon \abs{x - x_0}\) for all \(x \in [a, b]\), and then use this to complete the proof of \labelcrefs{ii:3.7.1}.)
\end{ex}
...
\begin{proof}
  ...
  \end{align*}
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{ii:3.4.2} we have
  \begin{align*}
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:1.4.6} \((f_n)_{n = 1}^\infty\) is a Cauchy sequence in \(\big(C([a, b] \to \R), d_{C([a, b] \to \R)}\big)\).
  Since \((\R, d_{l^1}|_{\R \times \R})\) is complete, by \labelcrefs{ii:3.4.5} we know that there exists some \(f : [a, b] \to \R\) such that
  \[
    \begin{dcases}
      f \in C([a, b] \to \R) \\
    ...
    \end{dcases}
  \]
  By \labelcrefs{ii:3.4.4} we know that \((f_n)_{n = 1}^\infty\) convergent uniformly to \(f\) on \([a, b]\) with respect to \(d_{l^1}|_{\R \times \R}\).
...
\begin{ex}\label{ii:ex:3.7.3}
  Prove \labelcrefs{ii:3.7.3}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.7.3}.
\end{proof}
