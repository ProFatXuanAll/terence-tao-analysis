...
\begin{note}
  Comparing this with \labelcrefs{ii:2.1.1}, we see that \(f\) is continuous at \(x_0\) iff
  \[
...
\begin{prop}\label{ii:3.1.5}
  ...
  \begin{enumerate}
    \item\label{ii:3.1.5.a} \(\lim_{x \to x_0 ; x \in E} f(x) = L\).
    \item\label{ii:3.1.5.b} For every sequence \((x^{(n)})_{n = 1}^\infty\) in \(E\) which converges to \(x_0\) with respect to the metric \(d_X\), the sequence \(\big(f(x^{(n)})\big)_{n = 1}^\infty\) converges to \(L\) with respect to the metric \(d_Y\).
    \item\label{ii:3.1.5.c} For every open set \(V \subseteq Y\) which contains \(L\), there exists an open set \(U \subseteq X\) containing \(x_0\) such that \(f(U \cap E) \subseteq V\).
    \item\label{ii:3.1.5.d} If one defines the function \(g : E \cup \set{x_0} \to Y\) by defining \(g(x_0) \coloneqq L\), and \(g(x) \coloneqq f(x)\) for \(x \in E \setminus \set{x_0}\), then \(g\) is continuous at \(x_0\).
...
\begin{proof}
  ...
  \]
  By \labelcrefs{ii:3.1.1} we have
  \[
  ...
  \]
  By \labelcrefs{ii:1.1.14} we have \(\lim_{n \to \infty} d_Y\big(f(x^{(n)}), L\big) = 0\).
  ...
  \]
  Then by \labelcrefs{ii:3.1.1} we have
  \[
  ...
  \begin{align*}
    ...
    \implies & \exists \delta \in \R^+ :                                                                                                    \\
             & \begin{dcases}
               ...
               \end{dcases}
  \end{align*}
  and by \labelcrefs{ii:1.2.15.c} we know that \(B_{(X, d_X)}(x_0, \delta)\) is open in \((X, d_X)\).
  ...
  \]
  Let \(\varepsilon \in \R^+\).
  By \labelcrefs{ii:1.2.15.c} we know that \(B_{(Y, d_Y)}(L, \varepsilon)\) is open in \((Y, d_Y)\).
  ...
  \end{align*}
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{ii:3.1.1} we have
  \[
    d_Y - \lim_{x \to x_0 ; x \in E} f(x) = L
  \]
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:2.1.1} \(g\) is continuous at \(x_0\) from \((E \cup \set{x_0}, d_X|_{(E \cup \set{x_0}) \times (E \cup \set{x_0})})\) to \((Y, d_Y)\).

  Now suppose that \(x_0 \in E\).
  We claim that \(f(x_0) = L\).
  Suppose for the sake of contradiction that \(f(x_0) \neq L\).
  Then by \labelcrefs{ii:1.1.2.b} we have \(d_Y\big(f(x_0), L\big) > 0\).
  ...
  \begin{align*}
    ...
    \implies & \forall \varepsilon \in \R^+, \exists \delta \in \R^+ :                                                         \\
             & \Big(\forall x \in E, d_X(x, x_0) < \delta \implies d_Y\big(f(x), L\big) < \varepsilon\Big).
  \end{align*}
  By \labelcrefs{ii:3.1.1} this means
  \[
    d_Y - \lim_{x \to x_0 ; x \in E} f(x) = L.
  \]
...
\begin{rmk}\label{ii:3.1.6}
  Observe from \labelcrefs{ii:3.1.5.b} and \labelcrefs{ii:1.1.20} that a function \(f(x)\) can converge to at most one limit \(L\) as \(x\) converges to \(x_0\).
...
\begin{proof}
  Let \(L \in Y\).
  By \labelcrefs{ii:1.1.2.a} we know that
  \[
...
\begin{ex}\label{ii:ex:3.1.2}
  Prove \labelcrefs{ii:3.1.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.1.5}.
\end{proof}

\begin{ex}\label{ii:ex:3.1.3}
  Use \labelcrefs{ii:3.1.5.c} to define a notion of a limiting value of a function \(f : E \to Y\) from one topological space \((X, \mathcal{F}_X)\) to another \((Y, \mathcal{F}_Y)\) where \(E \subseteq X\).
  If \(X\) is a topological space and \(Y\) is a Hausdorff topological space (see \labelcrefs{ii:ex:2.5.4}), prove the equivalence of \labelcrefs{ii:3.1.5.c,ii:3.1.5.d} in this setting, as well as an analogue of \labelcrefs{ii:3.1.6}.
...
\begin{proof}
  ...
  \[
    ...
    \end{dcases}
  \]
  Then by \labelcrefs{ii:2.5.7} we have \(U \cap (E \cup \set{x_0}) \in \mathcal{F}_{E \cup \set{x_0}}\) and
  \begin{align*}
    g\big(U \cap (E \cup \set{x_0})\big) & = g\big((U \cap E) \cup \set{x_0}\big)                        \\
                                         ...
                                         & \subseteq V.
  \end{align*}
  Since \(V\) was arbitrary, by \labelcrefs{ii:2.5.8} we know that \(g\) is continuous at \(x_0\) from \((E \cup \set{x_0}, \mathcal{F}_{E \cup \set{x_0}})\) to \((Y, \mathcal{F}_Y)\).

  Next suppose that \(x_0 \in E\) and \(f(x)\) converges to \(L\) in \(Y\) as \(x\) converges to \(x_0\) in \(E\).
  We want to show that \(f(x_0) = L\).
  Suppose for the sake of contradiction that \(f(x_0) \neq L\).
  Since \((Y, \mathcal{F}_Y)\) is Hausdorff, by \labelcrefs{ii:ex:2.5.4} we know that
  \[
  ...
  \[
    ...
    \end{dcases}
  \]
  By \labelcrefs{ii:2.5.1} we know that \(U_V \cap U_W \in \mathcal{F}_X\).
  But then we have
  \[
    \begin{dcases}
      x_0 \in U_V \cap U_W                                       \\
  ...
  By \labelcrefs{ii:2.5.8} we know that
  \[
    ...
    \end{dcases}
  \]
  By \labelcrefs{ii:2.5.7} we know that
  \[
...
\begin{ex}\label{ii:ex:3.1.4}
  Recall from \labelcrefs{ii:ex:2.5.5} that the extended real line \(\R^*\) comes with a standard topology (the order topology).
  ...
  Show that \(\lim_{n \to +\infty ; n \in \N} a_n = L\) (in the sense of \labelcrefs{ii:ex:3.1.3}) iff \(\lim_{n \to \infty} a_n = L\) (in the sense of \labelcrefs{ii:2.5.4}).
  This shows that the notions of limiting values of a sequence, and limiting values of a function, are compatible.
\end{ex}

\begin{proof}
  Let \((\R^*, \mathcal{F}_{\R^*})\) be the order topology in \labelcrefs{ii:ex:2.5.5}.
  Let \(f : \N \to Y\) be the function where \(f(n) = a_n\) for each \(n \in \N\).
  First, suppose that
  \[
    \lim_{n \to +\infty ; n \in \N} a_n = \lim_{n \to +\infty ; n \in \N} f(n) = L.
  \]
  By \labelcrefs{ii:ex:3.1.3} we have
  \[
    ...
    \end{dcases}
  \]
  Since \(U \in \mathcal{F}_{\R^*}\), by \labelcrefs{ii:ex:2.5.5} we know that there exists an interval \(I \subseteq \R^*\) such that \(I \subseteq U\) and \(+\infty \in I\).
  ...
  \]
  in the sense of \labelcrefs{ii:2.5.4}.
  ...
  \]
  Let \(I = (N, +\infty]\).
  Then we know that \(I\) is an interval of \(\R^*\) and by \labelcrefs{ii:ex:2.5.5} we have \(I \in \mathcal{F}_{\R^*}\).
  Observe that
  \begin{align*}
  ...
  \end{align*}
  Since this is true for arbitrary \(V \in \mathcal{F}_Y\), by \labelcrefs{ii:ex:3.1.3} we have
  \[
    \lim_{n \to +\infty ; n \in \N} a_n = \lim_{n \to +\infty ; n \in \N} f(n) = L.
  \]
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  and by \labelcrefs{ii:3.1.1} \(d_Z - \lim_{x \to x_0 ; x \in E} g \circ f(x) = z_0\).
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    \end{dcases}
  \]
  By \labelcrefs{ii:3.1.5.c} we know that \(f^*\) and \(g^*\) are continuous at \(x_0\) from \((X, d)\) to \((\R, d_1)\).
  Thus, by \labelcrefs{ii:2.2.3} we have
  \begin{align*}
    ...
    \forall x \in E \cup \set{x_0}, & (f^* + g^*)(x) = \begin{dcases}
                                    ...
                                    & (f^* / g^*)(x) = \begin{dcases}
                                                       ...
                                                       \end{dcases} \text{ when } \begin{dcases}
                                                                                  ...
                                                                                  \end{dcases}
  \end{align*}
  by \labelcrefs{ii:3.1.5.a,ii:3.1.5.d} we know that
  \begin{align*}
     & d_1 - \lim_{x \to x_0 ; x \in E} (f + g)(x) = L + M                                          \\
     & d_1 - \lim_{x \to x_0 ; x \in E} (f - g)(x) = L - M                                          \\
     & d_1 - \lim_{x \to x_0 ; x \in E} (fg)(x) = LM                                                \\
