\section{Uniform convergence and continuity}\label{ii:sec:3.3}

\begin{thm}[Uniform limits preserve continuity I]\label{ii:3.3.1}
...
\begin{proof}
  ...
  For each \(n \in \Z^+\), since \(f^{(n)}\) is continuous at \(x_0\) from \((X, d_X)\) to \((Y, d_Y)\), by \labelcrefs{ii:2.1.1} we have
  \begin{align*}
...
\begin{cor}[Uniform limits preserve continuity II]\label{ii:3.3.2}
...
\begin{proof}
  By applying \labelcrefs{ii:3.3.1} to each \(x \in X\) we conclude that \(f\) is continuous on \(X\) from \((X, d_X)\) to \((Y, d_Y)\).
\end{proof}

\begin{prop}[Interchange of limits and uniform limits]\label{ii:3.3.3}
  Let
...
\begin{proof}
  ...
  Since \((Y, d_Y)\) is complete, by \labelcrefs{ii:1.4.10} it suffices to show that \((L^{(n)})_{n = 1}^\infty\) is a Cauchy sequence in \((Y, d_Y)\).
  Let \(n_1, n_2 \in \Z^+\).
  Then by \labelcrefs{ii:3.2.7} we have
  \begin{align*}
  ...
  Since \(L^{(n)}\) exists for each \(n \in \N\), by \labelcrefs{ii:3.1.1} we have
  \begin{align*}
  ...
  \]
  and by \labelcrefs{ii:1.4.6} \((L^{(n)})_{n = 1}^\infty\) is a Cauchy sequence in \((Y, d_Y)\).

  Let \(L \in Y\) such that \(d_Y - \lim_{n \to \infty} L^{(n)} = L\).
  Again by \labelcrefs{ii:3.2.7} we have
  \begin{align*}
             & (f^{(n)})_{n = 1}^\infty \text{ converges uniformly to } f \text{ on } X \text{with respect to } d_Y                                              \\
  ...
  Since \(L\) exists, by \labelcrefs{ii:3.1.1} we have
  \begin{align*}
  ...
  \end{align*}
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{ii:3.1.1} we know that \(d_Y - \lim_{x \to x_0 ; x \in E} f(x) = L\).
\end{proof}

\begin{prop}\label{ii:3.3.4}
...
\begin{proof}
  ...
  \]
  By \labelcrefs{ii:3.3.1} we know that \(f\) is continuous at \(x_0\) from \((X, d_X)\) to \((Y, d_Y)\).
  Thus, by \labelcrefs{ii:2.1.4.a,ii:2.1.4.b} we have
  \[
    \lim_{n \to \infty} d_Y\big(f(x^{(n)}), f(x_0)\big) = 0
  \]
  and by \labelcrefs{ii:1.1.14} we have
  \[
  ...
  Since \((f^{(n)})_{n = 1}^\infty\) converges uniformly to \(f\) on \(X\) with respect to \(d_Y\), by \labelcrefs{ii:3.2.7} we have
  \begin{align*}
  ...
  \end{align*}
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{ii:1.1.14} we know that
  \[
...
\begin{defn}[Bounded functions]\label{ii:3.3.5}
...
\begin{prop}[Uniform limits preserve boundedness]\label{ii:3.3.6}
  Let \((f^{(n)})_{n = 1}^\infty\) be a sequence of functions from one metric space \((X, d_X)\) to another \((Y, d_Y)\), and suppose that this sequence converges uniformly to another function \(f : X \to Y\).
  If the functions \(f^{(n)}\) are bounded on \(X\) for each \(n\), then the limiting function \(f\) is also bounded on \(X\).
\end{prop}

\begin{proof}
  Since \(f^{(n)}\) is bounded in \((Y, d_Y)\) for each \(n \in \Z^+\), by \labelcrefs{ii:3.3.5} and \labelcrefs{ii:1.5.3} we have
  \begin{align*}
  ...
  \]
  and by \labelcrefs{ii:1.5.3} and \labelcrefs{ii:3.3.5} \(f\) is bounded in \((Y, d_Y)\).
\end{proof}

\begin{rmk}\label{ii:3.3.7}
...
\begin{ex}\label{ii:ex:3.3.1}
  Prove \labelcrefs{ii:3.3.1}.
  Explain briefly why your proof requires uniform convergence, and why pointwise convergence would not suffice.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.3.1}.
  Without uniform convergence we cannot make \(f^{(n)}(x)\) and \(f(x)\) arbitrary close.
\end{proof}

\begin{ex}\label{ii:ex:3.3.2}
  Prove \labelcrefs{ii:3.3.3}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.3.3}.
\end{proof}

\begin{ex}\label{ii:ex:3.3.3}
  Compare \labelcrefs{ii:3.3.3} with Example 1.2.8 in Analysis I.
  Can you now explain why the interchange of limits in Example 1.2.8 in Analysis I led to a false statement, whereas the interchange of limits in \labelcrefs{ii:3.3.3} is justified?
\end{ex}

\begin{proof}
  By \labelcrefs{ii:ex:3.2.2.b} we know that \(f^{(n)}(x) = x^{(n)}\) does not converge uniformly to any function \(f : (-1, 1) \to \R\), thus the interchange of limits in Example 1.2.8 in Analysis I failed.
\end{proof}

\begin{ex}\label{ii:ex:3.3.4}
  Prove \labelcrefs{ii:3.3.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.3.4}.
\end{proof}

\begin{ex}\label{ii:ex:3.3.5}
  Give an example to show that \labelcrefs{ii:3.3.4} fails if the phrase ``converges uniformly'' is replaced by ``converges pointwise.''
\end{ex}
...
\begin{proof}
  ...
  By \labelcrefs{ii:ex:3.2.2.b} we know that \(f^{(n)}(x) = x^{(n)}\) does not converge uniformly to \(f\) on \(X\) with respect to \(d_{l^1}|_{\R \times \R}\).
  ...
  \]
  Thus, \labelcrefs{ii:3.3.4} fails when the phrase ``converges uniformly'' is replaced by ``converges pointwise.''
\end{proof}

\begin{ex}\label{ii:ex:3.3.6}
  Prove \labelcrefs{ii:3.3.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.3.6}.
\end{proof}

\begin{ex}\label{ii:ex:3.3.7}
  Give an example to show that \labelcrefs{ii:3.3.6} fails if the phrase ``converges uniformly'' is replaced by ``converges pointwise.''
\end{ex}

\begin{proof}
  By \labelcrefs{ii:ex:3.2.2.c} we know that \(g\) is unbounded since
  \[
...
\begin{proof}
  ...
  Since \((f_n)_{n = 1}^\infty\) and \((g_n)_{n = 1}^\infty\) are uniformly bounded, by \labelcrefs{ii:3.3.5} we know that \(f_n\) and \(g_n\) are bounded in \((\R, d_1)\) for each \(n \in \Z^+\).
  By \labelcrefs{ii:3.3.6} we know that \(f\) and \(g\) are bounded in \((\R, d_1)\), i.e.,
  \[
    ...
    \end{dcases}
  \]
  Since \((f_n)_{n = 1}^\infty\) and \((g_n)_{n = 1}^\infty\) converge uniformly to \(f\) on \(X\) with respect to \(d_1\), by \labelcrefs{ii:3.2.7} we have
  \begin{align*}
  ...
  \]
  and by \labelcrefs{ii:3.2.7} \((f_n g_n)_{n = 1}^\infty\) converges uniformly to \(fg\) on \(X\) with respect to \(d_1\).
\end{proof}
