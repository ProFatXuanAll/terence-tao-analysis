...
\begin{defn}[Sup norm]\label{ii:3.5.5}
  ...
  \]
  In other words, \(\norm*{f}_\infty = d_\infty(f, 0)\), where \(0 : X \to \R\) is the zero function \(0(x) \coloneqq 0\), and \(d_\infty\) was defined in \labelcrefs{ii:3.4.2}.
...
\begin{proof}
  ...
  Since \(f^{(n)} \in C(X \to \R)\), by \labelcrefs{ii:ex:3.5.1} we know that \(\sum_{n = 1}^N f^{(n)} \in C(X \to \R)\) for each \(N \in \Z^+\).
  ...
  \]
  By \labelcrefs{ii:1.4.6} \(\bigg(\sum_{n = 1}^N f^{(n)}\bigg)_{N = 1}^\infty\) is a Cauchy sequence in \(\big(C(X \to \R), d_{C(X \to \R)}\big)\).
  Since \((\R, d_{l^1}|_{\R \times \R})\) is complete, by \labelcrefs{ii:3.4.5} we know that \(\bigg(\sum_{n = 1}^N f^{(n)}\bigg)_{N = 1}^\infty\) converges uniformly to some \(f \in C(X \to \R)\) on \(X\) with respect to \(d_{l^1}|_{\R \times \R}\).
\end{proof}
...
\begin{eg}\label{ii:3.5.8}
  ...
  in \labelcrefs{ii:ex:3.2.2.c} we see that this function must in fact be the function \(f : [-r, r] \to \R\) defined by \(f(x) \coloneqq x / (1 - x)\).
...
\begin{proof}
  ...
  Then by \labelcrefs{ii:ac:2.2.1}
  \[
  ...
  Then by \labelcrefs{ii:ex:2.3.5}
  \[
    \sum_{n = 1}^{N + 1} f^{(n)} = \bigg(\sum_{n = 1}^N f^{(n)}\bigg) \oplus f^{(N + 1)}
  \]
...
\begin{ex}\label{ii:ex:3.5.2}
  Prove \labelcrefs{ii:3.5.7}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.5.7}.
\end{proof}
