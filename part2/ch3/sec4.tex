...
\begin{note}
  ...
  \begin{enumerate}
    \item limits \(\lim_{n \to \infty} x^{(n)}\) of sequences of points in a metric space
          (\labelcrefs{ii:1.1.14};
          see also \labelcrefs{ii:2.5.4});
...
\begin{rmk}\label{ii:3.4.1}
  If one is willing to work in topological spaces instead of metric spaces, we can also view (a) as a special case of (b), see \labelcrefs{ii:ex:3.1.4}, and (c) is also a special case of (a), see \labelcrefs{ii:ex:3.4.4}.
...
\begin{note}
  ...
  \labelcrefs{ii:3.3.2} asserts that this space \(C(X \to Y)\) is closed in \(\big(B(X \to Y), d_{B(X \to Y)}\big)\).
\end{note}
...
\begin{proof}
  ...
  \end{align*}
  By hypothesis we know that \((Y, d_Y)\) is complete, thus by \labelcrefs{ii:1.4.10} we have
  \[
  ...
  Since \(f_n \in C(X \to Y)\) for all \(n \in \Z^+\), by \labelcrefs{ii:2.1.1} we know that
  \[
  ...
  \]
  If we denote \(M_{x, x_0, \varepsilon} = \max(N_{x, \varepsilon}, N_{x_0, \varepsilon})\), then by \labelcrefs{ii:1.1.2.d} we have
  \begin{align*}
  ...
  \end{align*}
  By \labelcrefs{ii:2.1.1} this means \(f \in C(X \to Y)\).
  Since \((f_n)_{n = 1}^\infty\) was arbitrary, by \labelcrefs{ii:1.4.10} \(\big(C(X \to Y), d_{C(X \to Y)}\big)\) is complete.
\end{proof}
...
\begin{ex}\label{ii:ex:3.4.1}
  Let \((X, d_X)\) and \((Y, d_Y)\) be metric spaces.
  Show that the space \(B(X \to Y)\) defined in \labelcrefs{ii:3.4.2}, with the metric \(d_{B(X \to Y)}\), is indeed a metric space.
\end{ex}

\begin{proof}
  If \(X = \emptyset\), then by \labelcrefs{ii:3.4.2} we have
  \begin{itemize}
  ...
  \end{itemize}
  Thus, by \labelcrefs{ii:1.1.2} \(\big(B(\emptyset \to Y), d_{B(X \to Y)}\big)\) is a metric space.
  Now suppose that \(X \neq \emptyset\).
  Since
  \begin{align*}
    \forall f \in B(X \to Y), d_{B(X \to Y)}(f, f) & = \sup_{x \in X} d_Y\big(f(x), f(x)\big) &  & \by{ii:3.4.2}   \\
                                                   ...
                                                   & = 0,
  \end{align*}
  by \labelcrefs{ii:1.1.2.a} we know that \(\big(B(X \to Y), d_{B(X \to Y)}\big)\) is reflexive.
  Since
  \begin{align*}
  ...
  \end{align*}
  by \labelcrefs{ii:1.1.2.b} we know that \(\big(B(X \to Y), d_{B(X \to Y)}\big)\) is positive.
  Since
  \begin{align*}
  ...
  \end{align*}
  by \labelcrefs{ii:1.1.2.c} we know that \(\big(B(X \to Y), d_{B(X \to Y)}\big)\) is symmetry.
  Since
  \begin{align*}
  ...
  \end{align*}
  by \labelcrefs{ii:1.1.2.d} we know that \(\big(B(X \to Y), d_{B(X \to Y)}\big)\) is transitive.
  Combine all the proofs above we conclude by \labelcrefs{ii:1.1.2} that \(\big(B(X \to Y), d_{B(X \to Y)}\big)\) is a metric space.
\end{proof}

\begin{ex}\label{ii:ex:3.4.2}
  Prove \labelcrefs{ii:3.4.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.4.4}.
\end{proof}

\begin{ex}\label{ii:ex:3.4.3}
  Prove \labelcrefs{ii:3.4.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:3.4.5}.
\end{proof}
...
\begin{ex}\label{ii:ex:3.4.4}
  ...
  \begin{itemize}
    ...
    \item For each natural number \(n\), let \(f^{(n)} : X \to Y\) be a function from \(X\) to \(Y\), and let \(f : X \to Y\) be another function from \(X\) to \(Y\).
          Show that \(f^{(n)}\) converges to \(f\) in the topology \(\mathcal{F}\) (in the sense of \labelcrefs{ii:2.5.4}) iff \(f^{(n)}\) converges to \(f\) pointwise (in the sense of \labelcrefs{ii:3.2.1}).
  \end{itemize}
...
\begin{proof}
  ...
  By \labelcrefs{ii:1.2.15.c} we know that \(B_{(Y, d_Y)}\big(f(x_0), 1\big)\) is open in \((Y, d_Y)\).
  Then we have
  \[
  ...
  Combine all the proofs above we conclude by \labelcrefs{ii:2.5.1} that \((Y^X, \mathcal{F})\) is a topological space.
  ...
  Then by \labelcrefs{ii:2.5.4} we have
  \[
  ...
  \end{align*}
  Since \(x_0\) was arbitrary, by \labelcrefs{ii:3.2.1} \((f^{(n)})_{n = 1}^\infty\) converges pointwise to \(f\) on \(X\) with respect to \(d_Y\).
  ...
  \end{align*}
  Since \(E\) was arbitrary, by \labelcrefs{ii:2.5.4} we know that \((f^{(n)})_{n = 1}^\infty\) converges to \(f\) in \((Y^X, \mathcal{F})\).
\end{proof}
