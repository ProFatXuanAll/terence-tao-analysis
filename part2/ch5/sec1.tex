...
\begin{note}
  ...
  Thus, for instance, the functions \(x \mapsto \sin(2 \pi n x)\), \(x \mapsto \cos(2 \pi n x)\), and \(x \mapsto e^{2 \pi i n x}\) are all elements of \(C(\R / \Z ; \C)\), as are the constant functions, however the square wave function in \labelcrefs{ii:5.1.4} is not in \(C(\R / \Z ; \C)\) because it is not continuous at every integer.
...
\begin{lem}[Basic properties of \(C(\R / \Z ; \C)\)]\label{ii:5.1.5}
  ...
  \begin{enumerate}
    ...
    \item\label{ii:5.1.5.c} (Closure under uniform limits)
...
\begin{proof}[\pf{ii:5.1.5.a}]
  Since \(f \in C(\R / \Z ; \C)\), by \labelcrefs{ii:5.1.1} we have
  \[
    \set{f(x) : x \in \R} = \set{f(x) : x \in [0, 1)} = \set{f(x) : x \in [0, 1]}.
  \]
  So it suffices to show that \(\set{f(x) : x \in [0, 1]}\) is bounded.
  Let \(d_{\R} = d_{l^1}|_{\R \times \R}\) and let \(d_{\C}\) be the metric in \labelcrefs{ii:4.6.10}.
  Since \([0, 1]\) is closed and bounded in \((\R, d_{\R})\), by Heine-Borel theorem (\labelcrefs{ii:1.5.7}) we know that \(([0, 1], d_{\R}|_{[0, 1] \times [0, 1]})\) is compact.
  Since \(f\) is continuous on \([0, 1]\), by \labelcrefs{ii:2.3.1} we know that \(\big(f([0, 1]), d_{\C}|_{f([0, 1]) \times f([0, 1])}\big)\) is also compact.
  By \labelcrefs{ii:1.5.6} we know that compactness implies boundness, thus we have
  \begin{align*}
...
\begin{proof}[\pf{ii:5.1.5.c}]
  Let \(d_{\R} = d_{l^1}|_{\R \times \R}\) and let \(d_{\C}\) be the metric in \labelcrefs{ii:4.6.10}.
  Since \((f_n)_{n = 0}^\infty\) converges uniformly to \(f\) on \(\C\) with respect to \(d_{\C}\), by \labelcrefs{ii:3.3.2} we know that \(f\) is continuous from \((\R, d_{\R})\) to \((\C, d_{\C})\).
  Suppose for the sake of contradiction that \(f \notin C(\R / \Z ; \C)\).
  By \labelcrefs{ii:5.1.1} this means
  \[
    \exists x \in \R : f(x + 1) \neq f(x).
  \]
  By \labelcrefs{ii:ex:3.2.2} we know that \((f_n)_{n = 0}^\infty\) converges pointwise to \(f\) on \(\C\) with respect to \(d_{\C}\), thus by \labelcrefs{ii:3.2.1} we have
  \begin{align*}
             & \begin{dcases}
    ...
    \implies & f(x) = f(x + 1).
  \end{align*}
  But this contradicts the definition of \(x\).
  Thus, \(f \in C(\R / \Z ; \C)\).
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    \forall x \in \R, \text{mod}_n(x + n) & = x + n - \bigg[\dfrac{x + n}{n}\bigg] n                                   \\
                                          ...
                                          & = \text{mod}_n(x),
  \end{align*}
  by \labelcrefs{ii:5.1.1} we know that \(\text{mod}_n\) is \(n\)-periodic.
\end{proof}
...
\begin{ex}\label{ii:ex:5.1.2}
  Prove \labelcrefs{ii:5.1.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:5.1.5}.
\end{proof}
...
\begin{proof}
  ...
  \]
  we know that \(d_\infty\) satisfied \labelcrefs{ii:1.1.2.a}.
  Since
  \begin{align*}
  ...
  \end{align*}
  we know that \(d_\infty\) satisfied \labelcrefs{ii:1.1.2.b}.
  Since
  \begin{align*}
    \forall f, g \in C(\R / \Z ; \C), d_\infty(f, g) & = \sup_{x \in [0, 1)} \abs{f(x) - g(x)}                     \\
                                                     ...
                                                     & = d_\infty(g, f),
  \end{align*}
  we know that \(d_\infty\) satisfied \labelcrefs{ii:1.1.2.c}.
  Since
  \begin{align*}
  ...
  \end{align*}
  we know that \(d_\infty\) satisfied \labelcrefs{ii:1.1.2.d}.
  From all proofs above, we conclude by \labelcrefs{ii:1.1.2} that \(\big(C(\R / \Z ; \C), d_\infty\big)\) is a metric space.
  ...
  Then by \labelcrefs{ii:1.4.6} we have
  \begin{align*}
  ...
  \end{align*}
  Since \((\C, d_{\C})\) is complete (by \labelcrefs{ii:ex:4.6.10}), we know that
  \[
  ...
  \end{align*}
  Since \((f_n)_{n = 1}^\infty\) was arbitrary, by \labelcrefs{ii:1.4.10} we know that \(\big(C(\R / \Z ; \C), d_\infty\big)\) is complete.
\end{proof}
