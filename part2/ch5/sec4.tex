\section{Periodic convolutions}\label{ii:sec:5.4}
...
\begin{proof}
  ...
  we know that \(f\) is bounded (by \labelcrefs{ii:5.1.5.a}), so that we have some \(M > 0\) such that \(\abs{f(x)} \leq M\) for all \(x \in \R\).

  Let \(\varepsilon > 0\) be arbitrary.
  Since \(f\) is uniformly continuous (by \labelcrefs{ii:2.3.5}), there exists a \(\delta > 0\) such that \(\abs{f(x) - f(y)} \leq \varepsilon\) whenever \(\abs{x - y} \leq \delta\).
  Now use \labelcrefs{ii:5.4.6} to find a trigonometric polynomial \(P\) which is a \((\varepsilon, \delta)\) approximation to the identity.
  Then \(f * P\) is also a trigonometric polynomial (by \labelcrefs{ii:ac:5.4.1}).
  We now estimate \(\norm*{f - f * P}_{\infty}\).
  ...
  \begin{align*}
     & \abs{f(x) - f * P(x)}                                                                                     \\
     & = \abs{f(x) - P * f(x)}                                                   &  & \by{ii:5.4.4.a,ii:5.4.4.b} \\
...
\begin{note}
  \labelcrefs{ii:5.4.1} asserts that any continuous periodic function can be uniformly approximated by trigonometric polynomials.
...
\begin{note}
  It is possible to prove this theorem directly from the Weierstrass approximation theorem for polynomials (\labelcrefs{ii:3.8.3}), and both theorems are a special case of a much more general theorem known as the \emph{Stone-Weierstrass theorem}, which we will not discuss here.
  ...
  The proof here, though, should strongly remind you of the arguments used to prove \labelcrefs{ii:3.8.3}.
\end{note}

\begin{defn}[Periodic convolution]\label{ii:5.4.2}
  Let \(f, g \in C(\R / \Z ; \C)\).
...
\begin{rmk}\label{ii:5.4.3}
  Note that \labelcrefs{ii:5.4.2} is slightly different from the convolution for compactly supported functions defined in \labelcrefs{ii:3.8.9}, because we are only integrating over \([0, 1]\) and not on all of \(\R\).
...
\begin{lem}[Basic properties of periodic convolution]\label{ii:5.4.4}
  Let \(f, g, h \in C(\R / \Z ; \C)\).
  \begin{enumerate}
    \item\label{ii:5.4.4.a} (Closure)
    The convolution \(f * g\) is continuous and \(\Z\)-periodic.
    In other words, \(f * g \in C(\R / \Z ; \C)\).
    \item\label{ii:5.4.4.b} (Commutativity)
    We have \(f * g = g * f\).
    \item\label{ii:5.4.4.c} (Bilinearity)
    We have \(f * (g + h) = f * g + f * h\) and \((f + g) * h = f * h + g * h\).
    For any complex number \(c\), we have \(c(f * g) = (cf) * g = f * (cg)\).
  \end{enumerate}
\end{lem}

\begin{proof}[\pf{ii:5.4.4.a}]
  By \labelcrefs{ii:5.2.2} we know that \(f * g\) is continuous on \(\R\).
  Since
  \begin{align*}
...
\begin{proof}[\pf{ii:5.4.4.b}]
  ...
  \end{align*}
  Let \([x]\) be the integer defined in \labelcrefs{ii:ex:5.1.1}.
  Then we have
  \begin{align*}
             & [x] \leq x < [x] + 1     \\
...
\begin{proof}[\pf{ii:5.4.4.c}]
  By \labelcrefs{ii:5.1.5.b} we know that \(f + g, g + h, cf, cg \in C(\R / \Z ; \C)\).
  ...
  \begin{align*}
     ...
     & = \int_{[0, 1]} f(y) g(x - y) \; dy + \int_{[0, 1]} f(y) h(x - y) \; dy &  & \text{(cf the proof of \labelcrefs{ii:5.2.5.c})} \\
  ...
  \begin{align*}
    \big((cf) * g\big)(x) & = \int_{[0, 1]} (cf)(y) \cdot g(x - y) \; dy &  & \by{ii:5.4.2}                                    \\
                          ...
                          & = c \int_{[0, 1]} f(y) g(x - y) \; dy        &  & \text{(cf the proof of \labelcrefs{ii:5.2.5.c})} \\
                          & = c (f * g)(x).                              &  & \by{ii:5.4.2}
  \end{align*}
...
\begin{ac}\label{ii:ac:5.4.1}
  ...
  \begin{align*}
    f * e_n(x) & = \int_{[0, 1]} f(y) e_n(x - y) \; dy                        &  & \by{ii:5.4.2}                                    \\
               ...
               & = e^{2 \pi i n x} \int_{[0, 1]} f(y) e^{- 2 \pi i n y} \; dy &  & \text{(cf the proof of \labelcrefs{ii:5.2.5.c})} \\
               & = \inner*{f, e_n} e^{2 \pi i n x}                            &  & \by{ii:5.2.1}                                    \\
               & = \hat{f}(n) e^{2 \pi i n x}                                 &  & \by{ii:5.3.7}                                    \\
               & = \hat{f}(n) e_n                                             &  & \by{ii:5.3.1}
  \end{align*}
  as desired.
  More generally, we see from \labelcrefs{ii:5.4.4.c} that for any trigonometric polynomial \(P = \sum_{n = -N}^N c_n e_n\), we have
  \[
  ...
  (Compare with \labelcrefs{ii:3.8.13}.)
\end{ac}

\begin{defn}[Periodic approximation to the identity]\label{ii:5.4.5}
  ...
  \begin{enumerate}
    \item\label{ii:5.4.5.a} \(f(x) \geq 0\) for all \(x \in \R\), and \(\int_{[0, 1]} f(x) \; dx = 1\).
    \item\label{ii:5.4.5.b} We have \(f(x) < \varepsilon\) for all \(\delta \leq \abs{x} \leq 1 - \delta\).
  \end{enumerate}
\end{defn}

\begin{ac}[Fejér kernel]\label{ii:ac:5.4.2}
...
\begin{proof}
  Since \(f\) is compactly supported, by \labelcrefs{ii:3.8.4} we know that
  \[
    \exists L, U \in \R : \forall x \in \R \setminus [L, U], f(x) = 0.
  \]
  Fix such \(L, U\).
  Let \([U - L]\) be the integer defined in \labelcrefs{ii:ex:5.1.1}.
  Then we have
  \begin{align*}
...
\begin{ex}\label{ii:ex:5.4.2}
  Prove \labelcrefs{ii:5.4.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:5.4.4}.
\end{proof}

\begin{ex}\label{ii:ex:5.4.3}
  Fill in the gaps marked in \labelcrefs{ii:5.4.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:5.4.6}.
\end{proof}
