...
\begin{proof}
  ...
  By the Weierstrass approximation theorem (\labelcrefs{ii:5.4.1}), we can find a trigonometric polynomial \(P = \sum_{n = -N_0}^{N_0} c_n e_n\) such that \(\norm*{f - P}_{\infty} \leq \varepsilon\), for some \(N_0 > 0\).
  In particular, we have \(\norm*{f - P}_2 \leq \varepsilon\) (\labelcrefs{ii:ex:5.2.3}).
  ...
  \]
  where we have used \labelcrefs{ii:5.3.5} and \labelcrefs{ii:5.2.5}.
  In particular, we have
  \[
    \inner*{f - F_N, F_N - P} = 0
  \]
  since we can write \(F_N - P\) as a linear combination of the \(e_m\) for which \(\abs{m} \leq N\).
  By Pythagoras' theorem (\labelcrefs{ii:5.2.7.d}) we therefore have
  \[
    \norm*{f - P}_2^2 = \norm*{f - F_N}_2^2 + \norm*{F_N - P}_2^2
  \]
  and in particular
...
\begin{proof}
  By the Weierstrass \(M\)-test (\labelcrefs{ii:3.5.7}), we see that \(\sum_{n = -\infty}^\infty \hat{f}(n) e_n\) converges to some function \(F\), which by \labelcrefs{ii:5.1.5.c} is also continuous and \(\Z\)-periodic.
  ...
  \[
    \lim_{N \to \infty} \norm*{F - \sum_{n = -N}^N \hat{f}(n) e_n}_2 = 0
  \]
  since the \(L^2\) norm is always less than or equal to the \(L^\infty\) norm (\labelcrefs{ii:ex:5.2.3}).
  But the sequence \(\sum_{n = -N}^N \hat{f}(n) e_n\) is already converging in \(L^2\) metric to \(f\) by the Fourier theorem (\labelcrefs{ii:5.5.1}), so can only converge in \(L^2\) metric to \(F\) if \(F = f\)
  (cf. \labelcrefs{ii:1.1.20}).
...
\begin{thm}[Plancherel theorem]\label{ii:5.5.4}
...
\begin{proof}
  Let \(\varepsilon > 0\).
  By the Fourier theorem (\labelcrefs{ii:5.5.1}) we know that
  \[
    \norm*{f - \sum_{n = -N}^N \hat{f}(n) e_n}_2 \leq \varepsilon
  \]
  if \(N\) is large enough (depending on \(\varepsilon\)).
  In particular, by the triangle inequality (\labelcrefs{ii:5.2.7.c,ii:5.2.7.e}) this implies that
  \[
    \norm*{f}_2 - \varepsilon \leq \norm*{\sum_{n = -N}^N \hat{f}(n) e_n}_2 \leq \norm*{f}_2 + \varepsilon.
  \]
  On the other hand, by \labelcrefs{ii:5.3.6} we have
  \[
...
\begin{note}
  \labelcrefs{ii:5.5.4} is also known as \emph{Parseval's theorem}.
\end{note}
...
\begin{ex}\label{ii:ex:5.5.1}
  ...
  \]
  \begin{enumerate}
    \item\label{ii:ex:5.5.1.a} Show that the series
    \[
      \dfrac{1}{2} a_0 + \sum_{n = 1}^\infty \big(a_n \cos(2 \pi n x) + b_n \sin(2 \pi n x)\big)
    \]
    converges in \(L_2\) metric to \(f\).
    \item\label{ii:ex:5.5.1.b} Show that if \(\sum_{n = 1}^\infty a_n\) and \(\sum_{n = 1}^\infty b_n\) are absolutely convergent, then the above series actually converges uniformly to \(f\), and not just in \(L_2\) metric.
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:5.5.1.a}]
  ...
  \begin{align*}
     ...
     & = \dfrac{1}{2} (a_n + i b_n).
  \end{align*}
  By Fourier theroem (\labelcrefs{ii:5.5.1}) we know that
  \[
    \lim_{N \to \infty} \norm*{f - \sum_{n = -N}^N \hat{f}(n) e_n}_2 = 0.
  \]
...
\begin{proof}[\pf{ii:ex:5.5.1.b}]
  ...
  \]
  for all \(n \in \Z\) (cf. the proof of \labelcrefs{ii:ex:5.5.1.a}), we know that
  \begin{align*}
  ...
  By \labelcrefs{ii:5.5.3} we know that
  \[
    \lim_{N \to \infty} \norm*{f - \sum_{n = -N}^N \hat{f}(n) e_n}_{\infty} = 0.
  \]
  and \(\sum_{n = -\infty}^\infty \hat{f}(n) e_n\) converges uniformly to \(f\) on \(\R\) with respect to \(d_{l^1}|_{\C \times \C}\).
  In particular, we have (by \labelcrefs{ii:ex:5.2.3} and squeeze test)
  \[
    \lim_{N \to \infty} \norm*{f - \sum_{n = -N}^N \hat{f}(n) e_n}_2 = 0.
  \]
  By \labelcrefs{ii:ex:5.5.1.a} we know that
  \[
    \lim_{N \to \infty} \norm*{f - \bigg(\dfrac{a_0}{2} + \sum_{n = 1}^N a_n \cos(2 \pi n x) + b_n \sin(2 \pi n x)\bigg)}_2 = 0.
  \]
  Thus, by \labelcrefs{ii:1.1.20} we have
  \[
...
\begin{ex}\label{ii:ex:5.5.2}
  ...
  \begin{enumerate}
    \item\label{ii:ex:5.5.2.a} Using \labelcrefs{ii:ex:5.5.1}, show that the series
    \[
      \dfrac{1}{3} + \sum_{n = 1}^\infty \dfrac{4}{\pi^2 n^2} \cos(2 \pi n x)
    \]
    converges uniformly to \(f\) .
    \item\label{ii:ex:5.5.2.b} Conclude that \(\sum_{n = 1}^\infty \dfrac{1}{n^2} = \dfrac{\pi^2}{6}\).
    \item\label{ii:ex:5.5.2.c} Conclude that \(\sum_{n = 1}^\infty \dfrac{1}{n^4} = \dfrac{\pi^4}{90}\).
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:5.5.2.a}]
  For each \(n \in \N\), we define \(a_n, b_n\) as in \labelcrefs{ii:ex:5.5.1}.
  Observe that for all \(n \in \Z^+\), we have
  \begin{align*}
  ...
  \end{align*}
  are absolutely convergent (by Corollary 7.3.7 in Analysis I), by \labelcrefs{ii:ex:5.5.1.b} we know that the series
  \[
    \dfrac{a_0}{2} + \sum_{n = 1}^\infty (a_n \cos(2 \pi n x) + b_n \sin(2 \pi n x))
  \]
...
\begin{proof}[\pf{ii:ex:5.5.2.c}]
  By \labelcrefs{ii:ex:5.5.2.a} we know that
  \[
...
\begin{proof}
  ...
  By \labelcrefs{ii:ac:5.4.1} we know that
  \[
  ...
  \]
  Let \(\varepsilon \in \R^+\).
  Since \(g \in C(\R / \Z ; \C)\), by \labelcrefs{ii:5.4.1} we know that there exists a trigonometric polynomial \(P\) such that
  \[
    \norm*{g - P}_{\infty} \leq \varepsilon.
  \]
  Fix such \(P\).
  Since \(f \in C(\R / \Z ; \C)\), by \labelcrefs{ii:5.5.4} we know that \(\sum_{n = -\infty}^\infty \abs{\hat{f}(n)}^2 \in \R\), thus
  \[
