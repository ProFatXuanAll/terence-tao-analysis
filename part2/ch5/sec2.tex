...
\begin{proof}
  Let \(f \in C(\R / \Z ; \C)\), let \(d_{\R} = d_{l^1}|_{\R \times \R}\) and let \(d_{\C}\) be the metric in \labelcrefs{ii:4.6.10}.
  ...
  \end{align*}
  Since \((a_n)_{n = 0}^\infty\) was arbitrary, by \labelcrefs{ii:2.1.4} we know that \(\Re \circ f\) is continuous at \(x_0\) from \((\R, d_{\R})\) to \((\R, d_{\R})\).
  Since \(x_0\) was arbitrary, by \labelcrefs{ii:2.1.5} we know that \(\Re \circ f\) is continuous on \(\R\) from \((\R, d_{\R})\) to \((\R, d_{\R})\).
  ...
  \end{align*}
  by \labelcrefs{ii:5.1.5.a} we know that both \(\Re \circ f\) and \(\Im \circ f\) are bounded in \((\C, d_{\C})\).
  In particular, by \labelcrefs{ii:4.6.8} we know that \((\Re \circ f)(\R) \subseteq \R\) and \((\Im \circ f)(\R) \subseteq \R\).
...
\begin{ac}\label{ii:ac:5.2.1}
  From the positivity property (\labelcrefs{ii:5.2.5.b}), it makes sense to define the \(L^2\) norm \(\norm*{f}_2\) of a function \(f \in C(\R / \Z ; \C)\) by the formula
  \[
...
\begin{lem}\label{ii:5.2.7}
  Let \(f, g \in C(\R / \Z ; \C)\).
  \begin{enumerate}
    \item\label{ii:5.2.7.a} (Non-degeneracy)
    We have \(\norm*{f}_2 = 0\) iff \(f = 0\).
    \item\label{ii:5.2.7.b} (Cauchy-Schwarz inequality)
    We have \(\abs{\inner*{f, g}} \leq \norm*{f}_2 \norm*{g}_2\).
    \item\label{ii:5.2.7.c} (Triangle inequality)
    We have \(\norm*{f + g}_2 \leq \norm*{f}_2 + \norm*{g}_2\).
    \item\label{ii:5.2.7.d} (Pythagoras' theorem)
    If \(\inner*{f, g} = 0\), then \(\norm*{f + g}_2^2 = \norm*{f}_2^2 + \norm*{g}_2^2\).
    \item\label{ii:5.2.7.e} (Homogeneity)
    We have \(\norm*{cf}_2 = \abs{c} \norm*{f}_2\) for all \(c \in \C\).
  \end{enumerate}
\end{lem}

\begin{proof}[\pf{ii:5.2.7.a}]
  We have
  \begin{align*}
         & \norm*{f}_2 = 0                                \\
...
\begin{proof}[\pf{ii:5.2.7.b}]
  ...
  \end{align*}
  If we let \(h = \norm*{g}_2^2 \cdot f - \inner*{f, g} \cdot g\), then by \labelcrefs{ii:ac:5.2.1} we know that \(\inner*{h, h}\) is well-defined.
  Thus, we have
  \begin{align*}
     & \inner*{h, h}                                                                                                                                  \\
...
\begin{proof}[\pf{ii:5.2.7.c}]
  We have
  \begin{align*}
    \norm*{f + g}_2^2 & = \inner*{f + g, f + g}                                         &  & \by{ii:ac:5.2.1}           \\
                      & = \inner*{f, f} + \inner*{f, g} + \inner*{g, f} + \inner*{g, g} &  & \by{ii:5.2.5.c,ii:5.2.5.d} \\
...
\begin{proof}[\pf{ii:5.2.7.e}]
  We have
  \begin{align*}
    \norm*{cf}_2 & = \sqrt{\inner*{cf, cf}}              &  & \by{ii:ac:5.2.1}           \\
                 & = \sqrt{c \overline{c} \inner*{f, f}} &  & \by{ii:5.2.5.c,ii:5.2.5.d} \\
...
\begin{ex}\label{ii:ex:5.2.1}
  Prove \labelcrefs{ii:5.2.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:5.2.5}.
\end{proof}

\begin{ex}\label{ii:ex:5.2.2}
  Prove \labelcrefs{ii:5.2.7}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:5.2.7}.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{ii:5.2.7.a} we know that \(0 < \norm*{f}_2\).
  Thus, we only need to show that \(\norm*{f}_2 \leq \norm*{f}_\infty\).
  By \labelcrefs{ii:5.1.5.a} we know that \(f\) is bounded, thus by \labelcrefs{ii:3.5.5}
  \[
...
\begin{proof}
  ...
  \begin{align*}
    d_{L^2}(f, f) & = \norm*{f - f}_2 &  & \by{ii:ac:5.2.2} \\
                  ...
                  & = 0,              &  & \by{ii:5.2.7.a}
  \end{align*}
  we know that \(\big(C(\R / \Z ; \C), d_{L^2}\big)\) satisfies \labelcrefs{ii:1.1.2.a}.
  Since
  \begin{align*}
             & f \neq g                                  \\
    \implies & f - g \neq 0        &  & \by{ii:5.2.5.b}  \\
  ...
  \end{align*}
  we know that \(\big(C(\R / \Z ; \C), d_{L^2}\big)\) satisfies \labelcrefs{ii:1.1.2.b}.
  Since
  \begin{align*}
    d_{L^2}(f, g) & = \norm*{f - g}_2              &  & \by{ii:ac:5.2.2}           \\
                  & = \sqrt{\inner*{f - g, f - g}} &  & \by{ii:ac:5.2.1}           \\
                  & = \sqrt{\inner*{g - f, g - f}} &  & \by{ii:5.2.5.c,ii:5.2.5.d} \\
                  & = \norm*{g - f}_2              &  & \by{ii:ac:5.2.1}           \\
                  & = d_{L^2}(g, f),               &  & \by{ii:ac:5.2.2}
  \end{align*}
  we know that \(\big(C(\R / \Z ; \C), d_{L^2}\big)\) satisfies \labelcrefs{ii:1.1.2.c}.
  Since
  \begin{align*}
  ...
  \end{align*}
  we know that \(\big(C(\R / \Z ; \C), d_{L^2}\big)\) satisfies \labelcrefs{ii:1.1.2.d}.
  From all proofs above, we conclude by \labelcrefs{ii:1.1.2} that \(\big(C(\R / \Z ; \C), d_{L^2}\big)\) is a metric space.
\end{proof}
...
\begin{proof}
  By \labelcrefs{ii:5.1.4} we can define a \(\Z\)-periodic square wave function \(f : \R \to \C\) as follow:
  \[
...
\begin{ex}\label{ii:ex:5.2.6}
  ...
  \begin{enumerate}
    \item\label{ii:ex:5.2.6.a} Show that if \(f_n\) converges uniformly to \(f\), then \(f_n\) also converges to \(f\) in the \(L^2\) metric.
    \item\label{ii:ex:5.2.6.b} Give an example where \(f_n\) converges to \(f\) in the \(L^2\) metric, but does not converge to \(f\) uniformly.
    \item\label{ii:ex:5.2.6.c} Give an example where \(f_n\) converges to \(f\) in the \(L^2\) metric, but does not converge to \(f\) pointwise.
    \item\label{ii:ex:5.2.6.d} Give an example where \(f_n\) converges to \(f\) pointwise, but does not converge to \(f\) in the \(L^2\) metric.
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:5.2.6.a}]
  We have
  \begin{align*}
...
\begin{proof}[\pf{ii:ex:5.2.6.b}]
  ...
  \begin{align*}
    \lim_{n \to \infty} d_{L^2}(f_n, f) & = \lim_{n \to \infty} \int_{[0, 1]} \abs{f_n(x) - f(x)}^2 \; dx &  & \by{ii:ac:5.2.2} \\
                                        ...
                                        & = 0.
  \end{align*}
  Thus, by \labelcrefs{ii:1.1.14} we have
  \[
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:3.2.7} \((f_n)_{n = 2}^\infty\) does not converges uniformly to \(f\) on \(\R\) with respect to \(d_{l^1}|_{\R \times \R}\).
\end{proof}

\begin{proof}[\pf{ii:ex:5.2.6.c}]
  Using the definition of \(f, f_n\) in \labelcrefs{ii:ex:5.2.6.b}, we see that \((f_n)_{n = 2}^\infty\) does not converges pointwise to \(f\) on \(\R\) with respect to \(d_{l^1}|_{\R \times \R}\).
\end{proof}

\begin{proof}[\pf{ii:ex:5.2.6.d}]
  Let \(f \in C(\R / \Z ; \C)\) such that \(f = 0\) and let \(\N_{\geq 2} = \set{n \in \N : n \geq 2}\).
  For all \(n \in \N_{\geq 2}\), we define \(f_n \in C(\R / \Z ; \C)\) as follow:
  \[
    \forall x \in [0, 1), f_n(x) = \begin{dcases}
  ...
  \end{align*}
  by \labelcrefs{ii:3.2.1} we know that \((f_n)_{n = 2}^\infty\) converges pointwise to \(f\) on \(\R\) with respect to \(d_{l^1}|_{\R \times \R}\).
  But
  \begin{align*}
     & \int_{[0, 1]} \abs{f_n(x) - f(x)}^2 \; dx                                                                                                                    \\
