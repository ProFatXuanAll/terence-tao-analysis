\section{Some point-set topology of metric spaces}\label{ii:sec:1.2}
...
\begin{rmk}\label{ii:1.2.4}
  ...
  However, \(B(x_0 , r)\) always contains at least one point, namely the center \(x_0\), as long as \(r\) stays positive, thanks to \labelcrefs{ii:1.1.2.a}.
  (We don't consider balls of zero radius or negative radius since they are rather boring, being just the empty set.)
\end{rmk}

\begin{defn}[Interior, exterior, boundary]\label{ii:1.2.5}
...
\begin{note}
  By \labelcrefs{ii:1.2.6} we thus have
  \begin{align*}
...
\begin{proof}
  ...
  \end{align*}
  Since \(X = (X \setminus E) \cup E = \text{ext}(E) \cup \text{int}(E)\), every point in \((X, d_{\text{disc}})\) is either a exterior point or interior point, thus by \labelcrefs{ii:1.2.5} there are no boundary points in \((X, d_{\text{disc}})\).
\end{proof}
...
\begin{note}
  Notions in \labelcrefs{ii:1.2.9} are consistent with the corresponding notions on the real line.
\end{note}
...
\begin{prop}\label{ii:1.2.10}
  ...
  \begin{enumerate}
    \item\label{ii:1.2.10.a} \(x_0\) is an adherent point of \(E\).
    \item\label{ii:1.2.10.b} \(x_0\) is either an interior point or a boundary point of \(E\).
    \item\label{ii:1.2.10.c} There exists a sequence \((x_n)_{n = 1}^\infty\) in \(E\) which converges to \(x_0\) with respect to the metric \(d\).
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Suppose that \(x_0 \in \partial_{(X, d)}(E)\).
          By \labelcrefs{ii:1.2.5} for all \(r \in \R^+\), we have \(B_{(X, d)}(x_0, r) \cap E \neq \emptyset\).
  ...
  By \labelcrefs{ii:1.1.14} we have
  \[
  ...
  Since \(r\) was arbitrary, by \labelcrefs{ii:1.2.9} we know that \(x_0\) is an adherent point of \(E\).
\end{proof}

\begin{cor}\label{ii:1.2.11}
  Let \((X, d)\) be a metric space, and let \(E\) be a subset of \(X\).
  Then \(\overline{E} = \text{int}(E) \cup \partial E = X \setminus \text{ext}(E)\).
\end{cor}

\begin{proof}
  By \labelcrefs{ii:1.2.10.a,ii:1.2.10.b} we are done.
\end{proof}

\begin{defn}[Open and closed sets]\label{ii:1.2.12}
  Let \((X, d)\) be a metric space, and let \(E\) be a subset of \(X\).
...
\begin{rmk}\label{ii:1.2.14}
  ...
  For instance, using the discrete metric \(d_{\text{disc}}\), \emph{every} set is both open and closed! (See \labelcrefs{ii:1.2.8})
\end{rmk}

\begin{note}
  From \labelcrefs{ii:1.2.14} we see that the notions of being open and being closed are \emph{not} negations of each other;
...
\begin{prop}[Basic properties of open and closed sets]\label{ii:1.2.15}
  Let \((X, d)\) be a metric space.
  \begin{enumerate}
    \item\label{ii:1.2.15.a} Let \(E\) be a subset of \(X\).
    ...
    \item\label{ii:1.2.15.b} Let \(E\) be a subset of \(X\).
    ...
    \item\label{ii:1.2.15.c} For any \(x_0 \in X\) and \(r > 0\), then the ball \(B(x_0, r)\) is an open set.
    ...
    \item\label{ii:1.2.15.d} Any singleton set \(\set{x_0}\), where \(x_0 \in X\), is automatically closed.
    \item\label{ii:1.2.15.e} If \(E\) is a subset of \(X\), then \(E\) is open iff the complement \(X \setminus E \coloneqq \set{x \in X : x \notin E}\) is closed.
    \item\label{ii:1.2.15.f} If \(E_1, \dots, E_n\) are a finite collection of open sets in \(X\), then \(E_1 \cap E_2 \cap \dots \cap E_n\) is also open.
    If \(F_1, \dots, F_n\) is a finite collection of closed sets in \(X\), then \(F_1 \cup F_2 \cup \dots \cup F_n\) is also closed.
    \item\label{ii:1.2.15.g} If \(\set{E_\alpha}_{\alpha \in I}\) is a collection of open sets in \(X\) (where the index set \(I\) could be finite, countable, or uncountable), then the union \(\bigcup_{\alpha \in I} E_\alpha \coloneqq \set{x \in X : x \in E_\alpha \text{ for some } \alpha \in I}\) is also open.
    ...
    \item\label{ii:1.2.15.h} If \(E\) is any subset of \(X\), then \(\text{int}(E)\) is the largest open set which is contained in \(E\);
...
\begin{proof}[\pf{ii:1.2.15.c}]
  ...
  By \labelcrefs{ii:1.2.1} we have \(d(x, x_0) < r\), so \(r' > 0\).
  Then we have
  \begin{align*}
  ...
  By \labelcrefs{ii:1.2.15.b} we know that \(E\) is closed in \((X, d)\) iff \(E = \overline{E}_{(X, d)}\).
  By \labelcrefs{ii:1.2.10.c} we know that \(E \subseteq \overline{E}_{(X, d)}\).
  ...
  By \labelcrefs{ii:1.2.10.c}, \(\exists (y_n)_{n = 1}^\infty\) such that \(y_n \in E\) for all \(n \in \Z^+\) and \(\lim_{n \to \infty} d(y_n, y) = 0\).
...
\begin{proof}[\pf{ii:1.2.15.d}]
  By \labelcrefs{ii:1.2.15.b} we know that \(\set{x_0}\) is closed in \((X, d)\) iff \(\set{x_0} = \overline{\set{x_0}}_{(X, d)}\).
  By \labelcrefs{ii:1.2.10.c} we know that \(\set{x_0} \subseteq \overline{\set{x_0}}_{(X, d)}\).
  So we only need to show that \(\overline{\set{x_0}}_{(X, d)} \subseteq \set{x_0}\).
  Let \(y \in \overline{\set{x_0}}_{(X, d)}\).
  By \labelcrefs{ii:1.2.10.c} we know that \(\exists (y_n)_{n = 1}^\infty\) such that \(y_n \in \set{x_0}\) for all \(n \in \Z^+\) and \(\lim_{n \to \infty} d(y_n, y) = 0\).
  But \(y_n \in \set{x_0}\) implies \(y_n = x_0\) for all \(n \in \Z^+\), thus
  \[
    \lim_{n \to \infty} d(y_n, y) = \lim_{n \to \infty} d(x_0, y) = d(x_0, y) = 0.
  \]
  By \labelcrefs{ii:1.1.2.a} we have \(x_0 = y\).
  This means \(\overline{\set{x_0}}_{(X, d)} \subseteq \set{x_0}\), as desired.
\end{proof}

\begin{proof}[\pf{ii:1.2.15.e}]
  Since
  \begin{align*}
...
\begin{ex}\label{ii:ex:1.2.1}
  Verify the claims in \labelcrefs{ii:1.2.8}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.2.8}.
\end{proof}

\begin{ex}\label{ii:ex:1.2.2}
  Prove \labelcrefs{ii:1.2.10}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.2.10}.
\end{proof}

\begin{ex}\label{ii:ex:1.2.3}
  Prove \labelcrefs{ii:1.2.15}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.2.15}.
\end{proof}

\begin{ex}\label{ii:ex:1.2.4}
  ...
  \begin{enumerate}
    \item\label{ii:ex:1.2.4.a} Show that \(\overline{B} \subseteq C\).
    \item\label{ii:ex:1.2.4.b} Give an example of a metric space \((X, d)\), a point \(x_0\), and a radius \(r > 0\) such that \(\overline{B}\) is \emph{not} equal to \(C\).
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:1.2.4.a}]
  Since \(B \subseteq C\) and \(C\) is closed in \((X, d)\), by \labelcrefs{ii:1.2.15.h} we have \(\overline{B}_{(X, d)} \subseteq C\).
\end{proof}

\begin{proof}[\pf{ii:ex:1.2.4.b}]
  Let \(X = \R\) and let \(d = d_{\text{disc}}\) be the metric function.
  By \labelcrefs{ii:ex:1.1.11} we know that \((\R, d_{\text{disc}})\) is a metric space.
  Let \(B = B_{(\R, d_{\text{disc}})}(0, 1)\) and let \(C = \set{x \in \R : d_{\text{disc}}(x, 0) \leq 1}\).
  Then we know that \(B = \set{0}\) and \(C = \R\).
  But by \labelcrefs{ii:1.2.8} we have \(\overline{B} = B \neq C\).
\end{proof}
