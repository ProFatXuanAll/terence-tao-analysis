...
\begin{defn}[Metric spaces]\label{ii:1.1.2}
  ...
  \begin{enumerate}
    \item\label{ii:1.1.2.a} For any \(x \in X\), we have \(d(x, x) = 0\).
    \item\label{ii:1.1.2.b} (Positivity) For any distinct \(x, y \in X\), we have \(d(x, y) > 0\).
    \item\label{ii:1.1.2.c} (Symmetry) For any \(x, y \in X\), we have \(d(x, y) = d(y, x)\).
    \item\label{ii:1.1.2.d} (Triangle inequality) For any \(x, y, z \in X\), we have \(d(x, z) \leq d(x, y) + d(y, z)\).
  \end{enumerate}
\end{defn}
...
\begin{rmk}\label{ii:1.1.3}
  The conditions (a) and (b) of \labelcrefs{ii:1.1.1} can be rephrased as follows:
...
\begin{eg}[Induced metric spaces]\label{ii:1.1.5}
  ...
  Thus, for instance the metric on the real line in the \labelcrefs{ii:1.1.4} induces a metric space structure on any subset of the reals, such as the integers \(Z\), or an interval \([a, b]\), etc.
\end{eg}
...
\begin{note}
  ...
  Extending the convention from \labelcrefs{ii:1.1.4}, if we refer to \(\R^n\) as a metric space, we assume that the metric is given by the Euclidean metric unless otherwise specified.
\end{note}
...
\begin{rmk}\label{ii:1.1.15}
  In view of \labelcrefs{ii:1.1.1} we see that this definition generalizes our existing notion of convergence of sequences of real numbers.
...
\begin{prop}[Equivalence of \(l^1\), \(l^2\), \(l^\infty\)]\label{ii:1.1.18}
  ...
  \begin{enumerate}
    \item\label{ii:1.1.18.a} \((x^{(k)})_{k = m}^\infty\) converges to \(x\) with respect to the Euclidean metric \(d_{l^2}\).
    \item\label{ii:1.1.18.b} \((x^{(k)})_{k = m}^\infty\) converges to \(x\) with respect to the taxi-cab metric \(d_{l^1}\).
    \item\label{ii:1.1.18.c} \((x^{(k)})_{k = m}^\infty\) converges to \(x\) with respect to the sup norm metric \(d_{l^\infty}\).
    \item\label{ii:1.1.18.d} For every \(1 \leq j \leq n\), the sequence \((x_j^{(k)})_{k = m}^\infty\) converges to \(x_j\).
...
\begin{note}
  Because of the equivalence of \labelcrefs{ii:1.1.18.a}, (b) and (c), we say that the Euclidean, taxicab, and sup norm metrics on \(\R^n\) are \emph{equivalent}.
...
\begin{proof}
  By \labelcrefs{ii:1.1.14} we know that \(\lim_{n \to \infty} d_{\text{disc}}(x^{(n)}, x) = 0\) iff \(\forall \varepsilon \in \R^+\), \(\exists N \in \N\) and \(N \geq m\) such that \(d(x^{(n)}, x) \leq \varepsilon\) for all \(n \geq N\).
  By \labelcrefs{ii:1.1.11} we know that \(d_{\text{disc}}(x^{(n)}, x) \leq \varepsilon\) iff \(x^{(n)} = x\).
\end{proof}
...
\begin{proof}
  By \labelcrefs{ii:1.1.14} we have \(\lim_{n \to \infty} d(x^{(n)}, x) = 0\) and \(\lim_{n \to \infty} d(x^{(n)}, x') = 0\).
  So
  \begin{align*}
    ...
    \implies & d(x, x') = 0                                                               &  & \by{ii:1.1.2.a,ii:1.1.2.b} \\
    \implies & x = x'.                                                                    &  & \by{ii:1.1.2.a}
  \end{align*}
\end{proof}

\begin{note}
  Because of \labelcrefs{ii:1.1.20}, it is safe to introduce the following notation:
  ...
  however \labelcrefs{ii:1.1.20} assures us that once \(d\) is fixed, there can be at most one value of \(d - \lim_{n \to \infty} x^{(n)}\).
...
\begin{ex}\label{ii:ex:1.1.1}
  Prove \labelcrefs{ii:1.1.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.1.1}.
\end{proof}
...
\begin{proof}
  ...
  Thus, by \labelcrefs{ii:1.1.2} \((\R, d)\) is a metric space.
\end{proof}

\begin{ex}\label{ii:ex:1.1.3}
  Let \(X\) be a set, and let \(d : X \times X \to [0, \infty)\) be a function.
  \begin{enumerate}
    \item\label{ii:ex:1.1.3.a} Give an example of a pair \((X, d)\) which obeys axioms (bcd) of \labelcrefs{ii:1.1.2}, but not (a).
    \item\label{ii:ex:1.1.3.b} Give an example of a pair \((X, d)\) which obeys axioms (acd) of \labelcrefs{ii:1.1.2}, but not (b).
    \item\label{ii:ex:1.1.3.c} Give an example of a pair \((X, d)\) which obeys axioms (abd) of \labelcrefs{ii:1.1.2}, but not (c).
    \item\label{ii:ex:1.1.3.d} Give an example of a pair \((X, d)\) which obeys axioms (abc) of \labelcrefs{ii:1.1.2}, but not (d).
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:1.1.3.a}]
  Let \(X = \R\), and let \(d(x, y) = 1\) for all \(x, y \in \R\).
  Then \((X, d)\) does not satisfy \labelcrefs{ii:1.1.2.a} but (bcd).
\end{proof}

\begin{proof}[\pf{ii:ex:1.1.3.b}]
  Let \(X = \R\) and let \(d(x, y) = 0\) for all \(x, y \in \R\).
  Then \((X, d)\) does not satisfy \labelcrefs{ii:1.1.2.b} but (acd).
\end{proof}

\begin{proof}[\pf{ii:ex:1.1.3.c}]
  Let \(X = \set{1, 2}\), let \(x, y \in X\) and let \(d(x, y) = x^y\) if \(x \neq y\) and \(d(x, y) = 0\) if \(x = y\).
  Then \((X, d)\) does not satisfy \labelcrefs{ii:1.1.2.c} but (abd).
\end{proof}

\begin{proof}[\pf{ii:ex:1.1.3.d}]
  Let \(X = \R^+\), let \(x, y \in \R^+\) and let \(d(x, y) = \max(x, y)\) if \(x \neq y\) and \(d(x, y) = 0\) if \(x = y\).
  Then \((X, d)\) does not satisfy \labelcrefs{ii:1.1.2.d} but (abc).
\end{proof}

\begin{ex}\label{ii:ex:1.1.4}
  Show that the pair \((Y, d|_{Y \times Y})\) defined in \labelcrefs{ii:1.1.5} is indeed a metric space.
\end{ex}
...
\begin{proof}
  ...
  Thus, by \labelcrefs{ii:1.1.2} \((Y, d|_{Y \times Y})\) is a metric space.
\end{proof}
...
\begin{ex}\label{ii:ex:1.1.6}
  Show that \((\R^n, d_{l^2})\) in \labelcrefs{ii:1.1.6} is indeed a metric space.
\end{ex}
...
\begin{proof}
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:1.1.2} \((\R^n, d_{l^2})\) is a metric space.
\end{proof}

\begin{ex}\label{ii:ex:1.1.7}
  Show that \((\R^n, d_{l^1})\) in \labelcrefs{ii:1.1.7} is indeed a metric space.
\end{ex}
...
\begin{proof}
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:1.1.2} \((\R^n, d_{l^1})\) is a metric space.
\end{proof}
...
\begin{ex}\label{ii:ex:1.1.9}
  Show that \((\R^n, d_{l^\infty})\) in \labelcrefs{ii:1.1.9} is indeed a metric space.
\end{ex}
...
\begin{proof}
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:1.1.2} \((\R^n, d_{l^\infty})\) is a metric space.
\end{proof}
...
\begin{ex}\label{ii:ex:1.1.11}
  Show that \((X, d_{\text{disc}})\) in \labelcrefs{ii:1.1.11} is indeed a metric space.
\end{ex}
...
\begin{proof}
  ...
  \]
  Thus, by \labelcrefs{ii:1.1.2} \((X, d_{\text{disc}})\) is a metric space.
\end{proof}

\begin{ex}\label{ii:ex:1.1.12}
  Prove \labelcrefs{ii:1.1.18}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.1.18}.
\end{proof}

\begin{ex}\label{ii:ex:1.1.13}
  Prove \labelcrefs{ii:1.1.19}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.1.19}.
\end{proof}

\begin{ex}\label{ii:ex:1.1.14}
  Prove \labelcrefs{ii:1.1.20}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.1.20}.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:1.1.2} \((X, d_{l^1})\) and \((X, d_{l^\infty})\) are metric spaces.
  ...
  By \labelcrefs{ii:1.1.14} we need to show that
  \[
