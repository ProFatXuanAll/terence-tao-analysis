...
\begin{prop}\label{ii:1.3.4}
  Let \((X, d)\) be a metric space, let \(Y\) be a subset of \(X\), and let \(E\) be a subset of \(Y\).
  \begin{enumerate}
    \item\label{ii:1.3.4.a} \(E\) is relatively open with respect to \(Y\) iff \(E = V \cap Y\) for some set \(V \subseteq X\) which is open in \(X\).
    \item\label{ii:1.3.4.b} \(E\) is relatively closed with respect to \(Y\) iff \(E = K \cap Y\) for some set \(K \subseteq X\) which is closed in \(X\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{ii:1.3.4.a}]
  ...
  By \labelcrefs{ii:1.2.15.c} and (g), \(V\) is open in \((X, d)\).
...
\begin{proof}[\pf{ii:1.3.4.b}]
  First, suppose that \(E\) is relatively closed with respect to \(Y\).
  Then by \labelcrefs{ii:1.3.3} \(E\) is closed in \((Y, d|_{Y \times Y})\).
  By \labelcrefs{ii:1.2.15.e} \(Y \setminus E\) is open in \((Y, d|_{Y \times Y})\).
  By \labelcrefs{ii:1.3.4.a} we know that \(Y \setminus E = V \cap Y\) for some set \(V \subseteq X\) which is open in \((X, d)\).
  Let \(K = X \setminus V\).
  By \labelcrefs{ii:1.2.15.e} we know that \(K\) is closed in \((X, d)\).
  Then we have
  \begin{align*}
  ...
  Then by \labelcrefs{ii:1.2.15.e} \(X \setminus K\) is open in \((X, d)\).
  By \labelcrefs{ii:1.3.4.a} we know that \(F = (X \setminus K) \cap Y\) is relatively open with respect to \(Y\).
  Then by \labelcrefs{ii:1.2.15.e} \(Y \setminus F\) is relatively closed with respect to \(Y\) and
  \begin{align*}
...
\begin{ex}\label{ii:ex:1.3.1}
  Prove \labelcrefs{ii:1.3.4.b}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:1.3.4}.
\end{proof}
