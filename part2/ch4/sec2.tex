...
\begin{note}
  From \labelcrefs{ii:4.1.6.c} and (d) we see that if \(f\) is real analytic at a point \(a\), then \(f\) is both continuous and diﬀerentiable on \((a - r, a + r)\) for some \(r > 0\).
\end{note}
...
\begin{proof}
  ...
  By \labelcrefs{ii:4.2.1} we know that \(r \leq R\) and thus
  \[
    \forall x \in (a - r, a + r), \abs{x - a} < r \leq R.
  \]
  We induct on \(k\).
  For \(k = 0\), by \labelcrefs{ii:4.2.4} we have
  \[
  ...
  \]
  for some \(k \geq 0\).
  By \labelcrefs{ii:ac:4.2.1} we know that
  \begin{align*}
  ...
  \end{align*}
  Thus, by \labelcrefs{ii:4.1.6.b} we know that \(f^{(k)}(x)\) converges for all \(x \in (a - r, a + r)\).
  Now we define
  \[
    \forall n \in \N, b_n = c_{n + k} \dfrac{(n + k)!}{n!}.
  \]
  Then by \labelcrefs{ii:4.1.6.d} we have
  \begin{align*}
...
\begin{proof}
  For every point \(a \in E\) and \(k \geq 0\), we know from \labelcrefs{ii:4.2.6} that \(f\) is \(k\)-times differentiable at \(a\)
  ...
  Also, from \labelcrefs{ii:4.2.6} we see that each derivative \(f^{(k)}\) of \(f\) has a convergent power series expansion at every \(x \in E\) and thus \(f^{(k)}\) is real analytic.
\end{proof}

\setcounter{thm}{8}
\begin{rmk}\label{ii:4.2.9}
  The converse statement to \labelcrefs{ii:4.2.7} is not true;
  there are infinitely differentiable functions which are not real analytic.
\end{rmk}

\begin{note}
  \labelcrefs{ii:4.2.6} has an important corollary (\labelcrefs{ii:4.2.10}), due to Brook Taylor (1685--1731).
\end{note}
...
\begin{proof}
  By \labelcrefs{ii:4.2.10}, we have \(f^{(k)}(a) = k! c_k\) for all \(k \geq 0\).
...
\begin{proof}
  ...
  By \labelcrefs{ii:4.2.4} we have
  \[
  ...
  \[
    \forall k > n, f^{(k)}(x) = 0
  \]
  and by \labelcrefs{ii:4.2.4} \(f\) is infinitely differentiable.
\end{proof}

\begin{ex}\label{ii:ex:4.2.2}
  Show that the function \(f\) defined in \labelcrefs{ii:4.2.2} is real analytic on all of \(\R \setminus \set{1}\).
\end{ex}
...
\begin{proof}
  ...
  By \labelcrefs{ii:4.2.1} we know that \(f\) is real analytic at \(a\).
  Since \(a\) was arbitrary, by \labelcrefs{ii:4.2.1} we know that \(f\) is real analysis at \(a\) for each \(a \in \R \setminus \set{1}\).
\end{proof}

\begin{ex}\label{ii:ex:4.2.3}
  Prove \labelcrefs{ii:4.2.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:4.2.6}.
\end{proof}

\begin{ex}\label{ii:ex:4.2.4}
  Use \labelcrefs{ii:4.2.6} and \labelcrefs{ii:ex:4.2.1} to prove \labelcrefs{ii:4.2.10}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:4.2.10}.
\end{proof}
...
\begin{ex}\label{ii:ex:4.2.5}
  ...
  Explain why this identity is consistent with Taylor's theorem and \labelcrefs{ii:ex:4.2.1}.
  (Note however that Taylor's theorem cannot be rigorously applied until one verifies \labelcrefs{ii:ex:4.2.6} below.)
\end{ex}
...
\begin{proof}
  ...
  By \labelcrefs{ii:4.2.1} we know that \(f\) is real analytic at \(b\).
  By \labelcrefs{ii:4.2.10} we have
  \begin{align*}
    \forall x \in \R, f(x) = \sum_{m = 0}^\infty \dfrac{f^{(m)}(b)}{m!} (x - b)^m.
  \end{align*}
  By \labelcrefs{ii:ex:4.2.1} we have
  \[
...
\begin{ex}\label{ii:ex:4.2.6}
  Using \labelcrefs{ii:ex:4.2.5}, show that every polynomial \(P(x)\) of one variable is real analytic on \(\R\).
\end{ex}
...
\begin{proof}
  ...
  By \labelcrefs{ii:4.2.1} \(P\) is real analytic at \(0\).
  ...
  By \labelcrefs{ii:4.2.1} \(P\) is real analytic at \(a\).
...
\begin{ex}\label{ii:ex:4.2.7}
  ...
  \]
  for all \(x \in (-r, r)\).
  Using \labelcrefs{ii:4.2.6}, conclude the identity
  \[
...
\begin{proof}
  ...
  \]
  and by \labelcrefs{ii:4.2.1} \(x \mapsto \dfrac{r}{r - x}\) is real analytic at \(0\).
  ...
  \]
  for some \(m \geq 0\).
  Then by \labelcrefs{ii:4.2.4} we have
  \[
  ...
  \]
  By \labelcrefs{ii:4.2.6} we know that
  \begin{align*}
...
\begin{ex}\label{ii:ex:4.2.8}
  ...
  \begin{enumerate}
    \item\label{ii:ex:4.2.8.a} Prove that \(\abs{a - b} \leq r - s\), so, in particular, \(\abs{a - b} < r\).
    \item\label{ii:ex:4.2.8.b} Show that for every \(0 < \varepsilon < r\), there exists a \(C > 0\) such that \(\abs{c_n} \leq C(r - \varepsilon)^{-n}\) for all integers \(n \geq 0\).
    \item\label{ii:ex:4.2.8.c} Show that the numbers \(d_0, d_1, \dots\) given by the formula
    \[
    ...
    \item\label{ii:ex:4.2.8.d} Show that for every \(0 < \varepsilon < s\) there exists a \(C > 0\) such that
    \[
      \abs{d_m} \leq C(s - \varepsilon)^{-m}
    \]
    for all integers \(m \geq 0\).
    \item\label{ii:ex:4.2.8.e} Show that the power series \(\sum_{m = 0}^\infty d_m (x - b)^m\) is absolutely convergent for \(x \in (b - s, b + s)\) and converges to \(f(x)\).
    \item\label{ii:ex:4.2.8.f} Conclude that \(f\) is real analytic at every point in \((a - r, a + r)\).
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:4.2.8.a}]
  \begin{align*}
...
\begin{proof}[\pf{ii:ex:4.2.8.c}]
  By \labelcrefs{ii:ex:4.2.8.a} we know that \(\abs{b - a} = \abs{a - b} < r\), thus
  \[
  ...
  \]
  Fix such \(\varepsilon\).
  By \labelcrefs{ii:ex:4.2.8.b} we know that
  \[
...
\begin{proof}[\pf{ii:ex:4.2.8.d}]
  Let \(\varepsilon \in (0, s)\).
  By \labelcrefs{ii:ex:4.2.8.a} we have
  \begin{align*}
             & \abs{a - b} \leq r - s                                                    \\
...
\begin{proof}[\pf{ii:ex:4.2.8.f}]
  ...
  \end{align*}
  by \labelcrefs{ii:ex:4.2.8.e} we know that \(f\) is real analytic at \(b\).
