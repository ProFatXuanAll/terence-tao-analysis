\section{Abel's theorem}\label{ii:sec:4.3}
...
\begin{note}
  ...
  From \labelcrefs{ii:4.1.6} we know that the power series converges absolutely whenever \(\abs{x - a} < R\), and diverges when \(\abs{x - a} > R\).
  However, at the boundary \(\abs{x - a} = R\) the situation is more complicated;
  the series may either converge or diverge (see \labelcrefs{ii:ex:4.1.2}).
...
\begin{thm}[Abel's theorem]\label{ii:4.3.1}
...
\begin{proof}
  ...
  \]
  Applying the summation by parts formula (\labelcrefs{ii:4.3.2}), and noting that \(\lim_{n \to \infty} y^n = 0\), we obtain
  \[
...
\begin{lem}[Summation by parts formula]\label{ii:4.3.2}
...
\begin{ex}\label{ii:ex:4.3.1}
  Prove \labelcrefs{ii:4.3.2}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:4.3.2}.
\end{proof}
