\section{Formal power series}\label{ii:sec:4.1}

\begin{defn}[Formal power series]\label{ii:4.1.1}
...
\setcounter{thm}{2}
\begin{defn}[Radius of convergence]\label{ii:4.1.3}
...
\begin{thm}\label{ii:4.1.6}
  ...
  \begin{enumerate}
    \setcounter{enumi}{2}
    \item\label{ii:4.1.6.c} (Uniform convergence on compact sets)
    ...
    \item\label{ii:4.1.6.d} (Differentiation of power series)
    ...
    \item\label{ii:4.1.6.e} (Integration of power series)
...
\begin{proof}[\pf{ii:4.1.6.a,ii:4.1.6.b}]
...
\begin{proof}[\pf{ii:4.1.6.c}]
  Let \(r \in (0, R)\).
  Since
  \[
    \forall x \in [a - r, a + r] \implies \abs{x - a} < r < R,
  \]
  by \labelcrefs{ii:4.1.6.b} we know that \(\sum_{n = 0}^\infty c_n (x - a)^n\) is absolutely convergent for all \(x \in [a - r, a + r]\).
  ...
  \end{align*}
  by \labelcrefs{ii:3.5.7} we know that \(\big(\sum_{n = 0}^N f_n\big)_{N = 0}^\infty\) converges uniformly to some function \(g : [a - r, a + r] \to \R\) on \([a - r, a + r]\) with respect to \(d_{l^1}|_{\R \times \R}\) and \(g\) is continuous on \([a - r, a + r]\).
  But by \labelcrefs{ii:3.5.2} we know that
  \[
...
\begin{proof}[\pf{ii:4.1.6.d}]
  ...
  By \labelcrefs{ii:4.1.6.c} we know that
  \begin{align*}
    ...
    \implies & \Bigg(\bigg(\sum_{n = 1}^N f_n\bigg)'\Bigg)_{N = 1}^\infty \text{ converges uniformly to some } g : [a - r, a + r] \to \R \\
             & \text{ on } [a - r, a + r] \text{ with respect to } d_{l^1}|_{\R \times \R}
  \end{align*}
  By \labelcrefs{ii:3.5.2} we know that
  \[
...
\begin{note}
  \labelcrefs{ii:4.1.6} (a) and (b) of the above theorem give another way to find the radius of convergence, by using your favorite convergence test to work out the range of \(x\) for which the power series converges.
\end{note}

\setcounter{thm}{7}
\begin{rmk}\label{ii:4.1.8}
  \labelcrefs{ii:4.1.6} is silent on what happens when \(\abs{x - a} = R\), i.e., at the points \(a - R\) and \(a + R\).
  Indeed, one can have either convergence or divergence at those points.
\end{rmk}

\begin{rmk}\label{ii:4.1.9}
  Note that while \labelcrefs{ii:4.1.6} assures us that the power series \(\sum_{n = 0}^\infty c_n (x - a)^n\) will converge pointwise on the interval \((a - R, a + R)\), it need not converge uniformly on that interval
  (see \labelcrefs{ii:ex:4.1.2.e}).
  On the other hand, \labelcrefs{ii:4.1.6.c} assures us that the power series will converge uniformly on any smaller interval \([a - r, a + r]\).
...
\begin{ex}\label{ii:ex:4.1.1}
  Prove \labelcrefs{ii:4.1.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:4.1.6}.
\end{proof}

\begin{ex}\label{ii:ex:4.1.2}
  Give examples of a formal power series \(\sum_{n = 0}^\infty c_n x^n\) centered at \(0\) with radius of convergence \(1\), which
  \begin{enumerate}
    \item\label{ii:ex:4.1.2.a} diverges at both \(x = 1\) and \(x = -1\);
    \item\label{ii:ex:4.1.2.b} diverges at \(x = 1\) but converges at \(x = -1\);
    \item\label{ii:ex:4.1.2.c} converges at \(x = 1\) but diverges at \(x = -1\);
    \item\label{ii:ex:4.1.2.d} converges at both \(x = 1\) and \(x = -1\).
    \item\label{ii:ex:4.1.2.e} converges pointwise on \((-1, 1)\), but does not converge uniformly on \((-1, 1)\).
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{ii:ex:4.1.2.a}]
...
\begin{proof}[\pf{ii:ex:4.1.2.e}]
  ...
  But by \labelcrefs{ii:3.5.8} we know that \(\sum_{n = 0}^\infty x^n\) does not converge uniformly on \((-1, 1)\).
\end{proof}
