...
\begin{defn}[Connected spaces]\label{ii:2.4.1}
  ...
  (Equivalently, \(X\) is disconnected iff \(X\) contains a non-empty proper subset which is simultaneously closed and open, see \labelcrefs{ii:1.2.15.e}.)
...
\begin{thm}\label{ii:2.4.5}
  ...
  \begin{enumerate}
    \item\label{ii:2.4.5.a} \(X\) is connected.
    \item\label{ii:2.4.5.b} Whenever \(x, y \in X\) and \(x < y\), the interval \([x, y]\) is also contained in \(X\).
    \item\label{ii:2.4.5.c} \(X\) is an interval (in the sense of Definition 9.1.1 in Analysis I).
  \end{enumerate}
\end{thm}
...
\begin{thm}[Continuity preserves connectedness]\label{ii:2.4.6}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies & f^{-1}(V_1), f^{-1}(V_2) \text{ are open in } (E, d_X|_{E \times E})                                &  & \by{ii:2.1.5.a,ii:2.1.5.c} \\
...
\begin{proof}
  Since \(f\) is continuous from \((X, d_X)\) to \((\R, d_{l^1}|_{\R \times \R})\) and \((E, d_X|_{E \times E})\) is connected, by \labelcrefs{ii:2.4.6} we know that \(\big(f(E), d_{l^1}|_{f(E) \times f(E)}\big)\) is connected.
  By \labelcrefs{ii:2.4.5.a,ii:2.4.5.c} we know that \(f(E)\) is an interval.
  Thus, we have
  \begin{align*}
...
\begin{proof}
  ...
  By \labelcrefs{ii:1.2.8} we know that \(V, W\) are open in \((E, d_{\text{disc}}|_{E \times E})\).
  Since \(V \neq \emptyset \neq W\), \(V \cup W = E\) and \(V \cap W = \emptyset\), by \labelcrefs{ii:2.4.3} we know that \((E, d_{\text{disc}}|_{E \times E})\) is disconnected.
\end{proof}
...
\begin{proof}
  ...
  \]
  and by \labelcrefs{ii:ex:2.4.1} we know that \(\big(f(E), d_{\text{disc}}|_{E \times E}\big)\) is disconnected.
  But \((X, d)\) is connected, thus by \labelcrefs{ii:2.4.6} we know that \(\big(f(E), d_{\text{disc}}|_{E \times E}\big)\) is connected, a contradiction.
...
\begin{ex}\label{ii:ex:2.4.3}
  Prove the equivalence of statements (b) and (c) in \labelcrefs{ii:2.4.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:2.4.5}.
\end{proof}

\begin{ex}\label{ii:ex:2.4.4}
  Prove \labelcrefs{ii:2.4.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:2.4.6}.
\end{proof}

\begin{ex}\label{ii:ex:2.4.5}
  Use \labelcrefs{ii:2.4.6} to prove \labelcrefs{ii:2.4.7}.
\end{ex}

\begin{proof}
  See \labelcrefs{ii:2.4.7}.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{ii:2.4.1} we know that
  \[
...
\begin{proof}
  ...
  But then we have
  \begin{align*}
             & ([0, 1], d_{l^1}|_{\R \times \R}) \text{ is connected}                                   &  & \by{ii:2.4.5.a,ii:2.4.5.c} \\
...
\begin{proof}
  ...
  \[
    ...
    \end{dcases}
  \]
  By \labelcrefs{ii:1.2.10.a,ii:1.2.10.b} we know that \(E \subseteq \overline{E}\), thus we know that at least one of \(E \cap V\) and \(E \cap W\) is not empty.
  ...
  \begin{itemize}
    \item \(E \cap V = \emptyset\).
          ...
          \end{align*}
          which contradict to \labelcrefs{ii:1.2.10.c}.
  ...
  But then we have
  \begin{align*}
             & E \subseteq \overline{E}                                     &  & \by{ii:1.2.10.a,ii:1.2.10.b} \\
    \implies & \begin{dcases}
  ...
  But by \labelcrefs{ii:2.4.5.a,ii:2.4.5.c} we know that \((\overline{E}_{(\R, d_{l^1}|_{\R \times \R})}, d_{l^1}|_{\R \times \R})\) is connected but \((E, d_{l^1}|_{\R \times \R})\) is not.
\end{proof}
...
\begin{proof}
  ...
  To show that \(E_x\) is closed in \((X, d)\), by \labelcrefs{ii:1.2.15.b} we need to show that \(E_x = \overline{E}_x\).
  Since \((E_x, d|_{E_x \times E_x})\) is connected, by \labelcrefs{ii:ex:2.4.8} we know that \((\overline{E}_x, d|_{\overline{E}_x \times \overline{E}_x})\) is connected.
  Thus, we have
  \begin{align*}
             & E_x \subseteq \overline{E}_x                                            &  & \by{ii:1.2.10.a,ii:1.2.10.b} \\
...
\begin{ex}\label{ii:ex:2.4.10}
  Combine \labelcrefs{ii:2.3.2} and \labelcrefs{ii:2.4.7} to deduce a theorem for continuous functions on a compact connected domain which generalizes Corollary 9.7.4 in Analysis I.
\end{ex}
...
\begin{proof}
  ...
  Since \((E, d|_{E \times E})\) is compact, by \labelcrefs{ii:2.3.2} we know that
  \[
    \exists x_{\min}, x_{\max} \in E : \forall x \in E, f(x_{\min}) \leq f(x) \leq f(x_{\max}).
  \]
  Then by \labelcrefs{ii:2.4.7} we have
  \[
