...
\begin{proof}{(a)}
    ...
    This means \((x^{(n)})_{n = 1}^\infty\) converges to some \(x^{(N)} \in X\) with respect to \(d\).
    Since \((x^{(n)})_{n = 1}^\infty\) is arbitrary, by Definition \ref{1.4.10} we know that \((X, d)\) is complete.
\end{proof}
