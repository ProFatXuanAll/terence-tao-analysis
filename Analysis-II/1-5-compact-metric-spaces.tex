...
\begin{proof}{(c)}
    ...
    Let \((x^{(n)})_{n = 1}^\infty\) be a sequence in \(X\) and let \(E = \{x^{(n)} : n \in \mathbf{Z}^+\}\).
    Since \(X\) is totally bounded in \((X, d)\), by definition we have
    \[
        \forall\ \varepsilon \in \mathbf{R}^+, \exists\ F \subseteq X : (F \text{ is finite}) \land \bigg(X \subseteq \bigcup_{y \in F} B_{(X, d)}(y, \varepsilon)\bigg).
    \]
    Since \(E \subseteq X\), we have
    \[
        \forall\ \varepsilon \in \mathbf{R}^+, \exists\ F \subseteq X : (F \text{ is finite}) \land \bigg(E \subseteq X \subseteq \bigcup_{y \in F} B_{(X, d)}(y, \varepsilon)\bigg).
    \]
    Since the choice of \(F\) depends on \(\varepsilon\) (and do not depends on \(E\)), we denote \(F\) as \(F_\varepsilon\) for every \(\varepsilon \in \mathbf{R}^+\).
    Note that here we use axiom of choice.

    Since \(E\) is infinite and \(F_\varepsilon\) is finite for every \(\varepsilon \in \mathbf{R}^+\), we know that there exists a \(y \in F_\varepsilon\) such that \(E \cap B_{(X, d)}(y, \varepsilon)\) is infinite.
    If not, then we would have
    \begin{align*}
                 & \exists\ \varepsilon \in \mathbf{R}^+ : \forall\ y \in F_\varepsilon, E \cap B_{(X, d)}(y, \varepsilon) \text{ is finite}                                                          \\
        \implies & \exists\ \varepsilon \in \mathbf{R}^+ : \bigcup_{y \in F_\varepsilon} \bigg(E \cap B_{(X, d)}(y, \varepsilon)\bigg) \text{ is finite} & \text{(since \(F_\varepsilon\) is finite)} \\
        \implies & \exists\ \varepsilon \in \mathbf{R}^+ : E \cap \bigg(\bigcup_{y \in F_\varepsilon} B_{(X, d)}(y, \varepsilon)\bigg) \text{ is finite}                                              \\
        \implies & \exists\ \varepsilon \in \mathbf{R}^+ : E \subseteq E \cap \bigg(\bigcup_{y \in F_\varepsilon} B_{(X, d)}(y, \varepsilon)\bigg)
    \end{align*}
    and \(E\) would be finite, a contradiction.
    Since the choice of \(y\) depends on \(F_\varepsilon\) (which depends on \(\varepsilon\)), we denote \(y\) as \(y_\varepsilon\) for every \(\varepsilon \in \mathbf{R}^+\).
    Again we use axiom of choice.

    Now we construct a subsequence of \(x^{(n)}\).
    For each \(j \in \mathbf{Z}^+\) we define \(N_j\) as follow:
    \[
        N_j = \bigg\{n \in \mathbf{Z}^+ : x^{(n)} \in E \cap B_{(X, d)}(y_{\frac{1}{j}}, \frac{1}{j})\bigg\}.
    \]
    From the claim above we know that \(N_j\) is infinite for every \(j \in \mathbf{Z}^+\).
    ...
    \[
        ...
        \end{cases}
    \]
    Since \(N_j\) is infinite for each \(j \in \mathbf{Z}^+\), we know that the set \(\{n \in N_{j - 1} : n > n_{j - 1}\}\) is also infinite.
    If not, then the maximum element of \(N_{j - 1}\) would be \(n_{j - 1}\) and \(N_{j - 1}\) is finite, a contradiction.
    Since \(N_j \subseteq \mathbf{Z}^+\) for each \(j \in \mathbf{Z}^+\), by well-ordering principle we know that the minimum of \(N_j\) or the subsets of \(N_j\) are well-defined.
    ...
    \begin{align*}
        ...
        \implies & \forall\ \varepsilon \in \mathbf{R}^+, \exists\ J \in \mathbf{Z}^+ :                                                                                         \\
                 & (\frac{1}{J} < \varepsilon) \land (\forall\ j \geq J, N_j \subseteq N_J)                                                                                     \\
        \implies & \forall\ \varepsilon \in \mathbf{R}^+, \exists\ J \in \mathbf{Z}^+ :                                                                                         \\
                 & (\frac{1}{J} < \varepsilon) \land \big(\forall\ j \geq J, d(x^{(n_j)}, y_{\frac{1}{J}}) < \frac{1}{J} < \varepsilon\big),
    \end{align*}
    If we fix \(J\) for each \(\varepsilon\), then we have
    \begin{align*}
        \forall\ i, j \geq 2J, d(x^{(n_i)}, x^{(n_j)}) & \leq d(x^{(n_i)}, y_{\frac{1}{2J}}) + d(x^{(n_j)}, y_{\frac{1}{2J}}) & \text{(by Definition \ref{1.1.2}(d))} \\
                                                       & < \frac{1}{2J} + \frac{1}{2J} = \frac{1}{J} < \varepsilon.
    \end{align*}
