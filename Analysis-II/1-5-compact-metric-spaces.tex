...
\begin{proof}
    ...
    \]
    is a open set and by Proposition \ref{1.2.15}(d)
    \[
        f\big((0, 2)\big) = \{1\} = [1, 1]
    \]
    is a closed set.
    By Proposition \ref{1.2.15}(a) we know that \(\{1\}\) is not a open set since we cannot find an \(r \in \mathbf{R}^+\) such that \(B_{(\mathbf{R}, d_{l^1})}(1, r) \subseteq \{1\}\).
...
\begin{proof}
    ...
    We first show that \((E, d_{l^1})\) is bounded in \(X\).
    Since
    \[
    ...
    Thus by Definition \ref{1.5.3} we know that \((E, d_{l^1})\) is bounded in \(X\).

    Next we show that \((E, d_{l^1})\) is closed.
    Let \(\overline{E}\) be the closure of \((E, d_{l^1})\) in \(X\) and let \(x \in \overline{E}\).
    ...
    \[
        d(a^{(k)}, a^{(k')}) = d(e^{(n)}, e^{(n')}) = \begin{cases}
            0 & \text{if } n = n';    \\
            2 & \text{if } n \neq n'.
        \end{cases}
    \]
    ...
    \begin{align*}
        ...
        \implies & x \in E
    \end{align*}
    Since \(x\) is arbitrary adherent point, by Proposition \ref{1.2.15}(b) we know that \((E, d_{l^1})\) is closed in \(X\).
    ...
    \[
        \forall\ N \in \mathbf{N}, \forall\ n, n' \geq N, d(e^{(n)}, e^{(n')}) = 2.
    \]
    Thus by Definition \ref{1.4.6} \((e^{(n)})_{n = 0}^\infty\) is not a Cauchy sequence in \((E, d_{l^1})\).
    Similarly any subsequence of \((e^{(n)})_{n = 0}^\infty\) is not a Cauchy sequence in \((E, d_{l^1})\).
    By Lemma \ref{1.4.7} this means no subsequence is convergent in \((E, d_{l^1})\), and by Definition \ref{1.5.1} \((E, d_{l^1})\) is not compact.
\end{proof}
