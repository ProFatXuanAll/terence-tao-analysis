...
\begin{remark}\label{3.4.1}
    If one is willing to work in topological spaces instead of metric spaces, we can also view (b) as a special case of (a), see Exercise \ref{ex 3.1.4}, and (c) is also a special case of (a), see Exercise \ref{ex 3.4.4}.
...
\begin{definition}[Metric space of bounded functions]\label{3.4.2}
    ...
    \]
    We define a metric \(d_\infty : B(X \to Y) \times B(X \to Y) \to \mathbf{R}^+\) by defining
    \[
        d_\infty(f, g) \coloneqq \sup_{x \in X} d_Y\big(f(x), g(x)\big) = \sup\{d_Y\big(f(x), g(x)\big) : x \in X\}
    \]
    for all \(f, g \in B(X \to Y)\).
    This metric is sometimes known as the \emph{sup norm metric} or the \(L^\infty\) metric.
