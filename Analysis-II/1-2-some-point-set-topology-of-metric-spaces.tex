...
\begin{proof}{(h)}
    ...
    % \begin{align*}
    %     & \forall\ x_0 \in \overline{\overline{E}} \\
    %     \implies & \forall\ r \in \mathbf{R}^+, B_{(X, d)}(x_0, r) \cap \overline{E} \neq \emptyset & \text{(by Definition \ref{1.2.9})} \\
    %     \implies & \forall\ r \in \mathbf{R}^+, B_{(X, d)}(x_0, r) \cap (\text{int}(E) \cup \partial E) \neq \emptyset & \text{(by Corollary \ref{1.2.11})} \\
    %     \implies & \forall\ r \in \mathbf{R}^+, \big(B_{(X, d)}(x_0, r) \cap \text{int}(E) \neq \emptyset\big) \\
    %     & \lor \big(B_{(X, d)}(x_0, r) \cap \partial E \neq \emptyset\big) \\
    %     \implies & x_0 \in \overline{E}, & \text{(by Definition \ref{1.2.9})}
    % \end{align*}
\end{proof}