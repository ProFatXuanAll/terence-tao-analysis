...
\begin{definition}[\(k\)-times differentiability]\label{4.2.4}
    ...
    We say a function \(f : E \to \mathbf{R}\) is \emph{once differentiable on \(E\)} iff it is differentiable.
...
\begin{exercise}\label{ex 4.2.2}
    Show that the function \(f\) defined in Example 4.2.2 is real analytic on all of \(\mathbf{R} \setminus \{1\}\).
\end{exercise}
...
\begin{exercise}\label{ex 4.2.8}
    Let \(E\) be a subset of \(\mathbf{R}\), let \(a\) be an interior point of \(E\), and let \(f : E \to \mathbf{R}\) be a function which is real analytic in \(a\), and has a power series expansion
    \[
        f(x) = \sum_{n = 0}^\infty c_n (x - a)^n
    \]
