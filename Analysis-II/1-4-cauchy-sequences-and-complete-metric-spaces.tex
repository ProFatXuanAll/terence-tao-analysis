...
\begin{proof}{(c)}
    To show that \((\overline{X}, d_{\overline{X}})\) is complete, by Definition \ref{1.4.10} we need to show that for any Cauchy sequence \((a^{(n)})_{n = m}^\infty\) in \(\overline{X}\), \(\exists\ x \in \overline{X}\) such that \(\lim_{n \to \infty} d_{\overline{X}}(a^{(n)}, x) = 0\).
    % Since \(x \in \overline{X}\), we can find a sequence \((b^{(k)})_{k = 1}^{\infty}\) in \(X\) such that \(\text{LIM}_{n \to \infty} b^{(k)} = x\).
    % Thus by Exercise \ref{ex 1.4.8}(b) it will suffice to show that
    % \[
    %     \lim_{n \to \infty} d_{\overline{X}}(a^{(n)}, \text{LIM}_{k \to \infty} b^{(k)}) = \lim_{n \to \infty} 
    % \]
    % Let \((b^{(k)})_{k = 1}^\infty\) be a Cauchy sequence in \(X\) such that \(\text{LIM}_{k \to \infty} b^{(k)} = x\).
    % We know that \((b^{(k)})_{k = 1}^\infty\) exists by the definition of \(\overline{X}\).
    Since \(a^{(n)} \in \overline{X}\), we know that \(\forall\ n \geq m\), there exists a Cauchy sequence \((a_k^{(n)})_{k = m}^\infty\) in \(X\) such that \(\text{LIM}_{k \to \infty} a_k^{(n)} = a^{(n)}\).
    Similarly since \(x \in \overline{X}\), there exists a Cauchy sequence \((b_k)_{k = m}^{\infty}\) in \(X\) such that \(\text{LIM}_{n \to \infty} b_k = x\).
    Thus by Exercise \ref{ex 1.4.8}(b) it will suffice to show that
    \[
    ...
    \]
    for some Cauchy sequence \((b_k)_{k = m}^{\infty}\) in \(X\).
    % Since \((a^{(n)})_{n = m}^\infty\) is a Cauchy sequence, by Definition \ref{1.4.6} we know that \(\forall\ \varepsilon \in \mathbf{R}^+\), \(\exists\ N \in \mathbf{N}\) and \(N \geq m\) such that \(\forall\ i, j \in \mathbf{N}\) and \(i, j \geq N\), we have \(d_{\overline{X}}(a^{(i)}, a^{(j)}) \leq \varepsilon\).
    % By Exercise \ref{ex 1.4.8}(b) we know that
    % \[
    %     d_{\overline{X}}(a^{(i)}, a^{(j)}) = d_{\overline{X}}(\text{LIM}_{k \to \infty} a_k^{(i)}, \text{LIM}_{k \to \infty} a_k^{(j)}) = \lim_{k \to \infty} d(a_k^{(i)}, a_k^{(j)})
    % \]
    % is well-defined.
    % Let \((b_k^{(i, j)})_{k = 1}^\infty\) be a sequence in \(\mathbf{R}\) where \(b_k^{(i, j)} = d(a_k^{(i)}, a_k^{(j)})\).
    % From the proof of Exercise \ref{ex 1.4.8}(b) we know that \((b_k^{(i, j)})_{k = 1}^\infty\) is a Cauchy sequence.

    Let \((b_k)_{k = m}^\infty\) be the sequence where \(b_k = a_k^{(k)}\).
    We can construct \((b_k)_{k = m}^\infty\) by axiom of choice.
    We now show that \((b_k)_{k = m}^\infty\) is a Cauchy sequence.
    Since \(\forall\ n \geq m\), \((a_k^{(n)})_{k = 1}^\infty\) is a Cauchy sequence, by Definition \ref{1.4.6} \(\forall\ \varepsilon \in \mathbf{R}^+\), \(\exists\ N_k \in \mathbf{Z}^+\) such that \(\forall\ i, j \in \mathbf{Z}^+\) and \(i, j \geq N_k\), we have \(d(a_i^{(n)}, a_j^{(n)}) \leq \varepsilon\).
\end{proof}