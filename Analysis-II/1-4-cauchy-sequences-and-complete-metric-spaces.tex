...
\begin{proof}{(c)}
    ...
    Let \(j, K, k_1, k_2, N, n_1, n_2 \in \mathbf{Z}^+\).
    ...
    This means \(\forall\ k, k_1, k_2, n, n_1, n_2 \geq P(j, n)\), we have
    \begin{align*}
        d(a_{k_1}^{(n)}, a_{k_2}^{(n)}) & < \frac{1}{n}     \\
        d(a_k^{(n_1)}, a_k^{(n_2)})     & \leq \frac{1}{j}.
    \end{align*}
    By axiom of choice, we can choose \(a_{P(j, n)}^{(P(j, n))}\) for every \(j \geq 1\) and let \((b_j)_{j = 1}^\infty\) be a sequence in \(X\) where \(b_j = a_{P(j, n)}^{(P(j, n))}\).
    Then \(\forall\ k_1 \leq k_2\), we have
    \begin{align*}
    ...
    \[
        d(b_{k_1}, b_{k_2}) \leq \frac{2}{K} \leq \varepsilon.
    \]
    ...
    Since \(\forall\ k \geq P(j, n)\) we have
    \begin{align*}
        d(a_k^{(n)}, b_k) & = d(a_k^{(n)}, a_{P(k, k)}^{(P(k, k))})                                                                                  \\
                          & \leq d(a_k^{(n)}, a_{P(k, k)}^{(n)}) + d(a_{P(k, k)}^{(n)}, a_{P(k, k)}^{(P_k)}) & \text{(by Definition \ref{1.1.2}(d))} \\
