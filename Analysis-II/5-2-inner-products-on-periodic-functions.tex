...
\begin{proof}
    ...
    For each \(k \in \mathbf{Z}^+\) and \(k \geq 10\), we define \(f_k : [0, 1) \to \mathbf{C}\) to be the function:
    \[
        ...
        \end{cases}
    \]
    If we extended \(f_k\) periodically with period \(1\), then \(f_k \in C(\mathbf{R} / \mathbf{Z} ; \mathbf{C})\) for all \(k \geq 10\).
    ...
    Since \(f\) and \(f_k\) are \(1\)-periodic for all \(k \geq 10\), we know that \((f_k)_{k = 10}^\infty\) converges to \(f\) on \(\mathbf{R}\) with respect to \(d_{L^2}\).
\end{proof}
