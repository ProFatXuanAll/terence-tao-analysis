\section{Uniform continuity}\label{i:sec:9.9}

\setcounter{thm}{1}
\begin{defn}[Uniform continuity]\label{i:9.9.2}
...
\begin{rmk}\label{i:9.9.3}
  This definition should be compared with the notion of continuity.
  From \labelcrefs{i:9.4.7.c}, we know that a function \(f\) is \emph{continuous} if for every \(\varepsilon > 0\), and every \(x_0 \in X\), there is a \(\delta > 0\) such that \(f(x)\) and \(f(x_0)\) are \(\varepsilon\)-close whenever \(x \in X\) is \(\delta\)-close to \(x_0\).
...
\setcounter{thm}{4}
\begin{defn}[Equivalent sequences]\label{i:9.9.5}
...
\begin{rmk}\label{i:9.9.6}
  One could debate whether \(\varepsilon\) should be assumed to be rational or real, but a minor modification of \labelcrefs{i:6.1.4} shows that this does not make any difference to the above definitions.
\end{rmk}

\begin{lem}\label{i:9.9.7}
...
\begin{proof}
  \begin{align*}
    ...
    \iff & \forall \varepsilon \in \R^+, \exists N \in \Z^+ : \forall n \geq N, \abs{a_n - b_n} \leq \varepsilon       &  & \by{i:9.9.5} \\
    \iff & \forall \varepsilon \in \R^+, \exists N \in \Z^+ : \forall n \geq N, \abs{(a_n - b_n) - 0} \leq \varepsilon                   \\
    \iff & \lim_{n \to \infty} (a_n - b_n) = 0.                                                                        &  & \by{i:6.1.5}
  \end{align*}
\end{proof}

\begin{prop}\label{i:9.9.8}
  Let \(X\) be a subset of \(\R\), and let \(f : X \to \R\) be a function.
...
\begin{proof}
  ...
  By \labelcrefs{i:9.9.2}, \(f\) is uniformly continuous on \(X\) iff
  \[
    \forall \varepsilon \in \R^+, \exists \delta \in \R^+ : \big(\forall x, y \in X, \abs{x - y} < \delta \implies \abs{f(x) - f(y)} \leq \varepsilon\big).
  \]
  By \labelcrefs{i:9.9.5}, \((x_n)_{n = 0}^\infty = (y_n)_{n = 0}^\infty\) iff
  \[
  ...
  \]
  and by \labelcrefs{i:9.9.5} \(\big(f(x_n)\big)_{n = 0}^\infty = \big(f(y_n)\big)_{n = 0}^\infty\).
  ...
  Suppose for the sake of contradiction that \(f\) is not uniformly continuous on \(X\).
  Then we have
  \[
...
\begin{rmk}\label{i:9.9.9}
  The reader should compare \labelcrefs{i:9.9.8} with \labelcrefs{i:9.3.9}.
  \labelcrefs{i:9.3.9} asserted that if \(f\) was continuous, then \(f\) maps convergent sequences to convergent sequences.
  In contrast, \labelcrefs{i:9.9.8} asserts that if \(f\) is \emph{uniformly} continuous, then \(f\) maps \emph{equivalent} pairs of sequences to equivalent pairs of sequences.
  To see how the two Propositions are connected, observe from \labelcrefs{i:9.9.7} that \((x_n)_{n = 0}^\infty\) will converge to \(x_*\) iff the sequences \((x_n)_{n = 0}^\infty\) and \((x_*)_{n = 0}^\infty\) are equivalent.
\end{rmk}

\setcounter{thm}{11}
\begin{prop}\label{i:9.9.12}
...
\begin{proof}
  Since \(f\) is uniformly continuous, by \labelcrefs{i:9.9.2} we have
  \[
    \forall \varepsilon \in \R^+, \exists \delta \in \R^+ : \big(\forall x, y \in X, \abs{x - y} < \delta \implies \abs{f(x) - f(y)} \leq \varepsilon\big).
  \]
  Let \(i, j \in \N\).
  Since \((x_n)_{n = 0}^\infty\) is a Cauchy sequence, by \labelcrefs{i:6.1.3} we have
  \[
  ...
  \]
  and by \labelcrefs{i:6.1.3} \(\big(f(x_n)\big)_{n = 0}^\infty\) is a Cauchy sequence.
\end{proof}

\setcounter{thm}{13}
\begin{cor}\label{i:9.9.14}
...
\begin{proof}
  Since \(x_0\) is an adherent point of \(X\), by \labelcrefs{i:9.1.14} there exists a sequence \((a_n)_{n = 0}^\infty\), consisting entirely of elements in \(X\), which converges to \(x_0\).
  Since \(\lim_{n \to \infty} a_n = x_0\), by \labelcrefs{i:6.1.12} \((a_n)_{n = 0}^\infty\) is a Cauchy sequence.
  Since \(f\) is uniformly continuous, by \labelcrefs{i:9.9.12} \(\big(f(a_n)\big)_{n = 0}^\infty\) is also a Cauchy sequence.
  Let \((b_n)_{n = 0}^\infty\) in \(X\) such that \(\lim_{n \to \infty} b_n = x_0\).
  Since
  \begin{align*}
             & \lim_{n \to \infty} b_n = x_0                                                        \\
    \implies & \lim_{n \to \infty} a_n - b_n = 0                                 &  & \by{i:6.1.19} \\
    \implies & (a_n)_{n = 0}^\infty = (b_n)_{n = 0}^\infty                       &  & \by{i:9.9.7}  \\
    \implies & \big(f(a_n)\big)_{n = 0}^\infty = \big(f(b_n)\big)_{n = 0}^\infty &  & \by{i:9.9.8}  \\
    \implies & \lim_{n \to \infty} f(a_n) = \lim_{n \to \infty} f(b_n)           &  & \by{i:9.9.12}
  \end{align*}
  and \((b_n)_{n = 0}^\infty\) was arbitrary, by \labelcrefs{i:9.3.9} we know that \(\lim_{x \to x_0 ; x \in X} f(x)\) exists.

  Now we show that \(f : (0, 2) \to \R\) defined by \(f(x) = 1 / x\) is not uniformly continuous.
  Suppose for the sake of contradiction that \(f\) is uniformly continuous.
...
\begin{proof}
  Suppose for the sake of contradiction that \(f(E)\) is unbounded.
  ...
  Since \((x_n)_{n = 0}^\infty\) in \(E\), by Bolzano-Weierstrass theorem (\labelcrefs{i:6.6.8}) there exists a subsequence \((x_{n_j})_{j = 0}^\infty\) which converges, where \(n_0 < n_1 < n_2 < \dots\) is an increasing sequence of natural numbers.
  In particular, we see that \(n_j \geq j\) for all \(j \in \N\) (use induction).

  Let \(\lim_{n \to \infty} x_n = x_*\).
  Since \((x_n)_{n = 0}^\infty\) in \(E\), by \labelcrefs{i:9.1.14} we know that \(x_*\) is an adherent point of \(E\).
  Since \(f\) is continuous on \(X\), by \labelcrefs{i:ex:9.4.6} \(f\) is continuous on \(E\).
  In particular, \(f\) is uniformly continuous on \(E\).
  Thus, by \labelcrefs{i:9.9.14} we know that \(\lim_{x \to x_* ; x \in E} f(x)\) exists.
  By \labelcrefs{i:9.3.9} we see that
  \[
...
\begin{proof}
  ...
  By \labelcrefs{i:9.9.8}, there must therefore exist two equivalent sequences \((x_n)_{n = 0}^\infty\) and \((y_n)_{n = 0}^\infty\) in \([a, b]\) such that the sequences \(\big(f(x_n)\big)_{n = 0}^\infty\) and \(\big(f(y_n)\big)_{n = 0}^\infty\) are not equivalent.
  ...
  By \labelcrefs{i:8.1.5}, \(E\) is countable;
  ...
  In particular, we have
  \[
    \abs{f(x_{n_j}) - f(y_{n_j})} > \varepsilon \text{ for all } j \in \N. \tag{i:9.3}\label{i:eq 9.3}
  \]
  On the other hand, the sequence \((x_{n_j})_{j = 0}^\infty\) is a sequence in \([a, b]\), and so by the Heine-Borel theorem (\labelcrefs{i:9.1.24}) there must be a subsequence \((x_{n_{j_k}})_{k = 0}^\infty\) which converges to some limit \(L\) in \([a, b]\).
  In particular, \(f\) is continuous at \(L\), and so by \labelcrefs{i:9.4.7},
  \[
    \lim_{k \to \infty} f(x_{n_{j_k}}) = f(L). \tag{i:9.4}\label{i:eq 9.4}
  \]
  Note that \((x_{n_{j_k}})_{k = 0}^\infty\) is a subsequence of \((x_n)_{n = 0}^\infty\), and \((y_{n_{j_k}})_{k = 0}^\infty\) is a subsequence of \((y_n)_{n = 0}^\infty\), by \labelcrefs{i:6.6.4}.
  On the other hand, from \labelcrefs{i:9.9.7} we have
  \[
    \lim_{n \to \infty} (x_n - y_n) = 0.
  \]
  By \labelcrefs{i:6.6.5}, we thus have
  \[
  ...
  \]
  But this contradicts (9.3) (by \labelcrefs{i:9.9.7}).
  From this contradiction we conclude that \(f\) is in fact uniformly continuous.
\end{proof}

\begin{rmk}\label{i:9.9.17}
  One should compare \labelcrefs{i:9.6.3}, \labelcrefs{i:9.9.15}, and \labelcrefs{i:9.9.16} with each other.
  Note, in particular, that \labelcrefs{i:9.6.3} follows from combining \labelcrefs{i:9.9.15} and \labelcrefs{i:9.9.16}.
\end{rmk}

\exercisesection

\begin{ex}\label{i:ex:9.9.1}
  Prove \labelcrefs{i:9.9.7}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.9.7}.
\end{proof}

\begin{ex}\label{i:ex:9.9.2}
  Prove \labelcrefs{i:9.9.8}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.9.8}.
\end{proof}

\begin{ex}\label{i:ex:9.9.3}
  Prove \labelcrefs{i:9.9.12}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.9.12}.
\end{proof}

\begin{ex}\label{i:ex:9.9.4}
  Use \labelcrefs{i:9.9.12} to prove \labelcrefs{i:9.9.14}.
  Use this corollary to give an alternate demonstration of the results in Example 9.9.10.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.9.14}.
\end{proof}

\begin{ex}\label{i:ex:9.9.5}
  Prove \labelcrefs{i:9.9.15}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.9.15}.
\end{proof}

\begin{ex}\label{i:ex:9.9.6}
...
\begin{proof}
  Since \(f\) is continuous on \(X\) and \(g\) is continuous on \(Y\), by \labelcrefs{i:9.4.13} we know that \(g \circ f\) is continuous on \(X\).
  Since \(g\) is uniformly continuous, by \labelcrefs{i:9.9.2} we have
  \[
  ...
  \]
  Similarly, since \(f\) is uniformly continuous, by \labelcrefs{i:9.9.2} we have
  \[
  ...
  \]
  and by \labelcrefs{i:9.9.2} \(g \circ f\) is uniformly continuous on \(X\).
\end{proof}
