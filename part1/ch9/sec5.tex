\section{Left and right limits}\label{i:sec:9.5}

\begin{defn}[Left and right limits]\label{i:9.5.1}
...
\begin{note}
  From \labelcrefs{i:9.3.9} we see that if the right limit \(f(x_0+)\) exists, and \((a_n)_{n = 0}^\infty\) is a sequence in \(X\) converging to \(x_0\) from the right (i.e., \(a_n > x_0\) for all \(n \in \N\)), then \(\lim_{n \to \infty} f(a_n) = f(x_0+)\).
...
\begin{proof}
  Since \(f\) is continuous at \(x_0\), by \labelcrefs{i:9.4.1} we know that
  \[
  ...
  \]
  and by \labelcrefs{i:9.5.1} we have \(f(x_0+) = f(x_0)\).
...
\begin{proof}
  ...
  From the first statement above and \labelcrefs{i:9.4.7} (applied to the restriction of \(f\) to \(X \cap (x_0, +\infty)\)), we know that there exists a \(\delta_+ > 0\) such that \(\abs{f(x) - L} < \varepsilon\) for all \(x \in X \cap(x_0, \infty)\) for which \(\abs{x - x_0} < \delta_+\).
  ...
  \end{itemize}
  By \labelcrefs{i:9.4.7} we thus have that \(f\) is continuous at \(x_0\), as desired.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{i:ac:9.5.2} we have
  \begin{align*}
  ...
  \end{align*}
  But this contradicts \((\abs{x - x_0} < \delta) \land \big(f(x) \leq \varepsilon\big)\).
  Thus, \(\lim_{x \to x_0 ; x \in E} f(x) = +\infty\).
\end{proof}

\begin{ac}\label{i:ac:9.5.4}
  Let \(X \subseteq \R\), let \(E \subseteq X\), let \(x_0 \in \overline{E}\).
  Show that the following two statements are equivalent:
  \begin{enumerate}
    \item \(\lim_{x \to x_0 ; x \in E} f(x) = -\infty\).
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies & \exists N = \max(N_1, N_2) : \forall n \geq N, (\abs{a_n - x_0} < \delta) \land \big(f(a_n) < -\varepsilon\big).
  \end{align*}
  But this contradicts \((\abs{x - x_0} < \delta) \land \big(f(x) \geq -\varepsilon\big)\).
  Thus, \(\lim_{x \to x_0 ; x \in E} f(x) = -\infty\).
\end{proof}
...
\begin{ex}\label{i:ex:9.5.1}
  ...
  Also, state and prove some analogue of \labelcrefs{i:9.3.9} when \(L = +\infty\) or \(L = -\infty\).
\end{ex}

\begin{proof}
  See \labelcrefs{i:ac:9.5.2}, \labelcrefs{i:ac:9.5.3} and \labelcrefs{i:ac:9.5.4}.
\end{proof}
