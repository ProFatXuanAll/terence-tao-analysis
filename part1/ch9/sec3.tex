...
\begin{proof}
  ...
  Since \(f\) converges to \(L\) at \(x_0\) in \(E\), by \labelcrefs{i:9.3.6} we have
  \[
  ...
  Such sequence exists since \labelcrefs{i:9.1.14}.
  By \labelcrefs{i:6.1.5} we have
  \[
  ...
  \]
  and by \labelcrefs{i:6.1.5} we have \(\lim_{n \to \infty} f(a_n) = L\).
  ...
  Then by \labelcrefs{i:9.3.6} we have
  \[
  ...
  By \labelcrefs{i:6.1.5} the following two statements are true:
  \begin{align*}
...
\begin{note}
  In view of \labelcrefs{i:9.3.9}, we will sometimes write ``\(f(x) \to L\) as \(x \to x_0\) in \(E\)'' or ``\(f\) has a limit \(L\) at \(x_0\) in \(E\)'' instead of ``\(f\) converges to \(L\) at \(x_0\),'' or ``\(\lim_{x \to x_0} f(x) = L\).''
\end{note}

\begin{rmk}\label{i:9.3.10}
  With the notation of \labelcrefs{i:9.3.9}, we have the following corollary:
...
\begin{proof}
  ...
  Since \(x_0\) is an adherent point of \(E\), we know by \labelcrefs{i:9.1.14} that there is a sequence \((a_n)_{n = 0}^\infty\) consisting of elements in \(E\) which converges to \(x_0\).
  Since \(f\) has a limit \(L\) at \(x_0\) in \(E\), we thus see by \labelcrefs{i:9.3.9}, that \((f(a_n))_{n = 0}^\infty\) converges to \(L\).
  But since \(f\) also has a limit \(L'\) at \(x_0\) in \(E\), we see that \((f(a_n))_{n = 0}^\infty\) also converges to \(L'\).
  But this contradicts the uniqueness of limits of sequences (\labelcrefs{i:6.1.7}).
\end{proof}
...
\begin{proof}
  ...
  By the limit laws for sequences (\labelcrefs{i:6.1.19}) we conclude that
  \begin{align*}
  ...
  \end{align*}
  If \(\forall x \in E, g(x) \neq 0\) and \(M \neq 0\), then by the limit laws for sequences (\labelcrefs{i:6.1.19}) and \labelcrefs{i:9.2.1} we have
  \[
    \lim_{n \to \infty} \big(f(a_n) / g(a_n)\big) = \lim_{n \to \infty} \big((f / g)(a_n)\big)_{n = 0}^\infty = L / M.
  \]
  By \labelcrefs{i:9.3.9} again, this implies
  \[
    \begin{dcases}
...
\begin{rmk}\label{i:9.3.15}
  One can phrase \labelcrefs{i:9.3.14} more informally as saying that
  \begin{align*}
...
\begin{note}
  Informally, \labelcrefs{i:9.3.18} asserts that
  \[
...
\begin{proof}
  ...
  By axiom of choice (\labelcrefs{i:8.1}) we know that \(\prod_{n \in \N} X_{\dfrac{1}{n}} \neq \emptyset\).
  ...
  \end{align*}
  a contradiction.
  By axiom of choice (\labelcrefs{i:8.1}) we know that \(\prod_{n \in \Z^+} Y_n \neq \emptyset\).
  ...
  By squeeze test (\labelcrefs{i:6.4.14}) we have \(\lim_{n \to \infty} f(b_n) = L\).
  By \labelcrefs{i:6.4.12.f} we have \(\limsup_{n \to \infty} f(b_n) = \lim_{n \to \infty} f(b_n) = L\).
...
\begin{ex}\label{i:ex:9.3.1}
  Prove \labelcrefs{i:9.3.9}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.3.9}.
\end{proof}

\begin{ex}\label{i:ex:9.3.2}
  Prove the remaining claims in \labelcrefs{i:9.3.14}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.3.14}.
\end{proof}

\begin{ex}\label{i:ex:9.3.3}
  Prove \labelcrefs{i:9.3.18}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.3.18}.
\end{proof}

\begin{ex}\label{i:ex:9.3.4}
  Propose a definition for limit superior \(\limsup_{x \to x_0 ; x \in E} f(x)\) and limit inferior \(\liminf_{x \to x_0 ; x \in E} f(x)\), and then propose an analogue of \labelcrefs{i:9.3.9} for your definition.
  (For an additional challenge: prove that analogue.)
\end{ex}

\begin{proof}
  See Addtional \labelcrefs{i:ac:9.3.1} and Addtional \labelcrefs{i:ac:9.3.2}.
\end{proof}
...
\begin{proof}
  Since \(\lim_{x \to x_0 ; x \in E} f(x) = L\), by \labelcrefs{i:9.3.6} we have
  \[
  ...
  \]
  and thus by \labelcrefs{i:9.3.6} \(\lim_{x \to x_0 ; x \in E} g(x) = L\).
\end{proof}
