...
\begin{prop}[Equivalent formulations of continuity]\label{i:9.4.7}
  ...
  \begin{enumerate}
    \item\label{i:9.4.7.a} \(f\) is continuous at \(x_0\).
    \item\label{i:9.4.7.b} For every sequence \((a_n)_{n = 0}^\infty\) consisting of elements of \(X\) with \(\lim_{n \to \infty} a_n = x_0\), we have \(\lim_{n \to \infty} f(a_n) = f(x_0)\).
    \item\label{i:9.4.7.c} For every \(\varepsilon > 0\), there exists a \(\delta > 0\) such that \(\abs{f(x) - f(x_0)} < \varepsilon\) for all \(x \in X\) with \(\abs{x - x_0} < \delta\).
    \item\label{i:9.4.7.d} For every \(\varepsilon > 0\), there exists a \(\delta > 0\) such that \(\abs{f(x) - f(x_0)} \leq \varepsilon\) for all \(x \in X\) with \(\abs{x - x_0} \leq \delta\).
  \end{enumerate}
\end{prop}

\begin{proof}
  We first show that the statement (a) and the statement (b) are equivalent.
  By \labelcrefs{i:9.4.1}, \(f\) is continuous at \(x_0\) iff \(f\) converges to \(f(x_0)\) at \(x_0\) in \(X\).
  Thus, by \labelcrefs{i:9.3.9} we know that the statement (a) and the statement (b) are equivalent.

  Next we show that the statement (a) and the statement (c) are equivalent.
  By \labelcrefs{i:9.4.1}, \(f\) is continuous at \(x_0\) iff \(f\) converges to \(f(x_0)\) at \(x_0\) in \(X\).
  By \labelcrefs{i:9.3.6} this is equivalent to the statement
  \[
  ...
  By \labelcrefs{i:5.4.14} we know such \(\delta\) exists.
  Then we have
  \[
...
\begin{rmk}\label{i:9.4.8}
  A particularly useful consequence of \labelcrefs{i:9.4.7} is the following:
...
\begin{proof}
  By \labelcrefs{i:9.3.14}, we have
  \begin{align*}
...
\begin{proof}
  Let \(x_0 \in \R\).
  By \labelcrefs{i:9.1.13} \(x_0\) is an adherent point.
  By \labelcrefs{i:6.7.3} we know that \(a^{x_0} > 0\).
  By \labelcrefs{i:6.5.3} we know that \(\lim_{n \to \infty} a^{\dfrac{1}{n}} = 1\), thus
  \begin{align*}
  ...
  \]
  Thus, by \labelcrefs{i:9.3.6} we have \(\lim_{x \to x_0 ; x \in \R} a^x = a^{x_0}\).
  Since \(x_0\) was arbitrary, by \labelcrefs{i:9.4.1} \(a^x\) is continuous on \(\R\).
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    \end{dcases}
  \]
  Thus, by squeeze test (\labelcrefs{i:ex:9.3.5}) we have \(\lim_{x \to 1 ; x \in (0, \infty)} x^p = 1\).

  Let \(x_0 \in (0, \infty)\).
  By \labelcrefs{i:9.1.12} \(x_0\) is an adherent point.
  By \labelcrefs{i:6.7.3} \(x_0^p > 0\).
  Since \(\lim_{x \to 1 ; x \in (0, \infty)} x^p = 1\), by \labelcrefs{i:9.3.6} we have
  \[
  ...
  \]
  Thus, by \labelcrefs{i:9.3.6} we have \(\lim_{y \to x_0 ; y \in (0, \infty)} y^p = x_0^p\).
  Since \(x_0\) was arbitrary, by \labelcrefs{i:9.4.1} \(x^p\) is continuous on \((0, \infty)\).
\end{proof}
...
\begin{proof}
  Since \(\lim_{y \to f(x_0) ; y \in Y} g(x) = g\big(f(x_0)\big)\), by \labelcrefs{i:9.3.6} we have
  \[
  ...
  \]
  Since \(\lim_{x \to x_0 ; x \in X} f(x) = f(x_0)\), by \labelcrefs{i:9.3.6} we have
  \[
  ...
  \]
  Thus, by \labelcrefs{i:9.3.6} we have \(\lim_{x \to x_0 ; x \in X} g\big(f(x)\big) = g\big(f(x_0)\big)\).
  By \labelcrefs{i:9.4.1} \(g \circ f\) is continuous at \(x_0\).
\end{proof}

\exercisesection

\begin{ex}\label{i:ex:9.4.1}
  Prove \labelcrefs{i:9.4.7}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.4.7}
\end{proof}
...
\begin{proof}
  ...
  Let \(x_0 \in X\).
  By \labelcrefs{i:9.1.11} we know that \(X \subseteq \overline{X}\), thus \(x_0\) is an adherent point of \(X\).
  Since
  \[
  ...
  Since \(f(x_0) = c\) and \(x_0\) was arbitrary, by \labelcrefs{i:9.4.1} we know that the constant functions \(f\) is continuous on \(X\).
  ...
  Since \(g(x_0) = x_0\) and \(x_0\) was arbitrary, by \labelcrefs{i:9.4.1} we know that the constant functions \(g\) is continuous on \(X\).
\end{proof}

\begin{ex}\label{i:ex:9.4.3}
  Prove \labelcrefs{i:9.4.10}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.4.10}.
\end{proof}

\begin{ex}\label{i:ex:9.4.4}
  Prove \labelcrefs{i:9.4.11}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.4.11}.
\end{proof}

\begin{ex}\label{i:ex:9.4.5}
  Prove \labelcrefs{i:9.4.13}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.4.13}.
\end{proof}
...
\begin{proof}
  By \labelcrefs{i:9.4.1}, \(\forall x_0 \in X\), we have \(\lim_{x \to x_0 ; x \in X} f(x) = f(x_0)\).
  Since \(Y \subseteq X\), we have \(\forall y \in Y \implies y \in X\).
  Thus, \(\forall y_0 \in Y\) we have \(\lim_{y \to y_0 ; y \in Y} f(y) = f(y_0)\).
  By \labelcrefs{i:9.4.1}, \(f|_Y\) is continuous on \(Y\).
\end{proof}
...
\begin{proof}
  ...
  \]
  By \labelcrefs{i:ex:9.4.2} we know that constant functions are continuous, and Thus, the base case holds.
  ...
  \end{align*}
  By the induction hypothesis we know that \(\sum_{i = 0}^n c_i x^i\) and \(x^n\) are continuous.
  By \labelcrefs{i:ex:9.4.2} we know that \(c_{n + 1}\) and \(x\) are continuous.
  Thus, by \labelcrefs{i:9.4.9} we know that \(f\) is continuous.
