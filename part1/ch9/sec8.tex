...
\begin{proof}
  ...
  Since \(a < b\) and \(f\) is strictly monotone increasing, by \labelcrefs{i:9.8.1} we know that \(f(a) < f(b)\).
  In particular, \(\forall c \in (a, b)\), we have \(a < c < b\) and \(f(a) < f(c) < f(b)\).
  By \labelcrefs{i:9.6.5} this means \(f\) attains its minimum at \(a\) and attains its maximum at \(b\).
  By \labelcrefs{i:9.7.4} we know that \(f([a, b]) = [f(a), f(b)]\), thus \(f\) is surjective from \([a, b]\) to \([f(a), f(b)]\).
  Since \(f\) is strictly monotone increasing, by \labelcrefs{i:9.8.1} \(x \neq y \implies f(x) \neq f(y)\), thus \(f\) is injective from \([a, b]\) to \([f(a), f(b)]\).
  ...
  Let \(y_0 \in [f(a), f(b)]\).
  By \labelcrefs{i:9.1.12} we know that \([f(a), f(b)]\) is closed, so \(y_0\) is an adherent point of \([f(a), f(b)]\).
  Since \(f\) is bijective from \([a, b]\) to \([f(a), f(b)]\), we know that \(\exists!\ x_0 \in [a, b]\) such that \(f(x_0) = y_0\) and thus \(f^{-1}(y_0) = x_0\).
  Again by \labelcrefs{i:9.1.12} we know that \([a, b]\) is closed, so \(x_0\) is an adherent point of \([a, b]\).
  To show that \(f^{-1}\) is continuous at \(y_0\), by \labelcrefs{i:9.4.1} we need to show that
  \[
    \lim_{y \to y_0 ; y \in [f(a), f(b)]} f^{-1}(y) = f^{-1}(y_0) = x_0.
  \]
  By \labelcrefs{i:9.3.6}, it suffice to show that
  \[
...
\begin{proof}
  ...
  By \labelcrefs{i:9.4.9} we know that \(h\) is continuous on \(\R\), thus by \labelcrefs{i:9.4.13} we know that \(g\) is continuous.
  ...
  Since \(g\) is monotone increasing and continuous, by \labelcrefs{i:9.8.3} we know that \(g : [a, b] \to [g(a), g(b)]\) is bijective and \(g^{-1} : [g(a), g(b)] \to [a, b]\) is continuous and strictly monotone increasing.
  Since \(h\) is bijective, by \labelcrefs{i:ex:3.3.7} we know that \(f = h \circ g\) is bijective from \([a, b]\) to \([-g(b), -g(a)] = [f(b), f(a)]\).
  Again by \labelcrefs{i:ex:3.3.7} we know that
  \[
    f^{-1} = g^{-1} \circ h^{-1}|_{[f(a), f(b)]}.
  \]
  Since \(h^{-1} = h\) is continuous, by \labelcrefs{i:9.4.13} we know that \(f^{-1}\) is continuous.
  ...
  \end{align*}
  and thus by \labelcrefs{i:9.8.1} \(f^{-1}\) is strictly monotone decreasing.
\end{proof}

\begin{eg}\label{i:9.8.4}
  Let \(n\) be a positive integer and \(R > 0\).
  Since the function \(f(x) \coloneqq x^n\) is strictly increasing on the interval \([0, R]\), we see from \labelcrefs{i:9.8.3} that this function is a bijection from \([0, R]\) to \([0, R^n]\), and hence there is an inverse from \([0, R^n]\) to \([0, R]\).
  This can be used to give an alternate means to construct the \(n^{\opTh}\) root \(x^{1 / n}\) of a number \(x \in [0, R]\) than what was done in \labelcrefs{i:5.6.5}.
\end{eg}
...
\begin{proof}
  ...
  Since \(\forall y \in [0.5, 1.5)\), \(\nexists x \in \N\) such that \(f(x) = y\), the intermediate value theorem (\labelcrefs{i:9.7.1}) does not hold.
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    \item \(f(a) < f(b)\).
          ...
          Since \(f(a) < f(b)\), by \labelcrefs{i:9.8.1} \(f\) is strictly monotone increasing.
    \item \(f(a) > f(b)\).
          Suppose for the sake of contradiction that \(f\) is not strictly monotone.
          Then \(\exists c \in (a, b)\) such that
          \[
          ...
          a contradiction.
          Thus, \(f\) is strictly monotone.
          Since \(f(a) > f(b)\), by \labelcrefs{i:9.8.1} \(f\) is strictly monotone decreasing.
  \end{itemize}
  From all cases above, we conclude that \(f\) is strictly monotone.
\end{proof}

\begin{ex}\label{i:ex:9.8.4}
  Prove \labelcrefs{i:9.8.3}.
...
\begin{proof}
  See \labelcrefs{i:9.8.3} and \labelcrefs{i:ac:9.8.1}.
\end{proof}
...
\begin{ex}\label{i:ex:9.8.5}
  ...
  \begin{enumerate}
    \item\label{i:ex:9.8.5.a} Show that \(f\) is strictly monotone increasing.
    \item\label{i:ex:9.8.5.b} Show that for every rational number \(r\), \(f\) is discontinuous at \(r\).
    \item\label{i:ex:9.8.5.c} Show that for every irrational number \(x\), \(f\) is continuous at \(x\).
  \end{enumerate}
\end{ex}

\begin{proof}[\pf{i:ex:9.8.5.a}]
  Let \(a, b \in \R\) and \(a < b\).
  By \labelcrefs{i:5.4.14}, \(\exists c \in \Q\) such that \(a < c < b\).
  Then we have
  \begin{align*}
    f(b) & = \sum_{r \in \Q : r < b} g(r)                                                                                                 \\
         ...
         & > f(a).
  \end{align*}
  and by \labelcrefs{i:9.8.1} \(f\) is strictly monotone increasing.
\end{proof}

\begin{proof}[\pf{i:ex:9.8.5.b}]
  Let \(\gamma \in \Q\).
...
\begin{proof}[\pf{i:ex:9.8.5.c}]
  ...
  Since \(\R \setminus \Q \subseteq \R\), by \labelcrefs{i:9.1.11} \(\forall x_0 \in \R \setminus \Q\), \(x_0\) is an adherent point of \(\R \setminus \Q\).
  ...
  \end{align*}
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{i:9.3.6} we have \(\lim_{x \to x_0 ; x \in \R \setminus \Q} f_n(x) = f_n(x_0)\), and by \labelcrefs{i:9.4.1} \(f_n\) is continuous at \(x_0\).
  ...
  \begin{align*}
     ...
     & = 2^{-n}.                                                                             &  & \by{i:7.3.3}
  \end{align*}
  By \labelcrefs{i:5.4.14}, \(\forall \varepsilon \in \R^+\), \(\exists n \in \N\) such that \(2^{-n} < \varepsilon / 2\).
  ...
  \begin{align*}
     ...
     & = \varepsilon.
  \end{align*}
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{i:9.3.6} we have \(\lim_{x \to x_0 ; x \in \R \setminus \Q} f(x) = f(x_0)\), and by \labelcrefs{i:9.4.1} \(f\) is continuous at \(x_0\).
\end{proof}
