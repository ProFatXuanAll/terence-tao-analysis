...
\begin{rmk}\label{i:9.6.2}
  ...
  Also, a function \(f : X \to \R\) is bounded iff its image \(f(X)\) is a bounded set in the sense of \labelcrefs{i:9.1.22}.
\end{rmk}
...
\begin{proof}
  Suppose for the sake of contradiction that \(f\) is not bounded.
  ...
  This sequence lies in \([a, b]\), and so by \labelcrefs{i:9.1.24} there exists a subsequence \((x_{n_j})_{j = 0}^\infty\) which converges to some limit \(L \in [a, b]\), where \(n_0 < n_1 < n_2 < \dots\) is an increasing sequence of natural numbers.
...
\begin{rmk}\label{i:9.6.4}
  There are two things about the proof of \labelcrefs{i:9.6.3} that are worth noting.
  Firstly, it shows how useful the Heine-Borel theorem (\labelcrefs{i:9.1.24}) is.
...
\begin{proof}
  From \labelcrefs{i:9.6.3} we know that \(f\) is bounded, thus there exists an \(M\) such that \(-M \leq f(x) \leq M\) for each \(x \in [a, b]\).
  ...
  by the Heine-Borel theorem (\labelcrefs{i:9.1.24}), we can thus find a subsequence \((x_{n_j})_{j = 1}^\infty\), where \(n_1 < n_2 < \dots\), which converges to some limit \(x_{\max} \in [a, b]\).
  ...
  (Again, this requires the axiom of choice)
  This is a sequence in \([a, b]\);
  by the Heine-Borel theorem (\labelcrefs{i:9.1.24}), we can thus find a subsequence \((x_{n_j})_{j = 1}^\infty\), where \(n_1 < n_2 < \dots\), which converges to some limit \(x_{\min} \in [a, b]\).
...
\begin{note}
  The maximum principle (\labelcrefs{i:9.6.7}) does not prevent a function from attaining its maximum or minimum at more than one point.
\end{note}

\begin{note}
  Let us write \(\sup_{x \in [a, b]} f(x)\) as short-hand for \(\sup\set{f(x) : x \in [a, b]}\), and similarly define \(\inf_{x \in [a, b]} f(x)\).
  The maximum principle (\labelcrefs{i:9.6.7}) thus asserts that \(m \coloneqq \sup_{x \in [a, b]} f(x)\) is a real number and is the maximum value of \(f\) on \([a, b]\), i.e., there is at least one point \(x_{\max}\) in \([a, b]\) for which \(f(x_{\max}) = m\), and for every other \(x \in [a, b]\), \(f(x)\) is less than or equal to \(m\).
...
\begin{proof}
  ...
  Then by \labelcrefs{i:9.4.12} and \labelcrefs{i:9.4.13} \(f_a\) is continuous.
  Since \(f_a\big((1, 2)\big) = [0, 0.5)\), by \labelcrefs{i:9.6.1} \(f_a\) is bounded.
  Since \(f_a\big((1, 2)\big) = [0, 0.5)\), by \labelcrefs{i:9.6.5} \(f_a\) attains its minimum at \(x = 1.5\).
  Since \(0.5 \notin f_a\big((1, 2)\big)\), by \labelcrefs{i:9.6.5} \(f_a\) does not attain its maximum anywhere.
  Since the domain of \(f_a\) is not closed, \(f_a\) does not violate the maximum principle (\labelcrefs{i:9.6.7}).

  Let \(f_b : [0, \infty) \to \R\) be a function where \(f_b(x) = 0.5^x\).
  Then by \labelcrefs{i:9.4.10} \(f_b\) is continuous.
  Since \(f_b\big([0, \infty)\big) = (0, 1]\), by \labelcrefs{i:9.6.1} \(f_b\) is bounded.
  Since \(f_b\big([0, \infty)\big) = (0, 1]\), by \labelcrefs{i:9.6.5} \(f_b\) attains its maximum at \(x = 0\).
  Since \(0 \notin f_b\big([0, \infty)\big)\), by \labelcrefs{i:9.6.5} \(f_b\) does not attain its minimum anywhere.
  Since the domain of \(f_b\) is not closed, \(f_b\) does not violate the maximum principle (\labelcrefs{i:9.6.7}).
  ...
  \[
    ...
    \end{dcases}
  \]
  Since \(f_c\big([-1, 1]\big) = (-1, 1)\), by \labelcrefs{i:9.6.1} \(f_c\) is bounded.
  Since \(-1 \notin f_c\big([-1, 1]\big)\), by \labelcrefs{i:9.6.5} \(f_c\) does not attain its minimum anywhere.
  Since \(1 \notin f_c\big([-1, 1]\big)\), by \labelcrefs{i:9.6.5} \(f_c\) does not attain its maximum anywhere.
  Since \(f_c\) is not continuous on its domain, \(f_c\) does not violate the maximum principle (\labelcrefs{i:9.6.7}).
  ...
  \[
    ...
    \end{dcases}
  \]
  Since \(f_d\big([-1, 1]\big) = (-\infty, -1) \cup \set{0} \cup (1, \infty)\), by \labelcrefs{i:9.6.1} \(f_d\) is unbounded.
  Since \(f_d\) is not bounded, \(f_d\) does not violate the maximum principle (\labelcrefs{i:9.6.7}).
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \implies & \begin{dcases}
               ...
               \end{dcases}
  \end{align*}
  Thus, by \labelcrefs{i:9.6.1} \(f + g\), \(f - g\), \(f \cdot g\) are bounded.
