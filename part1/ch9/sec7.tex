\section{The intermediate value theorem}\label{i:sec:9.7}

\begin{thm}[Intermediate value theorem]\label{i:9.7.1}
...
\begin{proof}
  ...
  \]
  By the squeeze test (\labelcrefs{i:6.4.14}) we thus have \(\lim_{n \to \infty} x_n = c\).
  ...
  By the comparison principle (\labelcrefs{i:6.4.13}) we thus have \(f(c) \leq y\).
...
\begin{rmk}\label{i:9.7.3}
  The intermediate value theorem gives another way to show that one can take \(n^{\opTh}\) roots of a number.
...
\begin{cor}[Images of continuous functions]\label{i:9.7.4}
  Let \(a < b\), and let \(f : [a, b] \to \R\) be a continuous function on \([a, b]\).
...
\begin{proof}
  ...
  By maximum principle (\labelcrefs{i:9.6.7}) we know that \(\exists x_M, x_m \in [a, b]\) such that \(f(x_M) = M\) and \(f(x_m) = m\).
  We have either \(x_m \leq x_M\) or \(x_m \geq x_M\).
  Without the loss of generality suppose that \(x_m \leq x_M\).
  Then we have \([x_m, x_M] \subseteq [a, b]\) and by \labelcrefs{i:ex:9.4.6} we know that \(f\) is continuous on \([x_m, x_M]\).
  Since \(m \leq y \leq M\), by \labelcrefs{i:9.7.1} \(\exists c \in [x_m, x_M]\) such that \(f(c) = y\).
  ...
  \end{align*}
  by \labelcrefs{i:3.1.18} we know that \(f([a, b]) = [m ,M]\).
\end{proof}

\exercisesection

\begin{ex}\label{i:ex:9.7.1}
  Prove \labelcrefs{i:9.7.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:9.7.4}.
\end{proof}
...
\begin{proof}
  ...
  Since \(x \mapsto x\) is continuous on \([0, 1]\), by \labelcrefs{i:9.4.9} \(F\) is continuous on \([0, 1]\).
