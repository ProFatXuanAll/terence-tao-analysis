...
\begin{proof}[\pf{i:ac:2.3.1}]
  ...
  We induct on \(n\).
  For \(n = 0\), by \labelcrefs{i:2.3.1}, we have \(0 \times m = 0\), which is a natural number by \labelcrefs{i:2.1}.
  ...
  By \labelcrefs{i:2.3.1}, \((n\pp) \times m = (n \times m) + m\).
  By the induction hypothesis, \(n \times m\) is a natural number.
  By \labelcrefs{i:ac:2.2.1}, \((n \times m) + m\) is a natural number.
...
\begin{proof}[\pf{i:ac:2.3.2}]
  We induct on \(n\).
  For \(n = 0\), by \labelcrefs{i:2.3.1}, we have \(0 \times 0 = 0\).
  So the base case holds.
...
\begin{proof}[\pf{i:ac:2.3.3}]
  ...
  For \(n = 0\), by \labelcrefs{i:2.3.1}, we have \(0 \times (m\pp) = 0\).
  So the base case holds.
...
\begin{proof}[\pf{i:2.3.2}]
  We induct on \(n\) and fix \(m\).
  For \(n = 0\), by \labelcrefs{i:2.3.1}, we have \(0 \times m = 0\), and by \labelcrefs{i:ac:2.3.2}, we have \(m \times 0 = 0\).
  So the base case holds.
...
\begin{proof}[\pf{i:2.3.3}]
  ...
  By \labelcrefs{i:2.2.7}, this means \(n\) and \(m\) are positive natural numbers.
  Then by \labelcrefs{i:2.2.10}, there exist some natural numbers \(a\) and \(b\) such that \(n = a\pp\) and \(m = b\pp\).
  Thus, we have
  \begin{align*}
    n \times m & = (a\pp) \times (b\pp)                        \\
               & = a \times (b\pp) + (b\pp). &  & \by{i:2.3.1}
  \end{align*}
  By \labelcrefs{i:2.3}, we know that \(b\pp \neq 0\).
  Thus, by \labelcrefs{i:2.2.8}, we know that \(n \times m\) is a positive natural number.
  But this contradicts \(n \times m = 0\).
  Thus, we must have either \(n = 0\) or \(m = 0\).

  Now suppose that \(n = 0\) or \(m = 0\).
  If \(n = 0\), then we have \(n \times m = 0 \times m = 0\) by \labelcrefs{i:2.3.1}.
  If \(m = 0\), then we have \(n \times m = n \times 0 = 0\) by \labelcrefs{i:ac:2.3.2}.
  In either case, we have \(n \times m = 0\).

  From all proofs above, we conclude that \(n \times m = 0 \iff (n = 0) \lor (m = 0)\).
  Thus, we have \(n \times m \neq 0 \iff (n \neq 0) \land (m \neq 0)\).
  By \labelcrefs{i:2.2.7}, we see that \(n\) and \(m\) are positive natural numbers iff \(n \times m \neq 0\).
\end{proof}
...
\begin{proof}[\pf{i:ac:2.3.4}]
  By \labelcrefs{i:2.3.2}, we know that \(n1 = 1n\).
  Thus, we only need to show that \(n1 = n\).
  We induct on \(n\).
  For \(n = 0\), we have \(0 \times 1 = 0\) by \labelcrefs{i:2.3.3}.
  So the base case holds.
  Suppose inductively that \(n1 = n\) is true for some natural number \(n\).
  Then for \(n + 1\), we have \((n + 1) \times 1 = n1 + 1\) by \labelcrefs{i:2.3.1}.
...
\begin{proof}[\pf{i:2.3.4}]
  ...
  The left-hand side is \(a((b + c)\pp) = a(b + c) + a\) by \labelcrefs{i:ac:2.3.3}, while the right-hand side is \(ab + ac + a = a(b + c) + a\) by the induction hypothesis, and so we can close the induction.
\end{proof}
...
\begin{proof}[\pf{i:2.3.5}]
  We keep \(a\) and \(b\) fixed, and we induct on \(c\).
  For \(c = 0\), by \labelcrefs{i:ac:2.3.2}, we have \((a \times b) \times 0 = 0 = a \times 0 = a \times (b \times 0)\).
  So the base case holds.
...
\begin{proof}[\pf{i:2.3.6}]
  Since \(a < b\), we have \(b = a + d\) for some positive \(d\) by \labelcrefs{i:2.2.12.f}.
  Multiplying by \(c\) and using the distributive law (\labelcrefs{i:2.3.4}), we obtain \(bc = ac + dc\).
  Since \(d\) is positive, and \(c\) is positive, \(dc\) is positive (\labelcrefs{i:2.3.3}), and hence \(ac < bc\) (by \labelcrefs{i:2.2.11}), as desired.
\end{proof}
...
\begin{proof}[\pf{i:2.3.7}]
  By the trichotomy of order (\labelcrefs{i:2.2.13}), we have three cases: \(a < b\), \(a = b\), \(a > b\).
  Suppose first that \(a < b\), then by \labelcrefs{i:2.3.6}, we have \(ac < bc\), a contradiction.
  We can obtain a similar contradiction when \(a > b\).
  Thus, the only possibility is that \(a = b\), as desired.
\end{proof}

\begin{rmk}\label{i:2.3.8}
  Just as \labelcrefs{i:2.2.6} will allow for a ``virtual subtraction'' which will eventually let us define genuine subtraction, \labelcrefs{i:2.3.7} provides a ``virtual division'' which will be needed to define genuine division later on.
\end{rmk}
...
\begin{proof}[\pf{i:2.3.9}]
  ...
  \begin{align*}
    n\pp & = (mq + r)\pp  &  & \byIH        \\
         & = mq + (r\pp). &  & \by{i:2.2.3} \\
  \end{align*}
  Since \(r < q\), we have \(r\pp \leq q\) by \labelcrefs{i:2.2.12.e}.
  Now we split into two cases:
  \begin{itemize}
    \item If \(r\pp < q\), then we have \(0 \leq r < r\pp < q\), and we are done in this case.
    \item If \(r\pp = q\), then by \labelcrefs{i:2.3.1}, we have
          \[
            n\pp = mq + (r\pp) = mq + q = (m\pp) \times q = (m\pp) \times q + r',
          \]
          where \(r' = 0\) and \(0 \leq r' < q\) by \labelcrefs{i:ac:2.2.4}, and we are also done in this case.
  \end{itemize}
...
\begin{ex}\label{i:ex:2.3.1}
  Prove \labelcrefs{i:2.3.2}.
\end{ex}

\begin{proof}[\pf{i:ex:2.3.1}]
  See \labelcrefs{i:2.3.2}
\end{proof}

\begin{ex}\label{i:ex:2.3.2}
  Prove \labelcrefs{i:2.3.3}
\end{ex}

\begin{proof}[\pf{i:ex:2.3.2}]
  See \labelcrefs{i:2.3.3}
\end{proof}

\begin{ex}\label{i:ex:2.3.3}
  Prove \labelcrefs{i:2.3.5}
\end{ex}

\begin{proof}[\pf{i:ex:2.3.3}]
  See \labelcrefs{i:2.3.5}
\end{proof}
...
\begin{ex}\label{i:ex:2.3.5}
  Prove \labelcrefs{i:2.3.9}
\end{ex}

\begin{proof}[\pf{i:ex:2.3.5}]
  See \labelcrefs{i:2.3.9}
\end{proof}
