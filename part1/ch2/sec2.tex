...
\begin{proof}[\pf{i:ac:2.2.1}]
  ...
  We induct on \(n\).
  For \(n = 0\), by \labelcrefs{i:2.2.1}, we have \(0 + m = m\), which is a natural number by the definition of \(m\).
  ...
  By \labelcrefs{i:2.2.1}, we have \((n\pp) + m = (n + m)\pp\).
  By the induction hypothesis, we know that \(n + m\) is a natural number.
  Thus, by \labelcrefs{i:2.2}, we know that \((n + m)\pp\) is again a natural number.
...
\begin{proof}[\pf{i:2.2.2}]
  We induct on \(n\).
  The base case \(0 + 0 = 0\) follows since we know that \(0 + m = m\) for every natural number \(m\) (\labelcrefs{i:2.2.1}), and \(0\) is a natural number (\labelcrefs{i:2.1}).
  Now suppose inductively that \(n + 0 = n\).
  We wish to show that \((n\pp) + 0 = n\pp\).
  But by \labelcrefs{i:2.2.1}, \((n\pp) + 0\) is equal to \((n + 0)\pp\), which is equal to \(n\pp\) since \(n + 0 = n\).
  This closes the induction.
\end{proof}
...
\begin{proof}[\pf{i:2.2.3}]
  ...
  But by \labelcrefs{i:2.2.1}, \(0 + (m\pp) = m\pp\) and \(0 + m = m\), so both sides are equal to \(m\pp\) and are thus equal to each other.
  So the base case holds.
  Suppose inductively that \(n + (m\pp) = (n + m)\pp\).
  We need to show that \((n\pp) + (m\pp) = ((n\pp) + m)\pp\).
  The left-hand side is \((n + (m\pp))\pp\) by \labelcrefs{i:2.2.1}, which is equal to \(((n+m)\pp)\pp\) by the inductive hypothesis.
  Similarly, we have \((n\pp) + m = (n + m)\pp\) by \labelcrefs{i:2.2.1}, and so the right-hand side is also equal to \(((n + m)\pp)\pp\).
...
\begin{proof}[\pf{i:ac:2.2.2}]
  Since \(n\) is a natural number, by \labelcrefs{i:2.2} we know that \(n\pp\) is also a natural number.
  Thus, we can apply \labelcrefs{i:2.2.2} to derive the following fact:
  \begin{align*}
...
\begin{proof}[\pf{i:2.2.4}]
  ...
  But by \labelcrefs{i:2.2.1}, \(0 + m = m\), while by \labelcrefs{i:2.2.2}, \(m + 0 = m\).
  ...
  By \labelcrefs{i:2.2.1}, \((n\pp) + m = (n + m)\pp\).
  By \labelcrefs{i:2.2.3}, \(m + (n\pp) = (m + n)\pp\), but this is equal to \((n + m)\pp\) by the induction hypothesis.
...
\begin{note}
  Because of associativity showed in \labelcrefs{i:2.2.5}, we can write sums such as \(a + b + c\) without having to worry about which order the numbers are being added together.
\end{note}
...
\begin{proof}[\pf{i:2.2.6}]
  We induct on \(a\).
  For \(a = 0\), we have \(0 + b = 0 + c\), which by \labelcrefs{i:2.2.1} implies that \(b = c\) as desired.
  ...
  By \labelcrefs{i:2.2.1}, we have \((a\pp) + b = (a + b)\pp\) and \((a\pp) + c = (a + c)\pp\), and so we have \((a + b)\pp = (a + c)\pp\).
  By \labelcrefs{i:2.4}, we have \(a + b = a + c\).
...
\begin{prop}\label{i:2.2.8}
  If \(a\) is a positive natural number and \(b\) is a natural number, then \(a + b\) is positive (and hence \(b + a\) is also, by \labelcrefs{i:2.2.4}).
\end{prop}
...
\begin{proof}[\pf{i:2.2.8}]
  ...
  Then \(a + (b\pp) = (a + b)\pp\), which cannot be zero by \labelcrefs{i:2.3}, and is hence positive by \labelcrefs{i:2.2.7}.
  This closes the induction.
\end{proof}
...
\begin{proof}[\pf{i:2.2.9}]
  ...
  If \(a \neq 0\), then \(a\) is positive, and hence \(a + b = 0\) is positive by \labelcrefs{i:2.2.8}, a contradiction.
  Similarly, if \(b \neq 0\), then \(b\) is positive, and again \(a + b = 0\) is positive by \labelcrefs{i:2.2.8}, a contradiction.
...
\begin{proof}[\pf{i:2.2.10}]
  ...
  Since \(a\) is a positive natural number, by \labelcrefs{i:2.2.7}, we know that \(a \neq 0\).
  ...
  Thus, by \labelcrefs{i:2.4}, we have \(b = c\).
\end{proof}
...
\begin{proof}[\pf{i:ac:2.2.3}]
  ...
  By \labelcrefs{i:ac:2.2.2}, we have \(n\pp = n + 1\).
  Since \(1\) is a natural number, by \labelcrefs{i:2.2.11}, we have \(n\pp \geq n\).
  To show that \(n\pp > n\), by \labelcrefs{i:2.2.11}, we only need to show that \(n\pp \neq n\).

  We induct on \(n\) to show that \(n\pp \neq n\) for any natural number \(n\).
  For \(n = 0\), we have \(0\pp \neq 0\), by \labelcrefs{i:2.3}.
  Thus, the base case holds.
...
\begin{proof}[\pf{i:ac:2.2.4}]
  Let \(n\) be a natural number.
  By \labelcrefs{i:2.2.1}, we have \(n = 0 + n\).
  Thus, by \labelcrefs{i:2.2.11}, we have \(n \geq 0\).

  Now suppose that \(n\) is a positive natural number.
  By \labelcrefs{i:2.2.7}, this means \(n \neq 0\).
  From the first paragraph we see that \(n \geq 0\).
  Thus, by \labelcrefs{i:2.2.11}, we have \(n > 0\).
\end{proof}
...
\begin{prop}[Basic properties of order for natural numbers]\label{i:2.2.12}
  ...
  Then
  \begin{enumerate}
    \item\label{i:2.2.12.a} (Order is reflexive) \(a \geq a\).
    \item\label{i:2.2.12.b} (Order is transitive) If \(a \geq b\) and \(b \geq c\), then \(a \geq c\).
    \item\label{i:2.2.12.c} (Order is anti-symmetric) If \(a \geq b\) and \(b \geq a\), then \(a = b\).
    \item\label{i:2.2.12.d} (Addition preserves order) \(a \geq b\) iff \(a + c \geq b + c\).
    \item\label{i:2.2.12.e} \(a < b\) iff \(a\pp \leq b\).
    \item\label{i:2.2.12.f} \(a < b\) iff \(b = a + d\) for some \emph{positive} number \(d\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{i:2.2.12.a}]
  By \labelcrefs{i:2.1}, \(0\) is a natural number, and by \labelcrefs{i:2.2.2}, \(a = a + 0\).
  Hence, by \labelcrefs{i:2.2.11}, \(a \geq a\).
\end{proof}

\begin{proof}[\pf{i:2.2.12.b}]
  Suppose that \(a \geq b\) and \(b \geq c\).
  By \labelcrefs{i:2.2.11}, there exist some natural numbers \(d\) and \(e\) such that \(a = b + d\) and \(b = c + e\).
  By \labelcrefs{i:2.2.5}, we have \(a = b + d = (c + e) + d = c + (e + d)\), and by \labelcrefs{i:ac:2.2.1}, \(e + d\) is a natural number.
  Hence, by \labelcrefs{i:2.2.11}, \(a \geq c\).
\end{proof}

\begin{proof}[\pf{i:2.2.12.c}]
  Suppose that \(a \geq b\) and \(b \geq a\).
  By \labelcrefs{i:2.2.11}, there exist some natural numbers \(c\) and \(d\) such that \(a = b + c\) and \(b = a + d\).
  By \labelcrefs{i:2.2.2}, we have \(a = a + 0\), and by \labelcrefs{i:2.2.5}, we have \(a = b + c = (a + d) + c = a + (d + c)\).
  Hence, by \labelcrefs{i:2.2.6}, \(0 = d + c\).
  By \labelcrefs{i:2.2.9}, this forces \(d = c = 0\).
  Therefore, by \labelcrefs{i:2.2.2}, \(a = b + 0 = b\).
\end{proof}

\begin{proof}[\pf{i:2.2.12.d}]
  First, suppose that \(a \geq b\).
  By \labelcrefs{i:2.2.11}, \(a = b + d\) for some natural number \(d\).
  By \labelcrefs{i:2.2.4}, we have \(a + c = c + a = c + (b + d)\), and by \labelcrefs{i:2.2.4,i:2.2.5}, this equals \((c + b) + d = (b + c) + d\).
  Hence, by \labelcrefs{i:2.2.11}, \(a + c \geq b + c\).
  Conversely, suppose that \(a + c \geq b + c\).
  By \labelcrefs{i:2.2.11}, \(a + c = (b + c) + d\) for some natural number \(d\).
  By \labelcrefs{i:2.2.4,i:2.2.5}, we have \(c + a = (c + b) + d = c + (b + d)\), so by \labelcrefs{i:2.2.6}, \(a = b + d\).
  Hence, by \labelcrefs{i:2.2.11}, \(a \geq b\).
  Thus, we conclude that \(a \geq b \iff a + c \geq b + c\).
\end{proof}

\begin{proof}[\pf{i:2.2.12.e}]
  First, suppose that \(a < b\).
  By \labelcrefs{i:2.2.11}, \(a \leq b\) and \(a \neq b\), so \(b = a + c\) for some natural number \(c\) with \(a \neq b\).
  By \labelcrefs{i:2.2.2}, \(c \neq 0\), so by \labelcrefs{i:2.2.7}, \(c\) is a positive natural number.
  By \labelcrefs{i:2.2.10}, there exists a natural number \(d\) such that \(d\pp = c\).
  By \labelcrefs{i:2.2.1,i:2.2.3}, we then have \(b = a + (d\pp) = (a + d)\pp = (a\pp) + d\).
  Hence, by \labelcrefs{i:2.2.11}, \(a\pp \leq b\).

  Now suppose that \(a\pp \leq b\).
  By \labelcrefs{i:2.2.11}, \(b = (a\pp) + c\) for some natural number \(c\).
  By \labelcrefs{i:2.2.1,i:2.2.3}, we have \(b = (a\pp) + c = (a + c)\pp = a + (c\pp)\), so by \labelcrefs{i:2.2.11}, \(a \leq b\).
  By \labelcrefs{i:2.3}, we know that \(c\pp \neq 0\).
  Thus, by \labelcrefs{i:2.2.6}, we must have \(b \neq a\).
  (If \(b = a\), then we would have \(b = a + (c\pp) = a + 0\), which implies \(c\pp = 0\), a contradiction.)
  By \labelcrefs{i:2.2.11}, this means \(a < b\).
  From both directions, we conclude that \(a < b \iff a\pp \leq b\).
\end{proof}

\begin{proof}[\pf{i:2.2.12.f}]
  By \labelcrefs{i:2.2.12.e}, \(a < b\) iff \(a\pp \leq b\), which by \labelcrefs{i:2.2.11} holds iff \(b = (a\pp) + c\) for some natural number \(c\).
  By \labelcrefs{i:2.2.1,i:2.2.3}, we have \(b = (a\pp) + c = (a + c)\pp = a + (c\pp)\), so by \labelcrefs{i:2.3,i:2.2.10}, this holds iff \(b = a + d\) for some positive natural number \(d\).
  Thus, we conclude that \(a < b \iff b = a + d\) for some positive number \(d\).
\end{proof}
...
\begin{proof}[\pf{i:2.2.13}]
  ...
  If \(a < b\), then \(a \neq b\) by \labelcrefs{i:2.2.11}, and if \(a > b\), then \(a \neq b\) by \labelcrefs{i:2.2.11}.
  If \(a > b\) and \(a < b\), then by \labelcrefs{i:2.2.12.c}, we have \(a = b\), a contradiction.
  ...
  When \(a = 0\), by \labelcrefs{i:2.2.1}, we have \(b = 0 + b\).
  Thus, by \labelcrefs{i:ac:2.2.4}, we have \(0 \leq b\) for any natural number \(b\).
  ...
  \begin{itemize}
    \item If \(a > b\), then by \labelcrefs{i:2.2.12.d}, we have \(a\pp \geq b\pp\).
          Since \(b\pp > b\), by \labelcrefs{i:2.2.12.b}, we have \(a\pp \geq b\).
          Then we must have \(a\pp > b\).
          Otherwise, by \labelcrefs{i:2.2.11}, we would have \(a\pp = b\), and by \labelcrefs{i:2.2.12.e}, this implies \(a < b\) and contradicts \(a > b\)
          (the first paragraph proves the contradiction).
    \item If \(a = b\), then by \labelcrefs{i:2.4}, we have \(a\pp = b\pp\).
          Since \(b\pp > b\), by \labelcrefs{i:2.2.12.b}, we have \(a\pp \geq b\).
          Then we must have \(a\pp > b\).
          Otherwise, by \labelcrefs{i:2.2.11}, we would have \(a\pp = b\), and by \labelcrefs{i:2.2.12.e}, this implies \(a < b\) and contradicts \(a = b\)
          (the first paragraph proves the contradiction).
    \item If \(a < b\), then by \labelcrefs{i:2.2.12.e}, we have \(a\pp \leq b\).
          Thus, either \(a\pp = b\) or \(a\pp < b\), and in either case, we are done.
  \end{itemize}
  This closes the induction.
\end{proof}

\begin{ac}\label{i:ac:2.2.5}
  Let \(m\) and \(n\) be natural numbers.
  Then \(m < n\pp\) iff \(m \leq n\).
\end{ac}

\begin{proof}[\pf{i:ac:2.2.5}]
  Let \(m\) and \(n\) be natural numbers.
  Then we have
  \begin{align*}
         & m < n\pp                              \\
    \iff & m\pp \leq n\pp   &  & \by{i:2.2.12.e} \\
    \iff & m + 1 \leq n + 1 &  & \by{i:ac:2.2.2} \\
    \iff & m \leq n.        &  & \by{i:2.2.12.d}
  \end{align*}
\end{proof}
...
\begin{proof}[\pf{i:2.2.14}]
  ...
  However, by \labelcrefs{i:ac:2.2.4}, we know that \(0 \leq m_0\) for any natural number \(m_0\).
  ...
  \begin{itemize}
    \item If \(n < m_0\), then there does not exist a natural number \(m\) satisfying \(m_0 \leq m < n\pp\), since such an \(m\) would satisfy \(m \leq n\) by \labelcrefs{i:ac:2.2.5}, and thus \(m_0 \leq m \leq n < m_0\), which is absurd.
          Thus, \(Q(n\pp)\) is vacuously true in this case.
    \item If \(n \geq m_0\), then by the induction hypothesis \(Q(n)\), we know that \(P(m')\) is true for all natural numbers \(m_0 \leq m' < n\).
          So by the hypothesis of \(P\), which is applicable since \(n \geq m_0\), we see that \(P(n)\) is true.
          Combining this with \(Q(n)\), we know that \(P(m)\) is true for any natural number \(m\) satisfying \(m_0 \leq m \leq n\), and hence, by \labelcrefs{i:ac:2.2.5}, for any natural number \(m\) satisfying \(m_0 \leq m < n\pp\).
          Thus, \(Q(n\pp)\) is true in this case.
  \end{itemize}
  From all cases above, we see that \(Q(n\pp)\) is true.
  This closes the induction.
  Hence, we can conclude that \(Q(n)\) is true for any natural number \(n\).

  Now let \(n\) be any natural number satisfying \(n \geq m_0\).
  By \labelcrefs{i:ac:2.2.3}, we have \(n < n\pp\), and so \(m_0 \leq n < n\pp\).
  Applying \(Q(n\pp)\), we conclude that \(P(n)\) is true.
  Hence, \(P(n)\) is true for all natural numbers \(n \geq m_0\), as desired.
\end{proof}

\begin{rmk}\label{i:2.2.15}
  In applications we usually use \labelcrefs{i:2.2.14} with \(m_0 = 0\) or \(m_0 = 1\).
\end{rmk}

\exercisesection

\begin{ex}\label{i:ex:2.2.1}
  Prove \labelcrefs{i:2.2.5}.
\end{ex}

\begin{proof}[\pf{i:ex:2.2.1}]
  See \labelcrefs{i:2.2.5}.
\end{proof}

\begin{ex}\label{i:ex:2.2.2}
  Prove \labelcrefs{i:2.2.10}.
\end{ex}

\begin{proof}[\pf{i:ex:2.2.2}]
  See \labelcrefs{i:2.2.10}.
\end{proof}

\begin{ex}\label{i:ex:2.2.3}
  Prove \labelcrefs{i:2.2.12}.
\end{ex}

\begin{proof}[\pf{i:ex:2.2.3}]
  See \labelcrefs{i:2.2.12}.
\end{proof}

\begin{ex}\label{i:ex:2.2.4}
  Justify the three statements marked in the proof of \labelcrefs{i:2.2.13}.
\end{ex}

\begin{proof}[\pf{i:ex:2.2.4}]
  See \labelcrefs{i:2.2.13}.
\end{proof}

\begin{ex}\label{i:ex:2.2.5}
  Prove \labelcrefs{i:2.2.14}.
\end{ex}

\begin{proof}[\pf{i:ex:2.2.5}]
  See \labelcrefs{i:2.2.14}.
\end{proof}
...
\begin{ex}\label{i:ex:2.2.6}
  ...
  Prove that \(P(m)\) is true for any natural number \(m \leq n\).
  This is known as the \emph{principle of backwards induction}.
\end{ex}

\begin{proof}[\pf{i:ex:2.2.6}]
  We induct on \(n\) to show that the implication ``if \(P(n)\) is true, then \(P(m)\) is true for any natural number \(m\) satisfying \(m \leq n\)'' holds for every natural number \(n\).
  For \(n = 0\), the only natural number \(m\) satisfying \(m \leq n = 0\) is \(0\).
  Thus, the implication ``if \(P(0)\) is true, then \(P(m)\) is true for any natural number \(m\) satisfying \(m \leq 0\)'' reduces to ``if \(P(0)\) is true, then \(P(0)\) is true,'' which holds trivially.
  ...
  Now any natural number \(m\) satisfying \(m \leq n\pp\) satisfies either \(m \leq n\) or \(m = n\pp\), by \labelcrefs{i:ac:2.2.5}.
  In the first case \(P(m)\) is true by the above, and in the second case \(P(m) = P(n\pp)\) is true.
  Thus, the statement ``\(P(m)\) is true for any natural number \(m\) satisfying \(m \leq n\pp\)'' is true.
  This closes the induction.

  Since \(P(n)\) is true by hypothesis, applying the implication for the given \(n\), we conclude that \(P(m)\) is true for any natural number \(m\) satisfying \(m \leq n\).
\end{proof}
...
\begin{proof}[\pf{i:ex:2.2.7}]
  ...
  For \(k = 0\), we have \(Q(0) = P(n + 0) = P(n)\) by \labelcrefs{i:2.2.2}.
  ...
  By \labelcrefs{i:2.2.3}, we have \(Q(k\pp) = P(n + (k\pp)) = P((n + k)\pp)\).
  ...
  By \labelcrefs{i:2.2.11}, we know that for every natural number \(m\), we have \(m \geq n \iff m = n + k\) for some natural number \(k\).
