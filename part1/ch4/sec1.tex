...
\begin{proof}[\pf{i:ac:4.1.1}]
  ...
  \end{align*}
  we know that \labelcrefs{i:4.1.1} is reflexive.
  ...
  \end{align*}
  Thus, \labelcrefs{i:4.1.1} is symmetric.
  ...
  \end{align*}
  Thus, \labelcrefs{i:4.1.1} is transitive.
\end{proof}
...
\begin{proof}[\pf{i:ac:4.1.3}]
  ...
  \end{align*}
  Thus, \labelcrefs{i:4.1.4} is well-defined.
\end{proof}
...
\begin{proof}[\pf{i:4.1.5}]
  ...
  By \labelcrefs{i:2.2.13}, we have three cases: \(a > b\), \(a = b\), or \(a < b\).
  If \(a > b\) then by \labelcrefs{i:2.2.12.f}, \(a = b + c\) for some positive natural number \(c\), which means that \(a \mm b = c \mm 0 = c\), which is (b).
  ...
  By \labelcrefs{i:2.2.7}, a positive natural number is non-zero, so (a) and (b) cannot simultaneously be true.
  ...
  If (b) and (c) were simultaneously true, then \(n = -m\) for some positive \(n, m\), so that \((n \mm 0) = (0 \mm m)\), so that \(n + m = 0 + 0\), which contradicts \labelcrefs{i:2.2.8}.
...
\begin{note}
  ...
  Thus, by \labelcrefs{i:4.1.5}, every integer is positive, zero, or negative, but not more than one of these at a time.
\end{note}

\begin{note}
  One could well ask why we don't use \labelcrefs{i:4.1.5} to \emph{define} the integers;
...
\begin{proof}[\pf{i:4.1.6}]
  ...
  One is to use \labelcrefs{i:4.1.5} and split into a lot of cases depending on whether \(x, y, z\) are zero, positive, or negative.
...
\begin{rmk}\label{i:4.1.7}
  The set of nine identities in \labelcrefs{i:4.1.6} have a name;
...
\begin{proof}[\pf{i:ac:4.1.5}]
  ...
  \begin{align*}
    -x & = (b \mm a)                  &  & \by{i:4.1.4}    \\
       ...
       & = (-1) x.
  \end{align*}
  So we can use \labelcrefs{i:4.1.6} to show that
  \begin{align*}
...
\begin{proof}[\pf{i:4.1.8}]
  ...
  By \labelcrefs{i:4.1.5} we can split into four cases:
  \begin{itemize}
    \item \(a\) is positive and \(b\) is positive.
          Then by \labelcrefs{i:2.2.7,i:ac:4.1.2}, we know that \(a, b\) are positive natural numbers.
          But by \labelcrefs{i:2.3.3}, we must have \((a = 0) \lor (b = 0)\), a contradiction.
    \item \(a\) is negative and \(b\) is positive.
          Then by \labelcrefs{i:4.1.4}, \(-a\) is positive and we have
          \begin{align*}
          ...
          But by \labelcrefs{i:ac:4.1.6,i:ac:4.1.8}, we know that \(-a = 0 \implies -(-a) = a = -0 = 0\).
    ...
    \item \(a\) is negative and \(b\) is negative.
          Then by \labelcrefs{i:4.1.4}, \(-a\) and \(-b\) are positive.
          By \labelcrefs{i:ac:4.1.7}, we know that \((-a)(-b) = ab\).
...
\begin{lem}[Properties of order]\label{i:4.1.11}
  ...
  \begin{enumerate}
    \item\label{i:4.1.11.a} \(a > b\) iff \(a - b\) is a positive natural number.
    \item\label{i:4.1.11.b} (Addition preserves order) If \(a > b\), then \(a + c > b + c\).
    \item\label{i:4.1.11.c} (Positive multiplication preserves order) If \(a > b\) and \(c\) is positive, then \(ac > bc\).
    \item\label{i:4.1.11.d} (Negation reverses order) If \(a > b\), then \(-a < -b\).
    \item\label{i:4.1.11.e} (Order is transitive) If \(a > b\) and \(b > c\), then \(a > c\).
    \item\label{i:4.1.11.f} (Order trichotomy) Exactly one of the statements \(a > b\), \(a < b\), or \(a = b\) is true.
  \end{enumerate}
\end{lem}
...
\begin{proof}[\pf{i:4.1.11.f}]
  By \labelcrefs{i:4.1.5}, \(a - b\) can be exactly one of the following three statements:
  \begin{itemize}
    \item \(a - b = 0\).
          Then by \labelcrefs{i:ac:4.1.8}, we have \(a = b\).
    \item \(a - b\) is a positive natural number.
          Then by \labelcrefs{i:4.1.11.a}, we have \(a > b\).
    \item \(a - b = -n\) where \(n\) is a positive natural number.
          ...
          \end{align*}
          and by \labelcrefs{i:4.1.11.a}, we have \(b > a\).
          By \labelcrefs{i:4.1.10}, we have \(b > a \iff a < b\).
  \end{itemize}
...
\begin{proof}[\pf{i:ex:4.1.3}]
  See \labelcrefs{i:ac:4.1.5}.
\end{proof}

\begin{ex}\label{i:ex:4.1.4}
  Prove the remaining identities in \labelcrefs{i:4.1.6}.
\end{ex}

\begin{proof}[\pf{i:ex:4.1.4}]
  See \labelcrefs{i:4.1.6}.
\end{proof}

\begin{ex}\label{i:ex:4.1.5}
  Prove \labelcrefs{i:4.1.8}.
\end{ex}

\begin{proof}[\pf{i:ex:4.1.5}]
  See \labelcrefs{i:4.1.8}.
\end{proof}

\begin{ex}\label{i:ex:4.1.6}
  Prove \labelcrefs{i:4.1.9}.
\end{ex}

\begin{proof}[\pf{i:ex:4.1.6}]
  See \labelcrefs{i:4.1.9}.
\end{proof}

\begin{ex}\label{i:ex:4.1.7}
  Prove \labelcrefs{i:4.1.11}.
\end{ex}

\begin{proof}[\pf{i:ex:4.1.7}]
  See \labelcrefs{i:4.1.11}.
\end{proof}

\begin{ex}\label{i:ex:4.1.8}
  Show that the principle of induction (\labelcrefs{i:2.5}) does not apply directly to the integers.
...
\begin{proof}[\pf{i:ex:4.1.8}]
  ...
  We use \labelcrefs{i:2.5} to prove the claim.
  For \(n = 0\), by \labelcrefs{i:4.1.10}, we know that \((0 + 1 = 1) \land (1 \neq 0) \implies 1 > 0\).
  ...
  Then by \labelcrefs{i:4.1.11.b}, we have \((n + 1) + 1 > 0 + 1 = 1\).
  Since \((n + 1) + 1 > 1\) and \(1 > 0\), by \labelcrefs{i:4.1.11.e}, we have \((n + 1) + 1 > 0\).
  This closes the induction.

  Now let \(n = -1\).
  By \labelcrefs{i:4.1.6}, we have \((-1) + 1 = 0\).
  But by \labelcrefs{i:4.1.10}, this means \(-1 > 0\), which implies \(0 > 1\) by \labelcrefs{i:4.1.11.b}.
