...
\begin{prop}[Basic properties of absolute value and distance]\label{i:4.3.3}
  ...
  \begin{enumerate}
    \item\label{i:4.3.3.a} (Non-degeneracy of absolute value)
    We have \(\abs{x} \geq 0\).
    Also, \(\abs{x} = 0\) iff \(x\) is \(0\).
    \item\label{i:4.3.3.b} (Triangle inequality for absolute value)
    We have \(\abs{x + y} \leq \abs{x} + \abs{y}\).
    \item\label{i:4.3.3.c} We have the inequalities \(-y \leq x \leq y\) iff \(y \geq \abs{x}\).
    In particular, we have \(-\abs{x} \leq x \leq \abs{x}\).
    \item\label{i:4.3.3.d} (Multiplicativity of absolute value)
    We have \(\abs{xy} = \abs{x} \abs{y}\).
    In particular, \(\abs{-x} = \abs{x}\).
    \item\label{i:4.3.3.e} (Non-degeneracy of distance)
    We have \(d(x, y) \geq 0\).
    Also, \(d(x, y) = 0\) iff \(x = y\).
    \item\label{i:4.3.3.f} (Symmetry of distance)
    \(d(x, y) = d(y, x)\).
    \item\label{i:4.3.3.g} (Triangle inequality for distance)
    \(d(x, z) \leq d(x, y) + d(y, z)\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{i:4.3.3.a}]
  By \labelcrefs{i:4.2.7} we know that exactly one of the three statements is true:
  \begin{itemize}
    \item \(x = 0\).
          Then by \labelcrefs{i:4.3.1} we have \(\abs{x} = 0\).
    \item \(x \in \Q^+\).
          Then by \labelcrefs{i:4.3.1} we have \(\abs{x} = x \in \Q^+\).
          By \labelcrefs{i:ac:4.2.9} we have \(\abs{x} = x > 0\).
    \item \(x \in \Q^-\).
          Then by \labelcrefs{i:4.3.1} we have \(\abs{x} = -x\).
          By \labelcrefs{i:ac:4.2.5,i:ac:4.2.7} we know that \(-x = (-1)x \in \Q^+\).
          Thus, by \labelcrefs{i:ac:4.2.9} we have \(\abs{x} = -x > 0\).
  \end{itemize}
  From all cases above, we conclude that \(\abs{x} \geq 0\) and \(\abs{x} = 0 \iff x = 0\).
\end{proof}

\begin{proof}[\pf{i:4.3.3.b}]
  By \labelcrefs{i:4.2.7} exactly one of the following three statements is true:
  \begin{itemize}
    \item \(x + y = 0\).
          By \labelcrefs{i:4.3.3.a} we have \(\abs{x + y} = 0 \leq \abs{x}\) and \(0 \leq \abs{y}\).
          Thus
          \begin{align*}
          ...
          \end{align*}
    \item \(x + y \in \Q^+\).
          By \labelcrefs{i:ac:4.2.6} we know that we cannot have both \(x \in \Q^-\) and \(y \in \Q^-\).
          So we can use \labelcrefs{i:4.2.7} to split into two further cases:
          \begin{itemize}
            \item If exactly one of \(x, y\) is a negative rational number, say \(x\), then by \labelcrefs{i:ac:4.2.9} we have \(x < 0\) and \(y > 0\).
                  Note that \(y \neq 0\), otherwise we would have \(x + y = x \in \Q^-\) by \labelcrefs{i:4.2.4}, which contradicts to \(x + y \in \Q^+\).
                  Since \(x \neq 0\), by \labelcrefs{i:4.3.3.a} we have \(0 < \abs{x}\).
                  Thus
                  \begin{align*}
                  ...
                  \end{align*}
            \item If none of \(x, y\) are negative rational numbers, then by \labelcrefs{i:ac:4.2.9} we have \(x \geq 0\) and \(y \geq 0\).
                  Thus
                  \begin{align*}
                    \abs{x + y} & = x + y              &  & \by{i:4.3.1} \\
                                & = \abs{x} + \abs{y}. &  & \by{i:4.3.1}
          ...
          \end{itemize}
    \item \(x + y \in \Q^-\).
          Then by \labelcrefs{i:ac:4.2.7} we have \(-(x + y) \in \Q^+\).
          Since
          \begin{align*}
          ...
          But by \labelcrefs{i:4.3.1} we have \(\abs{x + y} = -(x + y) = \abs{-(x + y)}\).
...
\begin{proof}[\pf{i:4.3.3.d}]
  By \labelcrefs{i:4.2.7} we know that exactly one of the following three statements is true:
  \begin{itemize}
    \item \(x = 0\).
          Then we have
          \begin{align*}
          ...
          \end{align*}
    \item \(x \in \Q^+\).
          By \labelcrefs{i:4.2.7} again we know that exactly one of the following three statements is true:
          \begin{itemize}
            \item \(y = 0\).
                  By \labelcrefs{i:4.2.4} we know that \(xy = yx\), thus this is the same case as \(x = 0\).
            \item \(y \in \Q^+\).
                  By \labelcrefs{i:ac:4.2.7} we know that \(xy \in \Q^+\).
                  Thus
                  \begin{align*}
                  ...
                  \end{align*}
            \item \(y \in \Q^-\).
                  By \labelcrefs{i:ac:4.2.8} we know that \(xy \in \Q^-\).
                  Thus
                  \begin{align*}
                    \abs{xy} & = -xy              &  & \by{i:4.3.1}    \\
                             & = x(-y)            &  & \by{i:ac:4.2.5} \\
    ...
    \item \(x \in \Q^-\).
          ...
          \begin{itemize}
            \item \(y = 0\).
                  By \labelcrefs{i:4.2.4} we know that \(xy = yx\), thus this is the same case as \(x = 0\).
            \item \(y \in \Q^+\).
                  By \labelcrefs{i:4.2.4} we know that \(xy = yx\), thus this is the same case as \(x \in \Q^+\) and \(y \in \Q^-\).
            \item \(y \in \Q^-\).
                  By \labelcrefs{i:ac:4.2.7}, \(xy\) is a positive.
                  Thus
                  \begin{align*}
                    \abs{xy} & = xy               &  & \by{i:4.3.1}    \\
                             & = (-1)(-1)xy       &  & \by{i:4.2.2}    \\
...
\begin{proof}[\pf{i:4.3.3.e}]
  Since \(x - y \in \Q\), by \labelcrefs{i:4.3.3.a} we have \(d(x, y) = \abs{x - y} \geq 0\) and
  \begin{align*}
         & d(x, y) = 0                         \\
...
\begin{prop}\label{i:4.3.7}
  ...
  \begin{enumerate}
    ...
    \item\label{i:4.3.7.e} Let \(\varepsilon \in \Q_{\geq 0}\).
    If \(x\) and \(y\) are \(\varepsilon\)-close, they are also \(\varepsilon'\)-close for every \(\varepsilon' > \varepsilon\).
    \item\label{i:4.3.7.f} Let \(\varepsilon \in \Q_{\geq 0}\).
    ...
    \item\label{i:4.3.7.g} Let \(\varepsilon \in \Q_{\geq 0}\).
    If \(x\) and \(y\) are \(\varepsilon\)-close, and \(z\) is non-zero, then \(xz\) and \(yz\) are \(\varepsilon\abs{z}\)-close.
    \item\label{i:4.3.7.h} Let \(\varepsilon, \delta \in \Q_{\geq 0}\).
...
\begin{proof}[\pf{i:4.3.7.a}]
  ...
  Then by \labelcrefs{i:4.3.3.e} we have \(d(x, y) > 0\).
...
\begin{proof}[\pf{i:4.3.7.c}]
  ...
  \begin{align*}
    ...
    \implies & d(x, y) + d(y, z) \leq \varepsilon + d(y, z) \leq \varepsilon + \delta                 &  & \by{i:4.2.9.c,i:4.2.9.d} \\
...
\begin{proof}[\pf{i:4.3.7.d}]
  ...
  \begin{align*}
    ...
    \implies & d(x, y) + d(z, w) \leq \varepsilon + d(z, w) \leq \varepsilon + \delta                 &  & \by{i:4.2.9.c,i:4.2.9.d} \\
...
\begin{proof}[\pf{i:4.3.7.e}]
  ...
  \begin{align*}
    ...
    \implies & d(x, y) < \varepsilon'                                                        &  & \by{i:4.2.9.b,i:4.2.9.c} \\
...
\begin{proof}[\pf{i:4.3.7.h}]
  ...
  Since \(y = x + a\) and \(w = z + b\), by \labelcrefs{i:4.2.4} we have
  \[
    yw = (x + a)(z + b) = xz + az + xb + ab.
  \]
  Thus, by \labelcrefs{i:4.3.3.b,i:4.3.3.d} we have
  \[
...
\begin{rmk}\label{i:4.3.8}
  One should compare statements (a)-(c) of \labelcrefs{i:4.3.7} with the reflexive, symmetric, and transitive axioms of equality.
...
\begin{prop}[Properties of exponentiation, I]\label{i:4.3.10}
  ...
  \begin{enumerate}
    \item\label{i:4.3.10.a} We have \(x^n x^m = x^{n + m}\), \((x^n)^m = x^{nm}\), and \((xy)^n = x^n y^n\).
    \item\label{i:4.3.10.b} Suppose \(n > 0\).
    Then we have \(x^n = 0\) iff \(x = 0\).
    \item\label{i:4.3.10.c} If \(x \geq y \geq 0\), then \(x^n \geq y^n \geq 0\).
    If \(x > y \geq 0\) and \(n > 0\), then \(x^n > y^n \geq 0\).
    \item\label{i:4.3.10.d} We have \(\abs{x^n} = \abs{x}^n\).
  \end{enumerate}
\end{prop}
...
\begin{proof}[\pf{i:4.3.10.c}]
  ...
  \begin{itemize}
    \item If \(y = 0\), then by \labelcrefs{i:4.3.10.b} we know that \(x \neq 0 \iff x^n \neq 0\) and \(y = 0 \iff y^n = 0\).
          By \labelcrefs{i:4.2.9.e} we know that \(x > 0 \implies x^n > 0\).
          Thus, by \labelcrefs{i:4.2.9.c} we have \(x^n > y^n = 0\).
...
\begin{defn}[Exponentiation to a negative number]\label{i:4.3.11}
  ...
  Note that when \(n = 1\), the definition of \(x^{-1}\) provided by \labelcrefs{i:4.3.11} coincides with the reciprocal of \(x\) defined previously, so there is no incompatibility of notation caused by this new definition.
\end{defn}

\begin{prop}[Properties of exponentiation, II]\label{i:4.3.12}
  Let \(x, y\) be nonzero rational numbers, and let \(n, m\) be integers.
  \begin{enumerate}
    \item\label{i:4.3.12.a} We have \(x^n x^m = x^{n + m}\), \((x^n)^m = x^{nm}\), and \((xy)^n = x^n y^n\).
    \item\label{i:4.3.12.b} If \(x \geq y > 0\), then \(x^n \geq y^n > 0\) if \(n\) is positive, and \(0 < x^n \leq y^n\) if \(n\) is negative.
    \item\label{i:4.3.12.c} If \(x, y > 0\), \(n \neq 0\), and \(x^n = y^n\), then \(x = y\).
    \item\label{i:4.3.12.d} We have \(\abs{x^n} = \abs{x}^n\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{i:4.3.12.a}]
  We first show that \(x^n x^m = x^{n + m}\).
  By \labelcrefs{i:4.1.11.f} exactly one of \(n > 0\), \(n = 0\) or \(n < 0\) is true.
    ...
    \item If \(n = 0\) or \(m = 0\), then we have
          ...
          \begin{align*}
            x^n x^0 & = x^0 x^n    &  & \by{i:4.2.4}                  \\
                    ...
                    & = x^{n + 0}. &  & \by{i:4.1.6}
          \end{align*}
    \item If \(n > 0\) and \(m > 0\), then by \labelcrefs{i:4.3.10.a} we have \(x^n x^m = x^{n + m}\).
    \item If \(n > 0\), \(m < 0\) and \(n + m \geq 0\), then by \labelcrefs{i:4.1.11.d} we have \(-m > 0\).
          Thus
          \begin{align*}
            x^n x^m & = \dfrac{1 x^n}{x^{-m}}            &  & \by{i:4.3.11}   \\
                    ...
                    & = x^{n + m}.                       &  & \by{i:4.2.3}
          \end{align*}
    \item If \(n > 0\), \(m < 0\) and \(n + m < 0\), then by \labelcrefs{i:4.1.11.d} we have \(-(n + m) > 0\).
          Thus
          \begin{align*}
            x^n x^m & = \dfrac{1 x^n}{x^{-m}}              &  & \by{i:4.3.11}   \\
    ...
    \item If \(n < 0\) and \(m > 0\), then we can use the previous two cases to conclude that
          \begin{align*}
            x^n x^m & = x^m x^n    &  & \by{i:4.2.4}                         \\
                    ...
                    & = x^{n + m}. &  & \by{i:4.1.6}
          \end{align*}
    \item If \(n < 0\) and \(m < 0\), then by \labelcrefs{i:4.1.11.d} we have \(-n > 0\), \(-m > 0\) and \(-(n + m) > 0\).
          Thus
          \begin{align*}
            x^n x^m & = \dfrac{1}{x^{-n} x^{-m}}   &  & \by{i:4.3.11}   \\
  ...
  Next we show that \((x^n)^m = x^{nm}\).
  By \labelcrefs{i:4.1.5} exactly one of the following two statements is true:
  \begin{itemize}
    \item \(m \geq 0\).
          ...
          This closes the induction.
    \item \(m < 0\).
          Then by \labelcrefs{i:ac:4.2.7} we have \(-m > 0\) and
          \begin{align*}
  ...
  \begin{itemize}
    \item \(n \geq 0\).
          Then by \labelcrefs{i:4.3.10.a} we know that \((xy)^n = x^n y^n\).
    \item \(n < 0\).
          Then by \labelcrefs{i:ac:4.2.7} we have \(-n > 0\) and
          \begin{align*}
...
\begin{proof}[\pf{i:4.3.12.b}]
  ...
  Then by \labelcrefs{i:4.3.10.b,i:4.3.10.c} we have \(x^n \geq y^n > 0\).
  Now suppose that \(n < 0\).
  Then by \labelcrefs{i:ac:4.2.7} we have \(-n > 0\) and
  \begin{align*}
             & x^{-n} \geq y^{-n} > 0                                                      &  & \by{i:4.3.10.b,i:4.3.10.c} \\
...
\begin{proof}[\pf{i:4.3.12.c}]
  Suppose for the sake of contradiction that \(x \neq y\).
  Then by \labelcrefs{i:4.2.9.a} exactly one of the following two statements is true:
  \begin{itemize}
    \item \(x > y\).
          By hypothesis we know that \(n \neq 0\).
          Then by \labelcrefs{i:4.1.11.f} exactly one of the following two statements is true:
          \begin{itemize}
            \item \(n > 0\).
                  But by \labelcrefs{i:4.3.10.c} we must have \(x^n > y^n\), which contradicts to \(x^n = y^n\).
            \item \(n < 0\).
                  Then by \labelcrefs{i:ac:4.2.7} we have \(-n > 0\) and by \labelcrefs{i:4.3.10.b,i:4.3.10.c} we have \(x^{-n} > y^{-n} > 0\).
                  Since
                  \begin{align*}
                    x^n x^{-n} & = x^{n + (-n)} &  & \by{i:4.3.12.a} \\
                               ...
                               & > 0
                  \end{align*}
                  and \(x^{-n} > 0\), by \labelcrefs{i:ac:4.2.8} we know that \(x^n > 0\).
                  Similarly, we have \(y^n > 0\).
                  Thus, by \labelcrefs{i:ac:4.2.7} \(x^n y^n > 0\).
                  But then we have
                  \begin{align*}
          ...
          \end{itemize}
    \item \(x < y\).
          By \labelcrefs{i:4.2.9} we know that \(x < y \implies y > x\), and we can use the first cases to derive either \(y^n > x^n\) or \(x^n > y^n\).
...
\begin{proof}[\pf{i:4.3.12.d}]
  ...
  By \labelcrefs{i:ac:4.2.3} we know that \(x^{-1} \neq 0\).
  By \labelcrefs{i:4.2.4} we have \(x x^{-1} = 1\).
  Thus, by \labelcrefs{i:ac:4.2.7,i:ac:4.2.8} both \(x, x^{-1}\) are positive or negative.
  So we use \labelcrefs{i:4.2.9.a} to split into two cases:
  \begin{itemize}
  ...
  By \labelcrefs{i:4.1.11.f} exactly one of the following two statements is true:
  \begin{itemize}
    \item \(n \geq 0\).
          Then by \labelcrefs{i:4.3.10.d} we have \(\abs{x^n} = \abs{x}^n\).
    \item \(n < 0\).
          Then by \labelcrefs{i:ac:4.2.7} we have \(-n > 0\) and
          \begin{align*}
...
\begin{ex}\label{i:ex:4.3.1}
  Prove \labelcrefs{i:4.3.3}.
\end{ex}

\begin{proof}[\pf{i:ex:4.3.1}]
  See \labelcrefs{i:4.3.3}.
\end{proof}

\begin{ex}\label{i:ex:4.3.2}
  Prove the remaining claims in \labelcrefs{i:4.3.7}.
\end{ex}

\begin{proof}[\pf{i:ex:4.3.2}]
  See \labelcrefs{i:4.3.7}.
\end{proof}

\begin{ex}\label{i:ex:4.3.3}
  Prove \labelcrefs{i:4.3.10}.
\end{ex}

\begin{proof}[\pf{i:ex:4.3.3}]
  See \labelcrefs{i:4.3.10}.
\end{proof}

\begin{ex}\label{i:ex:4.3.4}
  Prove \labelcrefs{i:4.3.12}.
\end{ex}

\begin{proof}[\pf{i:ex:4.3.4}]
  See \labelcrefs{i:4.3.12}.
\end{proof}
...
\begin{proof}[\pf{i:ex:4.3.5}]
  ...
  For \(N = 1\), by \labelcrefs{i:4.3.9} we have
  \[
  ...
  \begin{align*}
             & 2^N \geq N                      &  & \byIH                    \\
    \implies & 2^N \times 2 \geq 2N \geq N + 1 &  & \by{i:4.2.9.c,i:4.2.9.e} \\
