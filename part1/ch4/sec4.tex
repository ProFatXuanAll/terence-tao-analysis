...
\begin{proof}[\pf{i:ac:4.4.1}]
  ...
  \begin{itemize}
    \item \(n \geq 0\).
          Then by \labelcrefs{i:2.3.9} we know that there exists a pair of \((m, r) \in \N \times \N\) such that \(0 \leq r < q\) and \(n = mq + r\).
          By \labelcrefs{i:ac:4.1.2} we have \(m \in \Z\).
          Thus, there exists a pair of \((m, r) \in \Z \times \N\) such that \(0 \leq r < q\) and \(n = mq + r\).
    \item \(n < 0\).
          Then by \labelcrefs{i:ac:4.2.7} we have \(-n > 0\) and by \labelcrefs{i:2.3.9} there exists a pair of \((m, r) \in \N \times \N\) such that \(0 \leq r < q\) and \(-n = mq + r\).
          Fix one such pair \((m, r)\).
          Since \(0 \leq r\), by \labelcrefs{i:4.1.11.d} we have \(-r \leq 0\), and by \labelcrefs{i:4.1.11.b} we have \(q - r \leq q\).
          Since \(r < q\), by \labelcrefs{i:4.1.11.a} we have \(q - r > 0\).
          Thus, by \labelcrefs{i:4.1.11.e} we have \(0 < q - r \leq q\).
          Now observe that
          \begin{align*}
            n & = -(-n)                         &  & \by{i:ac:4.1.6}         \\
              & = -(mq + r)                     &  & \by{i:2.3.9}            \\
  ...
  \begin{itemize}
    \item \(r > r'\).
          Let \(a = r - r'\)
          By \labelcrefs{i:4.1.11.a} we know that \(a \in \Z^+\).
          Then we have
          \begin{align*}
                     & mq + r = m'q + r'                                     \\
    ...
    \item \(r < r'\).
          By \labelcrefs{i:4.1.10} we have \(r' > r\).
...
\begin{proof}[\pf{i:4.4.1}]
  By \labelcrefs{i:4.2.1} we know that \(x = a / b\) where \(a, b \in \Z\) and \(b > 0\).
  Since \(a \in \Z\) and \(b \in \Z^+\), by \labelcrefs{i:ac:4.4.1} we know that there exists a pair of \((m, r) \in \Z \times \N\) such that \(a = mb + r\) and \(0 \leq r < b\).
  Then we have
  \begin{align*}
  ...
  By \labelcrefs{i:4.2.9.a} we split into three cases:
  \begin{itemize}
    \item \(x = 0\).
    ...
    \item \(x > 0\).
          ...
          Since \(x > 0\), by \labelcrefs{i:4.2.9.c} we know that \(n + 1 > 0\).
          Thus, \(n + 1 \in \N\).
...
\begin{proof}[\pf{i:4.4.3}]
  ...
  Since \(x < y\), and \(1 / 2 = 1 // 2\) is positive, we have from \labelcrefs{i:4.2.9} that \(x / 2 < y / 2\).
  If we add \(y / 2\) to both sides using \labelcrefs{i:4.2.9} we obtain \(x / 2 + y / 2 < y / 2 + y / 2\), i.e., \(z < y\).
...
\begin{proof}[\pf{i:ac:4.4.2}]
  We induct on \(n\).
  For \(n = 0\), by \labelcrefs{i:2.3.1,i:2.3.2} we have \(0 = 0 \times 2 = 2 \times 0\).
  By \labelcrefs{i:2.3} we have \(0 \neq 2m + 1\).
  ...
  \begin{itemize}
    ...
    \item \(n = 2m + 1\) for some \(m \in \N\).
          Then by \labelcrefs{i:2.2.5,i:2.3.4} we have \(n + 1 = 2m + 2 = 2(m + 1)\), which means \(n + 1\) is even.
  \end{itemize}
  ...
  By \labelcrefs{i:2.3} we know that \(2m = n + 1 > 0\), thus by \labelcrefs{i:2.3.3} we must have \(m > 0\).
  By \labelcrefs{i:2.2.12.e} we have \(m \geq 1\), so by \labelcrefs{i:4.1.11.b} we have \(m - 1 \geq 0\), and by definition \(2(m - 1) + 1\) is odd.
  Now we use \labelcrefs{i:4.1.6} to rewrite \(2m = 2 (m - 1 + 1) = 2 (m - 1) + 2\).
  Therefore by \labelcrefs{i:2.2.10} \(n + 1 = 2m \implies n = 2 (m - 1) + 1\), which means \(n\) is odd.
  But by \labelcrefs{i:2.2.10} again we have \(n + 1 = 2k + 1 \implies n = 2k\), which means \(n\) is even, contradict to the induction hypothesis that \(n\) cannot be both even and odd.
...
\begin{proof}[\pf{i:ac:4.4.3}]
  ...
  Since \(n\) is even, by \labelcrefs{i:ac:4.4.2} there exists an \(m \in \N\) such that \(n = 2m\).
  By \labelcrefs{i:ac:2.3.1} we have \(m (2m) \in \N\).
  Since
  \begin{align*}
    n^2 & = (2m)^2                         \\
        & = (2m) (2m)   &  & \by{i:2.3.11} \\
        & = 2 (m (2m)), &  & \by{i:2.3.5}
  \end{align*}
  by \labelcrefs{i:ac:4.4.2} this means \(n^2\) is even.

  Now we show that \(n\) is odd implies \(n^2\) is odd.
  Since \(n\) is odd, by \labelcrefs{i:ac:4.4.2} there exists an \(m \in \N\) such that \(n = 2m + 1\).
  By \labelcrefs{i:ac:2.3.1} we know that \(m (2m) \in \N\), thus by \labelcrefs{i:ac:2.2.1} we have \(m (2m) + 2m \in \N\).
  Since
  \begin{align*}
    n^2 & = (2m + 1)^2                                   \\
  ...
  \end{align*}
  by \labelcrefs{i:ac:4.4.2} this means \(n^2\) is odd.
\end{proof}
...
\begin{proof}[\pf{i:4.4.4}]
  ...
  Clearly, \(x\) is not zero by \labelcrefs{i:4.3.12.c}.
  ...
  By \labelcrefs{i:ac:4.4.2}, every natural number is either even or odd, but not both.
  By \labelcrefs{i:ac:4.4.3}, if \(p\) is odd, then \(p^2\) is also odd, which contradicts \(p^2 = 2q^2\).
  ...
  \begin{itemize}
    \item If \(q = p\), then we have \(p^2 = q^2\).
          But \(p^2 = 2q^2 = q^2 + q^2\) implies \(q^2 = 0\), and by \labelcrefs{i:4.3.10.b} this means \(q = 0\), which contradicts to \(q > 0\).
    \item If \(q > p\), then by \labelcrefs{i:4.3.10.c} we have \(q^2 > p^2\).
          Since \(q > 0\), by \labelcrefs{i:4.3.10.b} we have \(q^2 > 0\).
          But by \labelcrefs{i:2.2.12.f} \(p^2 = 2q^2 = q^2 + q^2\) implies \(p^2 > q^2\), which contradicts to \(q^2 > p^2\).
  \end{itemize}
  ...
  But this contradicts the principle of infinite descent (see \labelcrefs{i:ex:4.4.2}).
...
\begin{proof}[\pf{i:4.4.5}]
  ...
  (note that \((x + \varepsilon)^2\) cannot equal \(2\), by \labelcrefs{i:4.4.4}).
  ...
  (Note that \(n\varepsilon\) is non-negative for every natural number \(n\) by \labelcrefs{i:ac:4.2.7})
  But, by \labelcrefs{i:4.4.1} we can find an integer \(n\) such that \(n > 2 / \varepsilon\), which implies that \(n\varepsilon > 2\), which implies that \((n\varepsilon)^2 > 4 > 2\), contradicting the claim that \((n\varepsilon)^2 < 2\) for all natural numbers \(n\).
  This contradiction gives the proof.
\end{proof}

\begin{note}
  \labelcrefs{i:4.4.5} indicates that, while the set \(\Q\) of rationals does not actually have \(\sqrt{2}\) as a member, we can get as close as we wish to \(\sqrt{2}\).
  ...
  This is how we shall construct the real numbers in \labelcrefs{i:ch:5}.
\end{note}
...
\begin{ex}\label{i:ex:4.4.1}
  Prove \labelcrefs{i:4.4.1}.
\end{ex}

\begin{proof}[\pf{i:ex:4.4.1}]
  See \labelcrefs{i:4.4.1}.
\end{proof}
...
\begin{ex}\label{i:ex:4.4.2}
  ...
  \begin{enumerate}
    \item\label{i:ex:4.4.2.a} Prove the \emph{principle of infinite descent}:
    that it is not possible to have a sequence of \emph{natural numbers} which is in infinite descent.
    \item\label{i:ex:4.4.2.b} Does the principle of infinite descent work if the sequence \(a_1, a_2, a_3, \dots\) is allowed to take integer values instead of natural number values?
...
\begin{proof}[\pf{i:ex:4.4.2.a}]
  ...
  For \(k = 0\), by \labelcrefs{i:ac:2.2.4} we have \(a_n \geq 0\) for all \(n \in \N\), so the base case holds.
  ...
  But both \(a_1 \geq a_0\) and \(a_0 > a_1\) being true contradict to \labelcrefs{i:2.2.13}.
...
\begin{ex}\label{i:ex:4.4.3}
  Fill in the gaps marked (why?) in the proof of \labelcrefs{i:4.4.4}.
\end{ex}

\begin{proof}[\pf{i:ex:4.4.3}]
  See \labelcrefs{i:4.4.4}.
\end{proof}
