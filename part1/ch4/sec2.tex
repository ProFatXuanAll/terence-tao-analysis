...
\begin{note}
  ...
  - think of L'H\^opital's rule (see \labelcrefs{i:sec:10.5}), which suffices for doing things like defining differentiation.
\end{note}
...
\begin{proof}[\pf{i:ac:4.2.1}]
  ...
  \end{align*}
  we know that \labelcrefs{i:4.2.1} is reflexive.
  ...
  \end{align*}
  Thus, \labelcrefs{i:4.2.1} is symmetric.
  ...
  \end{align*}
  Thus, \labelcrefs{i:4.2.1} is transitive.
\end{proof}
...
\begin{defn}\label{i:4.2.2}
  ...
  Note that if \(b\) and \(d\) are non-zero, then \(bd\) is also non-zero, by \labelcrefs{i:4.1.8}, so the sum or product of two rational numbers remains a rational number.
\end{defn}
...
\begin{proof}[\pf{i:4.2.3}]
  ...
  By \labelcrefs{i:4.2.2}, the left-hand side is \((ad + bc) // bd\) and the right-hand side is \((a'd + b'c) // b'd\).
  So by \labelcrefs{i:4.2.1}, we have to show that
  \[
  ...
  By \labelcrefs{i:4.2.2}, the left-hand side is \((ad + bc) // bd\) and the right-hand side is \((ad' + bc') // bd'\).
  So by \labelcrefs{i:4.2.1}, we have to show that
  \[
    (ad + bc)bd' = (ad' + bc')bd,
  \]
  ...
  By \labelcrefs{i:4.2.2}, the left-hand side is \((ac) // (bd)\) and the right-hand side is \((a'c) // (b'd)\).
  So by \labelcrefs{i:4.2.1}, we have to show that
  \[
    (ac)(b'd) = (a'c)(bd),
  \]
  ...
  Similarly, suppose \(c // d = c' // d'\), so that \(d\) and \(d'\) are non-zero and \(cd' = c'd\).
  We now show that \((a // b) \times (c // d) = (a // b) \times (c' // d')\).
  By \labelcrefs{i:4.2.2}, the left-hand side is \((ac) // (bd)\) and the right-hand side is \((ac') // (bd')\).
  So by \labelcrefs{i:4.2.1}, we have to show that
  \[
    (ac)(bd') = (ac')(bd),
  \]
  ...
  By \labelcrefs{i:4.2.2}, the left-hand side is \((-a) // b\) and the right-hand side is \((-a') // b'\).
  So by \labelcrefs{i:4.2.1}, we have to show that
  \[
    (-a)b' = (-a')b,
  \]
  which by \labelcrefs{i:ac:4.1.5} is equivalent to
  \[
    (-1)ab' = (-1)a'b.
  \]
  But since \(ab' = a'b\), the claim follows from \labelcrefs{i:4.1.3}.
\end{proof}
...
\begin{proof}[\pf{i:ac:4.2.2}]
  ...
  This is true by \labelcrefs{i:4.2.2}.
...
\begin{ac}\label{i:ac:4.2.3}
  ...
  The reciprocal operation on rational numbers is consistent with \labelcrefs{i:4.2.1}:
...
\begin{proof}[\pf{i:ac:4.2.3}]
  By \labelcrefs{i:4.2.1} and the definition of reciprocal we have \(a, a', b, b' \neq 0\).
  Then we have
  \begin{align*}
             & a // b = a' // b'                    \\
...
\begin{rmk}\label{i:4.2.5}
  The above set (\labelcrefs{i:4.2.4}) of ten identities have a name;
  ...
  Note that \labelcrefs{i:4.2.4} supercedes \labelcrefs{i:4.1.6}.
\end{rmk}
...
\begin{proof}[\pf{i:ac:4.2.5}]
  By \labelcrefs{i:4.2.2,i:ac:4.2.4} we have \(-x = (-a) / b\) and by \labelcrefs{i:4.2.3} we have \((-1) x = (-1) (a / b)\).
...
\begin{proof}[\pf{i:4.2.7}]
  ...
  By \labelcrefs{i:4.1.11.f}, \(a\) can only satisified one of the following three statements:
  \(a = 0\), \(a > 0\) and \(a < 0\).
  ...
  \begin{itemize}
    ...
    \item If \(a > 0\), then we need to consider \(b\):
          \begin{itemize}
            \item If \(b > 0\), then by \labelcrefs{i:4.2.6} \(x\) is positive.
            \item If \(b < 0\), then by \labelcrefs{i:4.1.4} \(b = -c\) for some \(c \in \Z^+\).
                  Thus, by \labelcrefs{i:ac:4.2.5} we have \(a / b = a / (-c) = (-a) / c\), which means \(x\) is negative by \labelcrefs{i:4.2.6}.
          \end{itemize}
    \item If \(a < 0\), then by \labelcrefs{i:4.1.4} \(a = -c\) for some \(c \in \Z^+\).
          Now we consider \(b\):
          \begin{itemize}
            \item If \(b > 0\), then \(a / b = (-c) / b\), which means \(x\) is negative by \labelcrefs{i:4.2.6}.
            \item If \(b < 0\), then by \labelcrefs{i:4.1.4} \(b = -d\) for some \(d \in \Z^+\).
                  Thus, by \labelcrefs{i:4.2.1} we have \(a / b = (-c) / (-d) = (-1) / (-1) \times (c / d) = c / d\), which means \(x\) is positive by \labelcrefs{i:4.2.6}.
          \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}
    \item If \(x\) is both positive and equal to \(0\), then by \labelcrefs{i:ac:4.2.2,i:4.2.6}, \(x = a / b = 0 / 1\), where \(a, b \in \Z^+\).
          But by \labelcrefs{i:ac:4.2.2}, \(a / b = 0 / 1\) means \(a = 0\), contradicted to \(a > 0\).
    \item If \(x\) is both negative and equal to \(0\), then by \labelcrefs{i:ac:4.2.2,i:4.2.6}, \(x = (-a) / b = 0 / 1\), where \(a, b \in \Z^+\).
          But by \labelcrefs{i:ac:4.2.2} \((-a) / b = 0 / 1\) means \(-a = 0\), so that \(a = 0\) by \labelcrefs{i:ac:4.1.8}, contradicted to \(a > 0\).
    \item If \(x\) is both positive and negative, then by \labelcrefs{i:4.2.6}, \(x = a / b = (-c) / d\), where \(a, b, c, d \in \Z^+\).
          By \labelcrefs{i:4.2.1} \(a / b = (-c) / d\) means \(ad = b(-c) = (-1)(bc)\).
          By \labelcrefs{i:2.3.3} we know that \(ad\) and \(bc\) are positive.
          But by \labelcrefs{i:4.1.4} we know that \((-1)(bc) = -(bc)\) is negative, which contradict to \labelcrefs{i:4.1.5}.
  \end{itemize}
...
\begin{proof}[\pf{i:ac:4.2.6}]
  ...
  By \labelcrefs{i:4.2.6} we have \(x = a / b\) and \(y = c / d\) where \(a, b, c, d \in \Z^+\).
  Then by \labelcrefs{i:4.2.2} we have \(x + y = (ad + bc) / bd\).
  By \labelcrefs{i:2.3.3} we know that \(ad, bc, bd \in \Z^+\).
  Since \(ad, bc \in \Z^+\), by \labelcrefs{i:2.2.8} we know that \(ad + bc \in \Z^+\).
  Thus, by \labelcrefs{i:4.2.6} we know that \(x + y = (ad + bc) / bd\) is a positive rational number.

  Now we show that if \(x\) and \(y\) are two negative rationals, then \(x + y\) is also negative.
  By \labelcrefs{i:4.2.6} we have \(x = (-a) / b\) and \(y = (-c) / d\) where \(a, b, c, d \in \Z^+\).
  Then by \labelcrefs{i:4.2.2} we have \(x + y = ((-a)d + b(-c)) / bd\).
  By \labelcrefs{i:ac:4.1.5,i:4.1.6} we have
  \[
    (-a)d + b(-c) = (-1) (ad) + (-1) (cb) = (-1)(ad + cb) = -(ad + cb).
  \]
  By \labelcrefs{i:2.3.3} we know that \(ad, cb, bd \in \Z^+\).
  Since \(ad, cb \in \Z^+\), by \labelcrefs{i:2.2.8} we have \(ad + cb \in \Z^+\).
  Thus, by \labelcrefs{i:4.1.4} we have \(-(ad + cb) \in \Z^-\) and by \labelcrefs{i:4.2.6} \(x + y = -(ad + cb) / bd\) is a negative rational number.
\end{proof}
...
\begin{proof}[\pf{i:ac:4.2.7}]
  ...
  By \labelcrefs{i:4.2.6} we know that \(x = a / b\) and \(y = c / d\) where \(a, b, c, d \in \Z^+\).
  By \labelcrefs{i:4.2.2} we have \(xy = ac / bd\).
  By \labelcrefs{i:2.3.3} we have \(ac, bd \in \Z^+\), thus by \labelcrefs{i:4.2.6} we know that \(xy\) is a positive rational number.

  Now we show that if \(x\) and \(y\) are two negative rationals, then \(xy\) is a positive rational number.
  By \labelcrefs{i:4.2.6} we know that \(x = (-a) / b\) and \(y = (-c) / d\) where \(a, b, c, d \in \Z^+\).
  By \labelcrefs{i:4.2.2} we have \(xy = (-a)(-c) / bd\).
  By \labelcrefs{i:ac:4.1.7} we have \((-a)(-c) = ac\).
  By \labelcrefs{i:2.3.3} we have \(ac, bd \in \Z^+\), thus by \labelcrefs{i:4.2.6} we know that \(xy\) is a positive rational number.
\end{proof}
...
\begin{proof}[\pf{i:ac:4.2.8}]
  By \labelcrefs{i:4.2.4} we know that \(xy = yx\), thus we only need to show that if \(x\) is negative and \(y\) is positive, then \(xy\) is negative.
  By \labelcrefs{i:4.2.6} we know that \(x = (-a) / b\) and \(y = c / d\) where \(a, b, c, d \in \Z^+\).
  By \labelcrefs{i:4.2.2} we have \(xy = (-a)c / bd\).
  By \labelcrefs{i:ac:4.1.5} we have \((-a)c = -(ac)\).
  By \labelcrefs{i:2.3.3} we know that \(ac, bd \in \Z^+\), thus by \labelcrefs{i:4.2.6} we know that \(xy\) is a negative rational number.
\end{proof}
...
\begin{proof}[\pf{i:ac:4.2.9}]
  By \labelcrefs{i:4.2.1} we have \(x = x - 0\).
  Thus, \(x\) is a positive rational number iff \(x - 0\) is a positive rational number, iff \(x > 0\) (\labelcrefs{i:4.2.8}).
  Similarly, \(x\) is a negative rational number iff \(x - 0\) is a negative rational number, iff \(x < 0\) (\labelcrefs{i:4.2.8}).
\end{proof}
...
\begin{prop}[Basic properties of order on the rationals]\label{i:4.2.9}
  ...
  \begin{enumerate}
    \item\label{i:4.2.9.a} (Order trichotomy)
    Exactly one of the three statements \(x = y\), \(x < y\), or \(x > y\) is true.
    \item\label{i:4.2.9.b} (Order is anti-symmetric)
    One has \(x < y\) iff \(y > x\).
    \item\label{i:4.2.9.c} (Order is transitive)
    If \(x < y\) and \(y < z\), then \(x < z\).
    \item\label{i:4.2.9.d} (Addition preserves order)
    If \(x < y\), then \(x + z < y + z\).
    \item\label{i:4.2.9.e} (Positive multiplication preserves order)
    If \(x < y\) and \(z\) is positive, then \(xz < yz\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{i:4.2.9.a}]
  By \labelcrefs{i:4.2.7} \(x - y\) satisfy exactly one of the following three statements:
  \begin{itemize}
    \item \(x - y = 0\).
          Then by \labelcrefs{i:4.2.4} we have \(x = y\).
    \item \(x - y\) is a positive rational number.
          Then by \labelcrefs{i:4.2.8} we have \(x > y\).
...
\begin{rmk}\label{i:4.2.10}
  The above five properties in \labelcrefs{i:4.2.9}, combined with the field axioms in \labelcrefs{i:4.2.4}, have a name:
  they assert that the rationals \(\Q\) form an \emph{ordered field}.
  It is important to keep in mind that \labelcrefs{i:4.2.9.e} only works when \(z\) is positive, see \labelcrefs{i:ex:4.2.6}.
\end{rmk}
...
\begin{proof}[\pf{i:ex:4.2.1}]
  See \labelcrefs{i:ac:4.2.1}.
\end{proof}

\begin{ex}\label{i:ex:4.2.2}
  Prove the remaining components of \labelcrefs{i:4.2.3}.
\end{ex}

\begin{proof}[\pf{i:ex:4.2.2}]
  See \labelcrefs{i:4.2.3}.
\end{proof}

\begin{ex}\label{i:ex:4.2.3}
  Prove the remaining components of \labelcrefs{i:4.2.4}.
\end{ex}

\begin{proof}[\pf{i:ex:4.2.3}]
  See \labelcrefs{i:4.2.4}.
\end{proof}

\begin{ex}\label{i:ex:4.2.4}
  Prove \labelcrefs{i:4.2.7}.
\end{ex}

\begin{proof}[\pf{i:ex:4.2.4}]
  See \labelcrefs{i:4.2.7}.
\end{proof}

\begin{ex}\label{i:ex:4.2.5}
  Prove \labelcrefs{i:4.2.9}.
\end{ex}

\begin{proof}[\pf{i:ex:4.2.5}]
  See \labelcrefs{i:4.2.9}.
\end{proof}
