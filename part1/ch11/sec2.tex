...
\begin{proof}
  Let \(K' \in \mathbf{P}'\).
  Since \(\mathbf{P}'\) is finer than \(\mathbf{P}\), by \labelcrefs{i:11.1.14} \(\exists K \in \mathbf{P}\) such that \(K' \subseteq K\).
  Since \(f\) is piecewise constant with respect to \(\mathbf{P}\), by \labelcrefs{i:11.2.3} we know that \(\forall x \in K\), \(f(x)\) is constant.
  Thus, for every \(x \in K'\), \(x \in K\) and \(f(x)\) is constant.
  Since \(K'\) was arbitrary, by \labelcrefs{i:11.2.3} \(f\) is piecewise constant with respect to \(\mathbf{P}'\).
\end{proof}
...
\begin{proof}
  Since \(f\) is piecewise constant function on \(I\), by \labelcrefs{i:11.2.5} \(\exists \mathbf{P}\) such that \(\mathbf{P}\) is a partition of \(I\) and \(f\) is piecewise constant with respect to \(\mathbf{P}\).
  Similarly, \(\exists \mathbf{P}'\) such that \(\mathbf{P}'\) is a partition of \(I\) and \(g\) is piecewise constant with respect to \(\mathbf{P}'\).
  By \labelcrefs{i:11.1.18} we know that \(\mathbf{P} \# \mathbf{P}'\) is also a partition of \(I\) and \(\mathbf{P} \# \mathbf{P}'\) is both finer than \(\mathbf{P}\) and finer than \(\mathbf{P}'\).
  By \labelcrefs{i:11.2.7} we know that both \(f\) and \(g\) are piecewise constant with respect to \(\mathbf{P} \# \mathbf{P}'\).
  ...
  Thus, by \labelcrefs{i:11.2.3} \(f + g\), \(f - g\), \(\max(f, g)\), \(\min(f, g)\), \(fg\) is piecewise constant with respect to \(\mathbf{P} \# \mathbf{P}'\), and when \(g(x) \neq 0\) we have \(f / g\) is piecewise constant with respect to \(\mathbf{P} \# \mathbf{P}'\).
  By \labelcrefs{i:11.2.5} \(f + g\), \(f - g\), \(\max(f, g)\), \(\min(f, g)\), \(fg\) is piecewise constant on \(I\), and when \(g(x) \neq 0\) we have \(f / g\) is piecewise constant on \(I\).
\end{proof}
...
\begin{proof}
  By \labelcrefs{i:11.1.18} we know that \(\mathbf{P} \# \mathbf{P}'\) is a partition of \(I\) and is both finer than \(\mathbf{P}\) and finer than \(\mathbf{P}'\), thus by \labelcrefs{i:11.2.9} we have
  \[
    p.c. \int_{[\mathbf{P} \# \mathbf{P}']} f = \sum_{J \in \mathbf{P} \# \mathbf{P}'} c_J \abs{J}.
  \]
  By \labelcrefs{i:11.1.13}, we know that
  \[
...
\begin{defn}[Piecewise constant integral II]\label{i:11.2.14}
  ...
  (Note that \labelcrefs{i:11.2.13} tells us that the precise choice of this partition is irrelevant.)
\end{defn}
...
\begin{thm}[Laws of integration]\label{i:11.2.16}
  Let \(I\) be a bounded interval, and let \(f : I \to \R\) and \(g : I \to \R\) be piecewise constant functions on \(I\).
  \begin{enumerate}
    \item\label{i:11.2.16.a} We have \(p.c. \int_I (f + g) = p.c. \int_I f + p.c. \int_I g\).
    \item\label{i:11.2.16.b} For any real number \(c\), we have \(p.c. \int_I (cf) = c (p.c. \int_I f)\).
    \item\label{i:11.2.16.c} We have \(p.c. \int_I (f - g) = p.c. \int_I f - p.c. \int_I g\).
    \item\label{i:11.2.16.d} If \(f(x) \geq 0\) for all \(x \in I\), then \(p.c. \int_I f \geq 0\).
    \item\label{i:11.2.16.e} If \(f(x) \geq g(x)\) for all \(x \in I\), then \(p.c. \int_I f \geq p.c. \int_I g\).
    \item\label{i:11.2.16.f} If \(f\) is the constant function \(f(x) = c\) for all \(x \in I\), then \(p.c. \int_I f = c \abs{I}\).
    \item\label{i:11.2.16.g} Let \(J\) be a bounded interval containing \(I\) (i.e., \(I \subseteq J\)), and let \(F : J \to \R\) be the function
    \[
    ...
    \item\label{i:11.2.16.h} Suppose that \(\set{J, K}\) is a partition of \(I\) into two intervals \(J\) and \(K\).
...
\begin{proof}[\pf{i:11.2.16.a}]
  Since \(f, g\) are both piecewise constant on \(I\), by \labelcrefs{i:11.2.3} \(f\) is piecewise constant with respect to \(\mathbf{P}_f\) and \(g\) is piecewise constant with respect to \(\mathbf{P}_g\) for some partitions \(\mathbf{P}_f, \mathbf{P}_g\) of \(I\).
  Let \(\mathbf{P} = (\mathbf{P}_f \# \mathbf{P}_g) \setminus \set{\emptyset}\).
  Then by \labelcrefs{i:11.1.18} we know that \(\mathbf{P}\) is a partition of \(I\) and by \labelcrefs{i:11.2.7} \(f, g\) are piecewise constant with respect to \(\mathbf{P}\).
  For each \(J \in \mathbf{P}\), we define \(c_{f|_J}, c_{g|_J} \in \R\) to be the constant value of \(f|_J, g|_J\), respectively.
  Then by \labelcrefs{i:11.2.1} \(c_{f|_J} + c_{g|_J}\) is the constant value of \((f + g)|_J\) for each \(J \in \mathbf{P}\).
...
\begin{proof}[\pf{i:11.2.16.g}]
  If \(I = \emptyset\), then by \labelcrefs{i:11.2.3} \(F\) is piecewise constant with respect to \(\set{J}\), and by \labelcrefs{i:11.2.16.f} we have
  \[
...
\begin{ex}\label{i:ex:11.2.1}
  Prove \labelcrefs{i:11.2.7}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.2.7}.
\end{proof}

\begin{ex}\label{i:ex:11.2.2}
  Prove \labelcrefs{i:11.2.8}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.2.8}.
\end{proof}

\begin{ex}\label{i:ex:11.2.3}
  Prove \labelcrefs{i:11.2.13}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.2.13}.
\end{proof}

\begin{ex}\label{i:ex:11.2.4}
  Prove \labelcrefs{i:11.2.16}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.2.16}.
\end{proof}
