...
\begin{proof}
  ...
  From \labelcrefs{i:ex:9.8.1} we know that \(f\) is bounded.
  ...
  Then by \labelcrefs{i:11.3.12} we have
  \[
  ...
  \]
  Using telescoping series (\labelcrefs{i:7.2.15}) we thus have
  \begin{align*}
  ...
  \end{align*}
  But \(N\) was arbitrary, so we can conclude as in the proof of \labelcrefs{i:11.5.1} that \(f\) is Riemann integrable.
\end{proof}

\begin{rmk}\label{i:11.6.2}
  From \labelcrefs{i:ex:9.8.5} we know that there exist monotone functions which are not piecewise continuous, so \labelcrefs{i:11.6.1} is not subsumed by \labelcrefs{i:11.5.6}.
\end{rmk}
...
\begin{proof}
  ...
  if \(I\) is a closed interval the claim follows from \labelcrefs{i:11.6.1}.
  ...
  The function \(f\) when restricted to the interval \([a + \varepsilon, b - \varepsilon]\) is monotone, and hence Riemann integrable by \labelcrefs{i:11.6.1}.
  ...
  by \labelcrefs{i:11.2.16} we have
  \[
  ...
  \]
  But \(\varepsilon\) was arbitrary, and so we can argue as in the proof of \labelcrefs{i:11.5.1} to conclude Riemann integrability.
\end{proof}
...
\begin{proof}
  Let \(N \in \Z^+\).
  Since \(f\) is monotone decreasing, by \labelcrefs{i:11.6.1} we know that \(f\) is Riemann integrable on both \([0, N]\) and every interval \([a, b] \subseteq [0, N]\).
  Then we have
  \begin{align*}
  ...
  Then by \labelcrefs{i:7.2.2} we know that
  \[
    \sum_{n = 0}^\infty f(n) = \lim_{m \to \infty} \sum_{n = 0}^m f(n)
  \]
  and by \labelcrefs{i:6.1.12} \(\big(\sum_{n = 0}^m f(n)\big)_{m = 0}^\infty\) is a Cauchy sequence.
  By \labelcrefs{i:5.1.15} we know that \(\big(\sum_{n = 0}^m f(n)\big)_{m = 0}^\infty\) is bounded by some \(M \in \R\).
  By comparison principle (\labelcrefs{i:6.4.13}) we have
  \[
  ...
  \]
  Thus, by \labelcrefs{i:7.3.1} \(\sum_{n = 0}^\infty f(n)\) is convergent.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{i:6.7.3}(a)(d) we know that \(f\) is positive and
  \[
    \begin{dcases}
    ...
    \end{dcases}
  \]
  By \labelcrefs{i:11.6.1} \(f\) is Riemann integrable on \([1, N]\) for every \(N \in \R\) and \(N \geq 1\).
  If \(p \neq 1\), then we have
  \begin{align*}
  ...
  \]
  Note that we use \labelcrefs{i:11.9.4} and logarithm without circularity.
  ...
  Thus, \(\sup_{N > 1} \int_{[1, N]} f\) is finite and by \labelcrefs{i:11.6.4} \(\sum_{n = 1}^\infty \dfrac{1}{n^p}\) is convergent.

  Next suppose that \(p = 1\).
  Since \(\int_{[1, N]} f = \ln N\) and \(\ln N\) is unbounded, we know that \(\sup_{N > 1} \int_{[1, N]} f = +\infty\) and by \labelcrefs{i:11.6.4} \(\sum_{n = 1}^\infty \dfrac{1}{n^p}\) is divergent.

  Next suppose that \(0 < p < 1\).
  Since \(\int_{[1, N]} f = \dfrac{1}{1 - p} (N^{1 - p} - 1)\) and \(\set{N^{1 - p} : N \in \R^+}\) is unbounded, we know that \(\sup_{N > 1} \int_{[1, N]} f = +\infty\) and by \labelcrefs{i:11.6.4} \(\sum_{n = 1}^\infty \dfrac{1}{n^p}\) is divergent.

  Finally suppose that \(p \leq 0\).
  By \labelcrefs{i:6.7.3}(e) we know that \(1 = x^0 \geq x^p\) for all \(x \in [1, \infty)\), thus \(1 = \dfrac{1}{x^0} \leq \dfrac{1}{x^p}\).
  By zero test (\labelcrefs{i:7.2.6}) we know that \(\lim_{n \to \infty} 1 \neq 0\) implies \(\sum_{n = 0}^\infty \dfrac{1}{x^0}\) diverges.
  Thus, by comparison test (\labelcrefs{i:7.3.1}) \(\sum_{n = 1}^N \dfrac{1}{x^p}\) is divergent.
\end{proof}

\exercisesection

\begin{ex}\label{i:ex:11.6.1}
  Use \labelcrefs{i:11.6.1} to prove \labelcrefs{i:11.6.3}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.6.3}.
\end{proof}
...
\begin{proof}
  ...
  Since \(f\) is bounded, \(f|_J\) is also bounded, by \labelcrefs{i:11.6.3} we know that \(f|_J\) is Riemann integrable on \(J\).
  ...
  \[
    ...
    \end{dcases}
  \]
  Then by \labelcrefs{i:11.4.1.g} we know that \(F_J\) is Riemann integrable and
  \begin{align*}
...
\begin{ex}\label{i:ex:11.6.3}
  Prove \labelcrefs{i:11.6.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.6.4}.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{i:11.6.5} we know that \(\sup_{N > 0} \int_{[0, N]} \dfrac{1}{x}\) is not finite, and since \(\int_{[0, N]} f_2 = \int_{[0, N]} \dfrac{1}{x}\) we also have \(\sup_{N > 0} \int_{[0, N]} f_2\) is not finite.
  But by \labelcrefs{i:11.6.5} we know that \(\sum_{n = 0}^\infty \dfrac{1}{x^2}\) converges.
\end{proof}

\begin{ex}\label{i:ex:11.6.5}
  Use \labelcrefs{i:11.6.4} to prove \labelcrefs{i:11.6.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.6.5}.
\end{proof}
