...
\begin{proof}
  Since \(f\) is Riemann integrable, it is bounded (by \labelcrefs{i:11.3.4}).
  ...
  \]
  by \labelcrefs{i:11.4.1.h}.
  By \labelcrefs{i:11.4.1.e} we thus have
  \[
  ...
  Since this is true for all sequences \(z_n \in [a, b]\) converging to \(z\), we thus see that \(F\) is continuous at \(z\) (by \labelcrefs{i:9.4.7}).
  Since \(z\) was an arbitrary element of \([a, b]\), we thus see that \(F\) is continuous
  (The above proof also show that when \(F\) is Lipschitz continuous, \(F\) is also continuous, see \labelcrefs{i:ex:10.2.6}).
  ...
  \]
  for all \(y \in I\), since \labelcrefs{i:10.1.7} will then imply that \(F\) is differentiable at \(x_0\) with derivative \(F'(x_0) = f(x_0)\) as desired.
  ...
  \]
  Since \(x_0\), \(y \in I\), and \(I\) is a connected set (by \labelcrefs{i:11.1.6}), then \([x_0, y]\) is a subset of \(I\), and thus we have
  \[
    f(x_0) - \varepsilon \leq f(x) \leq f(x_0) + \varepsilon \text{ for all } x \in [x_0, y],
  \]
  and thus by \labelcrefs{i:11.4.1.e}
  \[
  ...
  \]
  Since \(x_0\), \(y \in I\), and \(I\) is a connected set (by \labelcrefs{i:11.1.6}), then \([y, x_0]\) is a subset of \(I\), and thus we have
  \[
...
\begin{thm}[Second Fundamental Theorem of Calculus]\label{i:11.9.4}
  Let \(a < b\) be real numbers, and let \(f : [a, b] \to \R\) be a Riemann integrable function.
...
\begin{proof}
  The claim is trivial when \(b = a\), so assume \(b > a\).
  So, in particular, all points of \([a, b]\) are limit points.
  ...
  But by \labelcrefs{i:11.3.12}, this means that
  \[
  ...
  From \labelcrefs{i:11.8.4} we have
  \[
  ...
  Note that \(F\), being differentiable, is continuous, so we may use the simplified formula for the \(F\)-length as opposed to the more complicated one in \labelcrefs{i:11.8.1}.
  By the mean-value theorem (\labelcrefs{i:10.2.9}), this is equal to \((d - c) F'(e)\) for some \(e \in J\).
...
\begin{proof}
  ...
  Since \(F, G\) are antiderivatives of \(f\), by \labelcrefs{i:11.9.3} we know that \(F, G\) are differentiable on \(I\).
  By \labelcrefs{i:10.1.13.f} we know that \(F - G\) is differentiable on \(I\), and thus by \labelcrefs{i:10.1.12} we know that \(F - G\) are continuous on \(I\).
  Let \(x, y \in I\) and \(x < y\).
  Since \(I\) is a bounded interval, by \labelcrefs{i:11.1.10} we know that \(I\) is connected, and thus by \labelcrefs{i:11.1.1} \([x, y] \subseteq I\).
  By the mean-value theorem (\labelcrefs{i:10.2.9}) we know that
  \[
...
\begin{ex}\label{i:ex:11.9.1}
  Let \(f : [0, 1] \to \R\) be the function in \labelcrefs{i:ex:9.8.5}.
...
\begin{proof}
  By \labelcrefs{i:ex:9.8.5} we know that \(f\) is strictly monotone increasing, thus by \labelcrefs{i:11.6.1} we know that \(f\) is Riemann integrable and \(F\) is well-defined.
  By \labelcrefs{i:ex:9.8.5} \(f\) is not continuous at \(q\), thus by \labelcrefs{i:ex:11.9.3} we know that \(F\) is not differentiable at \(q\).
\end{proof}

\begin{ex}\label{i:ex:11.9.2}
  Prove \labelcrefs{i:11.9.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.9.5}.
\end{proof}
...
\begin{proof}
  Since \(f\) is monotone increasing, by \labelcrefs{i:11.6.1} we know that \(f\) is Riemann integrable and \(F\) is well-defined.
  If \(f\) is continuous at \(x_0\), then by \labelcrefs{i:11.9.1} we know that \(F\) is differentiable at \(x_0\).
  ...
  Suppose for the sake of contradiction that \(f\) is not continuous at \(x_0\).
  Since \(f\) is monotone increasing, by \labelcrefs{i:ac:11.8.1} we know that both \(f(x_0+)\) and \(f(x_0-)\) exist and \(f(x_0-) \leq f(x_0+)\).
  Since \(f\) is not continuous at \(x_0\), by \labelcrefs{i:9.5.3} we have \(f(x_0-) < f(x_0+)\).
