...
\begin{proof}
  Since \(F\) is an antiderivative of \(F'\) and \(F'\) is Riemann integrable on \([a, b]\), by \labelcrefs{i:11.9.1} we know that \(F\) is continuous on \([a, b]\).
  Similarly, \(G\) is continuous on \([a, b]\).
  By \labelcrefs{i:11.5.2} we know that \(F\) and \(G\) are Riemann integrable on \([a, b]\).
  By \labelcrefs{i:11.4.5} we know that \(F G'\) and \(F' G\) are Riemann integrable on \([a, b]\).
  By \labelcrefs{i:10.1.13.d} we have \((FG)' = F' G + F G'\).
  Thus, by \labelcrefs{i:11.4.1.a} \((FG)'\) is Riemann integrable on \([a, b]\) and
  \begin{align*}
...
\begin{proof}
  Since \(f\) is piecewise constant, it is Riemann integrable, and since \(\alpha'\) is also Riemann integrable, then \(f \alpha'\) is Riemann integrable by \labelcrefs{i:11.4.5}.
  ...
  On the other hand, from \labelcrefs{i:11.4.1.h} (and \labelcrefs{i:ex:11.4.3}) we have
  \[
    \int_{[a, b]} f \alpha' = \sum_{J \in \mathbf{P}} \int_J f \alpha' = \sum_{J \in \mathbf{P}} \int_J c_J \alpha' = \sum_{J \in \mathbf{P}} c_J \int_J \alpha'.
  \]
  But by the second fundamental theorem of calculus (\labelcrefs{i:11.9.4}), \(\int_J \alpha' = \alpha[J]\), and the claim follows.
\end{proof}
...
\begin{proof}
  ...
  Also, since \(\alpha\) is monotone increasing and differentable, \(\alpha'\) is non-negative (by \labelcrefs{i:10.3.1}).
  ...
  \]
  Applying \labelcrefs{i:11.10.2}, we obtain
  \[
...
\begin{rmk}\label{i:11.10.4}
  Informally, \labelcrefs{i:11.10.3} asserts that \(f \; d \alpha\) is essentially equivalent to \(f \dfrac{d \alpha}{dx} dx\), when \(\alpha\) is differentiable.
...
\begin{proof}
  ...
  Then \(\phi^{-1}(J)\) is connected (by \labelcrefs{i:9.8.3} and \labelcrefs{i:11.1.4}), and is thus an interval.
...
\begin{proof}
  This will be obtained from \labelcrefs{i:11.10.5} in a similar manner to how \labelcrefs{i:11.10.3} was obtained from \labelcrefs{i:11.10.2}.
  First, observe that since \(f\) is Riemann integrable, it is bounded, and then \(f \circ \phi\) must also be bounded (by \labelcrefs{i:9.8.3} \(\phi\) is a bijection).
  ...
  \]
  Applying \labelcrefs{i:11.10.5}, we obtain
  \[
...
\begin{proof}
  Since \(\phi\) is differentable on \([a, b]\), by \labelcrefs{i:10.1.12} we know that \(\phi\) is continuous on \([a, b]\).
  By \labelcrefs{i:11.10.6} we know that \(f \circ \phi\) is Riemann-Stieltjes integrable with respect to \(\phi\) on \([a, b]\).
  By \labelcrefs{i:11.10.3} we know that \((f \circ \phi) \phi'\) is Riemann integrable on \([a, b]\), and
  \begin{align*}
...
\begin{proof}
  Since \(\phi\) is differentiable on \([a, b]\), by \labelcrefs{i:10.1.12} we know that \(\phi\) is continuous on \([a, b]\).
  Since \(f\) is continuous on \([\phi(a), \phi(b)]\) and \(\phi\) is continuous on \([a, b]\), by \labelcrefs{i:9.4.13} we know that \(f \circ \phi\) is continuous on \([a, b]\).
  By \labelcrefs{i:11.5.2} we know that \(f \circ \phi\) is Riemann integrable on \([a, b]\).
  Since \(\phi'\) is Riemann integrable on \([a, b]\), by \labelcrefs{i:11.4.5} we know that \((f \circ \phi) \phi'\) is Riemann integrable on \([a, b]\).
  Thus
  \[
    \int_{[a, b]} (f \circ \phi) \phi'
  \]
  is well-defined.
  Since \(f\) is continuous on \([\phi(a), \phi(b)]\), by \labelcrefs{i:11.5.2} we know that \(f\) is Riemann integrable on \([\phi(a), \phi(b)]\).
  Thus
  \[
    \int_{[\phi(a), \phi(b)]} f
  \]
  ...
  \[
    F(x) = \int_{[\phi(a), x]} f
  \]
  Since \(f\) is continuous on \([\phi(a), \phi(b)]\), by \labelcrefs{i:11.9.1} we know that \(F'(x) = f(x)\) for each \(x \in [\phi(a), \phi(b)]\).
  Then by \labelcrefs{i:10.1.15} we have
  \[
...
\begin{ex}\label{i:ex:11.10.1}
  Prove \labelcrefs{i:11.10.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.10.1}.
\end{proof}

\begin{ex}\label{i:ex:11.10.2}
  Fill in the gaps marked (why?) in the proof of \labelcrefs{i:11.10.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.10.5}.
\end{proof}
...
\begin{proof}
  ...
  \]
  By \labelcrefs{i:11.3.2} and \labelcrefs{i:11.3.3} we have
  \[
...
\begin{ex}\label{i:ex:11.10.4}
  What is the analogue of \labelcrefs{i:11.10.7} when \(\phi\) is monotone decreasing instead of monotone increasing?
...
\begin{proof}
  ...
  \]
  Since \(\eta\) is differentiable on \([-b, -a]\), by chain rule (\labelcrefs{i:10.1.15}) we know that
  \[
  ...
  \end{align*}
  Thus, \(\gamma\) is monotone increasing and by \labelcrefs{i:11.10.7} we have
  \[
  ...
  \end{align*}
  By \labelcrefs{i:ex:11.10.3} we know that
  \[
