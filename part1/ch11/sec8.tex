...
\begin{proof}
  ...
  \]
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{i:9.3.6} and \labelcrefs{i:9.5.1} we have \(f(x_0+) = U\).
  ...
  \]
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{i:9.3.6} and \labelcrefs{i:9.5.1} we have \(f(x_0-) = L\).
\end{proof}
...
\begin{note}
  \labelcrefs{i:11.8.1} is well-defined, thanks to \labelcrefs{i:ac:11.8.1}.
  \labelcrefs{i:11.8.1} is can also be applied when \(\alpha\) is monotone decreasing, thanks to \labelcrefs{i:ac:11.8.2}.
\end{note}
...
\begin{proof}
  ...
  the only way that \(I\) can be partitioned into an empty partition is if \(I\) is itself empty, so by \labelcrefs{i:11.8.1} \(\alpha[I] = 0\).
  ...
  If \(I\) is the empty set or a point, then all the intervals in \(\mathbf{P}\) must also be either the empty set or a point, and by \labelcrefs{i:11.8.1} every interval either has \(\alpha\)-length zero or
  \[
  ...
  Either way, by \labelcrefs{i:11.8.1} we see that
  \begin{align*}
  ...
  \]
  Combining these two identities (and using the laws of addition for finite sets, see \labelcrefs{i:7.1.11.e}) we obtain
  \[
    \alpha[I] = \sum_{J \in \mathbf{P}} \alpha[J]
  \]
  as desired.
  ...
  Then one of the intervals \(K\) also is of the form \((c, b)\) or \([c, b)\) (see \labelcrefs{i:ex:11.1.3}).
      In particular, this means that the set \(I \setminus K\) is also an interval of the form \([a, c], (a, c), (a, c], [a, c)\) when \(c > a\), or a point or empty set when \(a = c\).
  By \labelcrefs{i:11.8.1} we see that
  \begin{align*}
...
\begin{proof}
  ...
  Thus, by \labelcrefs{i:11.8.4} we have
  \begin{align*}
             & (-\alpha)[I] = \sum_{J \in \mathbf{P}} (-\alpha)[J]                                                                                  \\
...
\begin{note}
  When \(\alpha\) is monotone decreasing, by \labelcrefs{i:11.8.5} we have
  \[
...
\begin{proof}
  By \labelcrefs{i:11.1.18} we know that \(\mathbf{P} \# \mathbf{P}'\) is a partition of \(I\) and is both finer than \(\mathbf{P}\) and finer than \(\mathbf{P}'\), thus by \labelcrefs{i:11.8.5} we have
  \[
    p.c. \int_{[\mathbf{P} \# \mathbf{P}']} f \; d \alpha = \sum_{J \in \mathbf{P} \# \mathbf{P}'} c_J \alpha[J].
  \]
  By \labelcrefs{i:11.8.4} we know that
  \[
  ...
  \]
  Since \(\mathbf{P} \# \mathbf{P}'\) is finer than \(\mathbf{P}\), by \labelcrefs{i:ac:11.1.4} we know that \(\mathbf{P}_K\) is a partition of \(K\), and \(\bigcup_{K \in \mathbf{P}} \mathbf{P}_K = \mathbf{P} \# \mathbf{P}'\).
  Since \(f\) is piecewise constant with respect to \(\mathbf{P}\), by \labelcrefs{i:11.2.7} we know that \(f\) is piecewise constant with respect to \(\mathbf{P} \# \mathbf{P}'\).
  So we have
  \begin{align*}
...
\begin{ac}\label{i:ac:11.8.5}
  ...
  (Note that \labelcrefs{i:ac:11.8.4} tells us that the precise choice of this partition is irrelevant.)
\end{ac}
...
\begin{proof}
  ...
  \begin{itemize}
    \item \(I = \emptyset\).
          Then by \labelcrefs{i:11.8.1} we have \(\alpha[\emptyset] = 0\).
...
\begin{ac}\label{i:ac:11.8.7}
  ...
  \begin{enumerate}
    \item\label{i:ac:11.8.7.a} We have \(p.c. \int_I (f + g) \; d \alpha = p.c. \int_I f \; d \alpha + p.c. \int_I g \; d \alpha\).
    \item\label{i:ac:11.8.7.b} For any real number \(c\), we have \(p.c. \int_I (cf) \; d \alpha = c (p.c. \int_I f \; d \alpha)\).
    \item\label{i:ac:11.8.7.c} We have \(p.c. \int_I (f - g) \; d \alpha = p.c. \int_I f \; d \alpha - p.c. \int_I g \; d \alpha\).
    \item\label{i:ac:11.8.7.d} If \(f(x) \geq 0\) for all \(x \in I\), then \(p.c. \int_I f \; d \alpha \geq 0\).
    \item\label{i:ac:11.8.7.e} If \(f(x) \geq g(x)\) for all \(x \in I\), then \(p.c. \int_I f \; d \alpha \geq p.c. \int_I g \; d \alpha\).
    \item\label{i:ac:11.8.7.f} If \(f\) is the constant function \(f(x) = c\) for all \(x \in I\), then \(p.c. \int_I f \; d \alpha = c \alpha[I]\).
    \item\label{i:ac:11.8.7.g} Let \(J\) be a bounded interval containing \(I\) (i.e., \(I \subseteq J\)), and let \(F : J \to \R\) be the function
    \[
    ...
    \item\label{i:ac:11.8.7.h} Suppose that \(\set{J, K}\) is a partition of \(I\) into two intervals \(J\) and \(K\).
...
\begin{proof}[\pf{i:ac:11.8.7.a}]
  Let \(\mathbf{P}\) be a partition of \(I\).
  By \labelcrefs{i:11.2.16.a} we know that \(f + g\) is piecewise constant with respect to \(\mathbf{P}\).
  For each \(J \in \mathbf{P}\), we define \(c_{f|_J}, c_{g|_J} \in \R\) to be the constant value of \(f|_J, g|_J\), respectively.
  Then by \labelcrefs{i:11.2.1} \(c_{f|_J} + c_{g|_J}\) is the constant value of \((f + g)|_J\) for each \(J \in P\).
  Thus, we have
  \begin{align*}
...
\begin{proof}[\pf{i:ac:11.8.7.b}]
  Let \(\mathbf{P}\) be a partition of \(I\).
  By \labelcrefs{i:11.2.16.b} we know that \(cf\) is piecewise constant with respect to \(\mathbf{P}\).
  For each \(J \in \mathbf{P}\), we define \(c_J \in \R\) to be the constant value of \(f|_J\).
  Then by \labelcrefs{i:11.2.1} \(c \cdot c_J\) is the constant value of \((cf)|_J\).
  Thus, we have
  \begin{align*}
...
\begin{proof}[\pf{i:ac:11.8.7.d}]
  By \labelcrefs{i:ac:11.8.5} \(f\) is piecewise constant with respect to \(\mathbf{P}\) for some partition \(\mathbf{P}\) of \(I\).
  Let \(J \in \mathbf{P}\) and let \(c_J \in \R\) be the constant value of \(f|_J\).
  By \labelcrefs{i:ac:11.8.6} we know that \(\alpha[J] \geq 0\) for all \(J \in \mathbf{P}\).
...
\begin{proof}[\pf{i:ac:11.8.7.g}]
  If \(I = \emptyset\), then by \labelcrefs{i:11.2.3} \(F\) is piecewise constant with respect to \(\set{J}\), and by \labelcrefs{i:ac:11.8.7.f} we have
  \[
    p.c. \int_J F \; d \alpha = 0 \alpha[J] = 0 = p.c \int_I f \; d \alpha.
  \]
  So suppose that \(I \neq \emptyset\).
  By \labelcrefs{i:11.2.3}, \(f\) is piecewise constant with respect to \(\mathbf{P}\) for some partition \(\mathbf{P}\) of \(I\).
  ...
  \]
  By \labelcrefs{i:ac:11.1.5} we know that \(\mathbf{P} \cup \set{I_1, I_2}\) is a partition of \(J\).
  ...
  \[
    ...
    \end{dcases}
  \]
  Thus, by \labelcrefs{i:11.2.5} \(F\) is piecewise constant on \(J\).
...
\begin{proof}[\pf{i:ac:11.8.7.h}]
  ...
  \[
    \mathbf{P}_K = \set{S \in \mathbf{P} \# \mathbf{P}' : S \subseteq K}.
  \]
  By \labelcrefs{i:11.1.8} we know that \(\mathbf{P} \# \mathbf{P}'\) is a partition of \(I\) and is finer than \(\mathbf{P}\).
  Since \(\mathbf{P} \# \mathbf{P}'\) is finer than \(\mathbf{P}\), by \labelcrefs{i:ac:11.1.4} we know that \(\mathbf{P}_J, \mathbf{P}_K\) are partitions of \(J, K\), respectively.
  Again by \labelcrefs{i:ac:11.1.4} we know that \(\mathbf{P}_J \cup \mathbf{P}_K\) is a partition of \(I\).
  Then by \labelcrefs{i:11.2.7} \(f\) is piecewise constant with respect to \(\mathbf{P}_J \cup \mathbf{P}_K\).
...
\begin{proof}
  Since \(f(x) \leq f(x)\) for every \(x \in I\), by \labelcrefs{i:ac:11.8.8} and \labelcrefs{i:ac:11.8.9} we have
  \[
    p.c. \int_I f \; d \alpha \leq \underline{\int}_I f \; d \alpha \leq \overline{\int}_I f \; d \alpha \leq p.c. \int_I f \; d \alpha
  \]
  Thus, by \labelcrefs{i:ac:11.8.8} we have
  \[
...
\begin{ac}[Laws of Riemann-Stieltjes integration]\label{i:ac:11.8.11}
  ...
  \begin{enumerate}
    \item\label{i:ac:11.8.11.a} The function \(f + g\) is Riemann-Stieltjes integrable, and we have \(\int_I (f + g) \; d \alpha = \int_I f \; d \alpha + \int_I g \; d \alpha\).
    \item\label{i:ac:11.8.11.b} For any real number \(c\), the function \(cf\) is Riemann-Stieltjes integrable, and we have \(\int_I (cf) \; d \alpha = c(\int_I f \; d \alpha)\).
    \item\label{i:ac:11.8.11.c} The function \(f - g\) is Riemann-Stieltjes integrable, and we have \(\int_I (f - g) \; d \alpha = \int_I f \; d \alpha - \int_I g \; d \alpha\).
    \item\label{i:ac:11.8.11.d} If \(f(x) \geq 0\) for all \(x \in I\), then \(\int_I f \; d \alpha \geq 0\).
    \item\label{i:ac:11.8.11.e} If \(f(x) \geq g(x)\) for all \(x \in I\), then \(\int_I f \; d \alpha \geq \int_I g \; d \alpha\).
    \item\label{i:ac:11.8.11.f} If \(f\) is the constant function \(f(x) = c\) for all \(x \in I\), then \(\int_I f \; d \alpha = c \alpha[I]\).
    \item\label{i:ac:11.8.11.g} Suppose that \(\set{J, K}\) is a partition of \(I\) into two intervals \(J\) and \(K\).
...
\begin{proof}[\pf{i:ac:11.8.11.a}]
  ...
  \(f_U, g_U, f_L, g_L\) are well-defined since by \labelcrefs{i:ac:11.8.8} \(f, g\) are bounded functions on a bounded interval \(I\).
  Then we have
  \[
  ...
  \]
  By \labelcrefs{i:ac:11.8.8} both \(f, g\) are bounded functions, so \(f + g\) is bounded function, and \(\underline{\int}_I (f + g) \; d \alpha, \overline{\int}_I (f + g) \; d \alpha\) are well-defined (by \labelcrefs{i:ac:11.8.8}).
  By \labelcrefs{i:ex:11.3.2} we know that \(f_U + g_U\) majorizes \(f + g_U\) and \(f + g_U\) majorizes \(f + g\), thus \(f_U + g_U\) majorizes \(f + g\).
  ...
  \end{align*}
  By \labelcrefs{i:ac:11.8.9} we have
  \[
  ...
  \]
  and thus by \labelcrefs{i:ac:11.8.8} we have
  \[
    \int_I (f + g) \; d \alpha = \underline{\int}_I (f + g) \; d \alpha = \overline{\int}_I (f + g) \; d \alpha = \int_I f \; d \alpha + \int_I g \; d \alpha.
  \]
\end{proof}

\begin{proof}[\pf{i:ac:11.8.11.b}]
  Since \(f\) is Riemann-Stieltjes integrable on \(I\) with respect to \(\alpha\), by \labelcrefs{i:ac:11.8.8} we have
  \[
  ...
  \(f_U, f_L\) are well-defined since by \labelcrefs{i:ac:11.8.8} \(f\) is a bounded function on a bounded interval \(I\).
  Then by \labelcrefs{i:ac:11.8.8} we have
  \[
    p.c. \int_I f_L \; d \alpha \leq \underline{\int}_I f \; d \alpha = \int_I f \; d \alpha = \overline{\int}_I f \; d \alpha \leq p.c. \int_I f_U \; d \alpha.
  \]
  Since \(f\) is a bounded function, \(cf\) is also a bounded function, by \labelcrefs{i:ac:11.8.8} both \(\overline{\int}_I (cf) \; d \alpha, \underline{\int}_I (cf) \; d \alpha\) are well-defined.
  Since \(c > 0\), by \labelcrefs{i:11.3.1} we know that \(c f_U\) majorizes \(c f\) and \(c f_L\) minorizes \(c f\).
  Then we have
  \begin{align*}
...
\begin{proof}[\pf{i:ac:11.8.11.d}]
  Let \(f_U : I \to \R\) be a piecewise constant function on \(I\) which majorizes \(f\).
  \(f_U\) is well-defined since by \labelcrefs{i:ac:11.8.8} \(f\) is a bounded function on a bounded interval \(I\).
...
\begin{proof}[\pf{i:ac:11.8.11.e}]
  We have \(f(x) - g(x) \geq 0\) for every \(x \in I\) and by \labelcrefs{i:ac:11.8.11.c} \(f - g\) is Riemann-Stieltjes integrable on \(I\) with respect to \(\alpha\).
  Thus
  \begin{align*}
...
\begin{proof}[\pf{i:ac:11.8.11.g}]
  ...
  \]
  By \labelcrefs{i:ac:11.8.7.h} we know that \(f_U|_J : J \to \R, f_L|_J : J \to \R\) are piecewise constant function on \(J\) and \(f_U|_K : K \to \R\), \(f_L|_K : K \to \R\) are piecewise constant functions on \(K\).
  By \labelcrefs{i:11.3.1} we know that \(f_U|_J\) majorizes \(f|_J\) and \(f_L|_J\) minorizes \(f|_J\), similarly \(f_U|_K\) majorizes \(f|_K\) and \(f_L|_K\) minorizes \(f|_K\).
  ...
  \end{align*}
  by \labelcrefs{i:ac:11.8.8} we have
  \begin{align*}
...
\begin{proof}
  Since \(g\) majorizes \(f\) and \(h\) minorizes \(f\), by \labelcrefs{i:11.3.1} we have \(h(x) \leq f(x) \leq g(x)\) for every \(x \in I\).
  Since \(\mathbf{P}\) is a partition of \(I\), by \labelcrefs{i:11.1.10} for every \(J \in \mathbf{P}\), we have \(h(x) \leq f(x) \leq g(x)\) for all \(x \in J\).
  ...
  \begin{align*}
    U(f, \alpha, \mathbf{P}) & = \sum_{J \in \mathbf{P} : J \neq \emptyset} \big(\sup_{x \in J} f(x)\big) \alpha[J] &  & \by{i:ac:11.8.12}          \\
                             ...
                             & = \sum_{J \in \mathbf{P}} c_{g|_J} \alpha[J]                                         &  & \by{i:7.1.11.a,i:7.1.11.e} \\
  ...
  \begin{align*}
    L(f, \alpha, \mathbf{P}) & = \sum_{J \in \mathbf{P} : J \neq \emptyset} \big(\inf_{x \in J} f(x)\big) \alpha[J] &  & \by{i:ac:11.8.12}          \\
                             ...
                             & = \sum_{J \in \mathbf{P}} c_{h|_J} \alpha[J]                                         &  & \by{i:7.1.11.a,i:7.1.11.e} \\
...
\begin{proof}
  ...
  By \labelcrefs{i:ac:11.8.13} we have
  \[
  ...
  \]
  Since \(g, h\) were arbitrary, by \labelcrefs{i:ac:11.8.8} we have
  \[
  ...
  By \labelcrefs{i:11.2.3} we know that \(G, H\) are piecewise constant functions with respect to \(\mathbf{P}\).
  Thus, we have
  \begin{align*}
    U(f, \alpha, \mathbf{P}) & = \sum_{J \in \mathbf{P} : J \neq \emptyset} \big(\sup_{x \in J} f(x)\big) \alpha[J] &  & \by{i:ac:11.8.12}          \\
                             & = \sum_{J \in \mathbf{P}} \big(\sup_{x \in J} f(x)\big) \alpha[J]                    &  & \by{i:7.1.11.a,i:7.1.11.e} \\
                             & = p.c. \int_{[\mathbf{P}]} G \; d \alpha                                             &  & \by{i:11.8.5}              \\
                             & = p.c. \int_I G \; d \alpha                                                          &  & \by{i:ac:11.8.5}
  \end{align*}
  and
  \begin{align*}
    L(f, \alpha, \mathbf{P}) & = \sum_{J \in \mathbf{P} : J \neq \emptyset} \big(\inf_{x \in J} f(x)\big) \alpha[J] &  & \by{i:ac:11.8.12}          \\
                             & = \sum_{J \in \mathbf{P}} \big(\inf_{x \in J} f(x)\big) \alpha[J]                    &  & \by{i:7.1.11.a,i:7.1.11.e} \\
                             & = p.c. \int_{[\mathbf{P}]} H \; d \alpha                                             &  & \by{i:11.8.5}              \\
                             & = p.c. \int_I H \; d \alpha.                                                         &  & \by{i:ac:11.8.5}
  \end{align*}
  By \labelcrefs{i:ac:11.8.8} we have
...
\begin{proof}
  From \labelcrefs{i:9.9.15} we see that \(f\) is bounded.
  By \labelcrefs{i:ac:11.8.8} we have to show that \(\underline{\int}_I f \; d \alpha = \overline{\int}_I f \; d \alpha\).
  ...
  By \labelcrefs{i:ac:11.8.14}, we thus have
  \[
  ...
  \]
  but by \labelcrefs{i:11.8.4} we thus have
  \[
  ...
  By \labelcrefs{i:ac:11.8.8} we thus have that \(f\) is Riemann-Stieltjes integrable on \(I\) with respect to \(\alpha\).
\end{proof}

\exercisesection

\begin{ex}\label{i:ex:11.8.1}
  Prove \labelcrefs{i:11.8.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.8.4}.
\end{proof}

\begin{ex}\label{i:ex:11.8.2}
  State and prove a version of \labelcrefs{i:11.2.13} for the Riemann-Stieltjes integral.
\end{ex}

\begin{proof}
  See \labelcrefs{i:ac:11.8.4}.
\end{proof}

\begin{ex}\label{i:ex:11.8.3}
  State and prove a version of \labelcrefs{i:11.2.16} for the Riemann-Stieltjes integral.
\end{ex}

\begin{proof}
  See \labelcrefs{i:ac:11.8.7}.
\end{proof}

\begin{ex}\label{i:ex:11.8.4}
  State and prove a version of \labelcrefs{i:11.5.1} for the Riemann-Stieltjes integral.
\end{ex}

\begin{proof}
  See \labelcrefs{i:ac:11.8.15}.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{i:9.9.16} \(f\) is uniformly continuous, and thus by \labelcrefs{i:9.9.15} \(f\) is bounded.
  Since \(\text{sgn}\) is monotone increasing, by \labelcrefs{i:ac:11.8.15} we know that \(f\) is Riemann-Stieltjes integrable on \([-1, 1]\) with respect to \(\text{sgn}\).

  Now we show that \(\int_{[-1, 1]} f \; d \, \text{sgn} = 2f(0)\).
  Since \(f\) is continuous, by \labelcrefs{i:9.4.7} we have
  \begin{align*}
