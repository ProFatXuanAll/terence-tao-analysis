...
\begin{thm}[Laws of Riemann integration]\label{i:11.4.1}
  ...
  \begin{enumerate}
    \item\label{i:11.4.1.a} The function \(f + g\) is Riemann integrable, and we have \(\int_I (f + g) = \int_I f + \int_I g\).
    \item\label{i:11.4.1.b} For any real number \(c\), the function \(cf\) is Riemann integrable, and we have \(\int_I (cf) = c(\int_I f)\).
    \item\label{i:11.4.1.c} The function \(f - g\) is Riemann integrable, and we have \(\int_I (f - g) = \int_I f - \int_I g\).
    \item\label{i:11.4.1.d} If \(f(x) \geq 0\) for all \(x \in I\), then \(\int_I f \geq 0\).
    \item\label{i:11.4.1.e} If \(f(x) \geq g(x)\) for all \(x \in I\), then \(\int_I f \geq \int_I g\).
    \item\label{i:11.4.1.f} If \(f\) is the constant function \(f(x) = c\) for all \(x \in I\), then \(\int_I f = c \abs{I}\).
    \item\label{i:11.4.1.g} Let \(J\) be a bounded interval containing \(I\) (i.e., \(I \subseteq J\)), and let \(F : J \to \R\) be the function
    \[
    ...
    \item\label{i:11.4.1.h} Suppose that \(\set{J, K}\) is a partition of \(I\) into two intervals \(J\) and \(K\).
...
\begin{proof}[\pf{i:11.4.1.a}]
  Since \(f, g\) are Riemann integrable on \(I\), by \labelcrefs{i:11.3.4} we have
  \[
  ...
  \(f_U, g_U, f_L, g_L\) are well-defined since by \labelcrefs{i:11.3.4} \(f, g\) are bounded functions on a bounded interval \(I\).
  By \labelcrefs{i:11.3.2} we have
  \[
  ...
  \]
  By \labelcrefs{i:11.3.4} both \(f, g\) are bounded functions, so \(f + g\) is bounded function, and \(\underline{\int}_I (f + g), \overline{\int}_I (f + g)\) are well-defined (by \labelcrefs{i:11.3.2}).
  ...
  \end{align*}
  By \labelcrefs{i:11.3.3} we have
  \[
    \int_I f + \int_I g \leq \underline{\int}_I (f + g) \leq \overline{\int}_I (f + g) \leq \int_I f + \int_I g
  \]
  and thus by \labelcrefs{i:11.3.4} we have
  \[
    \int_I (f + g) = \underline{\int}_I (f + g) = \overline{\int}_I (f + g) = \int_I f + \int_I g.
  \]
\end{proof}

\begin{proof}[\pf{i:11.4.1.b}]
  Since \(f\) is Riemann integrable on \(I\), by \labelcrefs{i:11.3.4} we have
  \[
    \int_I f = \overline{\int}_I f = \underline{\int}_I f.
  \]
  ...
  \(f_U, f_L\) are well-defined since by \labelcrefs{i:11.3.4} \(f\) is a bounded function on a bounded interval \(I\).
  Then by \labelcrefs{i:11.3.2} we have
  \[
    p.c. \int_I f_L \leq \underline{\int}_I f = \int_I f = \overline{\int}_I f \leq p.c. \int_I f_U.
  \]
  Since \(f\) is a bounded function, \(cf\) is also a bounded function, by \labelcrefs{i:11.3.2} both \(\overline{\int}_I (cf), \underline{\int}_I (cf)\) are well-defined.
...
\begin{proof}[\pf{i:11.4.1.d}]
  Let \(f_U : I \to \R\) be a piecewise constant function on \(I\) which majorizes \(f\).
  \(f_U\) is well-defined since by \labelcrefs{i:11.3.4} \(f\) is a bounded function on a bounded interval \(I\).
...
\begin{proof}[\pf{i:11.4.1.e}]
  We have \(f(x) - g(x) \geq 0\) for every \(x \in I\) and by \labelcrefs{i:11.4.1.c} \(f - g\) is Riemann integrable on \(I\).
  Thus
  \begin{align*}
...
\begin{proof}[\pf{i:11.4.1.g}]
  ...
  \[
    ...
    \end{dcases}
  \]
  We know that \(F_U\) majorizes \(F\) and \(F_L\) minorizes \(F\), and by \labelcrefs{i:11.2.16.g} we have \(p.c. \int_J F_U = p.c. \int_I f_U\) and \(p.c. \int_J F_L = p.c. \int_I f_L\).
  ...
  \begin{align*}
    ...
    \implies & \overline{\int}_J F \leq \overline{\int}_I f &  & \text{(by \labelcrefs{i:11.3.2} and \(f_U\) was arbitrary)} \\
  ...
  \begin{align*}
    ...
    \implies & \underline{\int}_J F \geq \underline{\int}_I f &  & \text{(by \labelcrefs{i:11.3.2} and \(f_L\) was arbitrary)} \\
...
\begin{proof}[\pf{i:11.4.1.h}]
  ...
  \[
    p.c. \int_I f_L \leq \underline{\int}_I f = \int_I f = \overline{\int}_I f \leq p.c. \int_I f_U.
  \]
  By \labelcrefs{i:11.2.16.h} we know that \(f_U|_J : J \to \R, f_L|_J : J \to \R\) are piecewise constant function on \(J\) and \(f_U|_K : K \to \R\), \(f_L|_K : K \to \R\) are piecewise constant functions on \(K\).
  ...
  \end{align*}
  by \labelcrefs{i:11.3.4} we have
  \begin{align*}
...
\begin{rmk}\label{i:11.4.2}
  ...
  We also observe from \labelcrefs{i:11.4.1.h} and \labelcrefs{i:11.3.8} that if \(f : [a, b] \to \R\) is Riemann integrable on a closed interval \([a, b]\), then \(\int_{[a, b]} f = \int_{(a, b]} f = \int_{[a, b)} f = \int_{(a, b)} f\).
\end{rmk}
...
\begin{proof}
  By \labelcrefs{i:11.4.3} we know that \(f_+, f_-\) are Riemann integrable.
  Since \(\abs{f} = f_+ - f_-\), by \labelcrefs{i:11.4.1.a} we know that \(\abs{f}\) is Riemann integrable.
\end{proof}
...
\begin{proof}
  ...
  by \labelcrefs{i:11.4.4}, the functions \(f_+\), \(f_-\), \(g_+\), \(g_-\) are Riemann integrable.
  Since
  \[
  ...
  Then, as in the proof of \labelcrefs{i:11.4.3}, we can find a piecewise constant function \(\underline{f_+}\) minorizing \(f_+\) on \(I\), and a piecewise constant function \(\overline{f_+}\) majorizing \(f_+\) on \(I\), such that
  \[
...
\begin{ex}\label{i:ex:11.4.1}
  Prove \labelcrefs{i:11.4.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.4.1}.
\end{proof}
...
\begin{proof}
  ...
  \]
  Thus, by \labelcrefs{i:11.1.8} we have \(\abs{[a, b] \cap (x_0 - \delta, x_0 + \delta)} > 0\).
  By \labelcrefs{i:11.1.6} we know that \([a, b] \cap (x_0 - \delta, x_0 + \delta)\) is a bounded interval.
  ...
  By \labelcrefs{i:11.2.16.g} we know that \(f_L\) is a piecewise constant function.
  By \labelcrefs{i:11.3.7} we have
  \[
    \int_{[a, b]} f_L = p.c. \int_{[a, b]} f_L = \dfrac{f(x_0)}{2}\abs{[a, b] \cap (x_0 - \delta, x_0 + \delta)} > 0.
  \]
  But by \labelcrefs{i:11.3.2} and \labelcrefs{i:11.3.4} we have
  \[
...
\begin{ex}\label{i:ex:11.4.4}
  Without repeating all the computations in the above proofs, give a short explanation as to why the remaining cases of \labelcrefs{i:11.4.3} and \labelcrefs{i:11.4.5} follow automatically from the cases presented in the text.
\end{ex}

\begin{proof}
  We first show that the remaining case of \labelcrefs{i:11.4.3} is true.
  By \labelcrefs{i:11.4.1.b} \(-f\) and \(-g\) are Riemann integrable on \(I\).
  Since \(\max(-f, -g)\) is Riemann integrable and \(\min(f, g) = -\max(-f, -g)\), by \labelcrefs{i:11.4.1.b} we know that \(\min(f, g)\) is Riemann integrable.

  Now we show that the remaining cases of \labelcrefs{i:11.4.5} are true.
  By \labelcrefs{i:11.4.4} \((-f)_+\) and \((-g)_+\) are Riemann integrable on \(I\).
  Since for any Riemann integrable functions \(p\) and \(q\), \(p_+ q_+\) are Riemann integrable (which is showed in the proof of \labelcrefs{i:11.4.5}), we have
  \begin{align*}
