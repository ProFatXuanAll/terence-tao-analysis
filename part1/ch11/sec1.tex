...
\begin{proof}
  ...
  Since \(X\) is bounded, by \labelcrefs{i:5.5.9} we know that \(\inf(X), \sup(X) \in \R\).
  Thus, \(X \subseteq [\inf(X), \sup(X)]\).
  Now we split into four cases:
  \begin{itemize}
    \item If \(\sup(X) \in X\) and \(\inf(X) \in X\), then by \labelcrefs{i:11.1.1} \(X\) is connected implies \([\inf(X), \sup(X)] \subseteq X\).
          Thus, by \labelcrefs{i:3.1.18} we have \(X = [\inf(X), \sup(X)]\).
    \item If \(\sup(X) \in X\) and \(\inf(X) \notin X\), then we claim that \(\big(\inf(X), \sup(X)] \subseteq X\).
          This is true since \(X\) is connected and by \labelcrefs{i:11.1.1} we have \(\big(a, \sup(X)] \subseteq X\) for every \(a \in X\).
    \item If \(\sup(X) \notin X\) and \(\inf(X) \in X\), then we claim that \([\inf(X), \sup(X)\big) \subseteq X\).
          This is true since \(X\) is connected and by \labelcrefs{i:11.1.1} we have \([\inf(X), b\big) \subseteq X\) for every \(b \in X\).
    \item If \(\sup(X) \notin X\) and \(\inf(X) \notin X\), then we claim that \(\big(\inf(X), \sup(X)\big) \subseteq X\).
          This is true since \(X\) is connected and by \labelcrefs{i:11.1.1} we have \((a, b) \subseteq X\) for every \(a, b \in X\) and \(a < b\).
  \end{itemize}
  ...
  Let \(a, b \in \R\).
  Then \(X\) can be one of \((a, b), [a, b], (a, b], [a, b)\), and by \labelcrefs{i:11.1.1} all of which are connected.
\end{proof}
...
\begin{proof}
  ...
  Since \(I, J\) are bounded intervals, by \labelcrefs{i:11.1.4} we know that \(I, J\) are bounded and connected.
  ...
  Thus, \([x, y] \subseteq I \cap J\) and by \labelcrefs{i:11.1.1} \(I \cap J\) is connected.
  Since \(I \cap J\) is bounded and connected, by \labelcrefs{i:11.1.4} \(I \cap J\) is bounded interval.
\end{proof}
...
\begin{proof}
  Let \(x \in I\).
  By \labelcrefs{i:11.1.10} we know that \(\exists!\ K \in \mathbf{P}\) such that \(x \in K\).
  Similarly, \(\exists!\ K' \in \mathbf{P}'\) such that \(x \in K'\), thus \(x \in K \cap K'\).
  By \labelcrefs{i:11.1.16} we know that \(K \cap K' \in \mathbf{P} \# \mathbf{P}'\), thus \(x \in \bigcup (\mathbf{P} \# \mathbf{P}')\).
  ...
  By \labelcrefs{i:11.1.16} we know that \(\exists J \in \mathbf{P}\) and \(\exists J' \in \mathbf{P}'\) such that \(S = J \cap J'\).
  Since \(S = J \cap J'\), we have \(S \subseteq I\).
  Since \(S\) was arbitrary, we have
  \[
    \bigcup \big(\mathbf{P} \# \mathbf{P}'\big) \subseteq I.
  \]
  Thus, by \labelcrefs{i:3.1.18} we have
  \[
...
\begin{proof}
  By \labelcrefs{i:ac:11.1.1} we know that at least one element in \(\mathbf{P} \# \mathbf{P}'\) contains \(x\).
  Suppose for the sake of contradiction that \(\exists S_1, S_2 \in \mathbf{P} \# \mathbf{P}'\) such that \(x \in S_1\) and \(x \in S_2\) but \(S_1 \neq S_2\).
  By \labelcrefs{i:11.1.16} we know that \(S_1 = K \cap K'\) for some \(K \in \mathbf{P}\) and \(K' \in \mathbf{P}'\).
  ...
  But by \labelcrefs{i:11.1.10} we know that \(K = J\), and a similar argument holds for \(K' = J'\).
...
\begin{proof}
  ...
  \]
  By \labelcrefs{i:11.1.16} we see that \(f\) is surjective.
  By \labelcrefs{i:11.1.10} we know that both \(\#(\mathbf{P}), \#(\mathbf{P}')\) are finite.
  Thus, by \labelcrefs{i:3.6.14.e} and \labelcrefs{i:ex:8.4.3} we have
  \[
    \#(\mathbf{P} \times \mathbf{P}') = \#(\mathbf{P}) \times \#(\mathbf{P}') \geq \#(\mathbf{P} \# \mathbf{P}').
  \]
  This means \(\mathbf{P} \# \mathbf{P}'\) is finite.

  By \labelcrefs{i:11.1.16} we know that for every \(S \in \mathbf{P} \# \mathbf{P}'\), \(S = K \cap K'\) for some \(K \in \mathbf{P}\) and \(K' \in \mathbf{P}'\).
  By \labelcrefs{i:11.1.10} we know that both \(K, K'\) are bounded interval, thus by \labelcrefs{i:11.1.6} we know that \(S\) is also a bounded interval.
...
\begin{proof}
  By \labelcrefs{i:ac:11.1.1} we know that \(I = \bigcup (\mathbf{P} \# \mathbf{P}')\).
  By \labelcrefs{i:ac:11.1.2} we know that every element in \(I\) contains in exactly one of the element \(\mathbf{P} \# \mathbf{P}'\).
  By \labelcrefs{i:ac:11.1.3} we know that \(\mathbf{P} \# \mathbf{P}'\) is finite and every element in  \(\mathbf{P} \# \mathbf{P}'\) is a bounded interval.
  Thus, by \labelcrefs{i:11.1.10} \(\mathbf{P} \# \mathbf{P}'\) is a partition of \(I\).

  By \labelcrefs{i:11.1.16} we know that for every \(S \in \mathbf{P} \# \mathbf{P}'\), \(S = K \cap K'\) for some \(K \in \mathbf{P}\) and \(K' \in \mathbf{P}'\).
  This means \(S \subseteq K\) and \(S \subseteq K'\), thus by \labelcrefs{i:11.1.14} \(\mathbf{P} \# \mathbf{P}'\) is both finer than \(\mathbf{P}\) and finer than \(\mathbf{P}'\)
\end{proof}
...
\begin{proof}
  Since \(\mathbf{P}_K \subseteq \mathbf{P}'\) and \(\mathbf{P}'\) is a partition of \(I\), by \labelcrefs{i:11.1.10} we know the following facts:
  \begin{itemize}
  ...
  \end{itemize}
  To show that \(\mathbf{P}_K\) is a partition of \(K\), by \labelcrefs{i:11.1.10} it suffices to show that \(K = \bigcup \mathbf{P}_K\).

  Let \(x \in K\).
  By \labelcrefs{i:11.1.10} we know that \(x \in I\), thus \(\exists!\ K' \in \mathbf{P}'\) such that \(x \in K'\).
  ...
  By the definition of \(\mathbf{P}_K\) we know that \(\bigcup \mathbf{P}_K \subseteq K\), thus by \labelcrefs{i:3.1.18} we have \(K = \bigcup \mathbf{P}_K\).
  ...
  Let \(K' \in \mathbf{P}'\).
  By \labelcrefs{i:11.1.18} we know that \(\mathbf{P}'\) is finer than \(\mathbf{P}\).
  By \labelcrefs{i:11.1.14} we know that \(K' \subseteq K\) for some \(K \in \mathbf{P}\).
  Thus, we have \(K' \in \mathbf{P}_K\).
  Since \(K'\) was arbitrary, we have \(\mathbf{P}' \subseteq \bigcup_{K \in \mathbf{P}} \mathbf{P}_K\).
  Thus, by \labelcrefs{i:3.1.18} we have \(\bigcup_{K \in \mathbf{P}} \mathbf{P}_K = \mathbf{P}'\).
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{i:3.1.18} we have \(J = \bigcup \mathbf{P}_J\).
  ...
  Since \(\mathbf{P}_J\) is finite (\(\#(\mathbf{P}_J) = 3\)), by \labelcrefs{i:11.1.10} \(\mathbf{P}_J\) is a partition of \(J\).
\end{proof}

\exercisesection

\begin{ex}\label{i:ex:11.1.1}
  Prove \labelcrefs{i:11.1.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:11.1.4}.
\end{proof}

\begin{ex}\label{i:ex:11.1.2}
  Prove \labelcrefs{i:11.1.6}.
\end{ex}
...
\begin{proof}
  ...
  If \(c = b\), then \((c, b) = \emptyset\), and thus by \labelcrefs{i:11.1.10} \(\mathbf{P} \cup \set{\emptyset}\) is a partition of \(I\).
  ...
  By \labelcrefs{i:11.1.10} this means for every \(j \in \set{1, \dots, n}\), \(x \in I_j\) implies \(x \geq b\) or \(x < c\).
