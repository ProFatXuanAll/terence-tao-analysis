...
\begin{proof}[\pf{i:5.5.3}]
  ...
  Since \(x > 0\), by \labelcrefs{i:5.4.7} we have \(M > 0\), thus \(M \in \R^+\).
...
\begin{note}
  ...
  This motivates the \labelcrefs{i:5.5.5}.
\end{note}
...
\begin{proof}[\pf{i:5.5.7}]
  ...
  By \labelcrefs{i:5.5.5} we know that \(x \leq M\) for all \(x \in \emptyset\).
  But by \labelcrefs{i:5.5.4} we know that \(M - 1\) is also a upper bound of \(\emptyset\), so by \labelcrefs{i:5.5.5} we have \(M < M - 1\), a contradiction.
...
\begin{proof}[\pf{i:5.5.9}]
  ...
  By \labelcrefs{i:5.5.8}, we know that \(E\) has at most one least upper bound;
  ...
  By the Archimedean property (\labelcrefs{i:5.4.13}), we can find a \(K \in \Z^+\) such that \(K / n \geq M\), and hence \(K / n\) is also an upper bound for \(E\).
  ...
  Since \(K / n\) is an upper bound for \(E\) and \(L / n\) is not, we can find an integer \(L < m_n \leq K\) with the property that \(m_n / n\) is an upper bound for \(E\), but \((m_n - 1) / n\) is not (see \labelcrefs{i:ex:5.5.2}).
  In fact, this integer \(m_n\) is unique (\labelcrefs{i:ex:5.5.3}).
  ...
  Since \(m_n / n\) is an upper bound for \(E\) and \((m_{n'} - 1) / n'\) is not, by \labelcrefs{i:5.5.1} we must have \(m_n / n > (m_{n'} - 1) / n'\).
  ...
  Applying \labelcrefs{i:ex:5.4.8}, we conclude that \(x \leq \LIM_{n \to \infty} m_n / n = S\).
  ...
  Applying \labelcrefs{i:ex:5.4.8}, we conclude that \(y \geq \LIM_{n \to \infty} (m_n - 1) / n = S\).
...
\begin{defn}[Supremum]\label{i:5.5.10}
  ...
  (this is well-defined by \labelcrefs{i:5.5.9}).
...
\begin{rmk}\label{i:5.5.11}
  ...
  In \labelcrefs{i:sec:6.2} we add \(+\infty\) and \(-\infty\) to the reals to form the \emph{extended real number system}, but this system is not as convenient to work with as the real number system, because many of the laws of algebra break down.
...
\begin{proof}[\pf{i:5.5.12}]
  ...
  Thus, by the least upper bound property (\labelcrefs{i:5.5.9}), we have a real number \(x \coloneqq \sup(E)\) which is the least upper bound of \(E\).
  ...
  Since \(x^2 < 2\), we see that we can use the Archimedean property (\labelcrefs{i:5.4.13}) to choose an \(\varepsilon \in \Q_{0 < 1}\) such that \(x^2 + 5\varepsilon < 2\), thus \((x + \varepsilon)^2 < 2\).
...
\begin{rmk}\label{i:5.5.13}
  Comparing \labelcrefs{i:5.5.12} with \labelcrefs{i:4.4.4}, we see that certain numbers are real but not rational.
  The proof of \labelcrefs{i:5.5.12} also shows that the rationals \(\Q\) do not obey the least upper bound property, otherwise one could use that property to construct a square root of \(2\), which by \labelcrefs{i:4.4.4} is not possible.
\end{rmk}

\begin{rmk}\label{i:5.5.14}
  In \labelcrefs{i:ch:6} we will use the least upper bound property to develop the theory of limits, which allows us to do many more things than just take square roots.
\end{rmk}
...
\begin{rmk}\label{i:5.5.15}
  ...
  A precise relationship between the two notions is given by \labelcrefs{i:ex:5.5.1}.
  See also \labelcrefs{i:sec:6.2}.
\end{rmk}
...
\begin{ex}\label{i:ex:5.5.2}
  ...
  Without using \labelcrefs{i:5.5.9}, show that there exists an integer \(L < m \leq K\) such that \(m / n\) is an upper bound for \(E\), but that \((m - 1) / n\) is not an upper bound for \(E\).
\end{ex}

\begin{proof}[\pf{i:ex:5.5.2}]
  Let \(d = K - L\), so \(d\) is positive by \labelcrefs{i:4.1.11.a}.
  ...
  \begin{itemize}
    ...
    \item If \((K - 1) / n\) is an upper bound for \(E\), then by \labelcrefs{i:5.5.1} we have \(L < K - 1\).
...
\begin{ex}\label{i:ex:5.5.3}
  ...
  Show that \(m = m'\).
  This shows that the integer \(m\) constructed in \labelcrefs{i:ex:5.5.2} is unique.
\end{ex}

\begin{proof}[\pf{i:ex:5.5.3}]
  Suppose for the sake of contradiction that \(m \neq m'\).
  Then by \labelcrefs{i:4.1.11.f} we have either \(m < m'\) or \(m > m'\).
  \begin{itemize}
    \item If \(m < m'\), then we have \(m \leq m' - 1\).
          By \labelcrefs{i:4.2.9.e} we have \(m / n \leq (m' - 1) / n\), which means \((m' - 1) / n\) is an upper bound for \(E\).
...
\begin{proof}[\pf{i:ex:5.5.4}]
  ...
  Let \(\varepsilon \in \Q^+\).
  By Archimedean property (\labelcrefs{i:5.4.13}) we know that there exists an \(M \in \Z^+\) such that \(M \varepsilon > 1\).
  By \labelcrefs{i:4.2.9.e} we have \(\varepsilon > 1 / M\).
  But by hypothesis we have \(\abs{q_n - q_{n'}} \leq \dfrac{1}{M} < \varepsilon\) for all \(n, n' \in \Z_{\geq M}\).
  Since \(\varepsilon\) was arbitrary, by \labelcrefs{i:5.1.8} this means \((q_n)_{n = 1}^{\infty}\) is a Cauchy sequence.
...
\begin{ex}\label{i:ex:5.5.5}
  Establish an analogue of \labelcrefs{i:5.4.14}, in which ``rational'' is replaced by ``irrational.''
\end{ex}

\begin{proof}[\pf{i:ex:5.5.5}]
  Let \(x, y, z \in \R\) where \(x < y\) and \(z^2 = 2\).
  (\(z\) is well-defined thanks to \labelcrefs{i:5.5.12}.)
  So by \labelcrefs{i:5.4.7} we have \(x - z < y - z\).
  But by \labelcrefs{i:5.4.14} there exists a \(q \in \Q\) such that \(x - z < q < y - z\).
  So by \labelcrefs{i:5.4.7} again we have \(x < q + z < y\).
  Because \(z\) is irrational, \(q + z\) is also irrational
  (otherwised we have \(a = q + z \in \Q\) and \(z = a - q \in \Q\), contradicts to \labelcrefs{i:4.4.4}).
