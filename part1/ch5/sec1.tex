...
\begin{rmk}\label{i:5.1.4}
  \labelcrefs{i:5.1.3} is not standard in the literature;
...
\begin{rmk}\label{i:5.1.9}
  ...
  However, once we do construct the real numbers, we shall see that \labelcrefs{i:5.1.8} will not change if we require \(\varepsilon\) to be real instead of rational (\labelcrefs{i:6.1.4}).
...
\begin{proof}[\pf{i:5.1.11}]
  ...
  \]
  Now since \(j, k \in \Z_{\geq N}\), by \labelcrefs{i:4.3.12.b} we know that \(0 < 1 / j, 1 / k \leq 1 / N\), so that
  \begin{align*}
             & \begin{dcases}
    ...
    \implies & \begin{dcases}
               ...
               \end{dcases} &  & \by{i:4.2.9.c,i:4.2.9.d}                        \\
  ...
  But this can be done thanks to \labelcrefs{i:4.4.1}.
\end{proof}
...
\begin{note}
  While \labelcrefs{i:5.1.14} shows that every finite rational sequence is bounded, no matter how long the finite sequence is, it does not say anything about whether an infinite rational sequence is bounded or not;
...
\begin{proof}[\pf{i:5.1.15}]
  Since \((a_n)_{n = m}^{\infty}\) is a rational Cauchy sequence, by \labelcrefs{i:5.1.8} we know that \((a_n)_{n = m}^{\infty}\) is eventually \(\varepsilon\)-steady for all \(\varepsilon \in \Q^+\).
  In particular, \((a_n)_{n = m}^{\infty}\) is eventually \(1\)-steady.
  By \labelcrefs{i:5.1.6} there exists an \(N \in \Z_{\geq m}\) such that \((a_n)_{n = N}^{\infty}\) is \(1\)-steady.
  Fix such \(N\).
  ...
  \end{align*}
  Thus, by \labelcrefs{i:5.1.12} \((a_n)_{n = N}^\infty\) is bounded by \(1 + \abs{a_N}\).
  Now we split into two cases:
  \begin{itemize}
    \item If \(N = m\), then we see that \((a_n)_{n = m}^\infty\) is bounded by \(1 + \abs{a_N}\).
    \item If \(N \neq m\), then we must have \(m < N\).
          By \labelcrefs{i:5.1.14} we know that the finite rational sequence \((a_n)_{n = m}^{N - 1}\) is bounded by some \(M \in \Q_{\geq 0}\).
...
\begin{ex}\label{i:ex:5.1.1}
  Prove \labelcrefs{i:5.1.15}.
\end{ex}

\begin{proof}[\pf{i:ex:5.1.1}]
  See \labelcrefs{i:5.1.15}.
\end{proof}
