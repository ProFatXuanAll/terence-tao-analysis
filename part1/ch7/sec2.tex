...
\begin{rmk}\label{i:7.2.3}
  Note that \labelcrefs{i:6.1.7} shows that if a series converges, then it has a unique sum, so it is safe to talk about \emph{the} sum \(L = \sum_{n = m}^\infty a_n\) of a convergent series.
\end{rmk}
...
\begin{proof}
  ...
  Since \((S_k)_{k = m}^\infty\) converges, by \labelcrefs{i:6.4.18} \((S_k)_{k = m}^\infty\) is a Cauchy sequence.
  ...
  \begin{itemize}
    \item If \(p \geq q\), then \(p + 1 > q\).
          By \labelcrefs{i:7.1.1} we have \(\abs{\sum_{n = p + 1}^q a_n} = \abs{0} = 0 \leq \varepsilon\).
    \item If \(p < q\), then
          \begin{align*}
  ...
  \end{align*}
  This means \((S_k)_{k = m}^\infty\) is a Cauchy Sequence, so by \labelcrefs{i:6.4.18} \((S_k)_{k = m}^\infty\) converges, and by \labelcrefs{i:7.2.2} \(\sum_{n = m}^\infty a_n\) converges.
...
\begin{proof}
  From the zero test (\labelcrefs{i:7.2.6}), we know that if \(\sum_{n = m}^\infty (-1)^n a_n\) is a convergent series, then the sequence \(((-1)^n a_n)_{n = m}^\infty\) converges to \(0\), which implies that \(a_n\) also converges to \(0\), since \((-1)^n a_n\) and \(a_n\) have the same distance from \(0\).
  \begin{align*}
...
\begin{prop}[Series law]\label{i:7.2.14}
  \begin{enumerate}
    \item\label{i:7.2.14.a} If \(\sum_{n = m}^\infty a_n\) is a series of real numbers converging to \(x\), and \(\sum_{n = m}^\infty b_n\) is a series of real numbers converging to \(y\), then \(\sum_{n = m}^\infty (a_n + b_n)\) is also a convergent series, and converges to \(x + y\).
    In particular, we have
    \[
      \sum_{n = m}^\infty (a_n + b_n) = \sum_{n = m}^\infty a_n + \sum_{n = m}^\infty b_n.
    \]
    \item\label{i:7.2.14.b} If \(\sum_{n = m}^\infty a_n\) is a series of real numbers converging to \(x\), and \(c\) is a real number, then \(\sum_{n = m}^\infty (c a_n)\) is also a convergent series, and converges to \(cx\).
    In particular, we have
    \[
      \sum_{n = m}^\infty (c a_n) = c \sum_{n = m}^\infty a_n.
    \]
    \item\label{i:7.2.14.c} Let \(\sum_{n = m}^\infty a_n\) be a series of real numbers, and let \(k \geq 0\) be an integer.
    ...
    \]
    \item\label{i:7.2.14.d} Let \(\sum_{n = m}^\infty a_n\) be a series of real numbers converging to \(x\), and let \(k\) be an integer.
    Then \(\sum_{n = m + k}^\infty a_{n - k}\) also converges to \(x\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{i:7.2.14.a}]
  We have
  \begin{align*}
...
\begin{note}
  From \labelcrefs{i:7.2.14.c} we see that the convergence of a series does not depend on the first few elements of the series
...
\begin{proof}
  ...
  We induct on \(N\).
  For \(N = 0\), by \labelcrefs{i:7.1.1} we have
  \[
...
\begin{proof}
  ...
  Since \(\lim_{n \to \infty} a_n = 1\), by alternating series test (\labelcrefs{i:7.2.12}) we know that \(\sum_{n = m}^\infty (-1)^n a_n\) diverges.
\end{proof}

\begin{ex}\label{i:ex:7.2.2}
  Prove \labelcrefs{i:7.2.5}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.2.5}.
\end{proof}

\begin{ex}\label{i:ex:7.2.3}
  Use \labelcrefs{i:7.2.5} to prove \labelcrefs{i:7.2.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.2.6}.
\end{proof}

\begin{ex}\label{i:ex:7.2.4}
  Prove \labelcrefs{i:7.2.9}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.2.9}.
\end{proof}

\begin{ex}\label{i:ex:7.2.5}
  Prove \labelcrefs{i:7.2.14}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.2.14}.
\end{proof}

\begin{ex}\label{i:ex:7.2.6}
  Prove \labelcrefs{i:7.2.15}.
