\section{Rearrangement of series}\label{i:sec:7.4}

\begin{note}
  One feature of finite sums is that no matter how one rearranges the terms in a sequence, the total sum is the same.
  A more rigorous statement of this, involving bijections, has already appeared earlier, see \labelcrefs{i:7.1.12}.
\end{note}
...
\begin{proof}
  ...
  By \labelcrefs{i:6.3.8} we know that \(L\) is finite, and in fact \(L = \sum_{n = 0}^\infty a_n\);
  by \labelcrefs{i:6.3.8} again we see that we will thus be done as soon as we can show that \(L' = L\).

  Fix \(M\), and let \(Y\) be the set \(Y \coloneqq \set{m \in \N : m \leq M}\).
  Note that \(f\) is a bijection between \(Y\) and \(f(Y)\).
  By \labelcrefs{i:7.1.11}, we have
  \[
  ...
  In particular, \(f(Y)\) is a subset of \(\set{n \in \N : n \leq N}\), and so by \labelcrefs{i:7.1.11} again (and the assumption that all the \(a_n\) are non-negative)
  \[
  ...
  In particular, \(f^{-1}(X)\) is a subset of \(\set{m \in \N : m \leq M}\), and so by \labelcrefs{i:7.1.11} again (and the assumption that all the \(a_n\) are non-negative)
  \[
...
\setcounter{thm}{2}
\begin{prop}[Rearrangement of series]\label{i:7.4.3}
...
\begin{proof}
  We apply \labelcrefs{i:7.4.1} to the infinite series \(\sum_{n = 0}^\infty \abs{a_n}\), which by hypothesis is a convergent series of non-negative numbers.
  If we write \(L \coloneqq \sum_{n = 0}^\infty \abs{a_n}\), then by \labelcrefs{i:7.4.1} we know that \(\sum_{m = 0}^\infty \abs{a_{f(m)}}\) also converges to \(L\).
  ...
  Since \(\sum_{n = 0}^\infty \abs{a_n}\) is convergent, we can use \labelcrefs{i:7.2.5} and find an \(N_1\) such that \(\sum_{n = p + 1}^q \abs{a_n} \leq \varepsilon / 2\) for all \(p, q \geq N_1\).
  ...
  So by \labelcrefs{i:7.1.11}, for any \(M' \geq M\),
  \[
...
\begin{note}
  ...
  - but doing so is risky, unless it is backed by a rigorous result such as \labelcrefs{i:7.4.3}.)
\end{note}
...
\begin{proof}
  ...
  Since \(\sum_{n = 0}^\infty a_n\) is absolutely convergent and \(S_N\) is an increasing sequence, by \labelcrefs{i:6.3.8} we have \(\lim_{N \to \infty} S_N = \sup(S_N)_{N = 0}^\infty\).
  ...
  Since \((T_N)_{N = 0}^\infty\) is an increasing sequence and is bounded, by \labelcrefs{i:6.3.8} \((T_N)_{N = 0}^\infty\) converges, and thus \(\sum_{n = 0}^\infty a_{f(n)}\) is absolutely convergent.
\end{proof}

\begin{ex}\label{i:ex:7.4.2}
  Obtain an alternate proof of \labelcrefs{i:7.4.3} using \labelcrefs{i:7.4.1}, \labelcrefs{i:7.2.14}, and expressing \(a_n\) as the difference of \(a_n + \abs{a_n}\) and \(\abs{a_n}\).
  (This proof is due to Will Ballard.)
\end{ex}

\begin{proof}
  From hypothesis we know that \(\sum_{n = 0}^\infty \abs{a_n}\) converges, thus by \labelcrefs{i:7.2.9} we know that \(\sum_{n = 0}^\infty a_n\) converges.
  Since \(\sum_{n = 0}^\infty \abs{a_n}\) converges, by \labelcrefs{i:7.2.14.b} we know that \(\sum_{n = 0}^\infty 2 \abs{a_n}\) converges and
  \begin{align*}
  ...
  \begin{align*}
    ...
    \implies & \sum_{n = 0}^\infty a_n = \sum_{m = 0}^\infty a_{f(m)}.                                                     &  & \by{i:7.2.14.a,i:7.2.14.b}
  \end{align*}
\end{proof}
