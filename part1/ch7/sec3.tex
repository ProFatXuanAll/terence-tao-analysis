...
\begin{proof}
  ...
  From \labelcrefs{i:6.3.8} and \labelcrefs{i:6.1.17}, we thus see that the sequence \((S_N)_{n = m}^\infty\) is convergent iff it has an upper bound \(M\).
\end{proof}
...
\begin{proof}
  ...
  \begin{enumerate}
    \item If \(x = 1\), then \(\lim_{n \to \infty} x^n = 1\). By zero test (\labelcrefs{i:7.2.6}) \(\sum_{n = 0}^\infty x^n\) diverges.
    \item If \(x = -1\) or \(\abs{x} > 1\), then by \labelcrefs{i:6.5.2} \(\lim_{n \to \infty} x^n\) diverges.
          Thus, by zero test (\labelcrefs{i:7.2.6}) \(\sum_{n = 0}^\infty x^n\) diverges.
  \end{enumerate}
...
\begin{proof}
  ...
  In light of \labelcrefs{i:7.3.1}, our task is to show that the sequence \((S_N)_{N = 1}^\infty\) is bounded iff the sequence \((T_K)_{K = 0}^\infty\) is bounded.
  From \labelcrefs{i:7.3.6} we see that if \((S_N)_{N = 1}^\infty\) is bounded, then \((S_{2^K})_{K = 0}^\infty\) is bounded, and hence \((T_K)_{K = 0}^\infty\) is bounded.
  Conversely, if \((T_K)_{K = 0}^\infty\) is bounded, then \labelcrefs{i:7.3.6} implies that \((S_{2^{K + 1} - 1})_{K = 0}^\infty\) is bounded, i.e., there is an \(M\) such that \(S_{2^{K + 1} - 1} \leq M\) for all natural numbers \(K\).
...
\begin{proof}
  ...
  \]
  Also, we have
  (using \labelcrefs{i:7.1.4.a} and (f), and the hypothesis that the \(a_n\) are decreasing)
  \[
...
\begin{proof}
  The sequence \((1 / n^q)_{n = 1}^\infty\) is non-negative and decreasing (by \labelcrefs{i:6.7.3}), and so the Cauchy criterion (\labelcrefs{i:7.3.4}) applies.
  Thus, this series is convergent iff
  \[
    \sum_{k = 0}^\infty 2^k \dfrac{1}{(2^k)^q}
  \]
  is convergent.
  But by the laws of exponentiation (\labelcrefs{i:6.7.3}) we can rewrite this as the geometric series
  \[
    \sum_{k = 0}^\infty (2^{1 - q})^k.
  \]
  By \labelcrefs{i:7.3.3}, the geometric series \(\sum_{k = 0}^\infty x^k\) converges iff \(\abs{x} < 1\).
...
\begin{ex}\label{i:ex:7.3.1}
  Use \labelcrefs{i:7.3.1} to prove \labelcrefs{i:7.3.2}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.3.2}.
\end{proof}

\begin{ex}\label{i:ex:7.3.2}
  Prove \labelcrefs{i:7.3.3}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.3.3}.
\end{proof}
