\section{Finite series}\label{i:sec:7.1}

\begin{defn}[Finite series]\label{i:7.1.1}
...
\begin{lem}\label{i:7.1.4}
  \begin{enumerate}
    \item\label{i:7.1.4.a} Let \(m \leq n < p\) be integers, and let \(a_i\) be a real number assigned to each integer \(m \leq i \leq p\).
    Then we have
    \[
      \sum_{i = m}^n a_i + \sum_{i = n + 1}^p a_i = \sum_{i = m}^p a_i.
    \]
    \item\label{i:7.1.4.b} Let \(m \leq n\) be integers, \(k\) be another integer, and let \(a_i\) be a real number assigned to each integer \(m \leq i \leq n\).
    Then we have
    \[
      \sum_{i = m}^n a_i = \sum_{j = m + k}^{n + k} a_{j - k}.
    \]
    \item\label{i:7.1.4.c} Let \(m \leq n\) be integers, and let \(a_i, b_i\) be real numbers assigned to each integer \(m \leq i \leq n\).
    Then we have
    \[
      \sum_{i = m}^n (a_i + b_i) = \Bigg(\sum_{i = m}^n a_i\Bigg) + \Bigg(\sum_{i = m}^n b_i\Bigg).
    \]
    \item\label{i:7.1.4.d} Let \(m \leq n\) be integers, and let \(a_i\) be a real number assigned to each integer \(m \leq i \leq n\), and let \(c\) be another real number.
    Then we have
    \[
      \sum_{i = m}^n (ca_i) = c\Bigg(\sum_{i = m}^n a_i\Bigg).
    \]
    \item\label{i:7.1.4.e} (Triangle inequality for finite series)
    ...
    \]
    \item\label{i:7.1.4.f} (Comparison test for finite series) Let \(m \leq n\) be integers, and let \(a_i\), \(b_i\) be real numbers assigned to each integer \(m \leq i \leq n\).
...
\begin{proof}[\pf{i:7.1.4.a}]
  Let \(k = p - m\).
  By hypothesis we know that \(k > 0\).
  Now we induct on \(k\) to show that \labelcrefs{i:7.1.4.a} is true and we start with \(k = 1\).
  For \(k = 1\), we have \(p = m + 1\) and by \labelcrefs{i:7.1.1} we have
  \[
    \sum_{i = m}^n a_i + \sum_{i = n + 1}^p a_i = \sum_{i = m}^m a_i + \sum_{i = m + 1}^p a_i = a_m + a_{m + 1} = \sum_{i = m}^p a_i.
  \]
  Thus, the base case holds.
  Suppose inductively that for some \(k \geq 1\) \labelcrefs{i:7.1.4.a} is true.
...
\begin{proof}[\pf{i:7.1.4.b}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:7.1.4.b} is true.
  ...
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:7.1.4.b} is true.
...
\begin{proof}[\pf{i:7.1.4.c}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:7.1.4.c} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:7.1.4.c} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{proof}[\pf{i:7.1.4.d}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:7.1.4.d} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:7.1.4.d} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{proof}[\pf{i:7.1.4.e}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:7.1.4.e} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:7.1.4.e} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{proof}[\pf{i:7.1.4.f}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:7.1.4.f} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:7.1.4.f} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{defn}[Summations over finite sets]\label{i:7.1.6}
...
\setcounter{thm}{7}
\begin{prop}[Finite summations are well-defined]\label{i:7.1.8}
...
\begin{proof}
  ...
  (More informally, \(P(n)\) is the assertion that \labelcrefs{i:7.1.8} is true for that value of \(n\).)
  We want to prove that \(P(n)\) is true for all natural numbers \(n\).

  We first check the base case \(P(0)\).
  In this case \(\sum_{i = 1}^0 f(g(i))\) and \(\sum_{i = 1}^0 f(h(i))\) both equal to \(0\), by definition of finite series (\labelcrefs{i:7.1.1}), so we are done.
  ...
  By definition of finite series (\labelcrefs{i:7.1.1}), we can expand the left-hand side of \labelcrefs{i:eq 7.1} as
  \[
    \sum_{i = 1}^{n + 1} f(g(i)) = \Bigg(\sum_{i = 1}^n f(g(i))\Bigg) + f(x).
  \]
  Now let us look at the right-hand side of \labelcrefs{i:eq 7.1}.
  ...
  We now use \labelcrefs{i:7.1.4} and the definition of finite series (\labelcrefs{i:7.1.1}) to write
  \begin{align*}
  ...
  We can thus write the right-hand side of \labelcrefs{i:eq 7.1} as
  \[
    = \Bigg(\sum_{i = 1}^{j - 1} f(\tilde{h}(i))\Bigg) + f(x) + \Bigg(\sum_{i = j}^n f(\tilde{h}(i))\Bigg) = \Bigg(\sum_{i = 1}^n f(\tilde{h}(i))\Bigg) + f(x)
  \]
  where we have used \labelcrefs{i:7.1.4} once again.
  Thus, to finish the proof of \labelcrefs{i:eq 7.1} we have to show that
  \[
  ...
  The function \(\tilde{h}\) is also a bijection from \(\set{i \in \N : 1 \leq i \leq n} \to X - \set{x}\) (cf. \labelcrefs{i:3.6.9}).
  Since \(X - \set{x}\) has \(n\) elements (by \labelcrefs{i:3.6.9}), the claim \labelcrefs{i:eq 7.2} then follows directly from the induction hypothesis \(P(n)\).
\end{proof}

\begin{rmk}\label{i:7.1.9}
  The issue is somewhat more complicated when summing over infinite sets;
  See \labelcrefs{i:sec:8.2}.
\end{rmk}
...
\begin{prop}[Basic properties of summation over finite sets]\label{i:7.1.11}
  \begin{enumerate}
    \item\label{i:7.1.11.a} If \(X\) is empty, and \(f : X \to \R\) is a function (i.e., \(f\) is the empty function), we have
    \[
      \sum_{x \in X} f(x) = 0.
    \]
    \item\label{i:7.1.11.b} If \(X\) consists of a single element, \(X = \set{x_0}\), and \(f : X \to \R\) is a function, we have
    \[
      \sum_{x \in X} f(x) = f(x_0).
    \]
    \item\label{i:7.1.11.c} (Substitution, part I) If \(X\) is a finite set, \(f : X \to \R\) is a function, and \(g : Y \to X\) is a bijection, then
    \[
      \sum_{x \in X} f(x) = \sum_{y \in Y} f(g(y)).
    \]
    \item\label{i:7.1.11.d} (Substitution, part II) Let \(n \leq m\) be integers, and let \(X\) be the set \(X \coloneqq \set{i \in \Z : n \leq i \leq m}\).
    If \(a_i\) is a real number assigned to each integer \(i \in X\), then we have
    \[
      \sum_{i = n}^m a_i = \sum_{i \in X} a_i.
    \]
    \item\label{i:7.1.11.e} Let \(X, Y\) be disjoint finite sets (so \(X \cap Y = \emptyset\)), and \(f : X \cup Y \to \R\) is a function.
    Then we have
    \[
      \sum_{z \in X \cup Y} f(z) = \Bigg(\sum_{x \in X} f(x)\Bigg) + \Bigg(\sum_{y \in Y} f(y)\Bigg).
    \]
    \item\label{i:7.1.11.f} (Linearity, part I) Let \(X\) be a finite set, and let \(f : X \to \R\) and \(g : X \to \R\) be functions.
    Then
    \[
      \sum_{x \in X} (f(x) + g(x)) = \sum_{x \in X} f(x) + \sum_{x \in X} g(x).
    \]
    \item\label{i:7.1.11.g} (Linearity, part II) Let \(X\) be a finite set, let \(f : X \to \R\) be a function, and let \(c\) be a real number.
    Then
    \[
      \sum_{x \in X} cf(x) = c \sum_{x \in X} f(x).
    \]
    \item\label{i:7.1.11.h} (Monotonicity) Let \(X\) be a finite set, and let \(f : X \to \R\) and \(g : X \to \R\) be functions such that \(f(x) \leq g(x)\) for all \(x \in \mathbf{X}\).
    Then we have
    \[
      \sum_{x \in X} f(x) \leq \sum_{x \in X} g(x).
    \]
    \item\label{i:7.1.11.i} (Triangle inequality) Let \(X\) be a finite set, and let \(f : X \to \R\) be a function, then
    \[
...
\begin{rmk}\label{i:7.1.12}
  The substitution rule in \labelcrefs{i:7.1.11.c} can be thought of as making the substitution \(x \coloneqq g(y)\) (hence the name).
  Note that the assumption that \(g\) is a bijection is essential.
  From \labelcrefs{i:7.1.11.c} and (d) we see that
  \[
...
\begin{proof}
  ...
  We will use induction on \(n\) (cf. \labelcrefs{i:7.1.8});
  i.e., we let \(P(n)\) be the assertion that \labelcrefs{i:7.1.13} is true for any set \(X\) with \(n\) elements, and any finite set \(Y\) and any function \(f : X \times Y \to \R\).
  We wish to prove \(P(n)\) for all natural numbers \(n\).

  The base case \(P(0)\) is easy, following from \labelcrefs{i:7.1.11.a}.
  Now suppose that \(P(n)\) is true;
  we now show that \(P(n + 1)\) is true.
  Let \(X\) be a set with \(n + 1\) elements.
  In particular, by \labelcrefs{i:3.6.9}, we can write \(X = X' \cup \set{x_0}\), where \(x_0\) is an element of \(X\) and \(X' \coloneqq X - \set{x_0}\) has \(n\) elements.
  Then by \labelcrefs{i:7.1.11.e} we have
  \[
  ...
  \]
  By \labelcrefs{i:7.1.11.c} this is equal to
  \[
    \sum_{(x, y) \in X' \times Y} f(x, y) + \bigg(\sum_{(x, y) \in \set{x_0} \times Y} f(x, y)\bigg).
  \]
  By \labelcrefs{i:7.1.11.e} this is equal to
  \[
...
\begin{cor}[Fubini's theorem for finite series]\label{i:7.1.14}
  Let \(X, Y\) be finite sets, and let \(f : X \times Y \to \R\) be a function.
  Then
  \begin{align*}
    \sum_{x \in X} \bigg(\sum_{y \in Y} f(x, y)\bigg) & = \sum_{(x, y) \in X \times Y} f(x, y)               \\
...
\begin{proof}
  In light of \labelcrefs{i:7.1.13}, it suffices to show that
  \[
    \sum_{(x, y) \in X \times Y} f(x, y) = \sum_{(y, x) \in Y \times X} f(x, y).
  \]
  But this follows from \labelcrefs{i:7.1.11.c} by applying the bijection \(h : Y \times X \to X \times Y\) defined by \(h(y, x) \coloneqq (x, y)\).
\end{proof}

\begin{rmk}\label{i:7.1.15}
  We anticipate something interesting to happen when we move from finite sums to infinite sums.
  However, see \labelcrefs{i:8.2.2}.
\end{rmk}

\begin{ac}[Products over finite sets]\label{i:ac:7.1.1}
  Let \(m, n\) be integers, and let \((a_i)_{i = m}^n\) be a finite sequence of real numbers, assigning a real number \(a_i\) to each integer \(i\) between \(m\) and \(n\) inclusive (i.e., \(m \leq i \leq n\)).
...
\begin{ac}\label{i:ac:7.1.2}
  \begin{enumerate}
    \item\label{i:ac:7.1.2.a} Let \(m \leq n < p\) be integers, and let \(a_i\) be a real number assigned to each integer \(m \leq i \leq p\).
    Then we have
    \[
      \prod_{i = m}^n a_i \times \prod_{i = n + 1}^p a_i = \prod_{i = m}^p a_i.
    \]
    \item\label{i:ac:7.1.2.b} Let \(m \leq n\) be integers, \(k\) be another integer, and let \(a_i\) be a real number assigned to each integer \(m \leq i \leq n\).
    Then we have
    \[
      \prod_{i = m}^n a_i = \prod_{j = m + k}^{n + k} a_{j - k}.
    \]
    \item\label{i:ac:7.1.2.c} Let \(m \leq n\) be integers, and let \(a_i, b_i\) be real numbers assigned to each integer \(m \leq i \leq n\).
    Then we have
    \[
      \prod_{i = m}^n (a_i \times b_i) = \Bigg(\prod_{i = m}^n a_i\Bigg) \times \Bigg(\prod_{i = m}^n b_i\Bigg).
    \]
    \item\label{i:ac:7.1.2.d} Let \(m \leq n\) be integers, and let \(a_i\) be a real number assigned to each integer \(m \leq i \leq n\), and let \(c\) be another real number.
    Then we have
    \[
      \prod_{i = m}^n (ca_i) = c^{n - m + 1} \Bigg(\prod_{i = m}^n a_i\Bigg).
    \]
    \item\label{i:ac:7.1.2.e} Let \(m \leq n\) be integers, and let \(a_i\) be a real number assigned to each integer \(m \leq i \leq n\).
    Then we have
    \[
...
\begin{proof}[\pf{i:ac:7.1.2.a}]
  Let \(k = p - m\).
  By hypothesis we know that \(k > 0\).
  Now we induct on \(k\) to show that \labelcrefs{i:ac:7.1.2.a} is true and we start with \(k = 1\).
  For \(k = 1\), we have \(p = m + 1\) and by \labelcrefs{i:ac:7.1.1} we have
  \[
    \prod_{i = m}^n a_i \times \prod_{i = n + 1}^p a_i = \prod_{i = m}^m a_i \times \prod_{i = m + 1}^p a_i = a_m \times a_{m + 1} = \prod_{i = m}^p a_i.
  \]
  Thus, the base case holds.
  Suppose inductively that for some \(k \geq 1\) \labelcrefs{i:ac:7.1.2.a} is true.
  Then for \(k + 1 = p - m\), we have \(p - 1 = k + m\) and
  \begin{align*}
...
\begin{proof}[\pf{i:ac:7.1.2.b}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:ac:7.1.2.b} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:ac:7.1.2.b} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{proof}[\pf{i:ac:7.1.2.c}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:ac:7.1.2.c} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:ac:7.1.2.c} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{proof}[\pf{i:ac:7.1.2.d}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:ac:7.1.2.d} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:ac:7.1.2.d} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{proof}[\pf{i:ac:7.1.2.e}]
  Let \(p = n - m\).
  By hypothesis we know that \(p \geq 0\).
  Now we induct on \(p\) to show that \labelcrefs{i:ac:7.1.2.e} is true.
  For \(p = 0\), we have \(n = m\) and
  \begin{align*}
  ...
  \end{align*}
  So the base case holds.
  Suppose inductively that for some \(p \geq 0\) \labelcrefs{i:ac:7.1.2.e} is true.
  Then for \(p + 1 = n - m\), we have \(p = n - m - 1\) and
  \begin{align*}
...
\begin{ac}[Finite products are well-defined]\label{i:ac:7.1.4}
  Let \(X\) be a finite set with \(n\) elements (where \(n \in \N\)), let \(f : X \to \R\) be a function, and let \(g : \set{i \in \N : 1 \leq i \leq n} \to X\) and \(h : \set{i \in \N : 1 \leq i \leq n} \to X\) be bijections.
  Then we have
  \[
...
\begin{proof}
  ...
  (More informally, \(P(n)\) is the assertion that \labelcrefs{i:ac:7.1.4} is true for that value of \(n\).)
  We induct on \(n\).

  We first check the base case \(P(0)\).
  In this case \(\prod_{i = 1}^0 f(g(i))\) and \(\prod_{i = 1}^0 f(h(i))\) both equal to \(1\), by \labelcrefs{i:ac:7.1.1}, so we are done.
  ...
  Let \(x \coloneqq g(n + 1)\);
  thus \(x\) is an element of \(X\).
  By \labelcrefs{i:ac:7.1.1}, we can expand the left-hand side of \labelcrefs{i:eq:ac:7.1} as
  \[
    \prod_{i = 1}^{n + 1} f(g(i)) = \Bigg(\prod_{i = 1}^n f(g(i))\Bigg) \times f(x).
  \]
  Now let us look at the right-hand side of \labelcrefs{i:eq:ac:7.1}.
  Since \(h\) is a bijection, we do know that there is \emph{some} index \(j\), with \(1 \leq j \leq n + 1\), for which \(h(j) = x\).
  We now use \labelcrefs{i:ac:7.1.1,i:ac:7.1.2} to write
  \begin{align*}
  ...
  \end{align*}
  We now define the function \(\tilde{h} : \set{i \in \N : 1 \leq i \leq n} \to X - \set{x}\) by setting \(\tilde{h}(i) \coloneqq h(i)\) when \(i < j\) and \(\tilde{h}(i) \coloneqq h(i + 1)\) when \(i \geq j\).
  We can thus write the right-hand side of \labelcrefs{i:eq:ac:7.1} as
  \[
  ...
  \]
  where we have used \labelcrefs{i:ac:7.1.2} once again.
  Thus, to finish the proof of \labelcrefs{i:eq:ac:7.1} we have to show that
  \[
    \prod_{i = 1}^n f(g(i)) = \prod_{i = 1}^n f(\tilde{h}(i)). \tag{i:ac:7.2}\label{i:eq:ac:7.2}
  \]
  But the function \(g\) (when restricted to \(\set{i \in \N : 1 \leq i \leq n}\)) is a bijection from \(\set{i \in \N : 1 \leq i \leq n} \to X - \set{x}\).
  The function \(\tilde{h}\) is also a bijection from \(\set{i \in \N : 1 \leq i \leq n} \to X - \set{x}\) (cf. \labelcrefs{i:3.6.9}).
  Since \(X - \set{x}\) has \(n\) elements (by \labelcrefs{i:3.6.9}), the claim \labelcrefs{i:eq:ac:7.2} then follows directly from the induction hypothesis \(P(n)\).
\end{proof}

\begin{ac}[Basic properties of product over finite sets]\label{i:ac:7.1.5}
  \begin{enumerate}
    \item\label{i:ac:7.1.5.a} If \(X\) is empty, and \(f : X \to \R\) is a function (i.e., \(f\) is the empty function), we have
    \[
      \prod_{x \in X} f(x) = 1.
    \]
    \item\label{i:ac:7.1.5.b} If \(X\) consists of a single element, \(X = \set{x_0}\), and \(f : X \to \R\) is a function, we have
    \[
      \prod_{x \in X} f(x) = f(x_0).
    \]
    \item\label{i:ac:7.1.5.c} (Substitution, part I) If \(X\) is a finite set, \(f : X \to \R\) is a function, and \(g : Y \to X\) is a bijection, then
    \[
      \prod_{x \in X} f(x) = \prod_{y \in Y} f(g(y)).
    \]
    \item\label{i:ac:7.1.5.d} (Substitution, part II) Let \(n \leq m\) be integers, and let \(X\) be the set \(X \coloneqq \set{i \in \Z : n \leq i \leq m}\).
    If \(a_i\) is a real number assigned to each integer \(i \in X\), then we have
    \[
      \prod_{i = n}^m a_i = \prod_{i \in X} a_i.
    \]
    \item\label{i:ac:7.1.5.e} Let \(X, Y\) be disjoint finite sets (so \(X \cap Y = \emptyset\)), and \(f : X \cup Y \to \R\) is a function.
    Then we have
    \[
      \prod_{z \in X \cup Y} f(z) = \Bigg(\prod_{x \in X} f(x)\Bigg) \times \Bigg(\prod_{y \in Y} f(y)\Bigg).
    \]
    \item\label{i:ac:7.1.5.f} Let \(X\) be a finite set, and let \(f : X \to \R\) and \(g : X \to \R\) be functions.
    Then
    \[
      \prod_{x \in X} (f(x) \times g(x)) = \prod_{x \in X} f(x) \times \prod_{x \in X} g(x).
    \]
    \item\label{i:ac:7.1.5.g} Let \(X\) be a finite set, let \(f : X \to \R\) be a function, and let \(c\) be a real number.
    Then
    \[
      \prod_{x \in X} cf(x) = c^{\#(X)} \prod_{x \in X} f(x).
    \]
    \item\label{i:ac:7.1.5.h} Let \(X\) be a finite set, and let \(f : X \to \R\) be a function, then
    \[
...
\begin{proof}
  Let \(n\) be the number of elements in \(X\).
  We will use induction on \(n\) (cf. \labelcrefs{i:ac:7.1.4});
  i.e., we let \(P(n)\) be the assertion that \labelcrefs{i:ac:7.1.6} is true for any set \(X\) with \(n\) elements, and any finite set \(Y\) and any function \(f : X \times Y \to \R\).
  We wish to prove \(P(n)\) for all natural numbers \(n\).

  The base case \(P(0)\) is easy, following from \labelcrefs{i:ac:7.1.5.a}.
  Now suppose that \(P(n)\) is true;
  we now show that \(P(n + 1)\) is true.
  Let \(X\) be a set with \(n + 1\) elements.
  In particular, by \labelcrefs{i:3.6.9}, we can write \(X = X' \cup \set{x_0}\), where \(x_0\) is an element of \(X\) and \(X' \coloneqq X - \set{x_0}\) has \(n\) elements.
  Then by \labelcrefs{i:ac:7.1.5.e} we have
  \[
...
\begin{proof}
  In light of \labelcrefs{i:ac:7.1.6}, it suffices to show that
  \[
    \prod_{(x, y) \in X \times Y} f(x, y) = \prod_{(y, x) \in Y \times X} f(x, y).
  \]
  But this follows from \labelcrefs{i:ac:7.1.5.c} by applying the bijection \(h : Y \times X \to X \times Y\) defined by \(h(y, x) \coloneqq (x, y)\).
\end{proof}

\exercisesection

\begin{ex}\label{i:ex:7.1.1}
  Prove \labelcrefs{i:7.1.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.1.4}.
\end{proof}

\begin{ex}\label{i:ex:7.1.2}
  Prove \labelcrefs{i:7.1.11}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:7.1.11}.
\end{proof}
...
\begin{proof}
  See \crefrange{i:ac:7.1.1}{i:ac:7.1.7}.
\end{proof}
