...
\begin{proof}
  Suppose for the sake of contradiction that the sets \(X\) and \(2^X\) had equal cardinality.
...
\begin{rmk}\label{i:8.3.2}
  The reader should compare the proof of Cantor's theorem with the statement of Russell's paradox (\labelcrefs{i:sec:3.2}).
...
\begin{proof}
  By \labelcrefs{i:8.3.1}, \(2^{\N}\) cannot have equal cardinality with \(\N\), hence is either uncountable or finite.
  ...
  Thus, \(2^{\N}\) cannot be finite (by \labelcrefs{i:3.6.14}), and is hence uncountable.
\end{proof}
...
\begin{proof}
  ...
  \]
  Observe that since \(\sum_{n = 0}^\infty 10^{-n}\) is an absolutely convergent series (by \labelcrefs{i:7.3.3}), the series \(\sum_{n \in A} 10^{-n}\) is also absolutely convergent (by \labelcrefs{i:8.2.6.c}).
  Thus, the map \(f\)  is well defined.
  We now claim that \(f\) is injective.
  Suppose for the sake of contradiction that there were two distinct sets \(A, B \in 2^{\N}\) such that \(f(A) = f(B)\).
  Since \(A \neq B\), the set \((A \setminus B) \cup (B \setminus A)\) is a non-empty subset of \(\N\).
  By the well-ordering principle (\labelcrefs{i:8.1.4}), we can then define the minimum of this set, say \(n_0 \coloneqq \min(A \setminus B) \cup (B \setminus A)\).
  ...
  \begin{align*}
    0 & = f(A) - f(B)                                                                                 \\
      ...
      & > 0,
  \end{align*}
  a contradiction, where we have used the geometric series lemma (\labelcrefs{i:7.3.3}) to sum
  \[
  ...
  Since \(f(2^{\N})\) is a subset of \(\R\), this forces \(\R\) to be uncountable also (otherwise this would contradict \labelcrefs{i:8.1.7}), and we are done.
\end{proof}

\setcounter{thm}{5}
\begin{rmk}\label{i:8.3.6}
  \labelcrefs{i:8.3.4} shows that the reals have strictly larger cardinality than the natural numbers (in the sense of \labelcrefs{i:ex:3.6.7}).
...
\begin{proof}
  ...
  Suppose for the sake of contradiction that \(D_{n + 1} \cap D_{n + k + 1} \neq \emptyset\).
  Then we have
  \begin{align*}
...
\begin{ex}\label{i:ex:8.3.3}
  Recall from \labelcrefs{i:ex:3.6.7} that a set \(A\) is said to have lesser or equal cardinality than a set \(B\) iff there is an injective map \(f : A \to B\) from \(A\) to \(B\).
  Using \labelcrefs{i:ex:8.3.2}, show that if \(A, B\) are sets such that \(A\) has lesser or equal cardinality to \(B\) and \(B\) has lesser or equal cardinality to \(A\), then \(A\) and \(B\) have equal cardinality.
...
\begin{proof}
  ...
  Then by \labelcrefs{i:ex:3.6.7} we know that \(\exists f : A \to B, g : B \to A\) such that both \(f, g\) are injective.
  By \labelcrefs{i:ex:3.3.2} we have \(f \circ g : B \to B\) is injective.
  By \labelcrefs{i:3.4.1} we know that \(f \circ g : B \to f\big(g(B)\big)\) is bijective and
  \[
  ...
  \end{align*}
  where \(n \in \N\).
  By \labelcrefs{i:ex:8.3.2} we know that \(\forall i, j \in \N\), \(i \neq j \implies D_i \cap D_j = \emptyset\).
  ...
  \[
    ...
    \end{dcases}
  \]
  By \labelcrefs{i:ex:8.3.2} we know that \(h\) is bijective.
  Thus, by \labelcrefs{i:3.6.1} we know \(f(A)\) and \(f\big(g(B)\big)\) have the same cardinality.
  Since \(f \circ g\) is bijective, we know that \(B, f\big(g(B)\big)\) have the same cardinality, thus by \labelcrefs{i:3.6.4} \(f(A), B\) have the same cardinality.
  But since \(f\) is injective, we know that \(f\) is bijective from \(A\) to \(f(A)\), which means \(A, f(A)\) have the same cardinality.
  Thus, by \labelcrefs{i:3.6.4} \(A, B\) have the same cardinality.
\end{proof}

\begin{ex}\label{i:ex:8.3.4}
  Let us say that a set \(A\) has \emph{strictly lesser cardinality} than a set \(B\) if \(A\) has lesser than or equal cardinality to \(B\) (in the sense of \labelcrefs{i:ex:3.6.7}) but \(A\) does not have equal cardinality to \(B\).
...
\begin{proof}
  ...
  By \labelcrefs{i:8.3.1} we know that \(g\) is not bijective.
  ...
  By \labelcrefs{i:ex:3.6.7} \(\exists f : A \to B, g : B \to C\) such that both \(f, g\) are injective.
  By \labelcrefs{i:ex:3.3.2} we know that \(g \circ f : A \to C\) is injective.
  ...
  Suppose for the sake of contradiction that \(A, C\) have the same cardinality.
  Then by \labelcrefs{i:3.6.1} \(\exists h : C \to A\) such that \(h\) is bijective.
  Since \(g\) is injective, by \labelcrefs{i:ex:3.3.2} we know that \(h \circ g : B \to A\) is injective.
  But \(f\) is also injective, by \labelcrefs{i:ex:8.3.3} we know that \(A, B\) have the same cardinality, a contradiction.
...
\begin{proof}
  Suppose for the sake of contradiction that there exists a set \(X\) such that \(2^X\) is countable.
  By \labelcrefs{i:ex:8.3.4} \(X\) has strictly lesser cardinality than \(2^X\).
  Since \(2^X\) is countable, by \labelcrefs{i:8.1.1} we know that \(X\) has strictly lesser cardinality than \(\N\) and \(X\) can not be countable.
  Since \(X\) has strictly lesser cardinality than \(\N\), by \labelcrefs{i:ex:3.6.7}, \(\exists f : X \to \N\) such that \(f\) is injective.
  Since \(f\) is injective, we know that \(f\) is bijective from \(X\) to \(f(X)\).
  Since \(f(X) \subseteq \N\), by \labelcrefs{i:8.1.6} \(f(X)\) is at most countable.
  Since \(X, f(X)\) have the same cardinality and \(X\) is not countable, \(X\) must be finite.
  But by \labelcrefs{i:ex:8.3.1} we know that \(X\) is finite implies \(2^X\) is finite, a contradiction.
