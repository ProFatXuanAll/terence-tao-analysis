\section{The axiom of choice}\label{i:sec:8.4}
...
\begin{defn}[Infinite Cartesian products]\label{i:8.4.1}
  ...
  \]
  where we recall (from \labelcrefs{i:3.10}) that \((\bigcup_{\alpha \in I} X_{\alpha})^I\) is the set of all functions \((x_{\alpha})_{\alpha \in I}\) which assign an element \(x_{\alpha} \in \bigcup_{\beta \in I} X_{\beta}\) to each \(\alpha \in I\).
...
\begin{eg}\label{i:8.4.2}
  ...
  If \(I\) is a set of the form \(I \coloneqq \set{i \in \N : 1 \leq i \leq n}\), then \(\prod_{\alpha \in I} X_{\alpha}\) is the same set as the set \(\prod_{1 \leq i \leq N} X_i\) defined in \labelcrefs{i:3.5.7}.
\end{eg}

\begin{note}
  Recall from \labelcrefs{i:3.5.12} that if \(X_1, \dots, X_n\) were any finite collection of non-empty sets, then the finite Cartesian product \(\prod_{1 \leq i \leq n} X_i\) was also non-empty.
...
\begin{rmk}\label{i:8.4.3}
  ...
  in some sense one is just applying \labelcrefs{i:3.1.6} over and over again.
  ...
  This problem is not unique to the axiom of choice - it already appears for instance in \labelcrefs{i:3.1.6} - but the objects shown to exist using the axiom of choice tend to be rather extreme in their level of non-constructiveness.
...
\begin{proof}
  ...
  Using the axiom of choice (\labelcrefs{i:8.1}, or the axiom of countable choice), we can then find a sequence \((a_n)_{n = 1}^\infty\) such that \(a_n \in X_n\) for all \(n \geq 1\).
  In particular, \(a_n \in E\) for all \(n\), and \(\sup(E) - 1 / n \leq a_n \leq \sup(E)\) for all \(n\).
  But then we have \(\lim_{n \to \infty} a_n = \sup(E)\) by the squeeze test (\labelcrefs{i:6.4.14}).
\end{proof}

\begin{rmk}\label{i:8.4.6}
  In many special cases, one can obtain the conclusion of \labelcrefs{i:8.4.5} without using the axiom of choice.
...
\begin{proof}
  We first show that axiom of choice (\labelcrefs{i:8.1}) implies \(\exists f : X \to Y\) such that \(P(x, f(x))\) is true for all \(x \in X\).
  Define
  \[
    Y_x \coloneqq \set{y \in Y : P(x, y) \text{ is true}}
  \]
  for each \(x \in X\).
  Such a set exist by \labelcrefs{i:3.5} and is non-empty by the hypothesis.
  By \labelcrefs{i:3.10}, we know that the set
  \[
    \prod_{x \in X} Y_x = \set{(y_x)_{x \in X} \in (\bigcup_{x \in X} Y_x)^X : y_x \in Y_x \text{ for all } x \in X}
  \]
  exists.
  By axiom of choice (\labelcrefs{i:8.1}) we also know that the set \(\prod_{x \in X} Y_x\) is non-empty.
  ...
  Thus, axiom of choice (\labelcrefs{i:8.1}) implies \(\exists f : X \to Y\) such that \(P(x, f(x))\) is true for all \(x \in X\).

  Now we show that if \(\forall x \in X\), \(\exists y \in Y\) such that \(P(x, y)\) is true and \(\exists f : X \to Y\) such that \(P(x, f(x))\) is true for all \(x \in X\), then axiom of choice (\labelcrefs{i:8.1}) is true.
  Using the definition of \(Y_x\) again we know that \(f(x) \in Y_x\), so \(Y_x \neq \emptyset\).
  By \labelcrefs{i:3.10} we can have a set \(\prod_{x \in X} Y_x\).
  We want to show that \(\prod_{x \in X} Y_x \neq \emptyset\).
  But this is true since \(f \in \prod_{x \in X} Y_x\).
  Thus, \labelcrefs{i:8.4.7} implies axiom of choice (\labelcrefs{i:8.1}).
\end{proof}

\exercisesection

\begin{ex}\label{i:ex:8.4.1}
  Show that the axiom of choice implies \labelcrefs{i:8.4.7}.
  Conversely, show that if \labelcrefs{i:8.4.7} is true, then the axiom of choice is also true.
\end{ex}

\begin{proof}
  See \labelcrefs{i:8.4.7}.
\end{proof}
...
\begin{proof}
  We first show that axiom of choice (\labelcrefs{i:8.1}) implies there exists a set \(Y\) such that \(\#(Y \cap X_{\alpha}) = 1\) for all \(\alpha \in I\).
  By \labelcrefs{i:8.1}, the set \(\prod_{\alpha \in I} X_{\alpha}\) is non-empty.
  ...
  We conclude that axiom of choice (\labelcrefs{i:8.1}) implies there exists a set \(Y\) such that \(\#(Y \cap X_{\alpha}) = 1\) for all \(\alpha \in I\).

  Now we show that if there exists a set \(Y\) such that \(\#(Y \cap X_{\alpha}) = 1\) for all \(\alpha \in I\), then axiom of choice (\labelcrefs{i:8.1}) is true.
  ...
  Thus, by \labelcrefs{i:3.6} we have a function \(f : I \to \bigcup_{\alpha \in I} X_{\alpha}\) such that \(f(\alpha) = x_{\alpha} \in X_{\alpha}\) for all \(\alpha \in I\).
  But this means \(f \in \prod_{\alpha \in I} X_{\alpha}\), so \(\prod_{\alpha \in I} X_{\alpha} \neq \emptyset\), and axiom of choice (\labelcrefs{i:8.1}) is true.
  Thus, if there exists a set \(Y\) such that \(\#(Y \cap X_{\alpha}) = 1\) for all \(\alpha \in I\), then axiom of choice (\labelcrefs{i:8.1}) is true.
\end{proof}

\begin{ex}\label{i:ex:8.4.3}
  ...
  in particular, \(A\) has lesser or equal cardinality to \(B\) in the sense of \labelcrefs{i:ex:3.6.7}.
  Compare this with \labelcrefs{i:ex:3.6.8}.
...
\begin{proof}
  ...
  By axiom of choice (\labelcrefs{i:8.1}), we know that the set \(\prod_{a \in A} g^{-1}(\set{a}) \neq \emptyset\).
  ...
  Otherwise by \labelcrefs{i:3.4.4} \(\exists b \in B\) such that \(g(b) = a_1 \land g(b) = a_2\), a contradiction.
  ...
  So by \labelcrefs{i:ex:8.4.2} we know that axiom of choice (\labelcrefs{i:8.1}) is true.
\end{proof}
