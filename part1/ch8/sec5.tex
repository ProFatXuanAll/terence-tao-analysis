...
\begin{eg}\label{i:8.5.2}
  The natural numbers \(\N\) together with the usual less-than-or-equal-to relation \(\leq\) (as defined in \labelcrefs{i:2.2.11}) forms a partially ordered set, by \labelcrefs{i:2.2.12}.
  ...
  Meanwhile, if \(X\) is any collection of sets, and one uses the relation of is-a-subset-of \(\subseteq\) (as defined in \labelcrefs{i:3.1.15}) for the ordering relation \(\leq_X\), then \(X\) is also partially ordered (\labelcrefs{i:3.1.18}).
...
\begin{eg}\label{i:8.5.4}
  ...
  (by \labelcrefs{i:2.2.13}, \labelcrefs{i:4.1.11}, \labelcrefs{i:4.2.9}, \labelcrefs{i:5.4.7}, and \labelcrefs{i:6.2.5} respectively).
...
\begin{eg}\label{i:8.5.9}
  The natural numbers \(\N\) are well-ordered by \labelcrefs{i:8.1.4}.
  However, the integers \(\Z\), the rationals \(\Q\), and the real numbers \(\R\) are not (see \labelcrefs{i:ex:8.1.2}).
...
\begin{proof}
  Since \((X, \leq_X)\) is well-ordered, by \labelcrefs{i:8.5.8} we know that \(X \neq \emptyset\) and \(\min\big((X, \leq_X)\big)\) exists.
  By \labelcrefs{i:8.5.5} we know that the statement ``\(\forall m \in X\), \(m <_X \min\big((X, \leq_X)\big) \implies P(m)\) is true'' is vacuously true since there is no \(m <_X n\).
  ...
  Since \(X\) is well-ordered and \(Y \subseteq X\), by \labelcrefs{i:8.5.8} we know that \(\min\big((Y, \leq_X)\big)\) exists.
  ...
  \]
  is not empty.
  Also by \labelcrefs{i:8.5.5} we know that \(\forall m \in Y'\), \(P(m)\) is true, otherwise contradict to the definition of \(\min\big((Y, \leq_X)\big)\).
  But this means
  \[
...
\begin{rmk}\label{i:8.5.11}
  It may seem strange that there is no ``base'' case in strong induction, corresponding to the hypothesis \(P(0)\) in \labelcrefs{i:2.5}.
...
\begin{proof}
  ...
  Using the axiom of choice (in the form of \labelcrefs{i:8.4.7}), we can thus assign a strict upper bound \(s(Y) \in X\) to each well-ordered subset \(Y\) of \(X\) which has \(x_0\) as its minimal element.
  ...
  But then \(Y_{\infty} \cup \set{s(Y_{\infty})}\) is well-ordered (by \labelcrefs{i:ex:8.5.11}) and has \(x_0\) as its minimal element.
...
\begin{proof}
  ...
  By \labelcrefs{i:8.5.12}, we know that \(s \in X\) and \(\forall y \in Y \implies y \leq s\).
  Since \(X\) has no maximal element, we know \(\exists s' \in X\) such that \(s < s'\), otherwise by \labelcrefs{i:8.5.5} we have \(s = \max((X, \leq))\), a contradiction.
  Since \(\forall y \in Y\), \(y < s'\), we know that \(s' \notin Y\), and thus by \labelcrefs{i:8.5.12} \(s'\) is a strict upper bound of \(Y\).

  Since \(X \neq \emptyset\), let \(x_0 \in X\).
  By \labelcrefs{i:8.5.14}, \(\exists Y \subseteq X\) such that \((Y, \leq)\) is well-ordered, \(\min\big((Y, \leq)\big) = x_0\) and \(Y\) has no strict upper bound.
...
\begin{proof}
  ...
  Since \((\emptyset, \leq_{\emptyset})\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} \((\emptyset, \leq_{\emptyset})\) is partially ordered.
  Since
  \[
    \forall x, y \in \emptyset, (x \leq_{\emptyset} y) \lor (y \leq_{\emptyset} x)
  \]
  is vacuously true, by \labelcrefs{i:8.5.3} we know that \((\emptyset, \leq_{\emptyset})\) is totally ordered.
  Since
  \[
    \forall X \subseteq \emptyset, X \neq \emptyset \implies \exists \min\big((X, \leq_{\emptyset})\big) \in X
  \]
  is vacuously true, by \labelcrefs{i:8.5.8} we know that \((\emptyset, \leq_{\emptyset})\) is well-ordered.
\end{proof}
...
\begin{proof}
  ...
  Since \((X, |)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} \((X, |)\) is partially ordered.
  Since \((2 | 3) \lor (3 | 2)\) is false, by \labelcrefs{i:8.5.3} \((X, |)\) is not totally ordered.
\end{proof}
...
\begin{proof}
  ...
  Since \((X, \leq_X)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} \((X, \leq_X)\) is partially ordered.
  ...
  Thus, by \labelcrefs{i:8.5.3} \((X, \leq_X)\) is not totally ordered.
  ...
  In either case, we get exactly one of \((x \leq_X x') \lor (x' \leq_X x)\) is true.
  Thus, by \labelcrefs{i:8.5.3} \((X, \leq_X)\) is totally ordered.
\end{proof}
...
\begin{proof}
  ...
  Since \((X, \leq_X)\) is partially ordered, by \labelcrefs{i:8.5.1} we know that
  \begin{align*}
...
\begin{proof}
  ...
  Then by \labelcrefs{i:8.5.5} we have \((y \leq_X y') \land (y' \leq_X y)\), so \(y = y'\), a contradiction.
...
\begin{ex}\label{i:ex:8.5.10}
  Prove \labelcrefs{i:8.5.10}, without using the axiom of choice.
\end{ex}

\begin{proof}
  See \labelcrefs{i:8.5.10}.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{i:8.5.8} we know that if \((Y \cup Y', \leq_X)\) is well-ordered, then \((Y \cup Y', \leq_X)\) is totally ordered.
  ...
  Since \(Z_Y \subseteq Y\) and \(Z_{Y'} \subseteq Y'\), by \labelcrefs{i:8.5.8} \(z_Y = \min\big((Z_Y, \leq_X)\big)\) and \(z_{Y'} = \min\big((Z_{Y'}, \leq_X)\big)\) exist.
  ...
  Thus, by \labelcrefs{i:8.5.8} \((Y \cup Y', \leq_X)\) is well-ordered.
\end{proof}
...
\begin{proof}
  ...
  If \(X = \emptyset \lor Y = \emptyset\), then by \labelcrefs{i:ex:3.5.8} \(X \times Y = \emptyset\) and by \labelcrefs{i:ex:8.5.1} we know that \(\emptyset\) is partially ordered.
  So suppose that \(X \neq \emptyset \land Y \neq \emptyset\).
  Since
  \[
  ...
  Since \((X \times Y, \leq_{X \times Y})\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} \((X \times Y, \leq_{X \times Y})\) is partially ordered.

  Now we show that if \((X, \leq_X), (Y, \leq_Y)\) are totally ordered, then \((X \times Y, \leq_{X \times Y})\) is totally ordered.
  If \(X = \emptyset \lor Y = \emptyset\), then by \labelcrefs{i:ex:3.5.8} \(X \times Y = \emptyset\) and by \labelcrefs{i:ex:8.5.1} we know that \(\emptyset\) is totally ordered.
  So suppose that \(X \neq \emptyset \land Y \neq \emptyset\).
  ...
  If \(X = \emptyset \lor Y = \emptyset\), then by \labelcrefs{i:ex:3.5.8} \(X \times Y = \emptyset\) and by \labelcrefs{i:ex:8.5.1} we know that \((\emptyset, \leq_{X \times Y})\) is well-ordered.
  So suppose that \(X \neq \emptyset \land Y \neq \emptyset\).
  ...
  Since \((X, \leq_X)\) is well-ordered and \(Z_X \subseteq X\), by \labelcrefs{i:8.5.8} we know that \(z_x = \min\big((Z_X, \leq_X)\big)\) exists.
  Since \(z_x \in Z_X\), we know that the set \(Z_Y = \set{y \in Y : (z_x, y) \in Z} \neq \emptyset\).
  Since \((Y, \leq_Y)\) is well-ordered and \(Z_Y \subseteq Y\), by \labelcrefs{i:8.5.8} we know that \(z_y = \min\big((Z_Y, \leq_Y)\big)\) exists.
  Then we have
  \begin{align*}
             & \forall (x, y) \in Z                                                                        \\
  ...
  Since \(Z\) was arbitrary, by \labelcrefs{i:8.5.8} we know that \((X \times Y, \leq_{X \times Y})\) is well-ordered.
\end{proof}

\begin{ex}\label{i:ex:8.5.13}
  Prove the claim in the proof of \labelcrefs{i:8.5.14}, namely that every element of \(Y' \setminus Y\) is an strict upper bound for \(Y\) and vice versa.
\end{ex}
...
\begin{proof}
  ...
  Note that if we can show that \(Q(n)\) is true for every \(n \in Y \cap Y'\), then by principle of strong induction (\labelcrefs{i:8.5.10}) we know that \(P(n)\) is true for every \(n \in Y \cap Y'\).
  ...
  \]
  Such \(n\) is well-defined since in the proof of \labelcrefs{i:8.5.14} we suppose for the sake of contradiction that every well-ordered subset \(Y\) of \(X\) which has \(x_0\) as its minimal element has at least one strict upper bound.
  ...
  By principle of strong induction (\labelcrefs{i:8.5.10}) we know that \(P(n)\) is true for all \(n \in Y \cap Y'\).
  Thus, we have
  \begin{align*}
  ...
  \begin{itemize}
    ...
    \item If \(Y \setminus Y' \neq \emptyset\), then \(Y' \setminus Y = \emptyset\) and \(Y' \subseteq Y\).
          Since \(Y \cap Y' = Y'\) and \(s(Y \cap Y') = s(Y') = \min(Y \setminus Y')\), by \labelcrefs{i:8.5.5} we know that every element of \(Y \setminus Y'\) is an strict upper bound of \(Y'\).
  \end{itemize}
...
\begin{ex}\label{i:ex:8.5.14}
  Use \labelcrefs{i:8.5.14} to prove \labelcrefs{i:8.5.15}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:8.5.15}
\end{proof}
...
\begin{ex}\label{i:ex:8.5.15}
  ...
  This exercise (combined with \labelcrefs{i:ex:8.3.3}) shows that the cardinality of any two sets is comparable, as long as one assumes the axiom of choice.
\end{ex}
...
\begin{proof}
  ...
  Since \((S, \leq_S)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} we know that \((S, \leq_S)\) is partially ordered.
  ...
  By axiom of choice (\labelcrefs{i:8.1}) we know that \(\prod_{X \in S'} F_X \neq \emptyset\).
  ...
  By Zorn's lemma (\labelcrefs{i:8.5.15}) we know that there exists at least one maximal element of \(S\).
  ...
  By hypothesis we know that we cannot have a injection from \(A\) to \(B\) (this is the definition of \(A\) having lesser or equal cardinality than \(B\), see \labelcrefs{i:ex:3.6.7}).
  Thus, we derived a contradiction.
  So \(B = \max\big((S, \leq_S)\big) \in S\) and \(\exists f_B : B \to A\) such that \(f_B\) is injective.
  By \labelcrefs{i:ex:8.3.3} \(B\) has lesser or equal cardinality than \(A\).
\end{proof}

\begin{ex}\label{i:ex:8.5.16}
  Let \(X\) be a set, and let \(P\) be the set of all partial orderings of \(X\).
  (For instance, if \(X \coloneqq \N \setminus \set{0}\), then both the usual partial ordering \(\leq\), and the partial ordering in \labelcrefs{i:ex:8.5.3}, are elements of \(P\).)
  ...
  Thus, for instance the partial ordering in \labelcrefs{i:ex:8.5.3} is coarser than the usual ordering \(\leq\).
  ...
  Using Zorn's lemma (\labelcrefs{i:8.5.15}), show that given any partial ordering \(\leq\) of \(X\) there exists a total ordering \(\leq'\) such that \(\leq\) is coarser than \(\leq'\).
\end{ex}
...
\begin{proof}
  ...
  Since \((P, \preceq)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} we know that \((P, \preceq)\) is partially ordered.
  ...
  Thus, by \labelcrefs{i:8.5.1} \((X, =_X)\) is partially ordered and \((X, =_X) \in P\).
  Since
  \begin{align*}
             & \forall (X, \leq) \in P                                           \\
  ...
  \end{align*}
  by \labelcrefs{i:8.5.5} we have \((X, =_X) = \min\big((P, \preceq)\big)\).
  ...
  Since \((X, \leq_S)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} we know that \((X, \leq_S)\) is partially ordered.
  Thus, \((X, \leq_S) \in P\).
  ...
  If \(S = \emptyset\), by \labelcrefs{i:ex:8.5.1} we know that \((\emptyset, \preceq)\) is totally ordered, and the statement
  \[
  ...
  By Zorn's lemma (\labelcrefs{i:8.5.15}) we know that \((X, \leq_{\max}) = \max\big((P, \preceq)\big)\) exists.
  ...
  Since \((X, \leq_p)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} we know that \((X, \leq_p)\) is partially ordered.
  Thus, \((X, \leq_p) \in P\).
...
\begin{ex}\label{i:ex:8.5.17}
  Use Zorn's lemma (\labelcrefs{i:8.5.15}) to give another proof of the claim in \labelcrefs{i:ex:8.4.2}.
  Deduce that Zorn's lemma (\labelcrefs{i:8.5.15}) and the axiom of choice (\labelcrefs{i:8.1}) are in fact logically equivalent
...
\begin{proof}
  ...
  By \labelcrefs{i:8.5.2} we know that \((\Omega, \subseteq)\) is partially ordered.
  ...
  \]
  by \labelcrefs{i:8.5.12} we know that \(\bigcup S\) is an upper bound of \(S\) (under the relation \(\subseteq\)).
  Thus, we conclude that every totally ordered subset of \(\Omega\) has an upper bound.
  By Zorn's lemma (\labelcrefs{i:8.5.15}) we know that \(M = \max\big((\Omega, \subseteq)\big)\) exists.
  ...
  By \labelcrefs{i:ex:8.4.2} this means Zorn's lemma and the axiom of choice are in fact logically equivalent.
\end{proof}

\begin{ex}\label{i:ex:8.5.18}
  Using Zorn's lemma (\labelcrefs{i:8.5.15}), prove \emph{Hausdorff's maximality principle}:
  ...
  Conversely, show that if Hausdorff's maximality principle is true, then Zorn's lemma (\labelcrefs{i:8.5.15}) is true.
  Thus, by \labelcrefs{i:ex:8.5.17}, these two statements are logically equivalent to the axiom of choice.
\end{ex}

\begin{proof}
  We first show that Zorn's lemma (\labelcrefs{i:8.5.15}) implies Hausdorff's maximality principle.
  Suppose that \((X, \leq)\) is partially ordered.
  Let \(X_T = \set{Y \subseteq X : (Y, \leq) \text{ is totally ordered}}\).
  By \labelcrefs{i:ex:8.5.1} we know that \((\emptyset, \leq)\) is totally ordered, thus \(\emptyset \in X_T\) and \(X_T \neq \emptyset\).
  By \labelcrefs{i:8.5.2} we know that \((X_T, \subseteq)\) is partially ordered.
  ...
  Since \((\bigcup S, \leq)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} we know that \((\bigcup S, \leq)\) is partially ordered.
  Since
  \begin{align*}
  ...
  \end{align*}
  by \labelcrefs{i:8.5.3} we know that \((\bigcup S, \leq)\) is totally ordered.
  ...
  By Zorn's lemma (\labelcrefs{i:8.5.15}) we know that \(\max\big((X_T, \subseteq)\big)\) exists.
  By \labelcrefs{i:8.5.5} we know that
  \[
  ...
  If such strict upper bound \(s(Y)\) does not exists, then by \labelcrefs{i:8.5.5} we must have \(u(Y) = \max\big((X, \leq)\big)\), a contradiction.
...
\begin{ex}\label{i:ex:8.5.19}
  ...
  Using Zorn's lemma (\labelcrefs{i:8.5.15}), conclude the \emph{well-ordering principle}:
  every set \(X\) has at least one well-ordering.
  Conversely, use the well-ordering principle to prove the axiom of choice, \labelcrefs{i:8.1}.
  We thus see that the axiom of choice, Zorn's lemma (\labelcrefs{i:8.5.15}), and the well-ordering principle are all logically equivalent to each other.
\end{ex}
...
\begin{proof}
  ...
  Since \((\Omega, \preceq)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} we know that \((\Omega, \preceq)\) is partially ordered.

  Next we show that \(\min\big((\Omega, \preceq)\big)\) exists.
  We claim that \((\emptyset, \leq_{\emptyset}) = \min\big((\Omega, \preceq)\big)\).
  By \labelcrefs{i:ex:8.5.1} we know that \((\emptyset, \leq_{\emptyset})\) is well-ordered.
  Let \((Y, \leq) \in \Omega\).
  Now we split into two cases:
  \begin{itemize}
    ...
    \item If \(Y \neq \emptyset\), then by \labelcrefs{i:8.5.8} we know that \(y_{\min} = \min\big((Y, \leq)\big)\) exists.
          By \labelcrefs{i:8.5.5} we know that
          \[
  ...
  By \labelcrefs{i:ex:8.5.1} we know that \((\emptyset, \leq_{\emptyset})\) is well-ordered, thus \((\emptyset, \leq_{\emptyset}) \in \Omega\).
  So suppose that \(S \neq \emptyset \land S \neq \set{\emptyset}\).
  Observe that
  \begin{align*}
  ...
  Since \((Y_S, \leq_S)\) is well-ordered and \(\bigcup S \subseteq Y_S\), by \labelcrefs{i:8.5.8} \((\bigcup S, \leq_S) \in \Omega\).
  ...
  By Zorn's lemma (\labelcrefs{i:8.5.15}) we know that \((Y_{\max}, \leq_{\max}) = \max\big((\Omega, \preceq)\big)\) exists.
  ...
  Since \((\set{y}, =)\) is reflexive, anti-symmetric and transitive, by \labelcrefs{i:8.5.1} we know that \((\set{y}, =)\) is partially ordered.
  Since
  \[
    \forall y_1, y_2 \in \set{y}, y_1 = y_2 = y,
  \]
  by \labelcrefs{i:8.5.3} we know that \((\set{y}, =)\) is totally ordered.
  Since every non-emptyset subset of \(\set{y} = \set{y}\), by \labelcrefs{i:8.5.5} \(\min\big((\set{y}, =)\big) = y\) and by \labelcrefs{i:8.5.8} \((\set{y}, =)\) is well-ordered.
  ...
  Finally we use well-ordering principle to prove axiom of choice (\labelcrefs{i:8.1}).
  ...
  Since \(X_{\beta} \subseteq \bigcup_{\alpha \in I} X_{\alpha}\) for every \(\beta \in I\), by \labelcrefs{i:8.5.8} we know that \(\min\big((X_{\beta}, \leq)\big)\) exists.
...
\begin{ex}\label{i:ex:8.5.20}
  ...
  Conversely, if the above claim is true, show that it implies the claim in \labelcrefs{i:ex:8.4.2}, and thus this is yet another claim which is logically equivalent to the axiom of choice.
\end{ex}
...
\begin{proof}
  ...
  \]
  By \labelcrefs{i:8.5.2} we know that \((S, \subseteq)\) is partially ordered.
  ...
  By Zorn's lemma (\labelcrefs{i:8.5.15}) we know that \(\Omega' = \max\big((S, \subseteq)\big)\) exists.
  ...
  \]
  then \labelcrefs{i:ex:8.4.2} is true.
  ...
  \end{align*}
  we conclude that \labelcrefs{i:ex:8.4.2} is true.
\end{proof}
