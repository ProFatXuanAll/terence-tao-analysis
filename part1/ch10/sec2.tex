...
\begin{rmk}\label{i:10.2.2}
  If \(f\) attains a maximum at \(x_0\), we sometimes say that \(f\) attains a \emph{global} maximum at \(x_0\), in order to distinguish it from the local maxima defined in \labelcrefs{i:10.2.1}.
...
\begin{proof}
  ...
  Then by \labelcrefs{i:10.2.1} we know that
  \[
    \exists \delta \in \R^+ : \forall x \in (a, b) \cap (x_0 - \delta, x_0 + \delta), f(x) \leq f(x_0).
  \]
  Since \(f\) is differentiable at \(x_0\), by \labelcrefs{i:10.1.1} we know that
  \[
    \lim_{x \to x_0 ; x \in (a, b) \setminus \set{x_0}} \dfrac{f(x) - f(x_0)}{x - x_0} = f'(x_0)
  \]
  By \labelcrefs{i:9.3.6} we know that \(\forall \varepsilon \in \R^+\), \(\exists \delta' \in \R^+\) such that
  \[
  ...
  \]
  Thus, by \labelcrefs{i:9.3.6} we must have
  \[
...
\begin{note}
  \(f\) must be differentiable for \labelcrefs{i:10.2.6} to work.
  Also, \labelcrefs{i:10.2.6} does not work if the open interval \((a, b)\) is replaced by a closed interval \([a, b]\).
...
\begin{proof}
  Since \(g\) is continuous on \([a, b]\), by \labelcrefs{i:9.6.7} \(g\) attains its maximum at some point \(x_{\max} \in [a, b]\), and also attains its minimum at some point \(x_{\min} \in [a, b]\).
  If \((x_{\min} \in \set{a, b}) \land (x_{\max} \in \set{a, b})\) is true, then by \labelcrefs{i:9.6.5} we have \(g(x) = g(a) = g(b)\) for every \(x \in [a, b]\), and by \labelcrefs{i:10.1.13.a} we know that \(g'(x) = 0\).
  So suppose that at least one of \(x_{\min}, x_{\max} \notin \set{a, b}\), i.e., \(\big(x_{\min} \in (a, b)\big) \lor \big(x_{\max} \in (a, b)\big)\) is true.
  If \(x_{\min} \in (a, b)\), then by \labelcrefs{i:10.2.6} we know that \(f'(x_{\min}) = 0\).
  Similarly, if \(x_{\max} \in (a, b)\), then by \labelcrefs{i:10.2.6} we know that \(f'(x_{\max}) = 0\).
  Thus, there exists an \(x \in (a, b)\) such that \(g'(x) = 0\).
\end{proof}

\begin{rmk}\label{i:10.2.8}
  We only assume \(f\) is differentiable on the open interval \((a, b)\), though of course \labelcrefs{i:10.2.7} also holds if we assume \(f\) is differentiable on the closed interval \([a, b]\), since this is larger than \((a, b)\).
\end{rmk}
...
\begin{proof}
  ...
  \]
  we have \(g(a) = g(b)\) and by \labelcrefs{i:10.2.7} \(\exists x_0 \in (a, b)\) such that \(g'(x_0) = 0\).
  Thus
  \begin{align*}
             & g'(x_0) = 0                              \\
...
\begin{ex}\label{i:ex:10.2.1}
  Prove \labelcrefs{i:10.2.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:10.2.6}.
\end{proof}
...
\begin{ex}\label{i:ex:10.2.2}
  ...
  Explain why this does not contradict \labelcrefs{i:10.2.6}.
\end{ex}

\begin{proof}
  Let \(f(x) = -\abs{x}\).
  Then \(f\) is continuous and attains a global maximum at \(0\), but which is not differentiable at \(0\).
  The fact that \(f\) is not differentiable at \(0\) does not contradict to \labelcrefs{i:10.2.6}.
\end{proof}
...
\begin{proof}
  Let \(f(x) = x^3\).
  Then by \labelcrefs{i:ex:10.1.5} we know that \(f'(x) = 3x^2\).
  ...
  This does not contradict to \labelcrefs{i:10.2.6} since \(0\) is not given to be a local minimum or local maximum.
\end{proof}

\begin{ex}\label{i:ex:10.2.4}
  Prove \labelcrefs{i:10.2.7}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:10.2.7}.
\end{proof}

\begin{ex}\label{i:ex:10.2.5}
  Use \labelcrefs{i:10.2.7} to prove \labelcrefs{i:10.2.9}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:10.2.9}.
\end{proof}
...
\begin{proof}
  ...
  By \labelcrefs{i:ex:9.4.6} we know that \(f|_{[x, y]}\) is continuous on \([x, y]\).
  By \labelcrefs{i:ex:10.1.1} we know that \(f|_{[x, y]}\) is differentiable on \((x, y)\).
  By mean value theorem (\labelcrefs{i:10.2.9}) we know that \(\exists c \in (x, y)\) such that
  \[
...
\begin{proof}
  Since \(f'\) is bounded, by \labelcrefs{i:9.6.1} we know that \(\exists M \in \R^+\) such that \(\abs{f'(x)} \leq M\) for every \(x \in \R\).
  By \labelcrefs{i:ex:10.2.6} we know that \(\abs{f(x) - f(y)} \leq M \abs{x - y}\) for every \(x, y \in \R\).
  Then we have
  \begin{align*}
  ...
  \end{align*}
  and by \labelcrefs{i:9.9.2} \(f\) is uniformly continuous.
\end{proof}
