...
\begin{proof}
  ...
  \end{align*}
  by \labelcrefs{i:9.3.14} we have \(f'(x_0) \geq 0\).
  ...
  \end{align*}
  by \labelcrefs{i:9.3.14} we have \(f'(x_0) \leq 0\).
\end{proof}
...
\begin{proof}
  ...
  Since \(f\) is differentiable on \([a, b]\), by \labelcrefs{i:ex:10.1.1} we know that \(f\) is differentiable on \((x_1, x_2)\), and by \labelcrefs{i:10.1.12} \(f\) is continuous on \([x_1, x_2]\).
  By mean value theorem (\labelcrefs{i:10.2.9}) \(\exists c \in (x_1, x_2)\) such that \(f'(c) = \dfrac{f(x_2) - f(x_1)}{x_2 - x_1}\).
  ...
  \begin{itemize}
    \item If \(f'(x) > 0\) for all \(x \in [a, b]\), then we have
          \begin{align*}
            ...
            \implies & f(x_2) > f(x_1).
          \end{align*}
          Since \(x_1, x_2\) was arbitrary, by \labelcrefs{i:9.8.1} we conclude that \(f\) is strictly monotone increasing.
    \item If \(f'(x) < 0\) for all \(x \in [a, b]\), then we have
          \begin{align*}
                     & f'(c) < 0                                            \\
            \implies & \dfrac{f(x_2) - f(x_1)}{x_2 - x_1} < 0               \\
            ...
            \implies & f(x_2) < f(x_1).
          \end{align*}
          Since \(x_1, x_2\) was arbitrary, by \labelcrefs{i:9.8.1} we conclude that \(f\) is strictly monotone decreasing.
    \item If \(f'(x) = 0\) for all \(x \in [a, b]\), then we have
          \begin{align*}
                     & f'(c) = 0                                            \\
            \implies & \dfrac{f(x_2) - f(x_1)}{x_2 - x_1} = 0               \\
...
\begin{ex}\label{i:ex:10.3.1}
  Prove \labelcrefs{i:10.3.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:10.3.1}.
\end{proof}
...
\begin{ex}\label{i:ex:10.3.2}
  ...
  Explain why this does not contradict \labelcrefs{i:10.3.1}.
\end{ex}
...
\begin{proof}
  ...
  Since \(f(0+) \neq f(0-)\), by \labelcrefs{i:ac:9.5.1} \(f\) is not continuous at \(0\), and by \labelcrefs{i:10.1.10} \(f\) is not differentiable at \(0\).
  This does not contradict to \labelcrefs{i:10.3.1} since \(0\) is not given to be differentiable.
\end{proof}
...
\begin{ex}\label{i:ex:10.3.3}
  ...
  Explain why this does not contradict \labelcrefs{i:10.3.1} or \labelcrefs{i:10.3.3}.
\end{ex}

\begin{proof}
  Let \(f(x) = x^3\).
  By \labelcrefs{i:ex:10.1.5} \(f\) is differentiable on \(\R\) and \(f'(x) = 3x^2\), thus \(f'(0) = 0\).
  If \(x, y \in \R\) and \(x < y\), then \(x^3 < y^3\), thus by \labelcrefs{i:9.8.1} \(f\) is strictly monotone increasing.
  This does not contradict to \labelcrefs{i:10.3.1} since \(f'(0) = 0 \geq 0\).
  This does not contradict to \labelcrefs{i:10.3.3} since \(\forall x \in \R\), \(3x^2 \geq 0\).
\end{proof}

\begin{ex}\label{i:ex:10.3.4}
  Prove \labelcrefs{i:10.3.3}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:10.3.3}.
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    \end{dcases}
  \]
  By \labelcrefs{i:ex:10.1.5} we know that \(x\) and \(2x\) are differentiable and
  \[
    \forall x \in X, f'(x) = \begin{dcases}
      2 & \text{if } x \in [0, 0.5], \\
      1 & \text{if } x \in [1, 2].
  ...
  Since \(0.5 < 1\) and \(f(0.5) = f(1) = 1\), by \labelcrefs{i:9.8.1} \(f\) is not strictly monotone increasing.
\end{proof}
