\section{Inverse functions and derivatives}\label{i:sec:10.4}
...
\begin{proof}
  From the chain rule (\labelcrefs{i:10.1.15}) we have
  \[
    (f^{-1} \circ f)'(x_0) = (f^{-1})'(y_0) f'(x_0).
  \]
  But \(f^{-1} \circ f\) is the identity function on \(X\), and hence by \labelcrefs{i:10.1.13.b} \((f^{-1} \circ f)'(x_0) = 1\).
  The claim follows.
\end{proof}

\begin{note}
  As a particular corollary of \labelcrefs{i:10.4.1}, we see that if \(f\) is differentiable at \(x_0\) with \(f'(x_0) = 0\), then \(f^{-1}\) cannot be differentiable at \(y_0 = f(x_0)\), since \(1 / f'(x_0)\) is undefined in that case.
\end{note}

\begin{note}
  If one writes \(y = f(x)\), so that \(x = f^{-1}(y)\), then one can write the conclusion of \labelcrefs{i:10.4.1} in the more appealing form \(dx / dy = 1 / (dy / dx)\).
...
\begin{note}
  \labelcrefs{i:10.4.1} seems to answer the question of how to differentiate the inverse of a function, however it has one significant drawback:
  the lemma only works if one assumes a \emph{priori} that \(f^{-1}\) is differentiable.
  Thus, if one does not already know that \(f^{-1}\) is differentiable, one cannot use \labelcrefs{i:10.4.1} to compute the derivative of \(f^{-1}\).
\end{note}

\begin{thm}[Inverse function theorem]\label{i:10.4.2}
  Let \(f : X \to Y\) be an invertible function, with inverse \(f^{-1} : Y \to X\).
...
\begin{proof}
  ...
  \]
  By \labelcrefs{i:9.3.9}, it suffices to show that
  \[
  ...
  Thus, since \(f\) is differentiable at \(x_0\), we have (by \labelcrefs{i:9.3.9} again)
  \[
...
\begin{ex}\label{i:ex:10.4.1}
  ...
  \begin{enumerate}
    \item\label{i:ex:10.4.1.a} Show that \(g\) is continuous on \((0, \infty)\).
    \item\label{i:ex:10.4.1.b} Show that \(g\) is differentiable on \((0, \infty)\), and that \(g'(x) = \dfrac{1}{n} x^{\dfrac{1}{n} - 1}\) for all \(x \in (0, \infty)\).
  \end{enumerate}
\end{ex}
...
\begin{proof}
  ...
  By \labelcrefs{i:ex:10.1.5} we know that \(x^n\) is differentiable on \((0, \infty)\), thus by \labelcrefs{i:10.1.12} \(f\) is continuous on \((0, \infty)\).
  By \labelcrefs{i:9.8.4} \(f\) is strictly monotone increasing.
  Then by \labelcrefs{i:9.8.3} we know that \(g\) is also continuous and strictly monotone increasing.
...
\begin{ex}\label{i:ex:10.4.2}
  ...
  \begin{enumerate}
    \item\label{i:ex:10.4.2.a} Show that \(f\) is differentiable on \((0, \infty)\) and that \(f'(x) = q x^{q - 1}\).
    \item\label{i:ex:10.4.2.b} Show that \(\lim_{x \to 1 ; x \in (0, \infty) \setminus \set{1}} \dfrac{x^q - 1}{x - 1} = q\) for every rational number \(q\).
  \end{enumerate}
\end{ex}
...
\begin{proof}
  ...
  \[
    x^q = x^{a / b} = (x^a)^{1 / b}.
  \]
  By \labelcrefs{i:ex:10.1.5} and \labelcrefs{i:ex:10.1.6} we know that \(x^a\) is differentiable on \((0, \infty)\).
  By \labelcrefs{i:ex:10.4.1.b} we know that \(x^{1 / b}\) is differentiable on \((0, \infty)\).
  Thus, by chain rule (\labelcrefs{i:10.1.15}) we know that \(x^q = x^{a / b}\) is differentiable and
  \begin{align*}
    (x^q)' & = (x^{a / b})'                                                              \\
  ...
  Thus, by \labelcrefs{i:10.1.1} we have
  \[
...
\begin{proof}
  ...
  By \labelcrefs{i:6.7.2} we know that \(1^{\alpha} = 1\).
  Then we have
  \begin{align*}
  ...
  Let \(x_0 \in (0, \infty)\).
  By \labelcrefs{i:10.1.13.b,i:10.1.13.e} we know that \(g(x) = x / x_0\) is differentiable at \(x_0\).
  Then by chain rule (\labelcrefs{i:10.1.15}) we know that \(f \circ g\) is differentiable at \(x_0\).
  Thus, we have
  \begin{align*}
    (f \circ g)'(x_0) & = f'(1) g'(x_0)                                                                                                                           &  & \by{i:10.1.15}               \\
                      & = \dfrac{\alpha}{x_0}                                                                                                                     &  & \by{i:10.1.13.b,i:10.1.13.e} \\
  ...
  \]
  By \labelcrefs{i:10.1.1} \(f\) is differentiable at \(x_0\).
