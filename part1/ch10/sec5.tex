\section{L'Hôpital's rule}\label{i:sec:10.5}

\begin{prop}[L'Hôpital's rule I]\label{i:10.5.1}
...
\begin{proof}
  Since \(g\) is differentiable at \(x_0\), by Newton's approximation (\labelcrefs{i:10.1.7}) we have \(\forall \varepsilon \in \R^+\), \(\exists \delta \in \R^+\) such that
  \begin{align*}
...
\begin{prop}[L'Hôpital's rule II]\label{i:10.5.2}
...
\begin{proof}
  ...
  But since \(g(a)\) is also zero, we can apply Rolle's theorem (\labelcrefs{i:10.2.7}) to obtain \(g'(y) = 0\) for some \(a < y < x\), but this contradicts the hypothesis that \(g'\) is non-zero on \([a, b]\).

  Now we show that \(\lim_{x \to a ; x \in (a, b]} \dfrac{f(x)}{g(x)} = L\).
  By \labelcrefs{i:9.3.9}, it will suffice to show that
  \[
  ...
  By Rolle's theorem (\labelcrefs{i:10.2.7}), we can thus find \(y_n \in (a, x_n)\) such that \(h_n'(y_n) = 0\), which implies that
  \[
    \dfrac{f(x_n)}{g(x_n)} = \dfrac{f'(y_n)}{g'(y_n)}.
  \]
  Since \(y_n \in (a, x_n)\) for all \(n\), and \(x_n\) converges to \(a\) as \(n \to \infty\), we see from the squeeze test (\labelcrefs{i:6.4.14}) that \(y_n\) also converges to \(a\) as \(n \to \infty\).
  Thus, \(\dfrac{f'(y_n)}{g'(y_n)}\) converges to \(L\), and thus \(\dfrac{f(x_n)}{g(x_n)}\) also converges to \(L\), as desired.
\end{proof}

\begin{note}
  In \labelcrefs{i:10.5.2}, the hypothesis that \(f, g\) be differentiable on \([a, b]\) may be weakened to being continuous on \([a, b]\) and differentiable on \((a, b]\), with \(g'\) only assumed to be non-zero on \((a, b]\) rather than \([a, b]\).
\end{note}
...
\begin{rmk}\label{i:10.5.3}
  ...
  Speaking very informally, \labelcrefs{i:10.5.2} states that
  \[
...
\begin{ex}\label{i:ex:10.5.1}
  Prove \labelcrefs{i:10.5.1}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:10.5.1}.
\end{proof}
