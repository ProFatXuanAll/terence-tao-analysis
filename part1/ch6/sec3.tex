...
\begin{proof}
  ...
  Then by \labelcrefs{i:6.3.7} \(\forall n \in \Z^+\) we must have \(0 < x \leq a_n\).
  But by \labelcrefs{i:5.4.12} \(\exists q \in \Q^+\) such that \(q \leq x\).
...
\begin{proof}
  ...
  By \labelcrefs{i:6.3.1} we have \(\sup(a_n)_{n = m}^\infty = \sup(\set{a_n : n \geq m})\).
  Thus, by \labelcrefs{i:6.2.11.a}, \(\forall a_n \in \set{a_n : n \geq m}\) we have \(a_n \leq x\).

  Next we show that \(M \in \R^*\) is an upper bound of \((a_n)_{n = m}^\infty\) implies \(x \leq M\).
  By \labelcrefs{i:6.3.1} we have \(\sup(a_n)_{n = m}^\infty = \sup(\set{a_n : n \geq m})\).
  Thus, by \labelcrefs{i:6.2.11.b} we have \(x \leq M\).
...
\begin{proof}
  Since \((a_n)_{n = m}^\infty\) have an upper bound \(M\), by \labelcrefs{i:6.3.1} the set \(E = \set{a_n : n \geq m}\) have an upper bound \(M\).
  By \labelcrefs{i:5.5.9} \(\sup(E)\) must exist and \(\sup(E) \leq M\).
  Now we want to show that \(\lim_{n \to \infty} a_n = \sup(E)\).
  By \labelcrefs{i:6.1.8,i:6.1.5}, we need to show that \(\forall \varepsilon \in \R^+\), \(\exists N \in \N\) and \(N \geq m\) such that \(\abs{a_n - \sup(E)} \leq \varepsilon\) for every \(n \geq N\).
...
\begin{note}
  ...
  From \labelcrefs{i:6.3.8} and \labelcrefs{i:6.1.17} we see that a monotone sequence converges iff it is bounded.
\end{note}
...
\begin{eg}\label{i:6.3.9}
  ...
  Hence by \labelcrefs{i:6.3.8} it must have a limit, which is a real number less than or equal to \(4\).
\end{eg}

\begin{note}
  \labelcrefs{i:6.3.8} asserts that the limit of a monotone sequence exists, but does not directly say what that limit is.
...
\begin{proof}
  ...
  Thus, by \labelcrefs{i:ac:6.3.1} the sequence \((x^n)_{n = 1}^\infty\) converges to some limit \(L\).
  Since \(x^{n + 1} = x \times x^n\), we thus see from the limit laws (\labelcrefs{i:6.1.19}) that \((x^{n + 1})_{n = 1}^\infty\) converges to \(xL\).
  But the sequence \((x^{n + 1})_{n = 1}^\infty\) is just the sequence \((x^n)_{n = 2}^\infty\) shifted by one, and so they must have the same limits by \labelcrefs{i:ex:6.1.3,i:ex:6.1.4}.
  So \(xL = L\).
...
\begin{ex}\label{i:ex:6.3.1}
  Verify the claim in \labelcrefs{i:6.3.4}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:6.3.4}.
\end{proof}

\begin{ex}\label{i:ex:6.3.2}
  Prove \labelcrefs{i:6.3.6}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:6.3.6}.
\end{proof}

\begin{ex}\label{i:ex:6.3.3}
  Prove \labelcrefs{i:6.3.8}.
\end{ex}

\begin{proof}
  See \labelcrefs{i:6.3.8}.
\end{proof}

\begin{ex}\label{i:ex:6.3.4}
  Explain why \labelcrefs{i:6.3.10} fails when \(x > 1\).
...
\begin{proof}
  ...
  Then by \labelcrefs{i:6.3.8} the sequence \((x^n)_{n = 1}^\infty\) converges to some limit \(L\).
  Since \((1 / x)^n x^n = 1\), we thus see from the limit laws (\labelcrefs{i:6.1.19}) that \(((1 / x)^n x^n)_{n = 1}^\infty\) converges to \(1\).
  But \(0 < (1 / x)^n < 1\), by \labelcrefs{i:6.3.10} we have \(((1 / x)^n)_{n = 1}^\infty\) converges to \(0\).
  So by \labelcrefs{i:6.1.19} we have \(0L = 1\), a contradiction.
  Thus, \((x^n)_{n = 1}^\infty\) does not have an upper bound.
  This means \((x^n)_{n = 1}^\infty\) is diverge by \labelcrefs{i:6.1.17}.
\end{proof}
