...
\begin{note}
  Implicit in \labelcrefs{i:3.3.1} is the assumption that whenever one is given two sets \(X, Y\) and a property \(P\) obeying the vertical line test, one can form a function object.
...
\begin{proof}[\pf{i:ac:3.3.1}]
  ...
  By \labelcrefs{i:3.3.1}, we know that \(f(x)\) is the unique object for which \(P(x, f(x))\) is true.
...
\begin{defn}[Equality of functions]\label{i:3.3.7}
  ...
  See the discussion after \labelcrefs{i:9.4.1} for an instance of this.
\end{defn}
...
\begin{eg}\label{i:3.3.9}
  ...
  Note that for each set \(X\), there is only one function from \(\emptyset\) to \(X\), since \labelcrefs{i:3.3.7} asserts that all functions from \(\emptyset\) to \(X\) are equal.
\end{eg}
...
\begin{proof}[\pf{i:3.3.12}]
  ...
  In order to check that they are equal, we see from \labelcrefs{i:3.3.7} that we have to verify that \((f \circ (g \circ h))(x) = ((f \circ g) \circ h)(x)\) for all \(x \in X\).
  But by \labelcrefs{i:3.3.10},
  \begin{align*}
    (f \circ (g \circ h))(x)
     & = f((g \circ h)(x))         \\
     & = f(g(h(x)))                \\
...
\begin{rmk}\label{i:3.3.19}
  ...
  See \labelcrefs{i:ex:3.3.2,i:ex:3.3.4,i:ex:3.3.5} for some evidence of this.
\end{rmk}
...
\begin{ex}\label{i:ex:3.3.1}
  Show that the definition of equality in \labelcrefs{i:3.3.7} is reflexive, symmetric, and transitive.
...
\begin{proof}[\pf{i:ex:3.3.1}]
  We first show that \labelcrefs{i:3.3.7} is reflexive.
  ...
  \end{align*}
  Thus, \labelcrefs{i:3.3.7} is reflexive.

  Next we show that \labelcrefs{i:3.3.7} is symmetric.
  ...
  \begin{align*}
    ...
    \iff & g = f.                       &  & \by{i:3.3.7}
  \end{align*}
  Thus, \labelcrefs{i:3.3.7} is symmetric.

  Next we show that \labelcrefs{i:3.3.7} is transitive.
  ...
  \end{align*}
  Thus, \labelcrefs{i:3.3.7} is transitive.
  ...
  By \labelcrefs{i:3.3.10}, \(g \circ f : X \to Z\) and \(\tilde{g} \circ \tilde{f} : X \to Z\) are well-defined.
  Since \(X = X\) and \(Z = Z\), to prove that \(g \circ f = \tilde{g} \circ \tilde{f}\), by \labelcrefs{i:3.3.7}, we only need to show that \((g \circ f)(x) = \pa{\tilde{g} \circ \tilde{f}}(x)\) for all \(x \in X\).
...
\begin{proof}[\pf{i:ex:3.3.2}]
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.14}, \(g \circ f\) is injective.
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.17}, \(g \circ f\) is surjective.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.3.3}]
  ...
  \(f\) is always injective since the statement ``for all \(x, x' \in \emptyset\), \(f(x) = f(x') \implies x = x'\)'' is vacuously true by \labelcrefs{i:3.2}.
...
\begin{proof}[\pf{i:ex:3.3.5}]
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.14}, \(f\) is injective.
...
\begin{proof}[\pf{i:ex:3.3.6}]
  ...
  \end{align*}
  we know that \(f^{-1}\) is injective by \labelcrefs{i:3.3.14}.
  Since
  \begin{align*}
             & \forall x \in X, \exists! y \in Y : f(x) = y      &  & \by{i:3.3.1}  \\
    \implies & \forall x \in X, \exists y \in Y : f^{-1}(y) = x, &  & \by{i:3.3.20}
  \end{align*}
  we know that \(f^{-1}\) is surjective by \labelcrefs{i:3.3.17}.
  Since \(f^{-1}\) is both injective and surjective, we know that \(f^{-1}\) is bijective by \labelcrefs{i:3.3.20}.
  ...
  \end{align*}
  we know that \(f = \pa{f^{-1}}^{-1}\) by \labelcrefs{i:3.3.7}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.3.7}]
  ...
  Since \(g \circ f\) has domain \(X\) and codomain \(Z\), by \labelcrefs{i:3.3.20}, we know that \((g \circ f)^{-1}\) has domain \(Z\) and codomain \(X\).
  By \labelcrefs{i:3.3.20}, we know that \(g^{-1} : Z \to Y\) and \(f^{-1} : Y \to X\) are well-defined.
  Thus, by \labelcrefs{i:3.3.10}, we know that \(f^{-1} \circ g^{-1} : Z \to X\) is well-defined.
  Since \((g \circ f)^{-1}\) and \(f^{-1} \circ g^{-1}\) both have the same domain and codomain, by \labelcrefs{i:3.3.7}, we only need to show that \((g \circ f)^{-1}(z) = (f^{-1} \circ g^{-1})(z)\) for every \(z \in Z\).
  Since
  \begin{align*}
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.7}, we conclude that \((g \circ f)^{-1} = f^{-1} \circ g^{-1}\).
\end{proof}
...
\begin{ex}\label{i:ex:3.3.8}
  ...
  \begin{enumerate}
    \item\label{i:ex:3.3.8.a} Show that if \(X \subseteq Y \subseteq Z\) then \(\iota_{Y \to Z} \circ \iota_{X \to Y} = \iota_{X \to Z}\).
    \item\label{i:ex:3.3.8.b} Show that if \(f : A \to B\) is any function, then \(f = f \circ \iota_{A \to A} = \iota_{B \to B} \circ f\).
    \item\label{i:ex:3.3.8.c} Show that if \(f : A \to B\) is a bijective function, then \(f \circ f^{-1} = \iota_{B \to B}\) and \(f^{-1} \circ f = \iota_{A \to A}\).
    \item\label{i:ex:3.3.8.d} Show that if \(X\) and \(Y\) are disjoint sets, and \(f : X \to Z\) and \(g : Y \to Z\) are functions, then there is a unique function \(h : X \cup Y \to Z\) such that \(h \circ \iota_{X \to X \cup Y} = f\) and \(h \circ \iota_{Y \to X \cup Y} = g\).
  \end{enumerate}
\end{ex}
...
\begin{proof}[\pf{i:ex:3.3.8.a}]
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.7,i:3.3.10}, we have \(\iota_{Y \to Z} \circ \iota_{X \to Y} = \iota_{X \to Z}\).
\end{proof}
...
\begin{proof}[\pf{i:ex:3.3.8.b}]
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.7,i:3.3.10}, we have \(f = f \circ \iota_{A \to A} = \iota_{B \to B} \circ f\).
\end{proof}
...
\begin{proof}[\pf{i:ex:3.3.8.c}]
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.7,i:3.3.10}, we have \(f^{-1} \circ f = \iota_{A \to A}\) and \(f \circ f^{-1} = \iota_{B \to B}\).
\end{proof}
...
\begin{proof}[\pf{i:ex:3.3.8.d}]
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.3.7,i:3.3.10}, we have \(h \circ \iota_{X \to X \cup Y} = f\) and \(h \circ \iota_{Y \to X \cup Y} = g\).
  ...
  \begin{align*}
    ...
    \forall y \in Y : g(y) & = (h' \circ \iota_{Y \to X \cup Y})(y)                    \\
                           ...
                           & = h(y).
  \end{align*}
  Thus, by \labelcrefs{i:3.3.7}, we have \(h = h'\), so \(h\) is unique.
\end{proof}
