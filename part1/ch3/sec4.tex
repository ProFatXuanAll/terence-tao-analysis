...
\begin{ac}\label{i:ac:3.4.1}
  ...
  The set \(f(S)\) is well-defined thanks to the axiom of replacement (\labelcrefs{i:3.6}).
  One can also define \(f(S)\) using the axiom of specification (\labelcrefs{i:3.5}) instead of replacement.
\end{ac}

\begin{proof}[\pf{i:ac:3.4.1}]
  First, we use \labelcrefs{i:3.4.1} to create the set \(f(X) = \set{f(x) : x \in X}\).
  Now we use \labelcrefs{i:3.5} to create the set
  \[
    E = \set{y \in f(X) \mid \exists x \in S : f(x) = y}.
  \]
  By \labelcrefs{i:3.1.4,i:3.4.1}, we see that \(E = f(S)\).
  Thus, we can define \(f(S)\) by \labelcrefs{i:3.5} instead of \labelcrefs{i:3.6}.
\end{proof}
...
\begin{rmk}\label{i:3.4.6}
  If \(f\) is a bijective function, then we have defined \(f^{-1}\) in two slightly different ways, but this is not an issue because both definitions are equivalent (\labelcrefs{i:ex:3.4.1}).
\end{rmk}
...
\begin{proof}[\pf{i:3.4.9}]
  ...
  By \labelcrefs{i:3.10}, there exists a set \(\set{0, 1}^X\) which consists of functions with domain \(X\) and codomain \(\set{0, 1}\).
  By \labelcrefs{i:3.6}, we can replace each \(f \in \set{0, 1}^X\) with \(f^{-1}\pa{\set{1}}\), i.e., there exists a set
  \[
  ...
  By \labelcrefs{i:3.4.4}, we know that \(Y = \set{x \in X : f(x) \in \set{1}}\), which is clearly a subset of \(X\) (\labelcrefs{i:3.1.15}).
  Thus, our claim is true.

  Finally we show that \(S\) is consist of all subsets of \(X\), and therefore \labelcrefs{i:3.4.9} is true.
  Let \(Y\) be a subset of \(X\).
  We want to show that \(Y \in S\), and we will use \labelcrefs{i:ex:3.3.8.d} to achieve this.
  We define two functions \(f : X \setminus Y \to \set{0, 1}\) and \(g : Y \to \set{0, 1}\) as follow:
  \begin{align*}
     & \forall z \in X \setminus Y, f(z) = 0; \\
     & \forall z \in Y, g(z) = 1.
  \end{align*}
  Since every element in \(X \setminus Y\) is mapped by \(f\) to the unique element \(0\) in \(\set{0, 1}\), we know that \(f\) obey the vertical line test, and therefore \(f\) is well-defined by \labelcrefs{i:3.3.1}.
  Similarly, \(g\) is well-defined.
  By \labelcrefs{i:3.1.28.g}, we know that \((X \setminus Y) \cap Y = \emptyset\) and \((X \setminus Y) \cup Y = X\).
  Thus, we can apply \labelcrefs{i:ex:3.3.8.d} to create a function \(h : X \to \set{0, 1}\) as follow:
  \[
...
\begin{note}
  The axiom of union (\labelcrefs{i:3.11}), combined with the axiom of pair set (\labelcrefs{i:3.3}), implies the axiom of pairwise union (\labelcrefs{i:3.4}) (see \labelcrefs{i:ex:3.4.8}).
  Another important consequence of \labelcrefs{i:3.11} is that if one has some set \(I\), and for every element \(\alpha \in I\) we have some set \(A_\alpha\), then we can form the union set \(\bigcup_{\alpha \in I} A_\alpha\) by defining
  \[
    \bigcup_{\alpha \in I} A_\alpha \coloneqq \bigcup \set{A_\alpha : \alpha \in I},
  \]
  which is a set thanks to the axiom of replacement (\labelcrefs{i:3.6}) and the axiom of union (\labelcrefs{i:3.11}).
...
\begin{note}
  ...
  \]
  which is a set by the axiom of specification (\labelcrefs{i:3.5}).
...
\begin{rmk}\label{i:3.4.12}
  The axioms of set theory that we have introduced (\crefrange{i:3.1}{i:3.11}, excluding the dangerous \labelcrefs{i:3.8}) are known as the \emph{Zermelo-Fraenkel axioms of set theory}, after Ernst Zermelo (1871--1953) and Abraham Fraenkel (1891--1965).
  There is one further axiom we will eventually need, the famous \emph{axiom of choice} (see \labelcrefs{i:sec:8.4}), giving rise to the \emph{Zermelo-Fraenkel-Choice (ZFC) axioms of set theory}, but we will not need this axiom for some time.
\end{rmk}
...
\begin{proof}[\pf{i:ex:3.4.1}]
  ...
  \end{align*}
  Thus, by \labelcrefs{i:3.1.4}, we have \(A = B\).
\end{proof}
...
\begin{proof}[\pf{i:ex:3.4.2}]
  ...
  \end{align*}
  we have \(S \subseteq f^{-1}(f(S))\) by \labelcrefs{i:3.1.15}.
  ...
  \end{align*}
  we have \(f(f^{-1}(U)) \subseteq U\) by \labelcrefs{i:3.1.15}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.4.3}]
  ...
  \end{align*}
  we have \(f(A \cap B) \subseteq f(A) \cap f(B)\) by \labelcrefs{i:3.1.15}.
  ...
  \end{align*}
  we have \(f(A) \setminus f(B) \subseteq f(A \setminus B)\) by \labelcrefs{i:3.1.15}.
  ...
  \end{align*}
  we have \(f(A \cup B) = f(A) \cup f(B)\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.4.4}]
  ...
  \end{align*}
  we have \(f^{-1}(U \cup V) = f^{-1}(U) \cup f^{-1}(V)\) by \labelcrefs{i:3.1.4}.
  ...
  \end{align*}
  we have \(f^{-1}(U \cap V) = f^{-1}(U) \cap f^{-1}(V)\) by \labelcrefs{i:3.1.4}.
  ...
  \end{align*}
  we have \(f^{-1}(U \setminus V) = f^{-1}(U) \setminus f^{-1}(V)\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.4.5}]
  ...
  By \labelcrefs{i:ex:3.4.2}, we have \(f\pa{f^{-1}(S)} \subseteq S\) for every \(S \subseteq Y\).
  ...
  By \labelcrefs{i:ex:3.4.2}, we have \(S \subseteq f^{-1}\pa{f(S)}\) for every \(S \subseteq X\).
  ...
  Then by \labelcrefs{i:3.3,i:3.1.15}, we know that \(\set{x_1} \subseteq X\) and \(\set{x_2} \subseteq X\).
  Thus, we can apply the hypothesis and \labelcrefs{i:ex:3.4.2} to derive
  \[
  ...
  \]
  By \labelcrefs{i:3.1.18}, this means \(\set{x_1} = f^{-1}\pa{f\pa{\set{x_1}}}\) and \(\set{x_2} = f^{-1}\pa{f\pa{\set{x_2}}}\).
  By \labelcrefs{i:3.4.1}, we see that \(f\pa{\set{x_1}}\) and \(f\pa{\set{x_2}}\) are singleton sets.
  Thus, by \labelcrefs{i:3.4.4}, we must have \(f(x_1) \neq f(x_2)\), otherwise we would have \(f(\set{x_1}) = f(\set{x_2})\), which implies \(\set{x_1} = \set{x_2}\), a contradiction.
  Therefore, \(f\) is injective by \labelcrefs{i:3.3.14}.
  ...
  Then by \labelcrefs{i:3.1.15}, there exists an \(x \in f^{-1}\pa{f(S)}\) such that \(x \notin S\).
  Fix one such \(x\).
  By \labelcrefs{i:3.4.4}, we know that there exists a \(y \in f(S)\) such that \(f(x) = y\).
  Fix one such \(y\).
  Since \(y \in f(S)\), by \labelcrefs{i:3.4.1}, there exists an \(x' \in S\) such that \(f(x') = y\).
...
\begin{ex}\label{i:ex:3.4.6}
  Prove \labelcrefs{i:3.4.9}.
\end{ex}

\begin{proof}[\pf{i:ex:3.4.6}]
  See \labelcrefs{i:3.4.9}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.4.7}]
  ...
  Then by \labelcrefs{i:3.4.9}, both the sets \(A = \set{X' : X' \subseteq X}\) and \(B = \set{Y' : Y' \subseteq Y}\) exist.
  Now we have
  \begin{align*}
...
\begin{ex}\label{i:ex:3.4.8}
  Show that \labelcrefs{i:3.4} can be deduced from \labelcrefs{i:3.1,i:3.3,i:3.11}.
\end{ex}

\begin{proof}[\pf{i:ex:3.4.8}]
  Let \(A, B\) be sets (the existence of \(A, B\) are guaranteed by \labelcrefs{i:3.1}).
  By \labelcrefs{i:3.3}, there exists a set \(\set{A, B}\) whose only elements are \(A\) and \(B\).
  By \labelcrefs{i:3.11}, we can create a set \(\bigcup \set{A, B}\).
  ...
  \end{align*}
  we see that \(A \cup B = \bigcup \set{A, B}\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.4.9}]
  ...
  \begin{align*}
    ...
    \iff & x \in B',
  \end{align*}
  we have \(B = B'\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.4.10}]
  ...
  \end{align*}
  we have \(\pa{\bigcup_{\alpha \in I} A_\alpha} \cup \pa{\bigcup_{\alpha \in J} A_\alpha} = \bigcup_{\alpha \in I \cup J} A_\alpha\) and \(\pa{\bigcap_{\alpha \in I} A_\alpha} \cap \pa{\bigcap_{\alpha \in J} A_\alpha} = \bigcap_{\alpha \in I \cup J} A_\alpha\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{ex}\label{i:ex:3.4.11}
  ...
  \]
  This should be compared with de Morgan's laws in \labelcrefs{i:3.1.28}
...
\begin{proof}[\pf{i:ex:3.4.11}]
  ...
  \end{align*}
  we have \(X \setminus \bigcup_{\alpha \in I} A_\alpha = \bigcap_{\alpha \in I} (X \setminus A_\alpha)\) and \(X \setminus \bigcap_{\alpha \in I} A_\alpha = \bigcup_{\alpha \in I} (X \setminus A_\alpha)\) by \labelcrefs{i:3.1.4}.
\end{proof}
