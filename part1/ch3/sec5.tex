...
\begin{defn}[Ordered pair]\label{i:3.5.1}
  ...
  \]
  This is consistent with the usual axioms of equality (\labelcrefs{i:ex:3.5.3}).
\end{defn}

\begin{rmk}\label{i:3.5.2}
  Strictly speaking, \labelcrefs{i:3.5.1} is partly an axiom, because we have simply postulated that given any two objects \(x\) and \(y\), that an object of the form \((x, y)\) exists.
  However, it is possible to define an ordered pair using the axioms of set theory in such a way that we do not need any further postulates (see \labelcrefs{i:ex:3.5.1}).
\end{rmk}
...
\begin{note}
  Again, \labelcrefs{i:3.5.7} simply postulates that an ordered \(n\)-tuple and a Cartesian product always exist when needed, but using the axioms of set theory one can explicitly construct these objects (\labelcrefs{i:ex:3.5.2}).
\end{note}
...
\begin{rmk}\label{i:3.5.8}
  ...
  One can generalize this construction to infinite Cartesian products, see \labelcrefs{i:8.4.1}.
\end{rmk}
...
\begin{rmk}\label{i:3.5.10}
  ...
  In \labelcrefs{i:ch:5} we shall also introduce the very useful concept of an \emph{infinite sequence}.
\end{rmk}
...
\begin{proof}[\pf{i:3.5.11}]
  ...
  By \labelcrefs{i:ex:3.5.2}, we see that \(x : \set{i \in \N : 1 \leq i \leq 0} \to Y\) is a surjective function, where \(Y\) is an arbitrary set.
...
\begin{proof}[\pf{i:3.5.12}]
  ...
  When \(n = 1\) the claim follows from \labelcrefs{i:3.1.6}.
  ...
  Also, since \(X_{n\pp}\) is non-empty, by \labelcrefs{i:3.1.6} we may find an object \(a\) such that \(a \in X_{n\pp}\).
...
\begin{ex}\label{i:ex:3.5.1}
  \begin{enumerate}
    \item\label{i:ex:3.5.1.a} Suppose we \emph{define} the ordered pair \((x, y)\) for any objects \(x\) and \(y\) by the formula \((x, y) \coloneqq \set{\set{x}, \set{x, y}}\)
    (thus using several applications of \labelcrefs{i:3.3}).
    Show that such a definition indeed obeys the \labelcrefs{i:3.5.1}.
    Thus, this definition can be validly used as a definition of an ordered pair.
    \item\label{i:ex:3.5.1.b} For an additional challenge, show that the alternate definition \((x, y) := \set{x, \set{x, y}}\) also verifies \labelcrefs{i:3.5.1} and is thus also an acceptable definition of ordered pair.
    \item\label{i:ex:3.5.1.c} Show that regardless of the definition of ordered pair, the Cartesian product \(X \times Y\) is a set.
  \end{enumerate}
\end{ex}
...
\begin{proof}[\pf{i:ex:3.5.1.a}]
  ...
  \end{align*}
  Thus, \labelcrefs{i:ex:3.5.1.a} is a valid definition of ordered pairs.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.5.1.b}]
  ...
  \end{align*}
  Thus, \labelcrefs{i:ex:3.5.1.b} is a valid definition of ordered pairs.
\end{proof}

\begin{proof}[\pf{i:ex:3.5.1.c}]
  We use \labelcrefs{i:ex:3.5.1.a} as in the definition of ordered pairs.
  Suppose that \(X, Y\) are sets.
  For each \(x \in X\), we can use \labelcrefs{i:3.3} to create singleton set \(\set{x}\).
  Using \labelcrefs{i:3.3} again, we can create pair sets \(\set{x, y}\) and \(\set{\set{x}, \set{x, y}}\) for each \(x \in X\) and each \(y \in Y\).
  Since \(Y\) is a set, each object in \(Y\) is uniquely identified (\labelcrefs{i:3.1.1}).
  Thus, for each \(x \in X\) and each \(y \in Y\), the statement ``\(P_y(x, y') \coloneqq y' = y\)'' is only true for one \(y' \in Y\), namely \(y\).
  Thus, for each \(y \in Y\), we can use \labelcrefs{i:3.6} to create the set \(S_y\) using \(P_y\):
  \begin{align*}
  ...
  \end{align*}
  By \labelcrefs{i:3.11}, we see that \(\bigcup_{y \in Y} S_y = \set{(x, y) : x \in X, y \in Y}\), which equals to \(X \times Y\) (\labelcrefs{i:3.5.4}).
...
\begin{ex}\label{i:ex:3.5.2}
  ...
  Also, show that if \((X_i)_{1 \leq i \leq n}\) are an ordered \(n\)-tuple of sets, then the Cartesian product, as defined in \labelcrefs{i:3.5.7}, is indeed a set.
  (Technically, this construction of ordered \(n\)-tuple is not compatible with the construction of ordered pair in \labelcrefs{i:ex:3.5.1}, but this does not cause a difficulty in practice;
  for instance, one can use the definition of an ordered \(2\)-tuple here to replace the construction in \labelcrefs{i:ex:3.5.1}, or one can make a rather pedantic distinction between an ordered \(2\)-tuple and an ordered pair in one's mathematical arguments.)
\end{ex}

\begin{proof}[\pf{i:ex:3.5.2}]
  First, we show that \labelcrefs{i:ex:3.5.2} is a valid definition of \(n\)-tuple.
  ...
  \end{align*}
  Thus, \labelcrefs{i:ex:3.5.2} is a valid definition of \(n\)-tuple.

  Now we show that \(\prod_{i = 1}^n X_i\) defined in \labelcrefs{i:3.5.7} is indeed a set.
  ...
  \end{align*}
  By \labelcrefs{i:ex:3.4.7}, there exists a set of all partial function with domain \(I\) and codomain \(\bigcup_{i \in I} X_i\), and we denote this set \(A\).
  By \labelcrefs{i:3.5}, there exists a set \(B = \set{x \in A : \forall i \in I, x_i \in X_i}\).
  Clearly, every \(x \in B\) has the form \(x = (x_i)_{1 \leq i \leq n}\).
  And every \(n\)-tuple \((x_i)_{1 \leq i \leq n}\) with \(x_i \in X_i\) for all \(i \in I\) is also in \(B\), thanks to \labelcrefs{i:ex:3.4.7}.
  Thus, by \labelcrefs{i:3.5.7}, we see that \(B = \prod_{i = 1}^n X_i\).
...
\begin{proof}[\pf{i:ex:3.5.3}]
  ...
  We first show that \labelcrefs{i:3.5.1} is reflexive.
  Since
  \begin{align*}
             & (x = x) \land (y = y)                   \\
    \implies & (x, y) = (x, y),      &  & \by{i:3.5.1}
  \end{align*}
  we see that \labelcrefs{i:3.5.1} is reflexive.

  Next we show that \labelcrefs{i:3.5.1} is symmetry.
  Since
  \begin{align*}
         & (x, y) = (x', y')                         \\
  ...
  \end{align*}
  we see that \labelcrefs{i:3.5.1} is symmetry.

  Next we show that \labelcrefs{i:3.5.1} is transitive.
  Since
  \begin{align*}
  ...
  \end{align*}
  we see that \labelcrefs{i:3.5.7} is symmetry.

  Finally we show that \labelcrefs{i:3.5.7} is transitive.
  Since
  \begin{align*}
  ...
  \end{align*}
  we see that \labelcrefs{i:3.5.7} is transitive.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.5.4}]
  ...
  \end{align*}
  we have \(A \times (B \cup C) = (A \times B) \cup (A \times C)\) by \labelcrefs{i:3.1.4}.
  ...
  \end{align*}
  we have \(A \times (B \cap C) = (A \times B) \cap (A \times C)\) by \labelcrefs{i:3.1.4}.
  ...
  \end{align*}
  we have \(A \times (B \setminus C) = (A \times B) \setminus (A \times C)\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.5.5}]
  ...
  \end{align*}
  we have \((A \times B) \cap (C \times D) = (A \cap C) \times (B \cap D)\) by \labelcrefs{i:3.1.4}.
  ...
  \end{align*}
  Clearly, \((A \times B) \cup (C \times D) \neq (A \cup C) \times (B \cup D)\) by \labelcrefs{i:3.1.4}.
  ...
  \begin{align*}
    ...
    (A \setminus C) \times (B \setminus D) & = \set{2} \times \set{4}                                      &  & \by{i:3.1.27} \\
                                           & = \set{(2, 4)}.                                               &  & \by{i:3.5.4}
  \end{align*}
  Clearly, \((A \times B) \setminus (C \times D) \neq (A \setminus C) \times (B \setminus D)\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{ex}\label{i:ex:3.5.7}
  ...
  (Compare this to \labelcrefs{i:ex:3.3.8.d}, and to \labelcrefs{i:ex:3.1.7}.)
...
\begin{proof}[\pf{i:ex:3.5.7}]
  ...
  Both \(\pi_{X \times Y \to X}\) and \(\pi_{X \times Y \to Y}\) are well-defined by \labelcrefs{i:3.6}.
  We now define a function \(h : Z \to X \times Y\) where
  \[
    \forall z \in Z, h(z) = (f(z), g(z)).
  \]
  To show that \(h\) is well-defined, by \labelcrefs{i:3.3.1}, we have to show that \(h\) pass the vertical line test.
  Since \(f, g\) are functions, by \labelcrefs{i:3.3.1}, we know that for each \(z \in Z\), \(f(z)\) and \(g(z)\) are unique objects in \(X\) and \(Y\), respectively.
  Thus, by \labelcrefs{i:ex:3.5.3}, \((f(z), g(z)) \in X \times Y\) is unique for each \(z \in Z\).
  ...
  \begin{align*}
    ...
    \forall z \in Z, (\pi_{X \times Y \to Y} \circ h)(z) & = \pi_{X \times Y \to Y}(h(z))       &  & \by{i:3.3.10} \\
                                                         ...
                                                         & = g(z).
  \end{align*}
  Thus, by \labelcrefs{i:3.3.7}, we have \(\pi_{X \times Y \to X} \circ h = f\) and \(\pi_{X \times Y \to Y} \circ h = g\).
...
\begin{proof}[\pf{i:ex:3.5.9}]
  ...
  \end{align*}
  we have \(\pa{\bigcup_{\alpha \in I} A_\alpha} \cap \pa{\bigcup_{\beta \in J} B_\beta} = \bigcup_{(\alpha, \beta) \in I \times J} (A_\alpha \cap B_\beta)\) by \labelcrefs{i:3.1.4}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.5.10}]
  ...
  For each \(x \in X\), we use \labelcrefs{i:3.6} to create the set \(S_x = \set{y \in Y : (x, y) \in G}\).
  ...
  Thus, we can use \labelcrefs{i:3.3.1} to create a function \(f : X \to Y\) such that
  \[
  ...
  \]
  By \labelcrefs{i:3.1.4}, this means \(G = \set{(x, f(x)) : x \in X}\).
...
\begin{ex}\label{i:ex:3.5.11}
  Show that \labelcrefs{i:3.10} can in fact be deduced from \labelcrefs{i:3.4.9} and the other axioms of set theory, and thus \labelcrefs{i:3.4.9} can be used as an alternate formulation of the power set axiom.
\end{ex}

\begin{proof}[\pf{i:ex:3.5.11}]
  Suppose that \(X, Y\) are sets.
  Then by \labelcrefs{i:ex:3.5.1.c}, \(X \times Y\) is a set.
  Thus, we can use \labelcrefs{i:3.4.9} to create a set of subsets of \(X \times Y\), i.e., \(S_1 = \set{S : S \subseteq X \times Y}\).
  If \(f : X \to Y\) is a function, then by \labelcrefs{i:3.3.1}, every element on \(X\) must be assigned exactly one object in \(Y\).
  Thus, we use \labelcrefs{i:3.5} to rule out those sets violating the vertical line test, and we derive the following set
  \[
    S_2 = \set{G \in S_1 \mid \forall x \in X, \exists! y \in Y : (x, y) \in G}.
  \]
  By \labelcrefs{i:ex:3.5.10}, we see that every element \(G \in S_2\) is a graph, and we know that there exists exactly one function \(f : X \to Y\) whose graph is \(G\).
  Thus, we can use \labelcrefs{i:3.6} to create the following set
  \[
    S_3 = \set{f : X \to Y \mid \text{the graph of } f \text{ is in } S_2}.
  \]
  Now we claim that every function \(f : X \to Y\) is an element of \(S_3\), and thus by \labelcrefs{i:3.10} we have \(S_3 = X^Y\), and \(X^Y\) is indeed a set.
  But by \labelcrefs{i:ex:3.5.10}, every function with domain \(X\) and codomain \(Y\) is uniquely identified by one graph and vice versa.
...
\begin{proof}[\pf{i:ex:3.5.12}]
  ...
  By \labelcrefs{i:ac:2.2.4}, we see that \(\set{0} = \set{n \in \N : n \leq 0}\), and the statement ``\(a_0(n\pp) = f(n, a_0(n))\) for all \(n \in \N\) such that \(n < 0\)'' is vacuously true.
  ...
  Since \(0\) is the only element in \(\set{0}\), and \(b_0(0) = c = a_0(0)\), we must have \(b_0 = a_0\) by \labelcrefs{i:3.3.7}.
  ...
  Since \(0 \neq N\pp\) (\labelcrefs{i:2.3}), we have \(a_{N\pp}(0) = a_N(0) = c\).
  ...
  By \labelcrefs{i:3.1.4}, this means \(b_{N\pp} = a_{N\pp}\), and this closes the induction.
  ...
  \]
  Thus, by \labelcrefs{i:2.5}, we know that \(b(n) = a(n)\) for all \(n \in \N\).
  By \labelcrefs{i:3.1.4}, this means \(b = a\).
  Thus, \(a\) is unique.
  ...
  By \labelcrefs{i:3.1.28.g}, we see that (a)(b) holds for \(A_0, B_0\).
  By \labelcrefs{i:3.3}, we have \(0 \in A_0\) and \(0\pp = 1 \notin A_0\).
  Thus, \(1 \in B_0\).
  So (c)(d) holds for \(A_0, B_0\).
  If \(n \in B_0\), then \(n \in \N\) and \(n\pp \neq 0\) (\labelcrefs{i:2.4}).
  ...
  By \labelcrefs{i:ex:3.1.9}, we have \(B_0' = \N \setminus A_0' = \N \setminus A_0 = B_0\).
  ...
  \begin{align*}
     ...
     & = \emptyset,                                                                              &  & \byIH
  \end{align*}
  we see that \(A_{N\pp} \cap B_{N\pp} = \emptyset\) by \labelcrefs{i:3.2}.
  ...
  Since \(A_{N\pp} = A_N \cup \set{N\pp}\), we have \(A_N \subseteq A_{N\pp}\) by \labelcrefs{i:ex:3.1.7}.
  By the induction hypothesis, we know that \(0 \in A_N\).
  Thus, by \labelcrefs{i:3.1.15}, we have \(0 \in A_{N\pp}\), and (c) is true for \(A_{N\pp}, B_{N\pp}\).
  ...
  If \(n \in B_{N\pp} = B_N \setminus \set{N\pp}\), then we have \(n \in B_N\) and \(n \neq N\pp\) by \labelcrefs{i:3.1.27,i:3.3}.
  We must have \(n\pp \neq N\pp\), for otherwise we would have \(n\pp = N\pp\) and \(n = N\) by \labelcrefs{i:2.4}.
  ...
  Thus, by (b) and \labelcrefs{i:ex:3.1.9}, we see that \(B_{N\pp}' = B_{N\pp}\).
...
\begin{proof}[\pf{i:ex:3.5.13}]
  ...
  \]
  By \labelcrefs{i:2.4}, we see that \(g\) pass the vertical line test.
  Thus, \(g\) is well-defined.
  By \labelcrefs{i:ex:3.5.12}, there exists a unique function \(a : \N \to \N'\) such that
  \[
  ...
  Since \(n_2 \neq 0\), by \labelcrefs{i:2.2.10}, there exists an \(m \in \N\) such that \(m\pp = n_2\).
  By the definition of \(a\), we see that \(a(n_2) = a(m\pp) = a(m)\pp'\).
  By \labelcrefs{i:2.3}, we know that \(a(m)\pp' \neq 0'\).
  ...
  \begin{itemize}
    ...
    \item If \(n_2 \neq 0\), then by \labelcrefs{i:2.2.10}, there exists an \(m \in \N\) such that \(m\pp = n_2\).
          Thus,
          \begin{align*}
  ...
  This closes the induction, and we see that \(a\) is injective by \labelcrefs{i:3.3.14}.

  Next we claim that \(a\) is surjective, i.e. (\labelcrefs{i:3.3.17}), for each \(n' \in \N'\), there exists an \(n \in \N\) such that \(a(n) = n'\).
  Since \labelcrefs{i:2.5} holds for \(\N'\), we can induct on \(n'\) to prove the claim.
  ...
  Since \(a\) is both injective and surjective, we know that \(a\) is bijective by \labelcrefs{i:3.3.20}.
