...
\begin{note}
  In \labelcrefs{i:ch:2} we defined the natural numbers axiomatically, assuming that they were equipped with a \(0\) and an increment operation, and assuming five axioms on these numbers.
  ...
  The purpose of \labelcrefs{i:sec:3.6} is to address this issue by noting that the natural numbers \emph{can} be used to count the cardinality of sets, as long as the set is finite.
...
\begin{proof}[\pf{i:3.6.4}]
  We first show that \labelcrefs{i:3.6.1} is reflexive.
  Let \(f : X \to X\) be a function where \(f = x \mapsto x\).
  By \labelcrefs{i:3.6}, \(f\) is well-defined.
  Since
  \[
    \forall x_1, x_2 \in X, x_1 \neq x_2 \implies x_1 = f(x_1) \neq f(x_2) = x_2,
  \]
  we know that \(f\) is injective (\labelcrefs{i:3.3.14}).
  Since
  \[
    \forall x \in X, f(x) = x,
  \]
  we know that \(f\) is surjective (\labelcrefs{i:3.3.17}).
  Thus, \(f\) is bijective (\labelcrefs{i:3.3.20}), and \(X\) has equal cardinality with \(X\) (\labelcrefs{i:3.6.1}).

  Next we show that \labelcrefs{i:3.6.1} is symmetric.
  Suppose that \(X\) has equal cardinality with \(Y\).
  Then there exists a bijective function \(f : X \to Y\) (\labelcrefs{i:3.6.1}).
  Since \(f\) is bijective, we know that \(f^{-1} : Y \to X\) is also bijective (\labelcrefs{i:ex:3.3.6}).
  Thus, \(Y\) has equal cardinality with \(X\) (\labelcrefs{i:3.6.1}).

  Finally we show that \labelcrefs{i:3.6.1} is transitive.
  Suppose that \(X\) has equal cardinality with \(Y\), and \(Y\) has equal cardinality with \(Z\).
  Then there exist two bijective functions \(f : X \to Y\) and \(g : Y \to Z\) (\labelcrefs{i:3.6.1}).
  Since \(f\) and \(g\) are bijective, we know that \(g \circ f : X \to Z\) is also bijective (\labelcrefs{i:ex:3.3.7}).
  Thus, \(X\) has equal cardinality with \(Z\) (\labelcrefs{i:3.6.1}).
\end{proof}
...
\begin{proof}[\pf{i:3.6.6}]
  ...
  By \labelcrefs{i:2.4,i:3.3.14}, we see that \(f\) is injective.
  Since \(0 \notin B\), by \labelcrefs{i:2.2.10}, we know that for every \(i \in B\), there exists an \(j \in \N\) such that \(j\pp = i\).
  Then we have
  \begin{align*}
             & i \in B                              \\
  ...
  \end{align*}
  Thus, \(f(j)\) is well-defined by \labelcrefs{i:3.3.1}, and \(f(j) = j\pp = i\).
  This means \(f\) is surjective (\labelcrefs{i:3.3.17}).
  Since \(f\) is both injective and surjective, we know that \(f\) is bijective (\labelcrefs{i:3.3.20}).
  Thus, \(A, B\) have equal cardinality (\labelcrefs{i:3.6.1}).
\end{proof}
...
\begin{proof}[\pf{i:3.6.8}]
  ...
  Then \(X\) must be empty (\labelcrefs{i:ex:3.3.3}), and so \(X\) cannot have any non-zero cardinality.
  ...
  By \labelcrefs{i:2.2.10}, there exists a \(p \in \N\) such that \(p\pp = m\).
  By \labelcrefs{i:3.6.9}, \(X\) is non-empty, and if \(x\) is any element of \(X\), then \(X \setminus \set{x}\) has cardinality \(n\) and also has cardinality \(p\), by \labelcrefs{i:3.6.9}.
...
\begin{lem}\label{i:3.6.9}
  ...
  (Such \(m\) exists by \labelcrefs{i:2.2.10}.)
\end{lem}

\begin{proof}[\pf{i:3.6.9}]
  If \(X\) is empty, then it clearly cannot have the same cardinality as the non-empty set \(\set{i \in \N : 1 \leq i \leq n}\), as there is no bijection from the empty set to a non-empty set (\labelcrefs{i:ex:3.3.3}).
  So let \(x\) be an element of \(X\).
  Since \(X\) has the same cardinality as \(\set{i \in \N : 1 \leq i \leq n}\) (\labelcrefs{i:3.6.5}), we thus have a bijection \(f\) from \(X\) to \(\set{i \in \N : 1 \leq i \leq n}\).
  ...
  Also note that since \(f(y) \geq 1\), \(k\) is well-defined by \labelcrefs{i:2.2.10}.
  ...
  \begin{itemize}
    ...
    \item If \(f(y_1) < f(x)\) and \(f(y_2) > f(x)\), then there exists a \(k \in \N\) such that \(k\pp = f(y_2)\).
          ...
          \end{align*}
          In particular, we have \(g(y_1) \neq g(y_2)\) by \labelcrefs{i:2.2.13}.
    ...
    \item If \(f(y_1) > f(x)\) and \(f(y_2) > f(x)\), then there exist some \(k_1, k_2 \in \N\) such that \(k_1\pp = f(y_1)\) and \(k_2\pp = f(y_2)\).
          ...
          Therefore, by \labelcrefs{i:2.4}, we have \(g(y_1) = k_1 \neq k_2 = g(y_2)\).
  \end{itemize}
  From all cases above, we see that \(g(y_1) \neq g(y_2)\).
  Therefore, \(g\) is injective (\labelcrefs{i:3.3.14}).
  ...
  \begin{itemize}
    \item If \(1 \leq i < f(x)\), then we have \(f^{-1}(i) \neq f^{-1}(f(x)) = x\) since \(f\) is bijective.
          Thus, by \labelcrefs{i:3.1.27}, we have \(f^{-1}(i) \in X \setminus \set{x}\).
          By \labelcrefs{i:3.3.1}, we know that \(g\pa{f^{-1}(i)}\) is well-defined, and by the definition of \(g\), we have \(i = f\pa{f^{-1}(i)} < f(x) \implies g\pa{f^{-1}(i)} = f\pa{f^{-1}(i)} = i\).
          Thus, we have found some \(y \in X \setminus \set{x}\) such that \(g(y) = i\).
    \item If \(f(x) \leq i \leq m\), then by \labelcrefs{i:2.2.12.d,i:2.2.12.e}, we have \(f(x) < i\pp \leq m\pp = n\).
          Since \(f\) is bijective, we know that \(f^{-1}(i\pp)\) is well-defined, and \(f(x) \neq i\pp \implies f^{-1}(f(x)) = x \neq f^{-1}(i\pp)\).
          Thus, by \labelcrefs{i:3.1.27}, we have \(f^{-1}(i\pp) \in X \setminus \set{x}\).
          By \labelcrefs{i:3.3.1}, we know that \(g\pa{f^{-1}(i\pp)}\) is well-defined, and by the definition of \(g\), we have \(i\pp = f\pa{f^{-1}(i\pp)} > f(x) \implies g\pa{f^{-1}(i\pp)} = i\).
          Thus, we have found some \(y \in X \setminus \set{x}\) such that \(g(y) = i\).
  \end{itemize}
  From all cases above, we see that there exists a \(y \in X \setminus \set{x}\) such that \(g(y) = i\).
  Therefore, \(g\) is surjective (\labelcrefs{i:3.3.17}).
  Since \(g\) is both injective and surjective, we know that \(g\) is bijective (\labelcrefs{i:3.3.20}).

  By \labelcrefs{i:3.6.5}, this means \(X \setminus \set{x}\) has equal cardinality with \(\set{i \in \N : 1 \leq i \leq m}\).
...
\begin{proof}[\pf{i:3.6.12}]
  ...
  One can show that the sequence \(f(1), f(2), \dots, f(n)\) is bounded, or more precisely that there exists a natural number \(M\) such that \(f(i) \leq M\) for all \(1 \leq i \leq n\) (\labelcrefs{i:ex:3.6.3}).
...
\begin{rmk}\label{i:3.6.13}
  ...
  See \labelcrefs{i:sec:8.3}.
\end{rmk}

\begin{prop}[Cardinal arithmetic]\label{i:3.6.14}
  \begin{enumerate}
    \item\label{i:3.6.14.a} Let \(X\) be a finite set, and let \(x\) be an object which is not an element of \(X\).
    Then \(X \cup \set{x}\) is finite and \(\#(X \cup \set{x}) = \#(X) + 1\).
    \item\label{i:3.6.14.b} Let \(X\) and \(Y\) be finite sets.
    Then \(X \cup Y\) is finite and \(\#(X \cup Y) \leq \#(X) + \#(Y)\).
    If in addition \(X\) and \(Y\) are disjoint (i.e., \(X \cap Y = \emptyset\)), then \(\#(X \cup Y) = \#(X) + \#(Y)\).
    \item\label{i:3.6.14.c} Let \(X\) be a finite set, and let \(Y\) be a subset of \(X\).
    Then \(Y\) is finite, and \(\#(Y) \leq \#(X)\).
    If in addition \(Y \neq X\) (i.e., \(Y\) is a proper subset of \(X\)), then we have \(\#(Y) < \#(X)\).
    \item\label{i:3.6.14.d} If \(X\) is a finite set, and \(f : X \to Y\) is a function, then \(f(X)\) is a finite set with \(\#(f(X)) \leq \#(X)\).
    If in addition \(f\) is one-to-one, then \(\#(f(X)) = \#(X)\).
    \item\label{i:3.6.14.e} Let \(X\) and \(Y\) be finite sets.
    Then Cartesian product \(X \times Y\) is finite and \(\#(X \times Y) = \#(X) \times \#(Y)\).
    \item\label{i:3.6.14.f} Let \(X\) and \(Y\) be finite sets.
    Then the set \(Y^X\) (defined in \labelcrefs{i:3.10}) is finite and \(\#\pa{Y^X} = \#(Y)^{\#(X)}\).
  \end{enumerate}
\end{prop}

\begin{proof}[\pf{i:3.6.14.a}]
  Since \(X\) is finite, by \labelcrefs{i:3.6.10}, there exists an \(n \in \N\) such that \(\#(X) = n\).
  By \labelcrefs{i:3.6.5}, there exists a bijective function \(f : X \to \set{i \in \N : 1 \leq i \leq n}\).
  ...
  With that and \(g(x) = n + 1\), we see that \(g\) is surjective (\labelcrefs{i:3.3.17}).
  To show that \(g\) is bijective, we also need to show that \(g\) is injective (\labelcrefs{i:3.3.20}).
  So let \(y, y' \in X \cup \set{x}\) where \(y \neq y'\).
  Now we split into three cases:
  \begin{itemize}
    \item If \(y \neq x \neq y'\), then we have \(g(y) = f(y) \neq f(y') = g(y')\) since \(f\) is bijective.
    \item If \(y \neq x = y'\), then by \labelcrefs{i:ac:2.2.3}, we have \(g(y) = f(y) \leq n < n + 1 = g(x) = g(y')\).
          By \labelcrefs{i:2.2.13}, this means \(g(y) \neq g(y')\).
    \item If \(y = x \neq y'\), then by \labelcrefs{i:ac:2.2.3}, we have \(g(y') = f(y') \leq n < n + 1 = g(x) = g(y)\).
          By \labelcrefs{i:2.2.13}, this means \(g(y) \neq g(y')\).
  \end{itemize}
  For all cases above we have \(g(y) \neq g(y')\).
  Thus, \(g\) is injective (\labelcrefs{i:3.3.14}).
  This means \(g\) is bijective.
  Therefore, by \labelcrefs{i:3.6.10}, we know that \(X \cup \set{x}\) is finite, and \(\#(X \cup \set{x}) = n + 1 = \#(X) + 1\).
\end{proof}

\begin{proof}[\pf{i:3.6.14.b}]
  Since \(X\) is finite, by \labelcrefs{i:3.6.10}, there exists an \(n \in \N\) such that \(\#(X) = n\).
  We induct on \(n\) to show that \(X \cup Y\) is finite and \(\#(X \cup Y) \leq \#(X) + \#(Y)\).
  For \(n = 0\), by \labelcrefs{i:ex:3.6.2}, we have \(X = \emptyset\).
  By \labelcrefs{i:3.1.28.a}, we have \(\emptyset \cup Y = Y\).
  ...
  \begin{itemize}
    \item If \(X \setminus Y = \emptyset\), then we have
          ...
          \end{align*}
          But by \labelcrefs{i:ex:3.1.7}, we have \(Y \subseteq X \cup Y\).
          Thus, by \labelcrefs{i:3.1.18}, we have \(X \cup Y = Y\).
          Since \(Y\) is finite, \(X \cup Y\) is also finite.
          By \labelcrefs{i:2.2.11}, this means \(\#(X \cup Y) = \#(Y) \leq \#(X) + \#(Y)\).
    \item If \(X \setminus Y \neq \emptyset\), then by \labelcrefs{i:3.1.6,i:3.1.27}, there exists a \(z \in X\) such that \(z \notin Y\).
          This implies \((X \setminus \set{z}) \cup Y = (X \cup Y) \setminus \set{z}\).
          By \labelcrefs{i:3.6.9}, we know that \(\#(X \setminus \set{z}) = n\).
          ...
          By \labelcrefs{i:3.6.14.a}, we know that \(X \cup Y = (X \setminus \set{z}) \cup Y \cup \set{z}\) is finite.
          Thus, we have
          \begin{align*}
  ...
  Since \(X, Y\) are finite, by \labelcrefs{i:3.6.5}, there exist two bijective functions \(f : X \to \set{i \in \N : 1 \leq i \leq \#(X)}\) and \(g : Y \to \set{i \in \N : 1 \leq i \leq \#(Y)}\).
  ...
  \begin{itemize}
    ...
    \item If both \(z_1\) and \(z_2\) are in \(Y\), then we have \(g(z_1) \neq g(z_2)\) since \(g\) is bijective.
          By \labelcrefs{i:2.2.13}, this means \(g(z_1) < g(z_2)\) or \(g(z_1) > g(z_2)\).
          Thus, we can apply \labelcrefs{i:2.2.12.d} to derive \(g(z_1) + \#(X) < g(z_2) + \#(X)\) or \(g(z_1) + \#(X) > g(z_2) + \#(X)\).
          By \labelcrefs{i:2.2.13} again, we have \(h(z_1) = g(z_1) + \#(X) \neq g(z_2) + \#(X) = h(z_2)\).
    \item If \(z_1 \in X\) and \(z_2 \in Y\), then by \labelcrefs{i:ac:2.2.3} and \labelcrefs{i:2.2.12.b,i:2.2.12.d}, we have
          \[
            1 \leq h(z_1) = f(z_1) \leq \#(X) < \#(X) + 1 \leq h(z_2) = g(z_2) + \#(X) \leq \#(X) + \#(Y).
          \]
          By \labelcrefs{i:2.2.13}, this means \(h(z_1) \neq h(z_2)\).
    \item If \(z_1 \in Y\) and \(z_2 \in X\), then by \labelcrefs{i:ac:2.2.3} and \labelcrefs{i:2.2.12.b,i:2.2.12.d}, we have
          \[
  ...
  \begin{itemize}
    ...
    \item If \(\#(X) + 1 \leq j \leq \#(X) + \#(Y)\), then by \labelcrefs{i:2.2.11}, we have \(j = \#(X) + 1 + k\) for some \(k \in \N\).
          By \labelcrefs{i:2.2.12.d}, we know that \(\#(X) + 1 \leq \#(X) + 1 + k \leq \#(X) + \#(Y) \implies 1 \leq 1 + k \leq \#(Y)\).
  ...
  Since \(h\) is both injective and surjective, we know that \(h\) is bijective (\labelcrefs{i:3.3.20}).
  By \labelcrefs{i:3.6.5}, this means \(X \cup Y\) has cardinality \(\#(X) + \#(Y)\).
  Thus, by \labelcrefs{i:3.6.10}, we have \(\#(X \cup Y) = \#(X) + \#(Y)\).
\end{proof}
...
\begin{proof}[\pf{i:3.6.14.c}]
  ...
  \begin{itemize}
    \item If \(z \in Y\), then we have \(Y \setminus \set{z} \subseteq X \setminus \set{z}\).
          ...
          By \labelcrefs{i:3.1.28.g}, we have \(Y = (Y \setminus \set{z}) \cup \set{z}\).
          Thus, by \labelcrefs{i:3.6.14.a}, we know that \(Y\) is finite.
          Therefore, we have
          \begin{align*}
...
\begin{proof}[\pf{i:3.6.14.d}]
  ...
  So let \(X\) be a set with cardinality \(n\pp\).
  Let \(x \in X\).
  By \labelcrefs{i:3.6.9}, we have \(\#(X \setminus \set{x}) = n\).
    ...
    \item If \(f(X \setminus \set{x}) = f(X)\), then \(f(X)\) is finite, and we have
          \begin{align*}
            \#(f(X)) & = \#(f(X \setminus \set{x}))                                  \\
                     ...
                     & = \#(X).                                 &  & \by{i:3.1.28.g}
          \end{align*}
    \item If \(f(X \setminus \set{x}) \neq f(X)\), then by \labelcrefs{i:ex:3.4.3,i:3.6.14.a}, we know that \(f(X) = f(X \setminus \set{x}) \cup \set{f(x)}\) is finite.
          Thus,
          \begin{align*}
  ...
  By \labelcrefs{i:3.4.1}, we know that \(g\) is surjective.
  Since \(f\) is injective, by \labelcrefs{i:3.3.14}, we know that
  \[
    \forall x_1, x_2 \in X, x_1 \neq x_2 \implies g(x_1) = f(x_1) \neq f(x_2) = g(x_2).
  \]
  Thus, \(g\) is injective.
  By \labelcrefs{i:3.3.20}, this means \(g\) is bijective.
  Therefore, by \labelcrefs{i:3.6.1}, \(X\) and \(f(X)\) have equal cardinality.
  From first part of the proof we know that \(f(X)\) is finite.
  Thus, by \labelcrefs{i:3.6.10}, we have \(\#(f(X)) = \#(X)\).
\end{proof}
...
\begin{proof}[\pf{i:3.6.14.e}]
  ...
  \]
  By \labelcrefs{i:ex:3.5.7}, we see that \(f = \pi_{\set{x} \times Y \to Y}\).
  ...
  \end{align*}
  we see that \(f\) is injective (\labelcrefs{i:3.3.14}).
  ...
  \begin{align*}
    ...
    \implies & f(x, y) = y.
  \end{align*}
  Thus, \(f\) is surjective (\labelcrefs{i:3.3.17}).
  Since \(f\) is both injective and surjective, we know that \(f\) is bijective (\labelcrefs{i:3.3.20}).
  Thus, by \labelcrefs{i:3.6.1}, \(\set{x} \times Y\) and \(Y\) have equal cardinality.
  Since \(Y\) is finite, by \labelcrefs{i:3.6.4}, we know that \(\set{x} \times Y\) is also finite.
  Thus, by \labelcrefs{i:3.6.10}, we have \(\#(\set{x} \times Y) = \#(Y)\).

  Now we show that \(X \times Y\) is finite, and \(\#(X \times Y) = \#(X) \times \#(Y)\).
  By \labelcrefs{i:3.6.10}, there exists an \(n \in \N\) such that \(\#(X) = n\).
  We induct on \(n\) to show that \(X \times Y\) is finite, and \(\#(X \times Y) = \#(X) \times \#(Y)\).
  For \(n = 0\), by \labelcrefs{i:ex:3.6.2}, we have \(X = \emptyset\).
  Thus, by \labelcrefs{i:3.5.4}, we have \(\emptyset \times Y = \emptyset\), and therefore \(\emptyset \times Y\) is finite.
  Then we have
  \begin{align*}
  ...
  \end{align*}
  we know that \(X \times Y\) is finite by \labelcrefs{i:3.6.14.b}.
  Thus, we have
  \begin{align*}
...
\begin{proof}[\pf{i:3.6.14.f}]
  ...
  Thus, \(f\) is well-defined (\labelcrefs{i:3.3.1}).
  ...
  By \labelcrefs{i:3.10}, we know that \(g \in Y^{\set{x}}\).
  Therefore, \(f(g) = y\).
  By \labelcrefs{i:3.3.17}, we see that \(f\) is surjective.
  Since \(f\) is both injective and surjective, we know that \(f\) is bijective (\labelcrefs{i:3.3.20}).
  Thus, by \labelcrefs{i:3.6.1}, \(Y^{\set{x}}\) and \(Y\) have equal cardinality.
  Since \(Y\) is finite, by \labelcrefs{i:3.6.10}, we know that \(Y^{\set{x}}\) is finite, and we have \(\#\pa{Y^{\set{x}}} = \#(Y)\).

  Now we show that \(Y^X\) is finite, and \(\#\pa{Y^X} = \#(Y)^{\#(X)}\).
  By \labelcrefs{i:3.6.10}, there exists an \(n \in \N\) such that \(\#(X) = n\).
  We induct on \(n\) to show that \(Y^X\) is finite and \(\#\pa{Y^X} = \#(Y)^{\#(X)}\).
  For \(n = 0\), by \labelcrefs{i:ex:3.6.2}, we have \(X = \emptyset\).
  By \labelcrefs{i:3.3.9}, there is only one function in \(Y^\emptyset\).
  Thus, by \labelcrefs{i:3.3}, \(Y^\emptyset\) is a singleton set, and therefore \(Y^\emptyset\) is finite.
  By \labelcrefs{i:3.6.10}, we have \(\#\pa{Y^\emptyset} = 1\).
  Thus,
  \begin{align*}
  ...
  \begin{align*}
    ...
    \implies & h(f) \neq h(g),
  \end{align*}
  we see that \(h\) is injective (\labelcrefs{i:3.3.14}).
  ...
  \begin{align*}
    ...
    \implies & h(f) = (p, q),
  \end{align*}
  we see that \(h\) is surjective (\labelcrefs{i:3.3.17}).
  Since \(h\) is both injective and surjective, we know that \(h\) is bijective (\labelcrefs{i:3.3.20}).
  Thus, by \labelcrefs{i:3.6.1}, we know that \(Y^X\) and \(Y^{X \setminus \set{x}} \times Y^{\set{x}}\) have equal cardinality.
  By \labelcrefs{i:3.6.9}, we know that \(\#(X \setminus \set{x}) = n\).
  ...
  Thus, by \labelcrefs{i:3.6.14.e}, we know that \(Y^{X \setminus \set{x}} \times Y^{\set{x}}\) is finite.
  By \labelcrefs{i:3.6.10}, this means \(Y^X\) is finite, and we have \(\#\pa{Y^X} = \#\pa{Y^{X \setminus \set{x}} \times Y^{\set{x}}}\).
...
\begin{rmk}\label{i:3.6.15}
  \labelcrefs{i:3.6.14} suggests that there is another way to define the arithmetic operations of natural numbers;
  not defined recursively as in \labelcrefs{i:2.2.1,i:2.3.1,i:2.3.11}, but instead using the notions of union, Cartesian product, and power set.
  ...
  we will not develop this arithmetic in this text, but we give some examples of how one would work with this arithmetic in \labelcrefs{i:ex:3.6.5,i:ex:3.6.6}.
\end{rmk}

\exercisesection

\begin{ex}\label{i:ex:3.6.1}
  Prove \labelcrefs{i:3.6.4}.
\end{ex}

\begin{proof}[\pf{i:ex:3.6.1}]
  See \labelcrefs{i:3.6.4}.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.6.2}]
  By \labelcrefs{i:3.6.5}, we know that \(\#(X) = 0\) iff there exists a bijective function \(f : X \to \set{i \in \N : 1 \leq i \leq 0}\).
  Since \(\set{i \in \N : 1 \leq i \leq 0} = \emptyset\), by \labelcrefs{i:ex:3.3.3}, we know that \(f\) is a bijective function iff \(X = \emptyset\).
...
\begin{proof}[\pf{i:ex:3.6.3}]
  We induct on \(n\).
  For \(n = 0\), any function \(f : \set{i \in \N : 1 \leq i \leq 0} \to \N\) is the empty function to \(\N\) (\labelcrefs{i:3.3.9}).
  ...
  \begin{itemize}
    ...
    \item If \(f(n\pp) > M\), then by \labelcrefs{i:2.2.12.b}, we know that \(f(i) < f(n\pp)\) for all \(1 \leq i \leq n\).
...
\begin{ex}\label{i:ex:3.6.4}
  Prove \labelcrefs{i:3.6.14}.
\end{ex}

\begin{proof}[\pf{i:ex:3.6.4}]
  See \labelcrefs{i:3.6.14}.
\end{proof}
...
\begin{ex}\label{i:ex:3.6.5}
  ...
  Then use \labelcrefs{i:3.6.14} to conclude an alternate proof of \labelcrefs{i:2.3.2}.
\end{ex}
...
\begin{proof}[\pf{i:ex:3.6.5}]
  ...
  \]
  By \labelcrefs{i:3.5.1}, we know that \(f\) passes the vertical line test.
  ...
  Therefore, \(f\) is surjective (\labelcrefs{i:3.3.17}).
  Since \(f\) is both injective and surjective, we know that \(f\) is bijective (\labelcrefs{i:3.3.20}).
  By \labelcrefs{i:3.6.1}, we conclude that \(A \times B\) and \(B \times A\) have equal cardinality.

  Now we give an alternative proof of \labelcrefs{i:2.3.2}.
  Let \(n, m \in \N\), let \(A = \set{i \in \N : 1 \leq i \leq n}\), and let \(B = \set{i \in \N : 1 \leq i \leq m}\).
  By \labelcrefs{i:3.6.10}, we know that \(\#(A) = n\) and \(\#(B) = m\).
  From the proof above we know that \(A \times B\) and \(B \times A\) have equal cardinality.
  Thus, by \labelcrefs{i:3.6.8}, we have \(\#(A \times B) = \#(B \times A)\).
  Then we have
  \begin{align*}
    n \times m & = \#(A) \times \#(B)                      \\
               ...
               & = m \times n.
  \end{align*}
  Thus, \labelcrefs{i:2.3.2} is true.
\end{proof}
...
\begin{proof}[\pf{i:ex:3.6.6}]
  ...
  By \labelcrefs{i:3.5.4}, \(B \times C\) is a set, and by \labelcrefs{i:3.10}, \(A^B, (A^B)^C, A^{B \times C}\) are sets.
  ...
  If \(h_g' : B \times C \to A\) is another function satisfying \(h_g'(b, c) = (g(c))(b)\) for any \((b, c) \in B \times C\), then we have \(h_g = h_g'\) (\labelcrefs{i:3.3.7}).
  ...
  Thus, \(f\) is surjective (\labelcrefs{i:3.3.17}).
  Since \(f\) is both injective and surjective, we know that \(f\) is bijective (\labelcrefs{i:3.3.20}).
  By \labelcrefs{i:3.6.1}, we conclude that \((A^B)^C\) and \(A^{B \times C}\) have equal cardinality.

  Now we show that \((a^b)^c = a^{bc}\) for any \(a, b, c \in \N\).
  Suppose that \(A, B, C\) are finite sets with cardinality \(\#(A) = a\), \(\#(B) = b\), and \(\#(C) = c\).
  Since \(B, C\) are finite, by \labelcrefs{i:3.6.14.e}, we know that \(B \times C\) is finite.
  By \labelcrefs{i:3.6.14.f}, we know that \(A^{B \times C}\) is finite.
  From the proof above we see that \((A^B)^C\) and \(A^{B \times C}\) have equal cardinality.
  Thus, \((A^B)^C\) is finite, and by \labelcrefs{i:3.6.8}, we have \(\#\pa{(A^B)^C} = \#\pa{A^{B \times C}}\).
  Then we have
  \begin{align*}
  ...
  By \labelcrefs{i:3.10}, \(A^B, A^C, A^{B \cup C}\) are sets, and by \labelcrefs{i:3.5.4}, \(A^B \times A^C\) is a set.
  ...
  If there exist some \(g' \in A^B\) and \(h' \in A^C\), such that \((f(g, h))(x) = g'(x)\) if \(x \in B\), and \((f(g, h))(x) = h'(x)\) if \(x \in C\), then we have \(g = g'\) and \(h = h'\) (\labelcrefs{i:3.3.7}).
  Thus, \(f\) passes the vertical line test, and \(f\) is well-defined.
  We now show that \(f\) is bijective.
  We start by showing that \(f\) is injective.
  ...
  Thus, \(f\) is surjective (\labelcrefs{i:3.3.17}).
  Since \(f\) is both injective and surjective, we know that \(f\) is bijective (\labelcrefs{i:3.3.20}).
  By \labelcrefs{i:3.6.1}, we conclude that \(A^B \times A^C\) and \(A^{B \cup C}\) have equal cardinality if \(B \cap C = \emptyset\).

  Now we show that \(a^b \times a^c = a^{b + c}\) for any \(a, b, c \in \N\).
  Suppose that \(A, B, C\) are finite sets with cardinality \(\#(A) = a\), \(\#(B) = b\), and \(\#(C) = c\).
  Suppose also that \(B \cap C = \emptyset\).
  Since \(B, C\) are finite, by \labelcrefs{i:3.6.14.b}, we know that \(B \cup C\) is finite.
  By \labelcrefs{i:3.6.14.f}, we know that \(A^{B \cup C}\) is finite.
  From the proof above we know that \(A^B \times A^C\) and \(A^{B \cup C}\) have equal cardinality.
  Thus, by \labelcrefs{i:3.6.8}, \(A^B \times A^C\) is finite, and \(\#(A^B \times A^C) = \#\pa{A^{B \cup C}}\).
  Then we have
  \begin{align*}
...
\begin{proof}[\pf{i:ex:3.6.7}]
  ...
  By \labelcrefs{i:3.4.1}, we have \(f(A) \subseteq B\).
  Thus,
  \begin{align*}
    \#(A) & = \#(f(A))  &  & \by{i:3.6.14.d} \\
          & \leq \#(B). &  & \by{i:3.6.14.c}
  ...
  By \labelcrefs{i:3.6.5}, there exists two bijective functions \(g_A : A \to \set{i \in \N : 1 \leq i \leq \#(A)}\) and \(g_B : B \to \set{i \in \N : 1 \leq i \leq \#(B)}\).
...
\begin{ex}\label{i:ex:3.6.8}
  ...
  see \labelcrefs{i:ex:8.4.3}.)
\end{ex}

\begin{proof}[\pf{i:ex:3.6.8}]
  Since \(f\) is injective, by \labelcrefs{i:3.3.14}, we know that if \(b \in f(A)\), then there exists only one element \(a \in A\) such that \(f(a) = b\).
  ...
  Let \(y \in A\).
  By \labelcrefs{i:3.4.1}, we have \(f(y) \in f(A) \subseteq B\).
  Thus, by \labelcrefs{i:ex:3.3.6}, we have \(g(f(y)) = f^{-1}(f(y)) = y\).
  Therefore, \(g\) is surjective (\labelcrefs{i:3.3.17}).
\end{proof}
...
\begin{proof}[\pf{i:ex:3.6.9}]
  By \labelcrefs{i:3.6.14.b}, \(A \cup B\) is finite.
  By \labelcrefs{i:ex:3.1.7}, we have \(A \cap B \subseteq A\).
  Thus, by \labelcrefs{i:3.6.14.c}, we know that \(A \cap B\) is finite since \(A\) is finite.
...
\begin{proof}[\pf{i:ex:3.6.10}]
  ...
  Since \(A_{n\pp}\) is finite, by \labelcrefs{i:3.6.14.b}, we know that \(\pa{\bigcup_{i \in \set{1, \dots, n}} A_i} \cup A_{n\pp} = \bigcup_{i \in \set{1, \dots, n\pp}} A_i\) is finite.
  Thus, we have
  \begin{align*}
